 ...
 */
static int mem_malloc_mempool_gen(uint16_t num_blocks, uint32_t block_size,
                                  void **out_buf) {
  block_size = OS_ALIGN(block_size, OS_ALIGNMENT);

  if (num_blocks > 0) {
    *out_buf =
      nimble_platform_mem_malloc(OS_MEMPOOL_BYTES(num_blocks, block_size));
    if (*out_buf == NULL) {
      return OS_ENOMEM;
    }
  } else {
...
int mem_malloc_mbufpkt_pool(struct os_mempool *mempool,
  ...
  rc = mem_malloc_mbuf_pool(mempool, mbuf_pool, num_blocks,
                            block_size + sizeof(struct os_mbuf_pkthdr), name,
                            out_buf);
  return rc;
}
