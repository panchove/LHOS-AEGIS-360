...
int ble_ll_iso_big_last_tx_timestamp_get(struct ble_ll_iso_bis *bis,
  ...
  *timestamp =
    (uint64_t)big->event_start * 1000000 / 32768 + big->event_start_us;

  return 0;
}

static void ble_ll_iso_big_biginfo_calc(struct ble_ll_iso_big *big,
                                        uint32_t seed_aa) {
  uint8_t *buf;
...
static void ble_ll_iso_big_free(struct ble_ll_iso_big *big) {
  struct ble_ll_iso_bis *bis;
...
static void ble_ll_iso_big_terminate_complete(struct ble_ll_iso_big *big) {
...
static void ble_ll_iso_big_update_event_start(struct ble_ll_iso_big *big) {
  os_sr_t sr;
...
static void ble_ll_iso_big_event_done(struct ble_ll_iso_big *big) {
  struct ble_ll_iso_bis *bis;
  struct ble_hci_ev *hci_ev;
  ...
  do {
    ...
    if (!big->control_active) {
      if (big->term_pending) {
        ...
        memcpy(big->chan_map_new, g_ble_ll_data.chan_map, BLE_LL_CHAN_MAP_LEN);
    ...
    big->sch.end_time =
      big->sch.start_time + ble_ll_tmr_u2t_up(big->sync_delay) + 1;
...
static void ble_ll_iso_big_event_done_ev(struct ble_npl_event *ev) {
  struct ble_ll_iso_big *big;
...
static void ble_ll_iso_big_event_done_to_ll(struct ble_ll_iso_big *big) {
  big_active = NULL;
  ble_ll_state_set(BLE_LL_STATE_STANDBY);
  ble_npl_eventq_put(&g_ble_ll_data.ll_evq, &big->event_done);
}

static uint8_t ble_ll_iso_big_control_pdu_cb(uint8_t *dptr, void *arg,
                                             uint8_t *hdr_byte) {
  struct ble_ll_iso_big *big;
  uint8_t len;
...
static void ble_ll_iso_big_control_txend_cb(void *arg) {
  struct ble_ll_iso_big *big;
...
static int ble_ll_iso_big_control_tx(struct ble_ll_iso_big *big) {
  uint16_t chan_idx;
  uint16_t chan_id;
  uint16_t foo, bar;
  int rc;
  ...
  chan_idx = ble_ll_utils_dci_iso_event(
    big->big_counter, chan_id, &foo, big->chan_map_used, big->chan_map, &bar);
...
static uint8_t ble_ll_iso_big_subevent_pdu_cb(uint8_t *dptr, void *arg,
                                              uint8_t *hdr_byte) {
  struct ble_ll_iso_big *big;
  struct ble_ll_iso_bis *bis;
  int idx;
...
static int ble_ll_iso_big_subevent_tx(struct ble_ll_iso_big *big) {
  struct ble_ll_iso_bis *bis;
  uint16_t chan_idx;
  int to_tx;
  int rc;
  ...
  if (bis->tx.subevent_num == 1) {
    chan_idx = ble_ll_utils_dci_iso_event(
      big->big_counter, bis->chan_id, &bis->tx.prn_sub_lu, big->chan_map_used,
      big->chan_map, &bis->tx.remap_idx);
  } else {
    chan_idx = ble_ll_utils_dci_iso_subevent(bis->chan_id, &bis->tx.prn_sub_lu,
  ...
  rc = ble_phy_tx(ble_ll_iso_big_subevent_pdu_cb, big,
                  to_tx ? BLE_PHY_TRANSITION_TX_TX : BLE_PHY_TRANSITION_NONE);
  return rc;
}

static void ble_ll_iso_big_subevent_txend_cb(void *arg) {
  struct ble_ll_iso_big *big;
  struct ble_ll_iso_bis *bis;
  int rc;
...
static int ble_ll_iso_big_event_sched_cb(struct ble_ll_sched_item *sch) {
  struct ble_ll_iso_big *big;
  struct ble_ll_iso_bis *bis;
  ...
  STAILQ_FOREACH(bis, &big->bis_q, bis_q_next) {
    ble_ll_isoal_mux_event_start(&bis->mux,
                                 (uint64_t)big->event_start * 1000000 / 32768 +
                                   big->event_start_us);
...
static void ble_ll_iso_big_calculate_gsk(struct ble_ll_iso_big *big,
...
static void ble_ll_iso_big_calculate_iv(struct ble_ll_iso_big *big) {
  struct ble_ll_iso_bis *bis;
  uint8_t *aa;
...
static int ble_ll_iso_big_create(uint8_t big_handle, uint8_t adv_handle,
                                 uint8_t num_bis, struct big_params *bp) {
...
static int ble_ll_iso_big_terminate(uint8_t big_handle, uint8_t reason) {
  struct ble_ll_iso_big *big = NULL;
  unsigned idx;
...
int ble_ll_iso_big_hci_create(const uint8_t *cmdbuf, uint8_t len) {
  ...
  if (!IN_RANGE(cmd->big_handle, 0x00, 0xef) ||
      ...
      !IN_RANGE(cmd->rtn, 0x00, 0x1e) || (cmd->packing > 1) ||
      (cmd->framing > 1) || (cmd->encryption) > 1) {
    return BLE_ERR_INV_HCI_CMD_PARMS;
  }
  ...
  rc =
    ble_ll_iso_big_create(cmd->big_handle, cmd->adv_handle, cmd->num_bis, &bp);
  switch (rc) {
  case 0:
    break;
  case -EINVAL:
...
int ble_ll_iso_big_hci_create_test(const uint8_t *cmdbuf, uint8_t len) {
  ...
  if (!IN_RANGE(cmd->big_handle, 0x00, 0xef) ||
      ...
      !IN_RANGE(cmd->bn, 0x01, 0x07) || !IN_RANGE(cmd->irc, 0x01, 0x0f) ||
      !IN_RANGE(cmd->pto, 0x00, 0x0f) || (cmd->encryption) > 1) {
    return BLE_ERR_INV_HCI_CMD_PARMS;
  }
