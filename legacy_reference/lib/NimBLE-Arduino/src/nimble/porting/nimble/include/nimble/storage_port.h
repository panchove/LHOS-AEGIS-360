...
typedef enum { READONLY, READWRITE } open_mode_t;

typedef uint32_t cache_handle_t;
typedef int (*open_cache)(const char *namespace_name, open_mode_t open_mode,
                          cache_handle_t *out_handle);
typedef void (*close_cache)(cache_handle_t handle);
typedef int (*erase_all_cache)(cache_handle_t handle);
typedef int (*write_cache)(cache_handle_t handle, const char *key,
                           const void *value, size_t length);
typedef int (*read_cache)(cache_handle_t handle, const char *key,
