...
public:
   ...
   */
protected:
  bool initImpl(const char *pin = NULL) { return thisModem().initImpl(pin); }
  ...
  String getModemNameImpl() {
    ...
    String res2;
    if (thisModem().waitResponse(5000L, res2) != 1) {
      return name;
    }
...
protected:
  bool restartImpl(const char *pin = NULL) {
    ...
    thisModem().waitResponse();
    if (!thisModem().setPhoneFunctionality(0)) {
      return false;
    }
    if (!thisModem().setPhoneFunctionality(1, true)) {
      return false;
    }
...
public:
  ...
  bool getNetworkSystemMode(bool &n, int16_t &stat) {
    ...
    if (thisModem().waitResponse(GF(GSM_NL "+CNSMOD:")) != 1) {
      return false;
    }
  ...
  String getLocalIPImpl() { return thisModem().getLocalIPImpl(); }
...
protected:
  ...
  bool gprsDisconnectImpl() { return thisModem().gprsDisconnectImpl(); }
...
protected:
  ...
  String getSimCCIDImpl() {
    ...
    if (thisModem().waitResponse(GF(GSM_NL)) != 1) {
      return "";
    }
    String res = stream.readStringUntil('\n');
    thisModem().waitResponse();
