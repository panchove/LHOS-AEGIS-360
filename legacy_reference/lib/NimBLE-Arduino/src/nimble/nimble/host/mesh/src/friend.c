 ...
 */
#define FRIEND_BUF_COUNT                                                       \
  ((MYNEWT_VAL(BLE_MESH_FRIEND_QUEUE_SIZE) + 1) *                              \
   MYNEWT_VAL(BLE_MESH_FRIEND_LPN_COUNT))

static os_membuf_t friend_buf_mem[OS_MEMPOOL_SIZE(
  FRIEND_BUF_COUNT, BT_MESH_ADV_DATA_SIZE + BT_MESH_MBUF_HEADER_SIZE)];
...
static int friend_cred_create(struct bt_mesh_friend *frnd, uint8_t idx) {
  return bt_mesh_friend_cred_create(&frnd->cred[idx], frnd->lpn,
                                    bt_mesh_primary_addr(), frnd->lpn_counter,
                                    frnd->counter, frnd->subnet->keys[idx].net);
}
...
void bt_mesh_friend_sec_update(uint16_t net_idx) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
    ...
    if (net_idx == BT_MESH_KEY_ANY || frnd->subnet->net_idx == net_idx) {
...
int bt_mesh_friend_clear(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  ...
  if (lpn_counter - frnd->lpn_counter > 255) {
    BT_WARN("LPN Counter out of range (old %u new %u)", frnd->lpn_counter,
            lpn_counter);
    return 0;
  }
  ...
  bt_mesh_ctl_send(&tx, TRANS_CTL_OP_FRIEND_CLEAR_CFM, &cfm, sizeof(cfm), NULL,
                   NULL);
...
static struct os_mbuf *create_friend_pdu(struct bt_mesh_friend *frnd,
  ...
  buf = bt_mesh_adv_create_from_pool(&friend_os_mbuf_pool, adv_alloc,
                                     BT_MESH_ADV_DATA, FRIEND_XMIT, K_NO_WAIT);
  if (!buf) {
    return NULL;
  }

  net_buf_add_u8(buf, (info->iv_index & 1)
                        << 7); /* Will be reset in encryption */
...
static int unseg_app_sdu_prepare(struct bt_mesh_friend *frnd,
  ...
  BT_DBG("Re-encrypting friend pdu (SeqNum %06x -> %06x)", meta.crypto.seq_num,
         bt_mesh.seq);
...
static int encrypt_friend_pdu(struct bt_mesh_friend *frnd, struct os_mbuf *buf,
  ...
  if (flooding_cred) {
    cred = &frnd->subnet->keys[SUBNET_KEY_TX_IDX(frnd->subnet)].msg;
  } else {
...
static struct os_mbuf *encode_friend_ctl(struct bt_mesh_friend *frnd,
                                         uint8_t ctl_op, struct os_mbuf *sdu) {
  struct friend_pdu_info info;
...
int bt_mesh_friend_sub_add(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  struct bt_mesh_friend *frnd;
  uint8_t xact;
...
int bt_mesh_friend_sub_rem(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  struct bt_mesh_friend *frnd;
  uint8_t xact;
...
static void friend_clear_sent(int err, void *user_data) {
  ...
  k_work_reschedule(&frnd->clear.timer, K_SECONDS(frnd->clear.repeat_sec));
...
static void send_friend_clear(struct bt_mesh_friend *frnd) {
  ...
  bt_mesh_ctl_send(&tx, TRANS_CTL_OP_FRIEND_CLEAR, &req, sizeof(req),
                   &clear_sent_cb, frnd);
}
...
int bt_mesh_friend_clear_cfm(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  ...
  if (lpn_counter != frnd->lpn_counter) {
    BT_WARN("LPN counter mismatch (0x%04x != 0x%04x)", lpn_counter,
            frnd->lpn_counter);
    return 0;
  }
...
static int32_t offer_delay(struct bt_mesh_friend *frnd, int8_t rssi,
                           uint8_t crit) {
...
init_friend:
  ...
  BT_DBG("LPN 0x%04x rssi %d recv_delay %u poll_to %ums", frnd->lpn,
         rx->ctx.recv_rssi, frnd->recv_delay, (unsigned)frnd->poll_to);
...
static bool is_seg(struct bt_mesh_friend_seg *seg, uint16_t src,
                   uint16_t seq_zero) {
...
static void enqueue_friend_pdu(struct bt_mesh_friend *frnd,
                               enum bt_mesh_friend_pdu_type type, uint16_t src,
                               uint8_t seg_count, struct os_mbuf *buf) {
  ...
  uint16_t seq_zero =
    (((buf->om_data[10] << 8 | buf->om_data[11]) >> 2) & TRANS_SEQ_ZERO_MASK);
...
static void buf_send_end(int err, void *user_data) {
  ...
  if (frnd->established) {
    ...
    BT_DBG("Waiting %u ms for next poll", (unsigned)frnd->poll_to);
  } else {
...
static void friend_timeout(struct ble_npl_event *work) {
  ...
  BT_DBG("lpn 0x%04x send_last %u last %p", frnd->lpn, frnd->send_last,
         frnd->last);
  ...
  BT_DBG("Sending buf %p from Friend Queue of LPN 0x%04x", frnd->last,
         frnd->lpn);
  frnd->queue_size--;
...
static void subnet_evt(struct bt_mesh_subnet *sub, enum bt_mesh_key_evt evt) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
    ...
    case BT_MESH_KEY_UPDATED:
      ...
      if (err) {
        BT_ERR("Failed updating friend cred for 0x%04x", frnd->lpn);
        friend_clear(frnd);
      }
      break;
    ...
    case BT_MESH_KEY_REVOKED:
      ...
      memcpy(&frnd->cred[0], &frnd->cred[1], sizeof(frnd->cred[0]));
...
static void friend_purge_old_ack(struct bt_mesh_friend *frnd,
                                 uint64_t *seq_auth, uint16_t src) {
  struct os_mbuf *cur, *prev = NULL;

  BT_DBG("SeqAuth %llx src 0x%04x", *seq_auth, src);

  for (cur = net_buf_slist_peek_head(&frnd->queue); cur != NULL;
       prev = cur, cur = net_buf_slist_peek_next(cur)) {
...
static void friend_lpn_enqueue_rx(struct bt_mesh_friend *frnd,
  ...
  BT_DBG("LPN 0x%04x queue_size %u", frnd->lpn, (unsigned)frnd->queue_size);
  ...
  BT_DBG("Queued message for LPN 0x%04x, queue_size %u", frnd->lpn,
...
static bool friend_queue_prepare_space(struct bt_mesh_friend *frnd,
                                       uint16_t addr, uint64_t *seq_auth,
                                       uint8_t seg_count) {
...
void bt_mesh_friend_enqueue_rx(struct bt_mesh_net_rx *rx,
  ...
  BT_DBG("recv_ttl %u net_idx 0x%04x src 0x%04x dst 0x%04x", rx->ctx.recv_ttl,
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
    ...
    if (friend_lpn_matches(frnd, rx->sub->net_idx, rx->ctx.addr)) {
      continue;
    }
    ...
    if (!friend_queue_prepare_space(frnd, rx->ctx.addr, seq_auth, seg_count)) {
      continue;
    }
...
bool bt_mesh_friend_enqueue_tx(struct bt_mesh_net_tx *tx,
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
    ...
    if (!friend_queue_prepare_space(frnd, tx->src, seq_auth, seg_count)) {
      continue;
    }

    friend_lpn_enqueue_tx(frnd, tx, type, seq_auth, seg_count, sbuf);
    matched = true;
  }
