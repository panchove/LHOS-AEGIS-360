...
NimBLEAttValue::NimBLEAttValue(uint16_t init_len, uint16_t max_len)
    : m_attr_value{static_cast<uint8_t *>(calloc(init_len + 1, 1))},
      ...
      m_attr_len{}, m_capacity{init_len}
...
NimBLEAttValue::NimBLEAttValue(const uint8_t *value, uint16_t len,
                               uint16_t max_len)
...
// Copy all the data from the source object to this object, including allocated
// space.
void NimBLEAttValue::deepCopy(const NimBLEAttValue &source) {
  uint8_t *res =
    static_cast<uint8_t *>(realloc(m_attr_value, source.m_capacity + 1));
...
bool NimBLEAttValue::setValue(const uint8_t *value, uint16_t len) {
  m_attr_len =
    0; // Just set the value length to 0 and append instead of repeating code.
  m_attr_value[0] =
    '\0'; // Set the first byte to 0 incase the len of the new value is 0.
  append(value, len);
