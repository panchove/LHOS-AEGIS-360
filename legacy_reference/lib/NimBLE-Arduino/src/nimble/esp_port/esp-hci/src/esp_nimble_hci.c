...
int ble_hci_trans_hs_cmd_tx(uint8_t *cmd) {
  ...
  *cmd = BLE_HCI_UART_H4_CMD;
  ...
  if (xSemaphoreTake(vhci_send_sem,
                     NIMBLE_VHCI_TIMEOUT_MS / portTICK_PERIOD_MS) == pdTRUE) {
...
static void dummy_controller_rcv_pkt_ready(void) { /* Dummy function */ }
...
static int host_rcv_pkt(uint8_t *data, uint16_t len) {
  ...
  if (data[0] == BLE_HCI_UART_H4_EVT) {
    ...
    if (totlen > MYNEWT_VAL(BLE_TRANSPORT_EVT_SIZE)) {
      esp_rom_printf("Received HCI data length at host (%d) exceeds maximum "
                     "configured HCI event buffer size (%d).",
