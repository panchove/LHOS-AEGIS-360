// Implementation for ELA Innovation Blue PUCK Digital Input
...
std::string ELA_PUCK_DI_decoder::decode(const char *macAddress,
  ...
  std::vector<uint8_t> diBytes;      // 0x2A06
  std::vector<uint8_t> batteryBytes; // Comes from manufacturer data bytes 1 & 2
  ...
  if (diBytes.size() == 2) {
    uint16_t raw = (diBytes[1] << 8) | diBytes[0];
    bool status = (raw & 0x0001) == 0x0001;
    uint16_t count = (raw >> 1) & 0x7FFF;
    out << "ble." << macAddress << ".digital.input.status:" << (status ? 1 : 0)
        << ",";
    out << "ble." << macAddress << ".digital.input.count:" << count << ",";
  }
