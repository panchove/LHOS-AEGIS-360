...
void bt_mesh_beacon_create(struct bt_mesh_subnet *sub, struct os_mbuf *buf) {
...
#define BEACON_THRESHOLD(sub)                                                  \
  (K_SECONDS(10 * ((sub)->beacons_last + 1)) - K_SECONDS(5))
...
static int secure_beacon_send(struct bt_mesh_subnet *sub, void *cb_data) {
  ...
  if (time_diff < (600 * MSEC_PER_SEC) && time_diff < BEACON_THRESHOLD(sub)) {
    return 0;
  }
...
static void unprovisioned_beacon_recv(struct os_mbuf *buf) {
  ...
  if (prov->unprovisioned_beacon) {
    prov->unprovisioned_beacon(uuid, (bt_mesh_prov_oob_info_t)oob_info,
                               uri_hash);
  }
}
...
static bool auth_match(struct bt_mesh_subnet_keys *keys,
  ...
  if (memcmp(params->auth, net_auth, 8)) {
    BT_WARN("Authentication Value %s != %s", bt_hex(params->auth, 8),
            bt_hex(net_auth, 8));
    return false;
  }
...
static void secure_beacon_recv(struct os_mbuf *buf) {
  ...
  BT_DBG("flags 0x%02x id %s iv_index 0x%08x", params.flags,
         bt_hex(params.net_id, 8), params.iv_index);
...
void bt_mesh_beacon_update(struct bt_mesh_subnet *sub) {
  ...
  err = bt_mesh_beacon_auth(keys->beacon, flags, keys->net_id, bt_mesh.iv_index,
                            sub->auth);
  if (err) {
