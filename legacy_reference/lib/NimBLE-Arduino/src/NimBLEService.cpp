...
NimBLEService::NimBLEService(const NimBLEUUID &uuid)
    : NimBLELocalAttribute{uuid, 0},
      m_pSvcDef{{0, getUUID().getBase(), nullptr, nullptr}, {}} {}

/**
 * @brief Destructor, make sure we release the resources allocated for the
 * service.
 */
...
void NimBLEService::dump() const {
  NIMBLE_LOGD(LOG_TAG, "Service: uuid:%s, handle: 0x%2x",
              getUUID().toString().c_str(), getHandle());
...
bool NimBLEService::start() {
  ...
  NIMBLE_LOGD(LOG_TAG, "Adding %d characteristics for service %s", numChrs,
              toString().c_str());
  if (numChrs) {
    int i = 0;

    // Nimble requires the last characteristic to have it's uuid = 0 to indicate
    // the end of the characteristics for the service. We create 1 extra and set
    // it to null for this purpose.
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gatts_count_cfg failed, rc= %d, %s", rc,
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gatts_add_svcs, rc= %d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
 ...
 * @param [in] max_len - The maximum length in bytes that the characteristic
 * value can hold.
...
NimBLEService::createCharacteristic(const NimBLEUUID &uuid, uint32_t properties,
                                    uint16_t max_len) {
  NimBLECharacteristic *pChar =
    new NimBLECharacteristic(uuid, properties, max_len, this);
 ...
 * @param[in] pChar A pointer to the characteristic instance to add to the
 * service.
 */
void NimBLEService::addCharacteristic(NimBLECharacteristic *pChar) {
  ...
  for (const auto &chr : m_vChars) {
    if (chr == pChar) {
      pChar->setService(
        this); // Update the service pointer in the characteristic.
      return;
    }
  }
 ...
 * @param[in] pChar A pointer to the characteristic instance to remove from the
 * service.
 * @param[in] deleteChr If true it will delete the characteristic instance and
 * free it's resources.
 */
void NimBLEService::removeCharacteristic(NimBLECharacteristic *pChar,
  ...
  pChar->setRemoved(deleteChr ? NIMBLE_ATT_REMOVE_DELETE
 ...
 * @param [in] uuid The UUID of the characteristic.
 * @param idx The index of the characteristic to return (used when multiple
 * characteristics have the same UUID).
 ...
 */
NimBLECharacteristic *
NimBLEService::getCharacteristicByHandle(uint16_t handle) const {
  for (const auto &chr : m_vChars) {
...
/**
 * @return A vector containing pointers to each characteristic associated with
 * this service.
 */
const std::vector<NimBLECharacteristic *> &
NimBLEService::getCharacteristics() const {
  return m_vChars;
} // getCharacteristics

/**
 * @return A vector containing pointers to each characteristic with the provided
 * UUID associated with this service.
 */
std::vector<NimBLECharacteristic *>
NimBLEService::getCharacteristics(const char *uuid) const {
 ...
 */
std::vector<NimBLECharacteristic *>
NimBLEService::getCharacteristics(const NimBLEUUID &uuid) const {
