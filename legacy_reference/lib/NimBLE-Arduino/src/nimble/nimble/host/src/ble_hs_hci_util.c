...
uint16_t ble_hs_hci_util_handle_pb_bc_join(uint16_t handle, uint8_t pb,
  ...
  return (handle << 0) | (pb << 12) | (bc << 14);
}

int ble_hs_hci_util_read_adv_tx_pwr(int8_t *out_tx_pwr) {
  struct ble_hci_le_rd_adv_chan_txpwr_rp rsp;
  int rc;

  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RD_ADV_CHAN_TXPWR), NULL, 0, &rsp,
    sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
int ble_hs_hci_util_read_rssi(uint16_t conn_handle, int8_t *out_rssi) {
  ...
  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_STATUS_PARAMS, BLE_HCI_OCF_RD_RSSI), &cmd,
...
int ble_hs_hci_util_set_random_addr(const uint8_t *addr) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_RAND_ADDR), &cmd, sizeof(cmd),
    NULL, 0);
}
...
int ble_hs_hci_util_set_data_len(uint16_t conn_handle, uint16_t tx_octets,
  ...
  rc =
    ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_DATA_LEN),
...
int ble_hs_hci_util_read_sugg_def_data_len(uint16_t *out_sugg_max_tx_octets,
  ...
  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RD_SUGG_DEF_DATA_LEN), NULL, 0,
    &rsp, sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
int ble_hs_hci_util_write_sugg_def_data_len(uint16_t sugg_max_tx_octets,
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_WR_SUGG_DEF_DATA_LEN), &cmd,
    sizeof(cmd), NULL, 0);
}
...
int ble_hs_hci_set_chan_class(const uint8_t *chan_map) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_HOST_CHAN_CLASS), &cmd,
    sizeof(cmd), NULL, 0);
}

int ble_hs_hci_util_set_data_addr_change(uint8_t adv_handle,
...
int ble_hs_hci_dtm_tx_start(const uint8_t tx_chan, const uint8_t test_data_len,
...
int ble_hs_hci_dtm_rx_start(const uint8_t rx_chan) {
...
int ble_hs_hci_dtm_stop(void) {
  return ble_hs_hci_cmd_tx_no_rsp(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_TEST_END), NULL, 0);
}

int ble_hs_hci_dtm_enh_tx_start(const uint8_t tx_chan,
                                const uint8_t test_data_len,
...
int ble_hs_hci_dtm_enh_rx_start(const uint8_t rx_chan, const uint8_t index,
                                const uint8_t phy) {
