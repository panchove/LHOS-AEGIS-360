...
class NimBLERemoteValueAttribute : public NimBLEValueAttribute,
                                   public NimBLEAttribute {
public:
  /**
   * @brief Read the value of the remote attribute.
   * @param [in] timestamp A pointer to a time_t struct to store the time the
   * value was read.
   ...
   * @param [in] length (optional) The length of the character string, uses
   * strlen if omitted.
   * @param [in] response Whether we require a response from the write.
   * @return false if not connected or otherwise cannot perform write.
   */
  bool writeValue(const char *str, size_t length = 0,
                  bool response = false) const {
    return writeValue(reinterpret_cast<const uint8_t *>(str),
                      length ? length : strlen(str), response);
  }
   ...
   * @details Only used for types without a `c_str()` and `length()` or `data()`
   * and `size()` method. <type\> size must be evaluatable by `sizeof()` if no
   * length is provided.
   */
  template <typename T>
#ifdef _DOXYGEN_
  ...
  typename std::enable_if<!std::is_pointer<T>::value &&
                            !Has_c_str_length<T>::value &&
                            !Has_data_size<T>::value,
                          bool>::type
#endif
  writeValue(const T &v, bool response = false) const {
    return writeValue(reinterpret_cast<const uint8_t *>(&v), sizeof(T),
                      response);
  }
  ...
  writeValue(const T &s, bool response = false) const {
    return writeValue(reinterpret_cast<const uint8_t *>(s.c_str()), s.length(),
                      response);
  }
   ...
   * @param [in] v The value to write.
   * @param [in] response True == request write response.
   * @details Only used if the <type\> has a `data()` and `size()` method with
   * `value_type`. Correctly calculates byte size for containers with multi-byte
   * element types.
   */
  template <typename T>
#ifdef _DOXYGEN_
...
#endif
  writeValue(const T &v, bool response = false) const {
    return writeValue(reinterpret_cast<const uint8_t *>(v.data()),
                      v.size() * sizeof(typename T::value_type), response);
  }
...
#endif
  writeValue(const T &v, bool response = false) const {
    return writeValue(reinterpret_cast<const uint8_t *>(v.data()), v.size(),
                      response);
  }
  ...
  writeValue(const T &v, bool response = false) const {
    if constexpr (Has_data_size<T>::value) {
      ...
      return writeValue(reinterpret_cast<const uint8_t *>(v.c_str()),
                        v.length(), response);
    } else {
      return writeValue(reinterpret_cast<const uint8_t *>(&v), sizeof(v),
                        response);
    }
  }
#endif
   ...
   * @param [in] timestamp A pointer to a time_t struct to store the time the
   * value was read.
   * @param [in] skipSizeCheck If true it will skip checking if the data size is
   * less than <tt>sizeof(<type\>)</tt>.
   * @return The data converted to <type\> or NULL if skipSizeCheck is false and
   * the data is less than <tt>sizeof(<type\>)</tt>.
...
protected:
   ...
   */
  NimBLERemoteValueAttribute(const ble_uuid_any_t &uuid, uint16_t handle)
  ...
  static int onReadCB(uint16_t conn_handle, const ble_gatt_error *error,
                      ble_gatt_attr *attr, void *arg);
  static int onWriteCB(uint16_t conn_handle, const ble_gatt_error *error,
                       ble_gatt_attr *attr, void *arg);
};
