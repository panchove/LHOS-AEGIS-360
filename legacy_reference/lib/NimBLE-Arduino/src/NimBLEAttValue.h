...
/* Used to determine if the type passed to a template has a data() and size()
 * method. */
template <typename T, typename = void, typename = void>
struct Has_data_size : std::false_type {};

template <typename T>
struct Has_data_size<T, decltype(void(std::declval<T &>().data())),
                     decltype(void(std::declval<T &>().size()))>
    : std::true_type {};

/* Used to determine if the type passed to a template has a c_str() and length()
 * method. */
template <typename T, typename = void, typename = void>
struct Has_c_str_length : std::false_type {};

template <typename T>
struct Has_c_str_length<T, decltype(void(std::declval<T &>().c_str())),
                        decltype(void(std::declval<T &>().length()))>
    : std::true_type {};

/* Used to determine if the type passed to a template has a value_type member
 * (std::vector, std::array, std::string, etc.). */
...
public:
  ...
  NimBLEAttValue(const std::string str, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN)
      : NimBLEAttValue(reinterpret_cast<const uint8_t *>(&str[0]), str.length(),
                       max_len) {}
   ...
   * @param[in] max_len The max size in bytes that the value can be.
   */
  NimBLEAttValue(const std::vector<uint8_t> vec,
                 uint16_t max_len = BLE_ATT_ATTR_MAX_LEN)
  ...
  NimBLEAttValue(const String str, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN)
      : NimBLEAttValue(reinterpret_cast<const uint8_t *>(str.c_str()),
                       str.length(), max_len) {}
#endif
  ...
  /** @brief Returns a pointer to the internal buffer of the value as a const
   * char* */
  const char *c_str() const {
    return reinterpret_cast<const char *>(m_attr_value);
  }
   ...
   * and `size()` method. <type\> size must be evaluatable by `sizeof()`.
   */
  template <typename T>
#ifdef _DOXYGEN_
  bool
...
#endif
  setValue(const T &v) {
    return setValue(reinterpret_cast<const uint8_t *>(v.data()),
  ...
  setValue(const T &s) {
    if constexpr (Has_data_size<T>::value) {
      if constexpr (Has_value_type<T>::value) {
        return setValue(reinterpret_cast<const uint8_t *>(s.data()),
  ...
  operator std::string() const {
    return std::string(reinterpret_cast<char *>(m_attr_value), m_attr_len);
  }
  ...
  bool operator==(const NimBLEAttValue &source) const {
    return (m_attr_len == source.size())
             ? memcmp(m_attr_value, source.data(), m_attr_len) == 0
             : false;
  }
