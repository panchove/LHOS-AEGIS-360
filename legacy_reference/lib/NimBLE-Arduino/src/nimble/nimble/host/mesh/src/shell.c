...
void bt_mesh_set_gen_onoff_srv_cb(int (*get)(struct bt_mesh_model *model,
                                             uint8_t *state),
                                  int (*set)(struct bt_mesh_model *model,
                                             uint8_t state)) {
  gen_onoff_srv.get = get;
  gen_onoff_srv.set = set;
}

void bt_mesh_set_gen_level_srv_cb(int (*get)(struct bt_mesh_model *model,
                                             int16_t *level),
                                  int (*set)(struct bt_mesh_model *model,
                                             int16_t level)) {
...
void bt_mesh_set_light_lightness_srv_cb(int (*get)(struct bt_mesh_model *model,
                                                   int16_t *level),
                                        int (*set)(struct bt_mesh_model *model,
                                                   int16_t level)) {
...
static void prov_complete(uint16_t net_idx, uint16_t addr) {
  printk("Local node provisioned, net_idx 0x%04x address 0x%04x\n", net_idx,
         addr);
  net.local = addr;
  net.net_idx = net_idx, net.dst = addr;
}
...
static void prov_input_complete(void) { printk("Input complete"); }
...
struct shell_cmd_help cmd_input_num_help = {NULL, "<number>", NULL};
...
struct shell_cmd_help cmd_input_str_help = {NULL, "<string>", NULL};
...
struct shell_cmd_help cmd_static_oob_help = {NULL, "[val: 1-16 hex values]",
                                             NULL};
...
struct shell_cmd_help cmd_uuid_help = {NULL, "<UUID: 1-16 hex values>", NULL};
...
struct shell_cmd_help cmd_reset_help = {NULL, "<addr>", NULL};
...
static bool str2bool(const char *str) { return str2u8(str); }
...
struct shell_cmd_help cmd_lpn_help = {NULL, "<value: off, on>", NULL};
...
struct shell_cmd_help cmd_dst_help = {NULL, "[destination address]", NULL};
...
struct shell_cmd_help cmd_netidx_help = {NULL, "[NetIdx]", NULL};
...
struct shell_cmd_help cmd_appidx_help = {NULL, "[AppIdx]", NULL};
...
struct shell_cmd_help cmd_net_send_help = {NULL, "<hex string>", NULL};
...
struct shell_cmd_help cmd_lpn_subscribe_help = {NULL, "<addr>", NULL};
...
struct shell_cmd_help cmd_lpn_unsubscribe_help = {NULL, "<addr>", NULL};
#endif
...
struct shell_cmd_help cmd_iv_update_test_help = {NULL, "<value: off, on>",
                                                 NULL};
#endif
...
struct shell_cmd_help cmd_timeout_help = {NULL, "[timeout in seconds]", NULL};
...
static int cmd_get_comp(int argc, char *argv[]) {
  ...
  err = bt_mesh_cfg_comp_data_get(net.net_idx, net.dst, page, &page, buf);
  if (err) {
  ...
  if (page != 0x00) {
    printk("Got page 0x%02x. No parser available.", page);
    goto done;
  }
  ...
  while (bt_mesh_comp_p0_elem_pull(&comp, &elem)) {
    ...
    for (i = 0; i < elem.nvnd; i++) {
      ...
      printk("\t\t\tCompany 0x%04x: 0x%04x", mod.company, mod.id);
    }
  }
  if (buf->om_len) {
...
struct shell_cmd_help cmd_get_comp_help = {NULL, "[page]", NULL};
...
struct shell_cmd_help cmd_beacon_help = {NULL, "[val: off, on]", NULL};

struct shell_cmd_help cmd_beacon_listen_help = {NULL, "[val: off, on]", NULL};
...
struct shell_cmd_help cmd_ttl_help = {NULL, "[ttl: 0x00, 0x02-0x7f]", NULL};
...
struct shell_cmd_help cmd_friend_help = {NULL, "[val: off, on]", NULL};
...
struct shell_cmd_help cmd_gatt_proxy_help = {NULL, "[val: off, on]", NULL};
...
static int cmd_net_transmit(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    err = bt_mesh_cfg_net_transmit_get(net.net_idx, net.dst, &transmit);
  } else {
    if (argc != 3) {
    ...
    err = bt_mesh_cfg_net_transmit_set(net.net_idx, net.dst, new_transmit,
                                       &transmit);
  }
  ...
  printk("Transmit 0x%02x (count %u interval %ums)", transmit,
...
static int cmd_relay(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    err = bt_mesh_cfg_relay_set(net.net_idx, net.dst, val, new_transmit, &relay,
                                &transmit);
  }
...
static int cmd_net_key_add(int argc, char *argv[]) {
  ...
  err = bt_mesh_cfg_net_key_add(net.net_idx, net.dst, key_net_idx, key_val,
                                &status);
  if (err) {
...
struct shell_cmd_help cmd_net_key_add_help = {NULL, "<NetKeyIndex> [val]",
                                              NULL};
...
struct shell_cmd_help cmd_net_key_get_help = {NULL, NULL, NULL};
...
static int cmd_net_key_del(int argc, char *argv[]) {
  ...
  if (status) {
    printk("NetKeyDel failed with status 0x%02x", status);
  } else {
...
struct shell_cmd_help cmd_net_key_del_help = {NULL, "<NetKeyIndex>", NULL};
...
static int cmd_app_key_add(int argc, char *argv[]) {
  ...
  err = bt_mesh_cfg_app_key_add(net.net_idx, net.dst, key_net_idx, key_app_idx,
                                key_val, &status);
  if (err) {
...
static int cmd_app_key_get(int argc, char *argv[]) {
  ...
  err =
    bt_mesh_cfg_app_key_get(net.net_idx, net.dst, net_idx, &status, keys, &cnt);
  if (err) {
  ...
  if (status) {
    printk("AppKeyGet failed with status 0x%02x", status);
    return 0;
  }

  printk("AppKeys for NetKey 0x%03x known by 0x%04x:", net_idx, net.dst);
  for (i = 0; i < cnt; i++) {
    printk("\t0x%03x", keys[i]);
  }
...
struct shell_cmd_help cmd_app_key_get_help = {NULL, "<NetKeyIndex>", NULL};
...
static int cmd_app_key_del(int argc, char *argv[]) {
  ...
  err = bt_mesh_cfg_app_key_del(net.net_idx, net.dst, key_net_idx, key_app_idx,
                                &status);
  if (err) {
  ...
  if (status) {
    printk("AppKeyDel failed with status 0x%02x", status);
  } else {
...
static int cmd_mod_app_bind(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    ...
    err = bt_mesh_cfg_mod_app_bind_vnd(net.net_idx, net.dst, elem_addr,
                                       mod_app_idx, mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_app_bind(net.net_idx, net.dst, elem_addr, mod_app_idx,
                                   mod_id, &status);
  }
...
static int cmd_mod_app_unbind(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_app_unbind_vnd(net.net_idx, net.dst, elem_addr,
                                         mod_app_idx, mod_id, cid, &status);
  } else {
  ...
  if (err) {
    printk("Unable to send Model App Unbind (err %d)", err);
    return 0;
  }

  if (status) {
    printk("Model App Unbind failed with status 0x%02x", status);
  } else {
...
static int cmd_mod_app_get(int argc, char *argv[]) {
  ...
  if (argc > 3) {
    ...
    err = bt_mesh_cfg_mod_app_get_vnd(net.net_idx, net.dst, elem_addr, mod_id,
                                      cid, &status, apps, &cnt);
  } else {
    err = bt_mesh_cfg_mod_app_get(net.net_idx, net.dst, elem_addr, mod_id,
                                  &status, apps, &cnt);
  }

  if (err) {
    printk("Unable to send Model App Get (err %d)", err);
    return 0;
  }

  if (status) {
    printk("Model App Get failed with status 0x%02x", status);
  } else {
    printk("Apps bound to Element 0x%04x, Model 0x%04x %s:", elem_addr, mod_id,
           argc > 3 ? argv[3] : "(SIG)");
...
static int cmd_mod_sub_add(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_sub_add_vnd(net.net_idx, net.dst, elem_addr, sub_addr,
                                      mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_add(net.net_idx, net.dst, elem_addr, sub_addr,
                                  mod_id, &status);
  }
...
static int cmd_mod_sub_del(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_sub_del_vnd(net.net_idx, net.dst, elem_addr, sub_addr,
                                      mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_del(net.net_idx, net.dst, elem_addr, sub_addr,
                                  mod_id, &status);
  }
...
static int cmd_mod_sub_add_va(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_sub_va_add_vnd(net.net_idx, net.dst, elem_addr, label,
                                         mod_id, cid, &sub_addr, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_va_add(net.net_idx, net.dst, elem_addr, label,
...
static int cmd_mod_sub_del_va(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_sub_va_del_vnd(net.net_idx, net.dst, elem_addr, label,
                                         mod_id, cid, &sub_addr, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_va_del(net.net_idx, net.dst, elem_addr, label,
                                     mod_id, &sub_addr, &status);
  }
...
static int cmd_mod_sub_get(int argc, char *argv[]) {
  ...
  if (argc > 3) {
    cid = strtoul(argv[3], NULL, 0);
    err = bt_mesh_cfg_mod_sub_get_vnd(net.net_idx, net.dst, elem_addr, mod_id,
                                      cid, &status, subs, &cnt);
  } else {
    err = bt_mesh_cfg_mod_sub_get(net.net_idx, net.dst, elem_addr, mod_id,
                                  &status, subs, &cnt);
  }
  ...
  if (status) {
    ...
    printk("Model Subscriptions for Element 0x%04x, "
           "Model 0x%04x %s:",
...
static int mod_pub_get(uint16_t addr, uint16_t mod_id, uint16_t cid) {
  ...
  if (cid == CID_NVAL) {
    err = bt_mesh_cfg_mod_pub_get(net.net_idx, net.dst, addr, mod_id, &pub,
                                  &status);
  } else {
    err = bt_mesh_cfg_mod_pub_get_vnd(net.net_idx, net.dst, addr, mod_id, cid,
                                      &pub, &status);
  }
...
static int mod_pub_set(uint16_t addr, uint16_t mod_id, uint16_t cid,
                       char *argv[]) {
  struct bt_mesh_cfg_mod_pub pub;
  ...
  if (cid == CID_NVAL) {
    err = bt_mesh_cfg_mod_pub_set(net.net_idx, net.dst, addr, mod_id, &pub,
                                  &status);
  } else {
    err = bt_mesh_cfg_mod_pub_set_vnd(net.net_idx, net.dst, addr, mod_id, cid,
                                      &pub, &status);
  }
...
struct shell_cmd_help cmd_hb_sub_help = {NULL, "<src> <dst> <period>", NULL};
...
struct shell_cmd_help cmd_pb_help = {NULL, "<val: off, on>", NULL};
...
static int cmd_provision(int argc, char *argv[]) {
  ...
  err = bt_mesh_provision(net_key, net_idx, 0, iv_index, addr, default_key);
  if (err) {
...
static int cmd_fault_get(int argc, char *argv[]) {
  ...
  err = bt_mesh_health_fault_get(net.dst, net.app_idx, cid, &test_id, faults,
                                 &fault_count);
  if (err) {
...
struct shell_cmd_help cmd_fault_get_help = {NULL, "<Company ID>", NULL};
...
static int cmd_fault_clear(int argc, char *argv[]) {
  ...
  err = bt_mesh_health_fault_clear(net.dst, net.app_idx, cid, &test_id, faults,
                                   &fault_count);
  if (err) {
...
struct shell_cmd_help cmd_fault_clear_help = {NULL, "<Company ID>", NULL};
...
static int cmd_fault_clear_unack(int argc, char *argv[]) {
  ...
  err = bt_mesh_health_fault_clear(net.dst, net.app_idx, cid, NULL, NULL, NULL);
  if (err) {
...
struct shell_cmd_help cmd_fault_clear_unack_help = {NULL, "<Company ID>", NULL};
...
static int cmd_fault_test(int argc, char *argv[]) {
  ...
  err = bt_mesh_health_fault_test(net.dst, net.app_idx, cid, test_id, faults,
                                  &fault_count);
  if (err) {
...
struct shell_cmd_help cmd_fault_test_help = {NULL, "<Company ID> <Test ID>",
                                             NULL};
...
static int cmd_fault_test_unack(int argc, char *argv[]) {
  ...
  err =
    bt_mesh_health_fault_test(net.dst, net.app_idx, cid, test_id, NULL, NULL);
  if (err) {
...
static int cmd_period_set(int argc, char *argv[]) {
  ...
  err =
    bt_mesh_health_period_set(net.dst, net.app_idx, divisor, &updated_divisor);
  if (err) {
...
struct shell_cmd_help cmd_period_set_help = {NULL, "<divisor>", NULL};
...
struct shell_cmd_help cmd_period_set_unack_help = {NULL, "<divisor>", NULL};

static int cmd_attention_get(int argc, char *argv[]) {
  uint8_t attention;
  int err;

  err = bt_mesh_health_attention_get(net.dst, net.app_idx, &attention);
  if (err) {
...
struct shell_cmd_help cmd_attention_set_help = {NULL, "<timer>", NULL};
...
static int cmd_attention_set_unack(int argc, char *argv[]) {
  ...
  err = bt_mesh_health_attention_set(net.dst, net.app_idx, attention, NULL);
  if (err) {
    printk("Failed to send Health Attention Set (err %d)\n", err);
  }
...
struct shell_cmd_help cmd_attention_set_unack_help = {NULL, "<timer>", NULL};
...
struct shell_cmd_help cmd_add_fault_help = {NULL, "<Fault ID>", NULL};
...
struct shell_cmd_help cmd_del_fault_help = {NULL, "[Fault ID]", NULL};

#if MYNEWT_VAL(BLE_MESH_CDB)
struct shell_cmd_help cmd_cdb_create_help = {NULL, "[NetKey]", NULL};
struct shell_cmd_help cmd_cdb_clear_help = {NULL, NULL, NULL};
struct shell_cmd_help cmd_cdb_show_help = {NULL, NULL, NULL};
struct shell_cmd_help cmd_cdb_node_add_help = {NULL,
                                               "<UUID> <addr> <num-elem> "
                                               "<NetKeyIdx> [DevKey]",
                                               NULL};
struct shell_cmd_help cmd_cdb_node_del_help = {NULL, "<addr>", NULL};
struct shell_cmd_help cmd_cdb_subnet_add_help = {NULL, "<NeyKeyIdx> [<NetKey>]",
                                                 NULL};
struct shell_cmd_help cmd_cdb_subnet_del_help = {NULL, "<NetKeyIdx>", NULL};
struct shell_cmd_help cmd_cdb_app_key_add_help = {
  NULL, "<NetKeyIdx> <AppKeyIdx> [<AppKey>]", NULL};
struct shell_cmd_help cmd_cdb_app_key_del_help = {NULL, "<AppKeyIdx>", NULL};
...
static void cdb_print_nodes(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.nodes); ++i) {
    ...
    printk("0x%04x   %-8d  %-5s  %s  %s", node->addr, node->num_elem,
           configured ? "C" : "-", uuid_hex_str, key_hex_str);
  }
...
static void cdb_print_subnets(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.subnets); ++i) {
    ...
    total++;
    bin2hex(subnet->keys[0].net_key, 16, key_hex_str, sizeof(key_hex_str));
    printk("0x%03x   %s", subnet->net_idx, key_hex_str);
  }
...
static void cdb_print_app_keys(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.app_keys); ++i) {
    ...
    total++;
    bin2hex(app_key->keys[0].app_key, 16, key_hex_str, sizeof(key_hex_str));
    printk("0x%03x   0x%03x   %s", app_key->net_idx, app_key->app_idx,
           key_hex_str);
  }
...
static int cmd_cdb_subnet_add(int argc, char *argv[]) {
...
static int cmd_cdb_subnet_del(int argc, char *argv[]) {
  struct bt_mesh_cdb_subnet *sub;
  uint16_t net_idx;
...
static int cmd_cdb_app_key_add(int argc, char *argv[]) {
...
static int cmd_cdb_app_key_del(int argc, char *argv[]) {
  struct bt_mesh_cdb_app_key *key;
  uint16_t app_idx;
...
static int cmd_gen_onoff_get(int argc, char *argv[]) {
  ...
  err = bt_mesh_gen_onoff_get(net.net_idx, net.dst, net.app_idx, &state);
  if (err) {
...
static int cmd_gen_onoff_set(int argc, char *argv[]) {
  ...
  err = bt_mesh_gen_onoff_set(net.net_idx, net.dst, net.app_idx, val, &state);
  if (err) {
    printk("Failed to send Generic OnOff Get (err %d)\n", err);
  } else {
    printk("Gen OnOff State %d\n", state);
...
struct shell_cmd_help cmd_gen_onoff_set_help = {NULL, "<0|1>", NULL};
...
static int cmd_gen_onoff_set_unack(int argc, char *argv[]) {
  ...
  err = bt_mesh_gen_onoff_set(net.net_idx, net.dst, net.app_idx, val, NULL);
  if (err) {
    printk("Failed to send Generic OnOff Get (err %d)\n", err);
  }
...
struct shell_cmd_help cmd_gen_onoff_set_unack_help = {NULL, "<0|1>", NULL};

static int cmd_gen_level_get(int argc, char *argv[]) {
  int16_t state;
  int err;

  err = bt_mesh_gen_level_get(net.net_idx, net.dst, net.app_idx, &state);
  if (err) {
...
static int cmd_gen_level_set(int argc, char *argv[]) {
  ...
  err = bt_mesh_gen_level_set(net.net_idx, net.dst, net.app_idx, val, &state);
  if (err) {
    printk("Failed to send Generic Level Get (err %d)\n", err);
  } else {
    printk("Gen Level State %d\n", state);
...
struct shell_cmd_help cmd_gen_level_set_help = {NULL, "<level>", NULL};
...
static int cmd_gen_level_set_unack(int argc, char *argv[]) {
  ...
  err = bt_mesh_gen_level_set(net.net_idx, net.dst, net.app_idx, val, NULL);
  if (err) {
    printk("Failed to send Generic Level Get (err %d)\n", err);
  }
...
struct shell_cmd_help cmd_gen_level_set_unack_help = {NULL, "<level>", NULL};
...
static void print_comp_elem(struct bt_mesh_elem *elem, bool primary) {
