...
NimBLERemoteService::NimBLERemoteService(NimBLEClient *pClient,
    ...
    : NimBLEAttribute{service->uuid, service->start_handle}, m_pClient{pClient},
      m_endHandle{service->end_handle} {}

/**
 * @brief When deleting the service make sure we delete all characteristics and
 * descriptors.
 */
NimBLERemoteService::~NimBLERemoteService() { deleteCharacteristics(); }

/**
 * @brief Get iterator to the beginning of the vector of remote characteristic
 * pointers.
 * @return An iterator to the beginning of the vector of remote characteristic
 * pointers.
 */
std::vector<NimBLERemoteCharacteristic *>::iterator
NimBLERemoteService::begin() const {
  return m_vChars.begin();
}

/**
 * @brief Get iterator to the end of the vector of remote characteristic
 * pointers.
 * @return An iterator to the end of the vector of remote characteristic
 * pointers.
 */
std::vector<NimBLERemoteCharacteristic *>::iterator
NimBLERemoteService::end() const {
  return m_vChars.end();
}
...
NimBLERemoteService::getCharacteristic(const char *uuid) const {
 ...
 */
NimBLERemoteCharacteristic *
NimBLERemoteService::getCharacteristic(const NimBLEUUID &uuid) const {
  NIMBLE_LOGD(LOG_TAG, ">> getCharacteristic: uuid: %s",
              uuid.toString().c_str());
 ...
 * @param [in] refresh If true the current characteristics vector will cleared
 * and all characteristics for this service retrieved from the peripheral. If
 * false the vector will be returned with the currently stored characteristics
 * of this service.
 * @return A read-only reference to the vector of characteristics retrieved for
 * this service.
 */
const std::vector<NimBLERemoteCharacteristic *> &
NimBLERemoteService::getCharacteristics(bool refresh) const {
  if (refresh) {
...
int NimBLERemoteService::characteristicDiscCB(uint16_t conn_handle,
                                              ...
                                              void *arg) {
  NIMBLE_LOGD(LOG_TAG, "Characteristic Discovery >> status: %d handle: %d",
              error->status, (error->status == 0) ? chr->def_handle : -1);
...
bool NimBLERemoteService::retrieveCharacteristics(
  const NimBLEUUID *uuidFilter) const {
  ...
  if (uuidFilter == nullptr) {
    rc = ble_gattc_disc_all_chrs(
      m_pClient->getConnHandle(), getHandle(), getEndHandle(),
      NimBLERemoteService::characteristicDiscCB, &taskData);
  } else {
    rc = ble_gattc_disc_chrs_by_uuid(m_pClient->getConnHandle(), getHandle(),
                                     getEndHandle(), uuidFilter->getBase(),
                                     NimBLERemoteService::characteristicDiscCB,
                                     &taskData);
  }

  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gattc_disc_chrs rc=%d %s", rc,
  ...
  NIMBLE_LOGE(LOG_TAG, "<< retrieveCharacteristics() rc=%d %s", rc,
              NimBLEUtils::returnCodeToString(rc));
  return false;
} // retrieveCharacteristics
 ...
 * @returns a string containing the value or an empty string if not found or
 * error.
 */
...
bool NimBLERemoteService::setValue(const NimBLEUUID &uuid,
                                   const NimBLEAttValue &value) const {
  const auto pChar = getCharacteristic(uuid);
  if (pChar) {
 ...
 * @details We maintain a vector called m_characteristicsVector that contains
 * pointers to BLERemoteCharacteristic object references. Since we allocated
 * these in this class, we are also responsible for deleting them. This method
 * does just that.
 */
 ...
 * @param [in] uuid The UUID of the characteristic to be removed from the local
 * database.
...
std::string NimBLERemoteService::toString() const {
  std::string res =
    "Service: uuid: " + m_uuid.toString() + ", start_handle: 0x";
  char val[5];
