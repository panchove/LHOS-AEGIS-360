...
public:
  ...
  bool disconnect(uint16_t connHandle,
                  uint8_t reason = BLE_ERR_REM_USER_CONN_TERM) const;
  bool disconnect(const NimBLEConnInfo &connInfo,
                  uint8_t reason = BLE_ERR_REM_USER_CONN_TERM) const;
  void setCallbacks(NimBLEServerCallbacks *pCallbacks,
                    bool deleteCallbacks = true);
  void updateConnParams(uint16_t connHandle, uint16_t minInterval,
  ...
  bool startAdvertising(uint8_t instanceId, int duration = 0,
                        int maxEvents = 0) const;
...
private:
  ...
  static int handleGattEvent(uint16_t connHandle, uint16_t attrHandle,
...
public:
   ...
   * @param [in] pServer A pointer to the BLE server that received the client
   * connection.
   * @param [in] connInfo A reference to a NimBLEConnInfo instance with
   * information. about the peer connection parameters.
   */
   ...
   * @param [in] connInfo A reference to a NimBLEConnInfo instance with
   * information Should be passed back to NimBLEDevice::injectConfirmPasskey
   ...
   * @param [in] connInfo A reference to a NimBLEConnInfo instance containing
   ...
   */
  virtual void onPhyUpdate(NimBLEConnInfo &connInfo, uint8_t txPhy,
                           uint8_t rxPhy);
