...
protected:
  ...
  NimBLELocalValueAttribute(
    const NimBLEUUID &uuid, uint16_t handle, uint16_t maxLen,
    uint16_t initLen = CONFIG_NIMBLE_CPP_ATT_VALUE_INIT_LENGTH)
      : NimBLELocalAttribute(uuid, handle),
        NimBLEValueAttribute(maxLen, initLen) {}
  /**
   ...
   * the peer info.
   * @details This function is called by NimBLEServer when a read request is
   * received.
   */
   ...
   * @param [in] connInfo A reference to a NimBLEConnInfo instance containing
   * the peer info.
   * @details This function is called by NimBLEServer when a write request is
   * received.
   */
  virtual void writeEvent(const uint8_t *val, uint16_t len,
                          NimBLEConnInfo &connInfo) = 0;

  /**
   * @brief Get a pointer to value of the attribute.
   * @return A pointer to the value of the attribute.
   * @details This function is used by NimBLEServer when handling read/write
   * requests.
   */
