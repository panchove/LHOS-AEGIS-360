...
static void ble_gap_update_l2cap_cb(uint16_t conn_handle, int status,
                                    void *arg);
#endif
...
#if NIMBLE_BLE_ADVERTISE && !MYNEWT_VAL(BLE_EXT_ADV)
static void ble_gap_log_adv(uint8_t own_addr_type,
...
static int ble_gap_find_snapshot(uint16_t handle,
                                 struct ble_gap_snapshot *snap) {
...
int ble_gap_read_rem_ver_info(uint16_t conn_handle, uint8_t *version,
...
int ble_gap_conn_find_handle_by_addr(const ble_addr_t *addr,
...
static int ble_gap_conn_foreach_handle_callback(struct ble_hs_conn *conn,
                                                void *arg) {
...
void ble_gap_conn_foreach_handle(ble_gap_conn_foreach_handle_fn *cb,
                                 void *arg) {
...
#if NIMBLE_BLE_CONNECT
static int ble_gap_extract_conn_cb(uint16_t conn_handle,
                                   ble_gap_event_fn **out_cb,
                                   void **out_cb_arg) {
...
int ble_gap_read_le_phy(uint16_t conn_handle, uint8_t *tx_phy,
                        uint8_t *rx_phy) {
#if NIMBLE_BLE_CONNECT
...
uint8_t *ble_resolve_adv_data(const uint8_t *adv_data, uint8_t adv_type,
                              uint8_t adv_data_len, uint8_t *length) {
  int rc = 0;
  const struct ble_hs_adv_field *fields;
  const uint8_t *data;

  rc = ble_hs_adv_find_field(adv_type, adv_data, adv_data_len,
                             &fields); /*Fill adv field*/
...
static int ble_gap_call_event_cb(struct ble_gap_event *event,
...
#if NIMBLE_BLE_CONNECT
static int ble_gap_call_conn_event_cb(struct ble_gap_event *event,
                                      uint16_t conn_handle) {
  ble_gap_event_fn *cb;
  void *cb_arg;
  int rc;
...
#if NIMBLE_BLE_ADVERTISE || NIMBLE_BLE_CONNECT
static void ble_gap_slave_extract_cb(uint8_t instance,
                                     ble_gap_event_fn **out_cb,
                                     void **out_cb_arg) {
  ble_hs_lock();
...
static void ble_gap_adv_finished(uint8_t instance, int reason,
                                 uint16_t conn_handle, uint8_t num_events) {
...
int ble_gap_master_connect_reattempt(uint16_t conn_handle) {
  struct ble_gap_snapshot snap;
  ...
  if (conn.role == BLE_GAP_ROLE_MASTER) {
...
#if MYNEWT_VAL(OPTIMIZE_MULTI_CONN)
    /* This reattempt will be done automatically. The `scheduling_len` maybe set
     * in the ble_gap_multi_connect(). */
    ble_gap_multi_conn.scheduling_len_set = true;
#endif // MYNEWT_VAL(OPTIMIZE_MULTI_CONN)

#if MYNEWT_VAL(BLE_EXT_ADV)
    rc = ble_gap_ext_connect(
      ble_conn_reattempt.own_addr_type,
      (ble_conn_reattempt.peer_addr_present == 1 ? &ble_conn_reattempt.peer_addr
                                                 : NULL),
      ble_conn_reattempt.duration_ms, ble_conn_reattempt.phy_mask,
      ble_conn_reattempt.phy_mask & BLE_GAP_LE_PHY_1M_MASK
        ? &ble_conn_reattempt.conn_params_1m
        : NULL,
      ble_conn_reattempt.phy_mask & BLE_GAP_LE_PHY_2M_MASK
        ? &ble_conn_reattempt.conn_params_2m
        : NULL,
      ble_conn_reattempt.phy_mask & BLE_GAP_LE_PHY_CODED_MASK
        ? &ble_conn_reattempt.conn_params_coded
        : NULL,
    ...
    rc = ble_gap_connect(
      ...
      (ble_conn_reattempt.peer_addr_present == 1 ? &ble_conn_reattempt.peer_addr
                                                 : NULL),
      ble_conn_reattempt.duration_ms, &ble_conn_reattempt.conn_params_1m,
      ble_conn_reattempt.cb, ble_conn_reattempt.cb_arg);
...
int ble_gap_slave_adv_reattempt(void) {
  ...
  case 0:
    ...
    rc = ble_gap_adv_start(ble_adv_reattempt.own_addr_type,
                           (ble_adv_reattempt.direct_addr_present == 1
                              ? &ble_adv_reattempt.direct_addr
                              : NULL),
                           ble_adv_reattempt.duration_ms,
                           &ble_adv_reattempt.adv_params, ble_adv_reattempt.cb,
                           ble_adv_reattempt.cb_arg);
    if (rc != 0) {
      return rc;
    }
    break;
  ...
  case 1:
    ...
    rc = ble_gap_ext_adv_configure(
      ble_adv_reattempt.instance, &ble_adv_reattempt.params,
      (ble_adv_reattempt.selected_tx_power_present == 1
         ? &ble_adv_reattempt.selected_tx_power
         : NULL),
    ...
    rc = ble_gap_ext_adv_set_data(
      ble_adv_reattempt.instance,
      ble_hs_mbuf_from_flat(ble_adv_reattempt.data,
                            ble_adv_reattempt.data_len));

    if (rc != 0) {
      return rc;
    }

    rc = ble_gap_ext_adv_start(ble_adv_reattempt.instance,
                               ble_adv_reattempt.duration,
...
void ble_gap_rx_update_complete(
  ...
  if (conn != NULL) {
    ...
    case BLE_ERR_UNSUPP_REM_FEATURE:
      ...
      if (entry != NULL && !(conn->bhc_flags & BLE_HS_CONN_F_MASTER)) {
        ...
        entry->exp_os_ticks = ble_npl_time_get() + ble_npl_time_ms_to_ticks32(
...
static int ble_gap_rx_adv_report_sanity_check(const uint8_t *adv_data,
  ...
  rc = ble_hs_adv_find_field(BLE_HS_ADV_TYPE_FLAGS, adv_data, adv_data_len,
                             &flags);
  ...
  if (ble_gap_master.disc.limited) {
    ...
    if (rc != 0) {
      ...
      return -1;
               ...
               !(flags->value[0] &
                 (BLE_HS_ADV_F_DISC_LTD | BLE_HS_ADV_F_DISC_GEN))) {
...
#if MYNEWT_VAL(BLE_EXT_ADV)
#if NIMBLE_BLE_SCAN
void ble_gap_rx_le_scan_timeout(void) { ble_gap_disc_complete(); }
...
void ble_gap_rx_adv_set_terminated(
...
static void ble_gap_slave_get_cb(uint8_t instance, ble_gap_event_fn **out_cb,
                                 void **out_cb_arg) {
  ble_hs_lock();
...
void ble_gap_rx_scan_req_rcvd(
  const struct ble_hci_ev_le_subev_scan_req_rcvd *ev) {
  struct ble_gap_event event;
  ble_gap_event_fn *cb;
  void *cb_arg;
    ...
    if (ev->status == BLE_ERR_CONN_ESTABLISHMENT) {
      if (ble_conn_reattempt.count < MAX_REATTEMPT_ALLOWED) {
        ...
        rc = ble_gap_periodic_adv_sync_create(
          &ble_conn_reattempt.periodic_addr, ble_conn_reattempt.adv_sid,
          &ble_conn_reattempt.periodic_params, ble_conn_reattempt.cb,
...
void ble_gap_rx_periodic_adv_rpt(
...
void ble_gap_rx_connless_iq_report(
  const struct ble_hci_ev_le_subev_connless_iq_rpt *ev) {
  struct ble_hs_periodic_sync *psync;
  struct ble_gap_event event;
  ble_gap_event_fn *cb = NULL;
  void *cb_arg = NULL;
  ...
  event.connless_iq_report.periodic_event_counter =
    le16toh(ev->periodic_event_counter);
  event.connless_iq_report.sample_count = ev->sample_count;
  event.connless_iq_report.i_samples = (int8_t *)(ev->iq_samples);
  event.connless_iq_report.q_samples =
    (int8_t *)(ev->iq_samples + ev->sample_count);
...
void ble_gap_rx_conn_iq_report(
  const struct ble_hci_ev_le_subev_conn_iq_rpt *ev) {
  struct ble_gap_event event;
  memset(&event, 0, sizeof event);
  ...
  event.conn_iq_report.q_samples =
    (int8_t *)(ev->iq_samples + ev->sample_count);
...
void ble_gap_rx_cte_req_failed(
  const struct ble_hci_ev_le_subev_cte_req_failed *ev) {
  struct ble_gap_event event;
  uint16_t conn_handle;
  conn_handle = le16toh(ev->conn_handle);
...
void ble_gap_rx_periodic_adv_sync_transfer(
  ...
  if (!conn || !conn->psync) {
    ...
    if (!ev->status) {
      opcode =
        BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_PERIODIC_ADV_TERM_SYNC);
...
void ble_gap_rx_biginfo_adv_rpt(
...
void ble_gap_rx_subrate_change(
  const struct ble_hci_ev_le_subev_subrate_change *ev) {
  struct ble_gap_event event;
  uint16_t conn_handle;
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
void ble_gap_rx_periodic_adv_subev_data_req(
  const struct ble_hci_ev_le_subev_periodic_adv_subev_data_req *ev) {
  struct ble_gap_event event;
  ble_gap_event_fn *cb;
  void *cb_arg;
...
void ble_gap_rx_periodic_adv_response(
  const struct ble_gap_periodic_adv_response resp) {
  struct ble_gap_event event;
  ble_gap_event_fn *cb;
  void *cb_arg;
...
static int ble_gap_rd_rem_sup_feat_tx(uint16_t handle) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RD_REM_FEAT), &cmd, sizeof(cmd),
    NULL, 0);
}
#endif
static int ble_gap_rd_rem_ver_tx(uint16_t handle) {
  struct ble_hci_rd_rem_ver_info_cp cmd;

  cmd.conn_handle = htole16(handle);

  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LINK_CTRL, BLE_HCI_OCF_RD_REM_VER_INFO), &cmd,
    sizeof(cmd), NULL, 0);
}
 ...
 */
int ble_gap_rx_conn_complete(struct ble_gap_conn_complete *evt,
                             uint8_t instance) {
#if NIMBLE_BLE_CONNECT
  struct ble_hs_conn *conn;
  int rc;
  ...
  if (evt->role == BLE_HCI_LE_CONN_COMPLETE_ROLE_MASTER) {
    ...
    /* This is also done when bonding is restored, so `conn` and
     * `ble_gatts_conn_aware_states` need to be kept in sync */
    ble_hs_conn_addrs(conn, &addrs);
    for (int i = 0; i < MYNEWT_VAL(BLE_STORE_MAX_BONDS); i++) {
      if (memcmp(ble_gatts_conn_aware_states[i].peer_id_addr,
        ...
        conn->bhc_gatt_svr.half_aware =
          ble_gatts_conn_aware_states[i].half_aware;
...
void ble_gap_rx_rd_rem_sup_feat_complete(
...
void ble_gap_rx_rd_rem_ver_info_complete(
  const struct ble_hci_ev_rd_rem_ver_info_cmp *ev) {
#if NIMBLE_BLE_CONNECT
  struct ble_hs_conn *conn;
...
void ble_gap_rx_phy_update_complete(
...
void ble_gap_rx_data_len_change(
  const struct ble_hci_ev_le_subev_data_len_chg *ev) {
#if NIMBLE_BLE_CONNECT
  struct ble_gap_event event;
  uint16_t conn_handle = le16toh(ev->conn_handle);
...
int ble_gap_set_event_cb(uint16_t conn_handle, ble_gap_event_fn *cb,
                         void *cb_arg) {
#if NIMBLE_BLE_CONNECT
  struct ble_hs_conn *conn;
...
int ble_gap_wl_tx_add(const ble_addr_t *addr) {
...
int ble_gap_wl_tx_clear(void) {
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_CLEAR_WHITE_LIST), NULL, 0, NULL,
    0);
}
...
int ble_gap_wl_tx_rmv(const ble_addr_t *addr) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RMV_WHITE_LIST), &cmd,
    sizeof(cmd), NULL, 0);
}
...
static int ble_gap_adv_params_tx(uint8_t own_addr_type,
...
static int ble_gap_adv_validate(uint8_t own_addr_type,
                                const ble_addr_t *peer_addr,
                                const struct ble_gap_adv_params *adv_params) {
  if (adv_params == NULL) {
    return BLE_HS_EINVAL;
...
int ble_gap_adv_start(uint8_t own_addr_type, const ble_addr_t *direct_addr,
  ...
  memcpy(&ble_adv_reattempt.adv_params, adv_params,
         sizeof(struct ble_gap_adv_params));
...
#if MYNEWT_VAL(BLE_ENABLE_CONN_REATTEMPT)
  memcpy(&ble_adv_reattempt.fields, adv_fields,
         sizeof(struct ble_hs_adv_fields));
#endif
...
#if MYNEWT_VAL(BLE_EXT_ADV)
static int ble_gap_set_ext_adv_params(
  struct ble_hci_le_set_ext_adv_params_cp *cmd, uint8_t instance,
  const struct ble_gap_ext_adv_params *params, int8_t *selected_tx_power) {
...
ble_gap_ext_adv_params_tx_v2(uint8_t instance,
  ...
  rc =
    ble_gap_set_ext_adv_params(&(cmd.cmd), instance, params, selected_tx_power);
...
int ble_gap_ext_adv_configure(uint8_t instance,
  ...
  memcpy(&ble_adv_reattempt.params, params,
...
static int ble_gap_ext_adv_set_addr_no_lock(uint8_t instance,
                                            const uint8_t *addr) {
...
int ble_gap_ext_adv_set_addr(uint8_t instance, const ble_addr_t *addr) {
  ...
  /*
     ...
     following requirements: The two most significant bits of the address shall
     be equal to 1 All bits of the random part of the address shall not be equal
     to 1 All bits of the random part of the address shall not be equal to 0
   */
  ...
  if ((addr->val[5] & BLE_STATIC_RAND_ADDR_MASK) == BLE_STATIC_RAND_ADDR_MASK) {
    invalid_static_rand_addr.val[5] =
      invalid_static_rand_addr.val[5] | BLE_STATIC_RAND_ADDR_MASK;

    if (memcmp(invalid_non_rpa_addr.val, addr->val, BLE_DEV_ADDR_LEN) == 0 ||
        memcmp(invalid_static_rand_addr.val, addr->val, BLE_DEV_ADDR_LEN) ==
          0) {
      return BLE_HS_EINVAL;
    }
  } else if ((addr->val[5] | BLE_NON_RPA_MASK) == BLE_NON_RPA_MASK) {
    invalid_non_rpa_addr.val[5] =
      invalid_non_rpa_addr.val[5] & BLE_NON_RPA_MASK;
...
static int ble_gap_ext_adv_set_data_validate(uint8_t instance,
                                             struct os_mbuf *data) {
...
static int ble_gap_ext_adv_set(uint8_t instance, uint16_t opcode,
                               struct os_mbuf **data) {
  ...
  *data = os_mbuf_trim_front(*data);
  ...
  return ble_hs_hci_cmd_tx(opcode, cmd, sizeof(*cmd) + cmd->adv_data_len, NULL,
                           0);
#else
  static uint8_t buf[sizeof(struct ble_hci_le_set_ext_adv_data_cp) +
  ...
  do {
    ...
    *data = os_mbuf_trim_front(*data);
    ...
    rc =
      ble_hs_hci_cmd_tx(opcode, cmd, sizeof(*cmd) + cmd->adv_data_len, NULL, 0);
    if (rc) {
      return rc;
    }
...
int ble_gap_ext_adv_set_data(uint8_t instance, struct os_mbuf *data) {
  ...
  if (!ble_adv_reattempt.retry) {
    ble_hs_mbuf_to_flat(data, ble_adv_reattempt.data, len,
                        &ble_adv_reattempt.data_len);
  } else {
...
static int ble_gap_ext_adv_rsp_set_validate(uint8_t instance,
                                            struct os_mbuf *data) {
  uint16_t len = OS_MBUF_PKTLEN(data);
...
int ble_gap_ext_adv_rsp_set_data(uint8_t instance, struct os_mbuf *data) {
  ...
  rc =
    ble_gap_ext_adv_set(instance, BLE_HCI_OCF_LE_SET_EXT_SCAN_RSP_DATA, &data);
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_PERIODIC_ADV_PARAMS_V2);
#else
...
static int ble_gap_periodic_adv_params_validate(
  const struct ble_gap_periodic_adv_params *params) {
  if (!params) {
    return BLE_HS_EINVAL;
  }
...
int ble_gap_periodic_adv_configure(
  uint8_t instance, const struct ble_gap_periodic_adv_params *params) {
  int rc;
...
static int ble_gap_periodic_adv_set_data_validate(uint8_t instance,
                                                  struct os_mbuf *data) {
  /* The corresponding extended advertising instance should be configured */
  if (!ble_gap_slave[instance].configured) {
    return BLE_HS_EINVAL;
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_ENH)
int ble_gap_periodic_adv_set_data(
...
int ble_gap_periodic_adv_sync_create(
  ...
  if (params != NULL) {
    memcpy(&ble_conn_reattempt.periodic_params, params,
           sizeof(struct ble_gap_periodic_sync_params));
  }
...
int ble_gap_periodic_adv_sync_create_cancel(void) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_PERIODIC_ADV_CREATE_SYNC_CANCEL);
...
int ble_gap_periodic_adv_sync_set_info(uint8_t instance, uint16_t conn_handle,
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_PERIODIC_ADV_SET_INFO_TRANSFER);
...
static int periodic_adv_transfer_enable(
...
int periodic_adv_set_default_sync_params(
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_DEFAULT_SYNC_TRANSFER_PARAMS);
...
int ble_gap_periodic_adv_sync_receive(
  uint16_t conn_handle, const struct ble_gap_periodic_sync_params *params,
  ble_gap_event_fn *cb, void *cb_arg) {
  struct ble_hs_conn *conn;
  int rc;
...
int ble_gap_rem_dev_from_periodic_adv_list(const ble_addr_t *peer_addr,
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_REM_DEV_FROM_PERIODIC_ADV_LIST);
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
static int ble_gap_calc_periodic_adv_data_size(
...
int ble_gap_set_periodic_adv_subev_data(
  uint8_t instance, uint8_t num_subevents,
  const struct ble_gap_set_periodic_adv_subev_data_params *params) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_PERIODIC_ADV_SUBEV_DATA);
  ...
  for (int i = 0; i < num_subevents; i++) {
    ...
    ble_hs_mbuf_to_flat(params[i].data, subevents->subevent_data,
                        OS_MBUF_PKTLEN(params[i].data),
                        (void *)&subev_data_len);
    subevents->subevent_data_length = OS_MBUF_PKTLEN(params[i].data);
    subevents =
      (struct periodic_adv_subevents *)((uint32_t)subevents +
                                        sizeof(struct periodic_adv_subevents) +
                                        subev_data_len);
  }
...
int ble_gap_periodic_adv_set_response_data(
  uint16_t sync_handle, struct ble_gap_periodic_adv_response_params *param,
  struct os_mbuf *data) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_PERIODIC_ADV_RESPONSE_DATA);

  return ble_hs_hci_cmd_tx(opcode, cmd, len + data_len, NULL, 0);
}

int ble_gap_periodic_adv_sync_subev(uint16_t sync_handle,
  ...
  uint16_t opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_PERIODIC_ADV_SYNC_SUBEVENT);
  int rc;
  ...
  rc = ble_hs_hci_cmd_tx(opcode, cmd, sizeof(*cmd) + num_subevents, &rsp,
                         sizeof(rsp));
  if (!rc) {
...
static int ble_gap_ext_disc_tx_params(
...
#if MYNEWT_VAL(BLE_AOA_AOD)
int ble_gap_set_connless_cte_transmit_params(
  uint8_t instance, const struct ble_gap_periodic_adv_cte_params *params) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_CONNLESS_CTE_TX_PARAMS);
  ...
  memcpy(cmd->switching_pattern, params->antenna_ids,
         params->switching_pattern_length);

  return ble_hs_hci_cmd_tx(opcode, cmd, len, NULL, 0);
}

int ble_gap_set_connless_cte_transmit_enable(uint8_t instance,
                                             uint8_t cte_enable) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_CONNLESS_CTE_TX_ENABLE);
...
int ble_gap_set_connless_iq_sampling_enable(
  uint16_t sync_handle, uint8_t sampling_enable, uint8_t max_sampled_ctes,
  ...
  struct ble_hci_le_set_connless_iq_sampling_enable_cp *cmd =
    (struct ble_hci_le_set_connless_iq_sampling_enable_cp *)buf;
  struct ble_hci_le_set_connless_iq_sampling_enable_rp rsp;
  uint8_t len = sizeof(buf);
  uint16_t opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_CONNLESS_IQ_SAMPLING_ENABLE);
  ...
  memcpy(cmd->antenna_ids, cte_sampling_params->antenna_ids,
         cte_sampling_params->switching_pattern_length);

  return ble_hs_hci_cmd_tx(opcode, cmd, len, &rsp, sizeof(rsp));
}

int ble_gap_set_conn_cte_recv_param(
  uint16_t conn_handle, uint8_t sampling_enable,
  const struct ble_gap_cte_sampling_params *cte_sampling_params) {
  uint8_t buf[sizeof(struct ble_hci_le_set_conn_cte_rx_params_cp) +
              cte_sampling_params->switching_pattern_length];
...
int ble_gap_set_conn_cte_transmit_param(uint16_t conn_handle, uint8_t cte_types,
  ...
  uint8_t buf[sizeof(struct ble_hci_le_set_conn_cte_tx_params_cp) +
  ...
  if (cmd->cte_types != BIT(0) &&
      (switching_pattern_len == 0 || antenna_ids == NULL)) {
...
int ble_gap_conn_cte_req_enable(uint16_t conn_handle, uint8_t enable,
...
int ble_gap_conn_cte_rsp_enable(uint16_t conn_handle, uint8_t enable) {
  ...
  uint16_t opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_CONN_CTE_RESP_ENABLE);
...
int ble_gap_read_antenna_information(uint8_t *switch_sampling_rates,
...
static int ble_gap_conn_create_tx(uint8_t own_addr_type,
                                  const ble_addr_t *peer_addr,
                                  const struct ble_gap_conn_params *params) {
...
static int ble_gap_check_conn_params(uint8_t phy,
                                     const struct ble_gap_conn_params *params) {
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
static int ble_gap_ext_conn_create_conn_synced_tx(
  uint8_t own_addr_type, uint8_t advertising_handle, uint8_t subevent,
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_LE_EXT_CREATE_CONN_V2), cmd, len, NULL,
    0);
}

int ble_gap_connect_with_synced(
  uint8_t own_addr_type, uint8_t advertising_handle, uint8_t subevent,
  const ble_addr_t *peer_addr, int32_t duration_ms, uint8_t phy_mask,
  const struct ble_gap_conn_params *phy_1m_conn_params,
  const struct ble_gap_conn_params *phy_2m_conn_params,
  const struct ble_gap_conn_params *phy_coded_conn_params, ble_gap_event_fn *cb,
  void *cb_arg) {
  int rc;
  ...
  rc = ble_gap_ext_conn_create_conn_synced_tx(
...
static int ble_gap_ext_conn_create_tx(
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_EXT_CREATE_CONN), cmd, len, NULL,
    0);
}
...
int ble_gap_ext_connect(uint8_t own_addr_type, const ble_addr_t *peer_addr,
  ...
  if (phy_mask & BLE_GAP_LE_PHY_1M_MASK) {
    memcpy(&ble_conn_reattempt.conn_params_1m, phy_1m_conn_params,
           sizeof(struct ble_gap_conn_params));
  }

  if (phy_mask & BLE_GAP_LE_PHY_2M_MASK) {
    memcpy(&ble_conn_reattempt.conn_params_2m, phy_2m_conn_params,
           sizeof(struct ble_gap_conn_params));
  }

  if (phy_mask & BLE_GAP_LE_PHY_CODED_MASK) {
    memcpy(&ble_conn_reattempt.conn_params_coded, phy_coded_conn_params,
           sizeof(struct ble_gap_conn_params));
  }
...
int ble_gap_connect(uint8_t own_addr_type, const ble_addr_t *peer_addr,
  ...
  if (peer_addr != NULL) {
    ble_conn_reattempt.peer_addr_present = 1;
    memcpy(&ble_conn_reattempt.peer_addr, &bhc_peer_addr, sizeof(ble_addr_t));
  } else {
    ble_conn_reattempt.peer_addr_present = 0;
    memset(&ble_conn_reattempt.peer_addr, 0, sizeof(ble_addr_t));
  }
  ...
  ble_conn_reattempt.duration_ms = duration_ms;
  memcpy(&ble_conn_reattempt.conn_params_1m, conn_params,
         sizeof(struct ble_gap_conn_params));
  ble_conn_reattempt.cb = cb;
  ble_conn_reattempt.cb_arg = cb_arg;
#endif
  ...
  if (peer_addr != NULL) {
    rc = ble_gap_conn_create_tx(own_addr_type, &bhc_peer_addr, conn_params);
  } else {
    rc = ble_gap_conn_create_tx(own_addr_type, NULL, conn_params);
  }
  if (rc != 0) {
    ble_gap_master_reset_state();
    goto done;
...
#if MYNEWT_VAL(OPTIMIZE_MULTI_CONN)
static bool ble_gap_interval_is_integer_multiple(uint32_t min_itvl,
                                                 uint32_t max_itvl) {
...
int ble_gap_common_factor_set(bool enable, uint32_t common_factor) {
  ...
  rc = ble_hs_hci_send_vs_cmd(0x10f, (const void *)vs_cmd, sizeof(vs_cmd), NULL,
                              0);
  if (rc == 0) {
    if (enable) {
...
int ble_gap_multi_connect(struct ble_gap_multi_conn_params *multi_conn_params,
  ...
  /* `scheduling_len_us == 0` is allowed.  It indicates that the optimization
   * for this connection is disabled. The connection interval must be an integer
   * multiple of `common factor`.  Note that the unit of the connection interval
   * is 1.25ms, while the common factor's unit is 0.625ms.
   */
  ...
  rc = ble_hs_hci_send_vs_cmd(0x110, (const void *)vs_cmd, sizeof(vs_cmd), NULL,
                              0);
  if (rc != 0) {
    return rc;
  }
...
#if MYNEWT_VAL(BLE_EXT_ADV)
  rc = ble_gap_ext_connect(
    multi_conn_params->own_addr_type, multi_conn_params->peer_addr,
...
#else
  rc = ble_gap_connect(multi_conn_params->own_addr_type,
                       multi_conn_params->peer_addr,
                       multi_conn_params->duration_ms,
                       multi_conn_params->phy_1m_conn_params, cb, cb_arg);
#endif // MYNEWT_VAL(BLE_EXT_ADV)
  ble_gap_multi_conn.scheduling_len_set = false;
...
static int ble_gap_tx_param_neg_reply(uint16_t conn_handle,
...
void ble_gap_rx_param_req(
  const struct ble_hci_ev_le_subev_rem_conn_param_req *ev) {
#if NIMBLE_BLE_CONNECT
...
#if NIMBLE_BLE_CONNECT
static int ble_gap_update_tx(uint16_t conn_handle,
                             const struct ble_gap_upd_params *params) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_CONN_UPDATE), &cmd, sizeof(cmd),
    NULL, 0);
}
...
ble_gap_validate_conn_params(const struct ble_gap_upd_params *params) {
  ...
  if (params->supervision_timeout < 0x000A ||
      params->supervision_timeout > 0xC80) {
    return false;
  }
...
int ble_gap_set_data_len(uint16_t conn_handle, uint16_t tx_octets,
   ...
   */
  if (g_max_tx_time[conn_handle] == tx_time &&
...
int ble_gap_dev_authorization(uint16_t conn_handle, bool authorized) {
  ble_hs_lock();
  ...
  if (conn != NULL) {
    if (!(conn->bhc_sec_state.authenticated)) {
      ...
      BLE_HS_LOG(ERROR, "Authorized should occur after successful "
...
static void ble_gap_reset_irk(void) {
...
int ble_gap_unpair(const ble_addr_t *peer_addr) {
  ...
  if (!rc) {
    if (value.sec.irk_present) {
      ...
      rc =
        ble_hs_pvcy_remove_entry(key.sec.peer_addr.type, key.sec.peer_addr.val);
      if (rc != 0) {
    ...
    if (!rc) {
      ...
      BLE_HS_LOG(
        ERROR, "No record found for the given address in ble store , rc = %x\n",
        rc);
      err = rc;
    }
  }
   ...
   * A best-practices approach, when all pairing and bonding records are
   * deleted, assign a new randomly-generated IRK.
   */
  ble_gap_reset_irk();
#endif
...
void ble_gap_enc_event(uint16_t conn_handle, int status, int security_restored,
                       int bonded) {
#if NIMBLE_BLE_SM && NIMBLE_BLE_CONNECT
  struct ble_gap_event event;
...
int ble_hs_send_vs_event_mask(uint32_t event_mask) {
  ...
  return ble_hs_hci_send_vs_cmd(BLE_HCI_OCF_VS_SET_EVT_MASK, &cmd, sizeof(cmd),
                                NULL, 0);
}
#endif
...
int ble_gap_event_listener_unregister(struct ble_gap_event_listener *listener) {
  ...
  if (!evl) {
    ...
    SLIST_REMOVE(&ble_gap_event_listener_list, listener, ble_gap_event_listener,
                 link);
    rc = 0;
  }
...

{
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_ENH_READ_TRANSMIT_POWER_LEVEL);
...
int ble_gap_read_remote_transmit_power_level(uint16_t conn_handle,
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_READ_REMOTE_TRANSMIT_POWER_LEVEL);
...
int ble_gap_set_path_loss_reporting_param(
...
int ble_gap_rd_local_resolv_addr(uint8_t peer_addr_type,
                                 const ble_addr_t *peer_addr,
                                 uint8_t *out_addr) {
...
int ble_gap_set_auto_pcl_param(struct ble_gap_set_auto_pcl_params *params) {
  int8_t vs_cmd[11] = {0,          0,           ESP_1M_LOW,  ESP_1M_HIGH,
                       ESP_2M_LOW, ESP_2M_HIGH, ESP_S2_LOW,  ESP_S2_HIGH,
                       ESP_S8_LOW, ESP_S8_HIGH, ESP_MIN_TIME};
  ...
  return ble_hs_hci_send_vs_cmd(BLE_HCI_OCF_VS_PCL_SET_RSSI, &vs_cmd,
                                sizeof(vs_cmd), NULL, 0);
}
#endif

int ble_gap_duplicate_exception_list(uint8_t subcode, uint8_t type,
                                     uint8_t *value, void *cb) {
  ...
  return ble_hs_hci_send_vs_cmd(BLE_HCI_OCF_VS_DUPLICATE_EXCEPTION_LIST,
                                &device_info_array, sizeof(device_info_array),
                                NULL, 0);
}

int ble_gap_clear_legacy_adv(void) {
  return ble_hs_hci_send_vs_cmd(BLE_HCI_OCF_VS_LEGACY_ADV_CLEAR, NULL, 0, NULL,
                                0);
}

int ble_gap_set_chan_select(uint8_t select) {
  return ble_hs_hci_send_vs_cmd(BLE_HCI_OCF_VS_SET_CHAN_SELECT, &select, 1,
                                NULL, 0);
}
#endif
...
int ble_gap_dtm_tx_start(uint8_t tx_chan, uint8_t test_data_len,
                         uint8_t payload) {
...
int ble_gap_dtm_stop(void) { return ble_hs_hci_dtm_stop(); }

int ble_gap_dtm_enh_tx_start(uint8_t tx_chan, uint8_t test_data_len,
...
void ble_gap_deinit(void) { ble_npl_mutex_deinit(&preempt_done_mutex); }
