 ...
 * Controller should use for a new connection (Range 0x001B-0x00FB).
 * @param tx_time          The preferred maximum number of microseconds that the
 * local Controller should use to transmit a single link layer packet (Range
 * 0x0148-0x4290).
 *
 * @return              0 on success,
...
/**
 * Read host's suggested values for the controller's maximum transmitted number
 * of payload octets and maximum packet transmission time (OGF = 0x08, OCF =
 * 0x0024).
 *
...
/**
 * Configure host's suggested maximum transmitted number of payload octets and
 * maximum packet transmission time in controller (OGF = 0x08, OCF = 0x0024).
 *
 ...
 *
 * @param size                  On success, total size of whitelist will be
 * stored here.
 *
 * @return                      0 on success; nonzero on failure.
 */
 ...
 *
 * @param addr                  On success, locally used address will be stored
 * here.
 *
 * @return                      0 on success; nonzero on failure.
 */
 ...
 * @param length                    Return the length of ADV data not including
 type.
 *
...
struct ble_gap_set_auto_pcl_params {
  ...
  /* Number of tx/rx packets to wait before initiating the LE power control
   * Request. The default value is (min time spent variable =  (tx/rxpackets
   * 15)).*/
  uint8_t min_time_spent;
};

/**
 * This API  is used to initiate the LE Power Control Request Procedure for the
 * ACL connection identified by the conn_handle parameter and other parameters.
 *
 * The parameters passed are used by controller for the subsquent LE Power
 * Control Requests that get initiated across all the connections.
 *
 * The Min_Time_Spent parameter indicates the number of tx/rx packets that the
 * Controller shall observe the RSSI  has crossed the threshold (upper and lower
 * limit of active phy) before the controller initiates the LE POWER CONTROL
 * PROCEDURE in the link layer.
 *
 * @param params	  Instance of ble_gap_set_auto_pcl_params with different
 * parameters
 *
 * @return                0 on success; nonzero on failure.
 */
...
/**
 * This API is used to clean up residue memory in controller for legacy
 * advertisement
 *
 * @return                0 on success; nonzero on failure.
 */
...
/**
 * This API is used to let controller know which CSA to use. Not applicable for
 * ESP32
 *
 ...
 *
 * @return                      0 on success;
 *                              BLE_HS_ENOTCONN if the connection handle is not
 * found. BLE_HS_EAUTHOR if the device is not authenticated before
 * authorization.
 */
 ...
 *
 * This function configures the vendor-specific event mask, enabling or
 * disabling specific vendor-defined events from being reported by the
 * controller.
 *
 * @param event_mask            Bitmask representing the events to enable.
 *
 * @return                      0 on success;
 *                              A nonzero value indicating an error if the
 * command fails.
 */
