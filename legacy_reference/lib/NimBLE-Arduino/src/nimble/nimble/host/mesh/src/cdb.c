...
static struct key_update
  cdb_key_updates[MYNEWT_VAL(BLE_MESH_CDB_SUBNET_COUNT) +
                  MYNEWT_VAL(BLE_MESH_CDB_APP_KEY_COUNT)];

struct bt_mesh_cdb bt_mesh_cdb = {
  .nodes = {[0 ...(CONFIG_BT_MESH_NODE_COUNT - 1)] =
              {
                .addr = BT_MESH_ADDR_UNASSIGNED,
              }},
  .subnets = {[0 ...(CONFIG_BT_MESH_SUBNET_COUNT - 1)] =
                {
                  .net_idx = BT_MESH_KEY_UNUSED,
                }},
  .app_keys = {[0 ...(CONFIG_BT_MESH_APP_KEY_COUNT - 1)] =
                 {
                   .net_idx = BT_MESH_KEY_UNUSED,
                 }},
};
...
static struct node_update *
cdb_node_update_find(uint16_t addr, struct node_update **free_slot) {
...
void bt_mesh_cdb_iv_update(uint32_t iv_index, bool iv_update) {
  ...
  atomic_set_bit_to(bt_mesh_cdb.flags, BT_MESH_CDB_IVU_IN_PROGRESS, iv_update);
...
struct bt_mesh_cdb_node *bt_mesh_cdb_node_get(uint16_t addr) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.nodes); i++) {
    ...
    if (addr >= node->addr && addr <= node->addr + node->num_elem - 1) {
      return node;
    }
  }
...
void bt_mesh_cdb_node_foreach(bt_mesh_cdb_node_func_t func, void *user_data) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.nodes); ++i) {
    ...
    if (func(&bt_mesh_cdb.nodes[i], user_data) == BT_MESH_CDB_ITER_STOP) {
      break;
    }
  }
}
