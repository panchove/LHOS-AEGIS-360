...
static os_membuf_t ble_hs_hci_os_event_buf[OS_MEMPOOL_SIZE(
  BLE_HS_HCI_EVT_COUNT, sizeof(struct ble_npl_event))];
...
struct ble_npl_eventq *ble_hs_evq_get(void) { return ble_hs_evq; }

void ble_hs_evq_set(struct ble_npl_eventq *evq) { ble_hs_evq = evq; }
...
void ble_hs_unlock_nested(void) {
  ...
  if (counter_lock > 0) {
    ...
    if (ble_hs_task_handles[ble_hs_task_handle_index - 1] ==
...
int ble_hs_synced(void) { return ble_hs_sync_state == BLE_HS_SYNC_STATE_GOOD; }
...
static void ble_hs_timer_sched(int32_t ticks_from_now) {
  ...
  if (!ble_npl_callout_is_active(&ble_hs_timer) ||
      ((ble_npl_stime_t)(abs_time - ble_npl_callout_get_ticks(&ble_hs_timer))) <
        0) {
    ...
    BLE_HS_LOG(DEBUG,
               "exp_time:%" PRId32 ".now:%" PRId32 ".ticks:%" PRId32
               ".active:%" PRId16 ".Need reset.",
               ble_npl_callout_get_ticks(&ble_hs_timer), ble_npl_time_get(),
               ticks_from_now, ble_npl_callout_is_active(&ble_hs_timer));
    ble_hs_timer_reset(ticks_from_now);
  }
}
...
static void ble_hs_event_reset(struct ble_npl_event *ev) { ble_hs_reset(); }
...
void ble_transport_hs_init(void) { ble_hs_init(); }
