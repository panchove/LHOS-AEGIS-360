 ...
 * host. DTM allows for testing and validation of the BLE radio performance by
 * enabling custom transmission and reception of data packets.
 *
 ...
 * currently running DTM test. It retrieves the number of packets transmitted
 * during the test and stores it in the provided `num_packets` variable.
 *
 * @param num_packets           Pointer to a `uint16_t` variable to store the
 * number of transmitted packets. If an error occurs, the value will be set to
 * 0.
 *
 * @return                      0 on success;
 *                              A non-zero value on failure.
