...
void RADIO_IRQHandler(void) { ((void (*)(void))radio_isr_addr)(); }

void RNG_IRQHandler(void) { ((void (*)(void))rng_isr_addr)(); }

void RTC0_IRQHandler(void) { ((void (*)(void))rtc0_isr_addr)(); }
...
ble_npl_error_t npl_freertos_sem_init(struct ble_npl_sem *sem,
                                      uint16_t tokens) {
  if (!sem) {
    return BLE_NPL_INVALID_PARAM;
  }
...
#if CONFIG_BT_NIMBLE_USE_ESP_TIMER
static void ble_npl_event_fn_wrapper(void *arg) {
...
int npl_freertos_callout_init(struct ble_npl_callout *co,
  ...
  esp_timer_create_args_t create_args = {
    .callback = ble_npl_event_fn_wrapper, .arg = co, .name = "nimble_timer"};
...
#else
  /* Workaround for bug in xTimerIsTimerActive with FreeRTOS V10.2.0, fixed in
   * V10.4.4 See: https://github.com/FreeRTOS/FreeRTOS-Kernel/pull/305 Sometimes
   * xTimerIsTimerActive returns pdTRUE even though the timer has expired, so we
   * double check.
   */
...
ble_npl_time_t npl_freertos_callout_get_ticks(struct ble_npl_callout *co) {
   ...
   * Drawback of this approach is that existing code to reset timer would be
   * called more often (since the if condition to invoke reset timer would
   * always succeed if timer is active).
   */
...
int na_npl_freertos_mempool_init(void) {
  int rc = -1;
  // These values taken from esp-idf:
  // bt/nimble/porting/npl/freertos/src/npl_os_freertos.c
  ...
  if (ble_total_evt_count) {
    ble_freertos_ev_buf = bt_osi_mem_malloc_internal(
      OS_MEMPOOL_SIZE(ble_total_evt_count,
    ...
    rc = os_mempool_init(&ble_freertos_ev_pool, ble_total_evt_count,
                         sizeof(struct ble_npl_event_freertos),
                         ble_freertos_ev_buf, "ble_freertos_ev_pool");
    if (rc) {
      goto _error;
    }
  }
  ...
  if (ble_total_evtq_count) {
    ble_freertos_evq_buf = bt_osi_mem_malloc_internal(
      OS_MEMPOOL_SIZE(ble_total_evtq_count,
    ...
    rc = os_mempool_init(&ble_freertos_evq_pool, ble_total_evtq_count,
                         sizeof(struct ble_npl_eventq_freertos),
                         ble_freertos_evq_buf, "ble_freertos_evq_pool");
    if (rc) {
      goto _error;
    }
  }
  ...
  if (ble_total_co_count) {
    ble_freertos_co_buf = bt_osi_mem_malloc_internal(
      OS_MEMPOOL_SIZE(ble_total_co_count,
    ...
    rc = os_mempool_init(&ble_freertos_co_pool, ble_total_co_count,
                         sizeof(struct ble_npl_callout_freertos),
                         ble_freertos_co_buf, "ble_freertos_co_pool");
    if (rc) {
      goto _error;
    }
  }
  ...
  if (ble_total_sem_count) {
    ble_freertos_sem_buf = bt_osi_mem_malloc_internal(
      OS_MEMPOOL_SIZE(ble_total_sem_count,
    ...
    rc = os_mempool_init(&ble_freertos_sem_pool, ble_total_sem_count,
                         sizeof(struct ble_npl_sem_freertos),
                         ble_freertos_sem_buf, "ble_freertos_sem_pool");
    if (rc) {
      goto _error;
    }
  }
  ...
  if (ble_total_mutex_count) {
    ble_freertos_mutex_buf = bt_osi_mem_malloc_internal(
      OS_MEMPOOL_SIZE(ble_total_mutex_count,
    ...
    rc = os_mempool_init(&ble_freertos_mutex_pool, ble_total_mutex_count,
                         sizeof(struct ble_npl_mutex_freertos),
                         ble_freertos_mutex_buf, "ble_freertos_mutex_pool");
    if (rc) {
      goto _error;
    }
  }
...
void na_npl_freertos_eventq_init(struct ble_npl_eventq *evq) {
  ...
  if (!os_memblock_from(&ble_freertos_evq_pool, evq->eventq)) {
    ...
    eventq->q = xQueueCreate(ble_freertos_total_event_cnt,
