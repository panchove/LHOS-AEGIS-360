...
#define BLE_EATT_DATABUF_SIZE                                                  \
  (MYNEWT_VAL(BLE_EATT_MTU) + 2 + sizeof(struct os_mbuf_pkthdr) +              \
   sizeof(struct os_mbuf))

#define BLE_EATT_MEMBLOCK_SIZE (OS_ALIGN(BLE_EATT_DATABUF_SIZE, 4))

#define BLE_EATT_MEMPOOL_SIZE                                                  \
  OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_EATT_CHAN_NUM) + 1, BLE_EATT_MEMBLOCK_SIZE)
static os_membuf_t ble_eatt_conn_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_EATT_CHAN_NUM), sizeof(struct ble_eatt))];
...
static struct ble_eatt *ble_eatt_find_not_busy(uint16_t conn_handle) {
...
static struct ble_eatt *ble_eatt_find_by_conn_handle(uint16_t conn_handle) {
  struct ble_eatt *eatt;
...
static struct ble_eatt *ble_eatt_find(uint16_t conn_handle, uint16_t cid) {
  struct ble_eatt *eatt;
...
static int ble_eatt_prepare_rx_sdu(struct ble_l2cap_chan *chan) {
  int rc;
  struct os_mbuf *om;
  ...
  if (rc) {
    BLE_EATT_LOG_ERROR(
      "eatt: Failed to supply RX SDU conn_handle 0x%04x (status=%d)\n",
      chan->conn_handle, rc);
...
static void ble_eatt_wakeup_cb(struct ble_npl_event *ev) {
  struct ble_eatt *eatt;
  struct os_mbuf *txom;
...
static struct ble_eatt *ble_eatt_alloc(void) {
  struct ble_eatt *eatt;
...
static void ble_eatt_free(struct ble_eatt *eatt) {
  struct os_mbuf_pkthdr *omp;
...
static int ble_eatt_l2cap_event_fn(struct ble_l2cap_event *event, void *arg) {
  ...
  case BLE_L2CAP_EVENT_COC_CONNECTED:
    ...
    ble_gap_eatt_event(event->connect.conn_handle, 0,
                       event->connect.chan->scid);
    break;
  case BLE_L2CAP_EVENT_COC_DISCONNECTED:
    BLE_EATT_LOG_DEBUG("eatt: Disconnected \n");
    ble_eatt_free(eatt);
    ble_gap_eatt_event(event->disconnect.conn_handle, 1,
...
static void ble_eatt_setup_cb(struct ble_npl_event *ev) {
  ...
  if (rc) {
    BLE_EATT_LOG_ERROR(
      "eatt: Failed to connect EATT on conn_handle 0x%04x (status=%d)\n",
      eatt->conn_handle, rc);
    os_mbuf_free_chain(om);
    ble_eatt_free(eatt);
  }
...
static int ble_gatt_eatt_write_cl_cb(uint16_t conn_handle,
                                     const struct ble_gatt_error *error,
                                     struct ble_gatt_attr *attr, void *arg) {
  if (error == NULL || (error->status != 0 && error->status != BLE_HS_EDONE)) {
    BLE_EATT_LOG_DEBUG(
      "eatt: Cannot write to Client Supported features on peer device\n");
    return 0;
  }
...
static int ble_gatt_eatt_read_cl_uuid_cb(uint16_t conn_handle,
                                         const struct ble_gatt_error *error,
  ...
  if (error == NULL || (error->status != 0 && error->status != BLE_HS_EDONE)) {
    BLE_EATT_LOG_DEBUG(
      "eatt: Cannot find Client Supported features on peer device\n");
    return BLE_HS_EDONE;
  }
  ...
  if (error->status == 0) {
    ...
    rc = ble_gattc_write_flat(conn_handle, attr->handle, &client_supported_feat,
                              1, ble_gatt_eatt_write_cl_cb, NULL);
...
static int ble_gatt_eatt_read_uuid_cb(uint16_t conn_handle,
                                      const struct ble_gatt_error *error,
                                      struct ble_gatt_attr *attr, void *arg) {
  ...
  if (error == NULL || (error->status != 0 && error->status != BLE_HS_EDONE)) {
    BLE_EATT_LOG_DEBUG(
      "eatt: Cannot find Server Supported features on peer device\n");
    return BLE_HS_EDONE;
  }
...
static void ble_gatt_eatt_read_svr_uuid(struct ble_npl_event *ev) {
  uint16_t conn_handle;

  conn_handle = (uint16_t)((uintptr_t)(ble_npl_event_get_arg(ev)));

  ble_gattc_read_by_uuid(
    conn_handle, 1, 0xffff,
...
static int ble_eatt_gap_event(struct ble_gap_event *event, void *arg) {
  struct ble_eatt *eatt;
  ...
  case BLE_GAP_EVENT_ENC_CHANGE:
    ...
    BLE_EATT_LOG_DEBUG(
      "eatt: Encryption enabled, connecting EATT (conn_handle=0x%04x)\n",
...
void ble_eatt_init(ble_eatt_att_rx_fn att_rx_cb) {
  int rc;

  rc = os_mempool_init(
    &ble_eatt_sdu_mbuf_mempool, MYNEWT_VAL(BLE_EATT_CHAN_NUM) + 1,
    BLE_EATT_MEMBLOCK_SIZE, ble_eatt_sdu_coc_mem, "ble_eatt_sdu");
  BLE_HS_DBG_ASSERT_EVAL(rc == 0);

  rc = os_mbuf_pool_init(&ble_eatt_sdu_os_mbuf_pool, &ble_eatt_sdu_mbuf_mempool,
                         BLE_EATT_MEMBLOCK_SIZE,
                         MYNEWT_VAL(BLE_EATT_CHAN_NUM) + 1);
  BLE_HS_DBG_ASSERT_EVAL(rc == 0);

  rc = os_mempool_init(&ble_eatt_conn_pool, MYNEWT_VAL(BLE_EATT_CHAN_NUM),
                       sizeof(struct ble_eatt), ble_eatt_conn_mem,
                       "ble_eatt_conn_pool");
  BLE_HS_DBG_ASSERT_EVAL(rc == 0);

  rc = ble_gap_event_listener_register(&ble_eatt_listener, ble_eatt_gap_event,
                                       NULL);
