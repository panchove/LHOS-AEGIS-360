...
uint8_t bt_mesh_default_ttl_get(void) { return bt_mesh.default_ttl; }
...
uint8_t bt_mesh_net_transmit_get(void) { return bt_mesh.net_xmit; }
...
static int cfg_set(int argc, char **argv, char *val) {
  ...
  if (len != sizeof(cfg)) {
    BT_ERR("Unexpected value length (%d != %zu)", len, sizeof(cfg));
    return -EINVAL;
  }
...
void bt_mesh_cfg_default_set(void) {
  ...
  bt_mesh.net_xmit = BT_MESH_TRANSMIT(CONFIG_BT_MESH_NETWORK_TRANSMIT_COUNT,
