...
bleSensorData LAYRZ_BLEFIER_1_decoder::parseData(const std::string bleData) {
...
std::string LAYRZ_BLEFIER_1_decoder::decode(const char *macAddress,
                                            const char *model, const char *rssi,
                                            const char *txPower,
                                            const char *manufData,
                                            const char *serviceData) {

  // debugPrint("[LAYRZ_BLEFIER_1] Decoding BLE data for MAC: %s\n",
  // macAddress);
  ...
  {
    bleSensorData manuf = LAYRZ_BLEFIER_1_decoder::parseData(manufData);
  ...
  for (const auto &service : serviceArray) {
    if (service.empty())
      continue;
    bleSensorData svc = LAYRZ_BLEFIER_1_decoder::parseData(service);
    if (svc.id.empty())
      continue;
    ...
    // 0x190F — Universal counter (mode-specific)
      ...
      if (b.size() >= 7) {
        ...
        case 1: { // Volumetric counter
          double liters = static_cast<double>(value) * 1000.0; // m^3 -> L
          double gallons = static_cast<double>(value) * 264.172;
          ...
          oss << "ble." << macAddress << ".volumetric.counter.liters:" << liters
              << ",";
          oss << "ble." << macAddress
              << ".volumetric.counter.cubic.meters:" << value << ",";
