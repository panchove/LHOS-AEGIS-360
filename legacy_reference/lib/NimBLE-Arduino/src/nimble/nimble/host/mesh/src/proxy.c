...
static void proxy_sar_timeout(struct ble_npl_event *work) {
  ...
  if ((client->conn_handle != BLE_HS_CONN_HANDLE_NONE)) {
    rc = ble_gap_terminate(client->conn_handle, BLE_ERR_REM_USER_CONN_TERM);
    assert(rc == 0);
  }
}
...
static void proxy_disconnected(uint16_t conn_handle, int reason) {
  ...
  for (i = 0; i < ARRAY_SIZE(clients); i++) {
    ...
    if (client->conn_handle == conn_handle) {
      if ((MYNEWT_VAL(BLE_MESH_PB_GATT)) && client->filter_type == PROV) {
...
static int proxy_send(uint16_t conn_handle, const void *data, uint16_t len) {
  ...
  if (gatt_svc == MESH_GATT_PROXY) {
    ...
    assert(om);
    err =
      ble_gatts_notify_custom(conn_handle, svc_handles.proxy_data_out_h, om);
    notify_complete();
  }
#endif
...
static int proxy_segment_and_send(uint16_t conn_handle, uint8_t type,
  ...
  BT_DBG("conn_handle %d type 0x%02x len %u: %s", conn_handle, type,
         msg->om_len, bt_hex(msg->om_data, msg->om_len));
...
static int node_id_adv(struct bt_mesh_subnet *sub) {
  ...
  err = bt_le_adv_start(&fast_adv_param, node_id_ad, ARRAY_SIZE(node_id_ad),
                        NULL, 0);
  if (err) {
...
static int net_id_adv(struct bt_mesh_subnet *sub) {
  ...
  err =
    bt_le_adv_start(&slow_adv_param, net_id_ad, ARRAY_SIZE(net_id_ad), NULL, 0);
  if (err) {
...
static int32_t gatt_proxy_advertise(struct bt_mesh_subnet *sub) {
  ...
  if (sub->node_id == BT_MESH_NODE_IDENTITY_RUNNING) {
    ...
    if (active < NODE_ID_TIMEOUT) {
      ...
      BT_DBG("Node ID active for %u ms, %d ms remaining", (unsigned)active,
             (int)remaining);
      node_id_adv(sub);
    } else {
...
int32_t bt_mesh_proxy_adv_start(void) {
  ...
  if (!bt_mesh_is_provisioned()) {
    ...
    if (bt_le_adv_start(param, prov_ad, ARRAY_SIZE(prov_ad), prov_sd,
                        prov_sd_len) == 0) {
      proxy_adv_enabled = true;
