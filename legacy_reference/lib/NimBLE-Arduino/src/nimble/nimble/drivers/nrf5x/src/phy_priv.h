...
#define PHY_USE_FEM_SINGLE_GPIO                                                \
  (PHY_USE_FEM &&                                                              \
   (!PHY_USE_FEM_PA || !PHY_USE_FEM_LNA ||                                     \
    (MYNEWT_VAL(BLE_FEM_PA_GPIO) == MYNEWT_VAL(BLE_FEM_LNA_GPIO))))
...
static inline void phy_gpiote_configure(int idx, int pin) {
  nrf_gpio_cfg_output(pin);
  nrf_gpiote_task_configure(
    NRF_GPIOTE, idx, pin,
    GPIOTE_CONFIG_POLARITY_None /*NRF_GPIOTE_POLARITY_NONE*/,
