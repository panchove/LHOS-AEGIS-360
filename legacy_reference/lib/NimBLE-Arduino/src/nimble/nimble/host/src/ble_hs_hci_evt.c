...
#endif
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
  [BLE_HCI_LE_SUBEV_PERIODIC_ADV_SYNC_ESTAB_V2] =
    ble_hs_hci_evt_le_periodic_adv_sync_estab,
  [BLE_HCI_LE_SUBEV_PERIODIC_ADV_RPT_V2] = ble_hs_hci_evt_le_periodic_adv_rpt,
  [BLE_HCI_LE_SUBEV_PERIODIC_ADV_SYNC_TRANSFER_V2] =
    ble_hs_hci_evt_le_periodic_adv_sync_transfer,
  [BLE_HCI_LE_SUBEV_PERIODIC_ADV_SUBEV_DATA_REQ] =
    ble_hs_hci_evt_le_periodic_adv_subev_data_req,
  [BLE_HCI_LE_SUBEV_PERIODIC_ADV_RESP_REPORT] =
    ble_hs_hci_evt_le_periodic_adv_subev_resp_rep,
...
static int ble_hs_hci_evt_disconn_complete(uint8_t event_code, const void *data,
  ...
  if (conn) {
    ...
    /* For master role, check if failure reason is 0x3E, to restart connect
     * attempt For slave role, check whether a. Failure reason is 0x3E b.
     * Connect event was not posted and 0x8 was received Restart advertising in
     * above reasons for slave.
     */
    ...
    if ((conn->bhc_flags & BLE_HS_CONN_F_MASTER) &&
      ...
      if (reattempt_conn.count < MAX_REATTEMPT_ALLOWED) {
        /* Go for connection */
        BLE_HS_LOG(INFO,
                   "Reattempt connection; reason = 0x%x, status = %d,"
                   "reattempt count = %d ",
                   ev->reason, ev->status, reattempt_conn.count);
...
#endif
static int ble_hs_hci_evt_hw_error(uint8_t event_code, const void *data,
                                   unsigned int len) {
...
static int ble_hs_hci_evt_num_completed_pkts(uint8_t event_code,
                                             const void *data,
                                             unsigned int len) {
...
#if MYNEWT_VAL(BLE_HCI_VS)
static int ble_hs_hci_evt_vs(uint8_t event_code, const void *data,
                             unsigned int len) {
...
static int ble_hs_hci_evt_rx_test(uint8_t event_code, const void *data,
                                  unsigned int len) {
...
static int ble_hs_hci_evt_tx_test(uint8_t event_code, const void *data,
                                  unsigned int len) {
...
static int ble_hs_hci_evt_end_test(uint8_t event_code, const void *data,
                                   unsigned int len) {
...
static int ble_hs_hci_evt_le_meta(uint8_t event_code, const void *data,
                                  unsigned int len) {
...
#if NIMBLE_BLE_CONNECT
static int ble_hs_hci_evt_le_enh_conn_complete(uint8_t subevent,
                                               const void *data,
                                               unsigned int len) {
  ...
  if (evt.status == BLE_ERR_SUCCESS) {
    ...
    ble_rpa_replace_peer_params_with_rl(evt.peer_addr, &evt.peer_addr_type,
                                        &rl);
#endif
...
static int ble_hs_hci_evt_le_adv_rpt_first_pass(const void *data,
                                                unsigned int len) {
...
static int ble_hs_hci_evt_le_adv_rpt(uint8_t subevent, const void *data,
                                     unsigned int len) {
  const struct ble_hci_ev_le_subev_adv_rpt *ev = data;
...
#if MYNEWT_VAL(BLE_QUEUE_CONG_CHECK)
  if (ble_get_adv_list_length() > BLE_ADV_LIST_MAX_LENGTH ||
      ble_adv_list_count > BLE_ADV_LIST_MAX_COUNT) {
...
#if MYNEWT_VAL(BLE_QUEUE_CONG_CHECK)
    if (ble_check_adv_list(ev->reports[i].addr, ev->reports[i].addr_type) ==
        true) {
      continue;
    }
#endif
...
static int ble_hs_hci_evt_rd_rem_ver_complete(uint8_t subevent,
                                              const void *data,
                                              unsigned int len) {
...
static int ble_hs_hci_evt_le_ext_adv_rpt_first_pass(const void *data,
                                                    unsigned int len) {
...
static int ble_hs_hci_evt_le_periodic_adv_rpt(uint8_t subevent,
                                              const void *data,
                                              unsigned int len) {
#if MYNEWT_VAL(BLE_PERIODIC_ADV)
...
#if MYNEWT_VAL(BLE_POWER_CONTROL)
static int ble_hs_hci_evt_le_pathloss_threshold(uint8_t subevent,
                                                const void *data,
                                                unsigned int len) {
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_SYNC_BIGINFO_REPORTS)
static int ble_hs_hci_evt_le_biginfo_adv_report(uint8_t subevent,
                                                const void *data,
                                                unsigned int len) {
...
static int ble_hs_hci_evt_le_adv_set_terminated(uint8_t subevent,
                                                const void *data,
                                                unsigned int len) {
#if MYNEWT_VAL(BLE_EXT_ADV)
...
#if MYNEWT_VAL(BLE_AOA_AOD)
static int ble_hs_hci_evt_le_connless_iq_report(uint8_t subevent,
                                                const void *data,
                                                unsigned int len) {
...
static int ble_hs_hci_evt_le_conn_iq_report(uint8_t subevent, const void *data,
                                            unsigned int len) {
...
static int ble_hs_hci_evt_le_cte_req_failed(uint8_t subevent, const void *data,
                                            unsigned int len) {
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
static int ble_hs_hci_evt_le_periodic_adv_subev_data_req(uint8_t subevent,
                                                         const void *data,
                                                         unsigned int len) {
...
static int ble_hs_hci_evt_le_periodic_adv_subev_resp_rep(uint8_t subevent,
                                                         const void *data,
                                                         unsigned int len) {
...
#if NIMBLE_BLE_CONNECT
static int ble_hs_hci_evt_le_conn_upd_complete(uint8_t subevent,
                                               const void *data,
                                               unsigned int len) {
  ...
  if (ev->status == 0) {
    ...
    BLE_HS_DBG_ASSERT(le16toh(ev->supervision_timeout) >=
...
static int ble_hs_hci_evt_le_phy_update_complete(uint8_t subevent,
                                                 const void *data,
                                                 unsigned int len) {
...
static int ble_hs_hci_evt_le_data_len_change(uint8_t subevent, const void *data,
                                             unsigned int len) {
...
uint32_t ble_get_adv_list_length(void) {
  ...
  SLIST_FOREACH(device, &ble_adv_list, next) { length++; }
...
bool ble_check_adv_list(const uint8_t *addr, uint8_t addr_type) {
  ...
  SLIST_FOREACH(device, &ble_adv_list, next) {
    if (!memcmp(addr, device->addr.val, BLE_DEV_ADDR_LEN) &&
