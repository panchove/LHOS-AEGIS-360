 ...
 */
NimBLEAdvertisedDevice::NimBLEAdvertisedDevice(const ble_gap_event *event,
                                               uint8_t eventType)
#if CONFIG_BT_NIMBLE_EXT_ADV
    : m_address{event->ext_disc.addr}, m_advType{eventType},
      m_rssi{event->ext_disc.rssi}, m_callbackSent{0},
      m_advLength{event->ext_disc.length_data},
      m_isLegacyAdv{!!(event->ext_disc.props & BLE_HCI_ADV_LEGACY_MASK)},
      m_dataStatus{event->ext_disc.data_status}, m_sid{event->ext_disc.sid},
      m_primPhy{event->ext_disc.prim_phy}, m_secPhy{event->ext_disc.sec_phy},
      m_periodicItvl{event->ext_disc.periodic_adv_itvl},
      m_payload(event->ext_disc.data,
                event->ext_disc.data + event->ext_disc.length_data) {
#else
    : m_address{event->disc.addr}, m_advType{eventType},
      m_rssi{event->disc.rssi}, m_callbackSent{0},
 ...
 * @param [in] event The advertisement event data.
 */
void NimBLEAdvertisedDevice::update(const ble_gap_event *event,
                                    uint8_t eventType) {
#if CONFIG_BT_NIMBLE_EXT_ADV
 ...
 * * BLE_HCI_ADV_RPT_EVTYPE_ADV_IND      (0) - indirect advertising -
 * connectable and scannable
 * * BLE_HCI_ADV_RPT_EVTYPE_DIR_IND      (1) - direct advertising - connectable
 * * BLE_HCI_ADV_RPT_EVTYPE_SCAN_IND     (2) - indirect scan response - not
 * connectable - scannable
 * * BLE_HCI_ADV_RPT_EVTYPE_NONCONN_IND  (3) - beacon only - not connectable -
 * not scannable
 ...
 *
 * A %BLE device can declare its own appearance.  The appearance is how it would
 * like to be shown to an end user typically in the form of an icon.
 *
 ...
 */
int8_t NimBLEAdvertisedDevice::getRSSI() const { return m_rssi; } // getRSSI
...
NimBLEAddress NimBLEAdvertisedDevice::getTargetAddress(uint8_t index) const {
  ...
  uint8_t count =
    findAdvField(BLE_HS_ADV_TYPE_PUBLIC_TGT_ADDR, index, &data_loc);
  if (count < index + 1) {
    index -= count;
  ...
  if (count > 0 && data_loc != ULONG_MAX) {
    ...
    if (field->length < index * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN) {
      // In the case of more than one field of target addresses we need to
      // adjust the index
      index -= count - field->length / BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN;
    }
    if (field->length > index * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN) {
      return NimBLEAddress{field->value +
                             (index - 1) * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN,
                           field->type};
    }
  }
  ...
  if (data_loc != ULONG_MAX) {
    ...
    if (field->length > bytes) {
      const char *field_data =
        reinterpret_cast<const char *>(field->value + bytes);
 ...
 */
std::string
NimBLEAdvertisedDevice::getServiceData(const NimBLEUUID &uuid) const {
  uint8_t bytes;
  uint8_t index = 0;
  size_t data_loc = findServiceData(index, &bytes);
 ...
 * @param [in] bytes A pointer to storage for the number of the bytes in the
 * UUID.
 * @return The index in the vector where the data is located, ULONG_MAX if not
 * found.
 */
size_t NimBLEAdvertisedDevice::findServiceData(uint8_t index,
                                               uint8_t *bytes) const {
  *bytes = 0;

  size_t data_loc = 0;
  uint8_t found =
    findAdvField(BLE_HS_ADV_TYPE_SVC_DATA_UUID16, index, &data_loc);
  if (found > index) {
    *bytes = 2;
    return data_loc;
  }
 ...
 * @return The Service UUID of the advertised service, or an empty UUID if not
 * found.
 */
 ...
 */
bool NimBLEAdvertisedDevice::isAdvertisingService(
  const NimBLEUUID &uuid) const {
 ...
 * @return The set ID.
 */
uint8_t NimBLEAdvertisedDevice::getSetId() const { return m_sid; } // getSetId
 ...
 * One of:
 * * BLE_GAP_EXT_ADV_DATA_STATUS_COMPLETE (0) - Complete extended advertising
 * data
 * * BLE_GAP_EXT_ADV_DATA_STATUS_INCOMPLETE (1) - Incomplete extended
 * advertising data, more to come
 * * BLE_GAP_EXT_ADV_DATA_STATUS_TRUNCATED (2) - Incomplete extended advertising
 * data, no more to come
 */
...
uint8_t NimBLEAdvertisedDevice::findAdvField(uint8_t type, uint8_t index,
                                             size_t *data_loc) const {
  ...
  while (length > 2) {
    const ble_hs_adv_field *field =
      reinterpret_cast<const ble_hs_adv_field *>(&m_payload[data]);
    if (field->length >= length) {
      return count;
    }

    if (field->type == type || (type == BLE_HS_ADV_TYPE_COMP_NAME &&
                                field->type == BLE_HS_ADV_TYPE_INCOMP_NAME)) {
      switch (type) {
...
std::string NimBLEAdvertisedDevice::toString() const {
  std::string res =
    "Name: " + getName() + ", Address: " + getAddress().toString();
  ...
  if (haveManufacturerData()) {
    ...
    res += NimBLEUtils::dataToHexString(
      reinterpret_cast<const uint8_t *>(mfgData.data()), mfgData.length());
  }
 ...
 */
uint16_t NimBLEAdvertisedDevice::getAdvLength() const { return m_advLength; }
...
bool NimBLEAdvertisedDevice::isScannable() const {
  return isLegacyAdvertisement() &&
         (m_advType == BLE_HCI_ADV_TYPE_ADV_IND ||
          m_advType == BLE_HCI_ADV_TYPE_ADV_SCAN_IND);
} // isScannable
...
/**
 * @brief Convenience operator to convert this NimBLEAdvertisedDevice to
 * NimBLEAddress representation.
...
NimBLEAdvertisedDevice::begin() const {
  return m_payload.cbegin();
}

/**
 * @brief Get the end iterator for the payload.
 * @return A read only iterator pointing to one past the last byte of the
 * payload.
 */
