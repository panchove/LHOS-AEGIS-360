 ...
 */
void ble_ll_ctrl_rej_ext_ind_make(uint8_t rej_opcode, uint8_t err,
                                  uint8_t *ctrdata) {
 ...
 * @param ble_err
 */
void ble_ll_ctrl_phy_update_cancel(struct ble_ll_conn_sm *connsm,
                                   uint8_t ble_err) {
 ...
 * @return int
 */
static int ble_ll_ctrl_conn_param_pdu_proc(struct ble_ll_conn_sm *connsm,
 ...
 * @param dptr
 */
static int ble_ll_ctrl_proc_unk_rsp(struct ble_ll_conn_sm *connsm,
                                    uint8_t *dptr, uint8_t *rspdata) {
  uint8_t ctrl_proc;
  uint8_t opcode;
...
static void ble_ll_ctrl_phy_update_ind_make(struct ble_ll_conn_sm *connsm,
                                            uint8_t *dptr, uint8_t *ctrdata,
                                            int periph_req) {
  uint8_t m_to_s;
  uint8_t s_to_m;
  uint8_t tx_phys;
 ...
 * @param ctrdata: Pointer to where CtrData starts in pdu
 */
static void ble_ll_ctrl_phy_req_rsp_make(struct ble_ll_conn_sm *connsm,
                                         uint8_t *ctrdata) {
 ...
 * @param ctrdata: Pointer to where CtrData starts in pdu
 */
static void ble_ll_ctrl_sca_req_rsp_make(struct ble_ll_conn_sm *connsm,
                                         uint8_t *ctrdata) {
  ctrdata[0] = BLE_LL_SCA_ENUM;
}
#endif

static uint8_t ble_ll_ctrl_rx_phy_req(struct ble_ll_conn_sm *connsm,
 ...
 * @return uint8_t
 */
static uint8_t ble_ll_ctrl_rx_phy_rsp(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rsp) {
  uint8_t rsp_opcode;
 ...
 * @return uint8_t
 */
static uint8_t ble_ll_ctrl_rx_phy_update_ind(struct ble_ll_conn_sm *connsm,
                                             uint8_t *dptr) {
  int no_change;
 ...
 * @return uint8_t
 */
static uint8_t ble_ll_ctrl_rx_sca_req(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rsp) {
 ...
 * @return uint8_t
 */
static uint8_t ble_ll_ctrl_rx_sca_rsp(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr) {
...
static uint8_t ble_ll_ctrl_rx_cis_req(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rspdata) {
  return BLE_LL_CTRL_UNKNOWN_RSP;
}

static uint8_t ble_ll_ctrl_rx_cis_rsp(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rspdata) {
  return BLE_LL_CTRL_UNKNOWN_RSP;
}

static uint8_t ble_ll_ctrl_rx_cis_ind(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr) {
  return BLE_LL_CTRL_UNKNOWN_RSP;
}
#endif
 ...
 */
static void ble_ll_ctrl_datalen_upd_make(struct ble_ll_conn_sm *connsm,
                                         uint8_t *dptr) {
 ...
 * @return int
 */
static int ble_ll_ctrl_enc_allowed_pdu(uint8_t llid, uint8_t len,
                                       uint8_t opcode) {
  int allowed;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_ISO)
static void ble_ll_ctrl_cis_create(struct ble_ll_conn_sm *connsm,
                                   uint8_t *dptr) {
  /* TODO Implement */
  return;
}
 ...
 * @param dptr: Pointer to where control pdu payload starts
 */
static void ble_ll_ctrl_enc_req_make(struct ble_ll_conn_sm *connsm,
                                     uint8_t *dptr) {
 ...
 * @param dptr
 */
static void ble_ll_ctrl_rx_enc_rsp(struct ble_ll_conn_sm *connsm,
                                   uint8_t *dptr) {
 ...
 * @param rspbuf
 */
static uint8_t ble_ll_ctrl_rx_enc_req(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rspdata) {
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
  if (connsm->conn_role == BLE_LL_CONN_ROLE_CENTRAL) {
    return BLE_LL_CTRL_UNKNOWN_RSP;
 ...
 *             of the data.
 */
static void ble_ll_ctrl_conn_param_pdu_make(struct ble_ll_conn_sm *connsm,
                                            uint8_t *dptr,
                                            struct ble_ll_conn_params *req) {
  uint16_t offset;
  struct hci_conn_update *hcu;
...
static void ble_ll_ctrl_version_ind_make(struct ble_ll_conn_sm *connsm,
                                         uint8_t *pyld) {
 ...
 */
static void ble_ll_ctrl_chanmap_req_make(struct ble_ll_conn_sm *connsm,
                                         uint8_t *pyld) {
 ...
 * @param opcode
 */
static int ble_ll_ctrl_rx_reject_ind(struct ble_ll_conn_sm *connsm,
                                     uint8_t *dptr, uint8_t opcode,
                                     uint8_t *rspdata) {
  uint8_t ble_error;
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
      case BLE_LL_CONN_ROLE_CENTRAL:
        /* As a central we should send connection update indication in this
         * point */
        rsp_opcode = BLE_LL_CTRL_CONN_UPDATE_IND;
        ble_ll_ctrl_conn_update_init_proc(connsm, NULL);
        connsm->reject_reason = BLE_ERR_SUCCESS;
 ...
 * @return int
 */
static int ble_ll_ctrl_rx_conn_update(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr) {
  uint8_t rsp_opcode;
...
static void ble_ll_ctrl_update_features(struct ble_ll_conn_sm *connsm,
                                        uint8_t *feat) {
 ...
 * @return int
 */
static int ble_ll_ctrl_rx_feature_req(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rspbuf,
                                      uint8_t opcode) {
  uint8_t rsp_opcode;
  uint64_t our_feat;
 ...
 *
 */
static void ble_ll_ctrl_rx_feature_rsp(struct ble_ll_conn_sm *connsm,
                                       uint8_t *dptr) {
  ble_ll_ctrl_update_features(connsm, dptr);
 ...
 * @return int
 */
static int ble_ll_ctrl_rx_version_ind(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr, uint8_t *rspbuf) {
  uint8_t rsp_opcode;
 ...
 * @param dptr
 */
static int ble_ll_ctrl_rx_chanmap_req(struct ble_ll_conn_sm *connsm,
                                      uint8_t *dptr) {
  uint16_t instant;
  uint16_t conn_events;
