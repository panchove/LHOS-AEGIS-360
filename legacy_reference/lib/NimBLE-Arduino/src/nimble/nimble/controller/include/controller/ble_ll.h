...
#define BLE_LL_CONN_HANDLE(t, h)                                               \
  ((((t) << 8) & BLE_LL_CONN_HANDLE_TYPE_MASK) |                               \
   ((h) & BLE_LL_CONN_HANDLE_IDX_MASK))
#define BLE_LL_CONN_HANDLE_TYPE_MASK (0x0700)
#define BLE_LL_CONN_HANDLE_IDX_MASK (0x00ff)
#define BLE_LL_CONN_HANDLE_TYPE(conn_h)                                        \
  (((conn_h) & BLE_LL_CONN_HANDLE_TYPE_MASK) >> 8)
 ...
 *
 * @param len  Length of PDU, including PDU header and excluding MIC (if
 * encrypted)
 *
 ...
 * Set public address
 *
 * This can be used to set controller public address from vendor specific
 * storage, usually should be done in hal_bsp_init(). Shall be *only* called
 * before LL is initialized, i.e. before sysinit stage.
 */
...
static inline int ble_ll_get_addr_type(uint8_t txrxflag) {
  if (txrxflag) {
