...
int na_ble_sm_alg_c1(const uint8_t *k, const uint8_t *r, const uint8_t *preq,
...
int na_ble_sm_alg_f4(const uint8_t *u, const uint8_t *v, const uint8_t *x,
   ...
   *
   * note:
   * na_ble_sm_alg_aes_cmac uses BE data; na_ble_sm_alg_f4 accepts LE so we
   * swap.
   */
  swap_buf(m, u, 32);
  swap_buf(m + 32, v, 32);
...
int ble_sm_alg_gen_dhkey(const uint8_t *peer_pub_key_x,
  ...
  if (mbedtls_ecp_group_load(&keypair.MBEDTLS_PRIVATE(grp),
                             MBEDTLS_ECP_DP_SECP256R1) != 0) {
    goto exit;
  }

  if (mbedtls_ecp_point_read_binary(&keypair.MBEDTLS_PRIVATE(grp), &pt, pub,
                                    65) != 0) {
    goto exit;
  }
  ...
  if (mbedtls_ecp_point_read_binary(&keypair.MBEDTLS_PRIVATE(grp), &Q, pub,
                                    65) != 0) {
    goto exit;
  }
...
void na_mbedtls_free_keypair(void) { mbedtls_ecp_keypair_free(&keypair); }
#endif
