...
os_membuf_t loopback_mbuf_membuf[OS_MEMPOOL_SIZE(
...
bool bt_mesh_net_iv_update(uint32_t iv_index, bool iv_update) {
  if (atomic_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS)) {
    ...
    if (iv_index != bt_mesh.iv_index) {
      BT_WARN("IV Index mismatch: 0x%08x != 0x%08x", (unsigned)iv_index,
    ...
    if (iv_index < bt_mesh.iv_index || iv_index > bt_mesh.iv_index + 42) {
      BT_ERR("IV Index out of sync: 0x%08x != 0x%08x", (unsigned)iv_index,
             (unsigned)bt_mesh.iv_index);
      return false;
    }
...
static const struct bt_mesh_net_cred *
net_tx_cred_get(struct bt_mesh_net_tx *tx) {
...
static int net_header_encode(struct bt_mesh_net_tx *tx, uint8_t nid,
  ...
  BT_DBG("src 0x%04x dst 0x%04x ctl %u seq 0x%06x", tx->src, tx->ctx->addr, ctl,
         bt_mesh.seq);
...
static int net_encrypt(struct os_mbuf *buf, const struct bt_mesh_net_cred *cred,
...
int bt_mesh_net_send(struct bt_mesh_net_tx *tx, struct os_mbuf *buf,
  ...
  BT_DBG("src 0x%04x dst 0x%04x len %u headroom %zu tailroom %zu", tx->src,
         tx->ctx->addr, buf->om_len, net_buf_headroom(buf),
  ...
  if (bt_mesh_fixed_group_match(tx->ctx->addr) ||
    ...
    if (BT_MESH_ADDR_IS_UNICAST(tx->ctx->addr) || tx->ctx->send_ttl == 1U) {
      if (!err) {
...
static bool net_decrypt(struct bt_mesh_net_rx *rx, struct os_mbuf *in,
  ...
  return bt_mesh_net_decrypt(cred->enc, out, BT_MESH_NET_IVI_RX(rx), proxy) ==
         0;
}
...
static void bt_mesh_net_relay(struct os_mbuf *sbuf, struct bt_mesh_net_rx *rx) {
  const struct bt_mesh_net_cred *cred;
  struct os_mbuf *buf;
  uint8_t transmit;
...
void bt_mesh_net_header_parse(struct os_mbuf *buf, struct bt_mesh_net_rx *rx) {
...
void bt_mesh_net_recv(struct os_mbuf *data, int8_t rssi,
  ...
  if ((MYNEWT_VAL(BLE_MESH_GATT_PROXY)) && net_if == BT_MESH_NET_IF_PROXY) {
...
static void ivu_refresh(struct ble_npl_event *work) {
  ...
  bt_mesh.ivu_duration =
    MIN(UINT8_MAX, bt_mesh.ivu_duration + BT_MESH_IVU_HOURS);
...
void bt_mesh_net_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_net conf");

  rc = conf_register(&bt_mesh_iv_conf_handler);

  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_iv conf");

  rc = conf_register(&bt_mesh_seq_conf_handler);

  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_seq conf");
#endif
  ...
  rc =
    os_mempool_init(&loopback_buf_mempool, MYNEWT_VAL(BLE_MESH_LOOPBACK_BUFS),
                    LOOPBACK_MAX_PDU_LEN + BT_MESH_MBUF_HEADER_SIZE,
