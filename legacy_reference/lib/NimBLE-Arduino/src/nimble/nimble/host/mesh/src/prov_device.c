...
static int reset_state(void) { return bt_mesh_prov_reset_state(pub_key_ready); }
...
static void prov_invite(const uint8_t *data) {
  ...
  net_buf_simple_add_u8(
    buf, bt_mesh_prov->public_key_be == NULL ? PUB_KEY_NO_OOB : PUB_KEY_OOB);
  ...
  memcpy(bt_mesh_prov_link.conf_inputs.capabilities, &buf->om_data[1],
...
static void prov_start(const uint8_t *data) {
  ...
  if (data[1] > PUB_KEY_OOB ||
      (data[1] == PUB_KEY_OOB && (!MYNEWT_VAL(BLE_MESH_PROV_OOB_PUBLIC_KEY) ||
  ...
  atomic_set_bit_to(bt_mesh_prov_link.flags, OOB_PUB_KEY,
                    data[1] == PUB_KEY_OOB);
...
static void send_pub_key(void) {
  ...
  sys_memcpy_swap(net_buf_simple_add(buf, BT_PUB_KEY_COORD_LEN),
                  &key[BT_PUB_KEY_COORD_LEN], 32);

  BT_DBG("Local Public Key: %s", bt_hex(buf->om_data + 1, BT_PUB_KEY_LEN));

  /* PublicKeyDevice */
  memcpy(bt_mesh_prov_link.conf_inputs.pub_key_device, &buf->om_data[1],
         PDU_LEN_PUB_KEY);
...
static void prov_pub_key(const uint8_t *data) {
  ...
  memcpy(bt_mesh_prov_link.conf_inputs.pub_key_provisioner, data,
         PDU_LEN_PUB_KEY);
  ...
  if (MYNEWT_VAL(BLE_MESH_PROV_OOB_PUBLIC_KEY) &&
    ...
    memcpy(bt_mesh_prov_link.conf_inputs.pub_key_device,
           bt_mesh_prov->public_key_be, PDU_LEN_PUB_KEY);
...
static void prov_link_opened(void) { bt_mesh_prov_link.expect = PROV_INVITE; }
