...
static int health_fault_status(struct bt_mesh_model *model,
  ...
  if (!bt_mesh_msg_ack_ctx_match(&health_cli->ack_ctx, OP_HEALTH_FAULT_STATUS,
                                 ctx->addr, (void **)&param)) {
    return -ENOENT;
  }
...
static int health_period_status(struct bt_mesh_model *model,
  ...
  if (!bt_mesh_msg_ack_ctx_match(&health_cli->ack_ctx, OP_HEALTH_PERIOD_STATUS,
                                 ctx->addr, (void **)&param)) {
    return -ENOENT;
  }
...
int32_t bt_mesh_health_cli_timeout_get(void) { return msg_timeout; }

void bt_mesh_health_cli_timeout_set(int32_t timeout) { msg_timeout = timeout; }
