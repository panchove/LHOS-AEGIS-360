...
static int ble_mesh_ext_adv_event_handler(struct ble_gap_event *event,
                                          void *arg) {
  int64_t duration;
...
static int adv_start(const struct ble_gap_ext_adv_params *param,
                     uint32_t timeout, const struct bt_data *ad, size_t ad_len,
  ...
  if (atomic_test_bit(adv.flags, ADV_FLAG_UPDATE_PARAMS)) {
    ...
    atomic_set_bit_to(adv.flags, ADV_FLAG_UPDATE_PARAMS, param != &adv_param);
  }
...
static int buf_send(struct os_mbuf *buf) {
  ...
  struct bt_le_ext_adv_start_param start = {
    .num_events = BT_MESH_TRANSMIT_COUNT(BT_MESH_ADV(buf)->xmit) + 1,
  };
  uint16_t duration, adv_int;
  struct bt_data ad;
  int err;

  adv_int = MAX(ADV_INT_FAST_MS, BT_MESH_TRANSMIT_INT(BT_MESH_ADV(buf)->xmit));
  /* Upper boundary estimate: */
  duration = start.num_events * (adv_int + 10);

  BT_DBG("type %u len %u: %s", BT_MESH_ADV(buf)->type, buf->om_len,
...
void bt_mesh_adv_buf_ready(void) { schedule_send(); }
...
int bt_mesh_adv_start(const struct ble_gap_adv_params *param, int32_t duration,
  ...
  struct ble_gap_ext_adv_params params = {.itvl_min = param->itvl_min,
