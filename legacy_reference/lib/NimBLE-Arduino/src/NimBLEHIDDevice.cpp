...
NimBLEHIDDevice::NimBLEHIDDevice(NimBLEServer *server) {
  ...
  m_pnpChr =
    m_deviceInfoSvc->createCharacteristic(pnpCharUuid, NIMBLE_PROPERTY::READ);

  // Mandatory characteristics for HID service
  m_hidInfoChr =
    m_hidSvc->createCharacteristic(hidInfoCharUuid, NIMBLE_PROPERTY::READ);
  m_reportMapChr =
    m_hidSvc->createCharacteristic(reportMapCharUuid, NIMBLE_PROPERTY::READ);
  m_hidControlChr = m_hidSvc->createCharacteristic(hidControlCharUuid,
                                                   NIMBLE_PROPERTY::WRITE_NR);
  m_protocolModeChr = m_hidSvc->createCharacteristic(
    protocolModeCharUuid, NIMBLE_PROPERTY::WRITE_NR | NIMBLE_PROPERTY::READ);

  // Mandatory battery level characteristic with notification and presence
  // descriptor
  m_batteryLevelChr = m_batterySvc->createCharacteristic(
    batteryLevelCharUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
  ...
  // This value is setup here because its default value in most usage cases,
  // it's very rare to use boot mode
...
bool NimBLEHIDDevice::setManufacturer(const std::string &name) {
  if (m_manufacturerChr == nullptr) {
    m_manufacturerChr = m_deviceInfoSvc->createCharacteristic(
      m_manufacturerChrUuid, NIMBLE_PROPERTY::READ);
  }
 ...
 */
void NimBLEHIDDevice::setPnp(uint8_t sig, uint16_t vid, uint16_t pid,
                             uint16_t version) {
  uint8_t pnp[] = {sig,
 ...
 * @param [in] notify If true sends a notification to the peer device, otherwise
 * not. default = false
 */
...
NimBLEHIDDevice::locateReportCharacteristicByIdAndType(uint8_t reportId,
                                                       uint8_t reportType) {
  NimBLECharacteristic *candidate =
    m_hidSvc->getCharacteristic(inputReportChrUuid, 0);
  for (uint16_t i = 1; (candidate != nullptr) && (i != 0); i++) {
    NimBLEDescriptor *dsc =
      candidate->getDescriptorByUUID(featureReportDscUuid);
 ...
 * @param [in] reportId Input report ID, the same as in report map for input
 * object related to the characteristic.
 * @return NimBLECharacteristic* A pointer to the input report characteristic.
 *                               Store this value to avoid computational
 * overhead.
...
NimBLECharacteristic *NimBLEHIDDevice::getInputReport(uint8_t reportId) {
  NimBLECharacteristic *inputReportChr =
    locateReportCharacteristicByIdAndType(reportId, 0x01);
  if (inputReportChr == nullptr) {
    inputReportChr = m_hidSvc->createCharacteristic(
      inputReportChrUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY |
                            NIMBLE_PROPERTY::READ_ENC);
    NimBLEDescriptor *inputReportDsc = inputReportChr->createDescriptor(
      featureReportDscUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::READ_ENC);
 ...
 * @param [in] reportId Output report ID, the same as in report map for output
 * object related to the characteristic.
...
NimBLECharacteristic *NimBLEHIDDevice::getOutputReport(uint8_t reportId) {
  NimBLECharacteristic *outputReportChr =
    locateReportCharacteristicByIdAndType(reportId, 0x02);
  if (outputReportChr == nullptr) {
    outputReportChr = m_hidSvc->createCharacteristic(
      inputReportChrUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE |
                            NIMBLE_PROPERTY::WRITE_NR |
                            NIMBLE_PROPERTY::READ_ENC |
                            NIMBLE_PROPERTY::WRITE_ENC);
    NimBLEDescriptor *outputReportDsc = outputReportChr->createDescriptor(
      featureReportDscUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE |
                              NIMBLE_PROPERTY::READ_ENC |
                              NIMBLE_PROPERTY::WRITE_ENC);
 ...
 * @param [in] reportId Feature report ID, the same as in report map for feature
 * object related to the characteristic.
...
NimBLECharacteristic *NimBLEHIDDevice::getFeatureReport(uint8_t reportId) {
  NimBLECharacteristic *featureReportChr =
    locateReportCharacteristicByIdAndType(reportId, 0x03);
 ...
 * @returns A pointer to the boot input report characteristic, or nullptr on
 * error.
 * @details This will create the characteristic if not already created.
 */
NimBLECharacteristic *NimBLEHIDDevice::getBootInput() {
  NimBLECharacteristic *bootInputChr =
    m_hidSvc->getCharacteristic(bootInputChrUuid);
  if (bootInputChr) {
    return bootInputChr;
  }

  return m_hidSvc->createCharacteristic(bootInputChrUuid,
                                        NIMBLE_PROPERTY::NOTIFY);
} // getBootInput

/**
 * @brief Create a keyboard boot output report characteristic
 * @returns A pointer to the boot output report characteristic, or nullptr on
 * error.
 * @details This will create the characteristic if not already created.
 */
NimBLECharacteristic *NimBLEHIDDevice::getBootOutput() {
  NimBLECharacteristic *bootOutputChr =
    m_hidSvc->getCharacteristic(bootOutputChrUuid);
  if (bootOutputChr) {
 ...
 */
NimBLECharacteristic *NimBLEHIDDevice::getPnp() { return m_pnpChr; } // getPnp
