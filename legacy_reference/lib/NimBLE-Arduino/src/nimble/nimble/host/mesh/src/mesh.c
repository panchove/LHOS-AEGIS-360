...
int bt_mesh_provision(const uint8_t net_key[16], uint16_t net_idx,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_CDB) &&
    ...
    node = bt_mesh_cdb_node_alloc(prov->uuid, addr, comp->elem_count, net_idx);
    if (node == NULL) {
...
int bt_mesh_provision_adv(const uint8_t uuid[16], uint16_t net_idx,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_PROVISIONER) &&
    ...
    return bt_mesh_pb_adv_open(uuid, net_idx, addr, attention_duration);
  }
...
static void model_resume(struct bt_mesh_model *mod, struct bt_mesh_elem *elem,
  ...
  if (mod->pub && mod->pub->update) {
    ...
    if (period_ms) {
      k_work_reschedule(&mod->pub->timer, K_MSEC(period_ms));
    }
  }
}
...
int bt_mesh_init(uint8_t own_addr_type, const struct bt_mesh_prov *prov,
  ...
  ble_gap_event_listener_register(&mesh_event_listener, bt_mesh_gap_event,
                                  NULL);
