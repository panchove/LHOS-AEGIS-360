...
static const uint8_t ble_sm_sc_init_ioa[5 /*resp*/][5 /*init*/] = {
  ...
  /*e*/
  {IOACT_NONE, IOACT_NUMCMP, IOACT_INPUT, IOACT_NONE, IOACT_NUMCMP},
  /*s*/
...
void ble_sm_sc_oob_confirm(struct ble_sm_proc *proc,
...
static void ble_sm_sc_gen_numcmp(struct ble_sm_proc *proc,
                                 struct ble_sm_result *res) {
  uint8_t *pka;
  uint8_t *pkb;
...
void ble_sm_sc_random_rx(struct ble_sm_proc *proc, struct ble_sm_result *res) {
  ...
  rc = na_ble_sm_alg_f5(proc->dhkey, proc->randm, proc->rands, iat, ia, rat, ra,
...
static void ble_sm_sc_dhkey_addrs(struct ble_sm_proc *proc,
                                  ble_addr_t *our_addr, ble_addr_t *peer_addr) {
...
static void ble_sm_dhkey_check_process(struct ble_sm_proc *proc,
  ...
  res->app_status = na_ble_sm_alg_f6(proc->mackey, ble_sm_peer_pair_rand(proc),
                                     ble_sm_our_pair_rand(proc), proc->tk,
                                     iocap, peer_addr.type, peer_addr.val,
