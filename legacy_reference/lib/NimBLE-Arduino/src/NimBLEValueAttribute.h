...
public:
   ...
   * @param [in] timestamp (Optional) A pointer to a time_t struct to get the
   * time the value set.
   * @return A copy of the attribute value.
   */
  NimBLEAttValue getValue(time_t *timestamp) const {
   ...
   * @param [in] timestamp (Optional) A pointer to a time_t struct to get the
   * time the value set.
   * @param [in] skipSizeCheck (Optional) If true it will skip checking if the
   * data size is less than <tt>sizeof(<type\>)</tt>.
   ...
   * Used for types that are trivially copyable and convertible to
   * NimBLEAttValue.
   */
  template <typename T>
   ...
   * the data is less than <tt>sizeof(<type\>)</tt>.
   * @details <b>Use:</b> <tt>getValue<type>(&timestamp, skipSizeCheck);</tt>
   * Used for types that are not trivially copyable and convertible to
   * NimBLEAttValue via it's operators.
   */
  template <typename T>
  typename std::enable_if<!std::is_trivially_copyable<T>::value &&
                            std::is_convertible<T, NimBLEAttValue>::value,
                          T>::type
  getValue(time_t *timestamp = nullptr, bool skipSizeCheck = false) const {
    return m_value;
  }
