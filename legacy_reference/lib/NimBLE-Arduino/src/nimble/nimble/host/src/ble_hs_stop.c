...
static void ble_hs_stop_done(int status) {
  ...
  SLIST_FOREACH(listener, &slist, link) { listener->fn(status, listener->arg); }
}
...
static int ble_hs_stop_terminate_conn(struct ble_hs_conn *conn, void *arg) {
  ...
  if (rc == 0) {
    ...
    /* If failed, just make sure we are not going to wait for connection
     * complete event, just count it as already disconnected
     */
...
/**
 * This is called when host graceful disconnect timeout fires. That means some
 * devices are out of range and disconnection completed did no happen yet.
 */
...
int ble_hs_stop(struct ble_hs_stop_listener *listener, ble_hs_stop_fn *fn,
                void *arg) {
  int rc;
...
#else
  ble_npl_callout_init(&ble_hs_stop_terminate_tmo,
