...
extern const uint8_t
  ca_cert_bundle_start[] asm("_binary_data_crt_x509_crt_bundle_bin_start");
extern const uint8_t
  ca_cert_bundle_end[] asm("_binary_data_crt_x509_crt_bundle_bin_end");
...
void testGET(const char *host, uint16_t port, const char *resource) {
  ...
  securePresentationLayer.printf(
    "GET %s HTTP/1.1\r\nHost: %s\r\nConnection: close\r\n\n",
    resource == nullptr ? "/" : resource, host);

  // Wait a little to receive some data
  uint32_t start = millis();
  while (securePresentationLayer.connected() &&
         !securePresentationLayer.available() &&
         ((millis() - start) < 10000L)) {
    delay(10);
  }
