...
static void store_rpl(struct bt_mesh_rpl *entry) {
  ...
  BT_DBG("src 0x%04x seq 0x%06x old_iv %u", entry->src, (unsigned)entry->seq,
         entry->old_iv);
...
void bt_mesh_rpl_pending_store(uint16_t addr) {
  ...
  if (!IS_ENABLED(CONFIG_BT_SETTINGS) ||
      (!BT_MESH_ADDR_IS_UNICAST(addr) && addr != BT_MESH_ADDR_ALL_NODES)) {
    return;
  }
  ...
  for (i = 0; i < ARRAY_SIZE(replay_list); i++) {
    if (addr != BT_MESH_ADDR_ALL_NODES && addr != replay_list[i].src) {
      continue;
    }
...
void bt_mesh_rpl_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_rpl conf");
#endif
}
