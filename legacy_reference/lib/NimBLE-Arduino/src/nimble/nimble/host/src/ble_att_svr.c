...
static os_membuf_t ble_att_svr_prep_entry_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_ATT_SVR_MAX_PREP_ENTRIES), sizeof(struct ble_att_prep_entry))];
...
uint16_t ble_att_svr_prev_handle(void) { return ble_att_svr_id; }
...
ble_att_svr_find_by_uuid(struct ble_att_svr_entry *prev, const ble_uuid_t *uuid,
  ...
  for (; entry != NULL && entry->ha_handle_id <= end_handle;
       entry = STAILQ_NEXT(entry, ha_next)) {
...
static int ble_att_svr_pullup_req_base(struct os_mbuf **om, int base_len,
                                       uint8_t *out_att_err) {
  uint8_t att_err;
  int rc;
...
static void ble_att_svr_get_sec_state(uint16_t conn_handle,
...
static int ble_att_svr_check_perms(uint16_t conn_handle, int is_read,
  ...
  if (ble_hs_cfg.sm_sc_only) {
    ...
    } else if (sec_state.authenticated && sec_state.encrypted &&
 ...
 */
static int ble_att_svr_pkt(struct os_mbuf **rxom, struct os_mbuf **out_txom,
                           uint8_t *out_att_err) {
...
static int ble_att_svr_read(uint16_t conn_handle,
                            struct ble_att_svr_entry *entry, uint16_t offset,
                            struct os_mbuf *om, uint8_t *out_att_err) {
  uint8_t att_err;
  int rc;
  ...
  rc = entry->ha_cb(conn_handle, entry->ha_handle_id, BLE_ATT_ACCESS_OP_READ,
                    offset, &om, entry->ha_cb_arg);
  if (rc != 0) {
    att_err = rc;
    rc = BLE_HS_EAPP;
    goto err;
...
static int ble_att_svr_read_flat(uint16_t conn_handle,
                                 struct ble_att_svr_entry *entry,
                                 uint16_t offset, uint16_t max_len, void *dst,
                                 uint16_t *out_len, uint8_t *out_att_err) {
  struct os_mbuf *om;
  uint16_t len;
  int rc;
...
int ble_att_svr_read_local(uint16_t attr_handle, struct os_mbuf **out_om) {
  ...
  rc =
    ble_att_svr_read_handle(BLE_HS_CONN_HANDLE_NONE, attr_handle, 0, om, NULL);
  if (rc != 0) {
    goto err;
  }
...
static int ble_att_svr_write(uint16_t conn_handle,
                             struct ble_att_svr_entry *entry, uint16_t offset,
                             struct os_mbuf **om, uint8_t *out_att_err) {
  uint8_t att_err = 0;
  int rc;
  ...
  BLE_HS_DBG_ASSERT(entry->ha_cb != NULL);
  rc = entry->ha_cb(conn_handle, entry->ha_handle_id, BLE_ATT_ACCESS_OP_WRITE,
                    offset, om, entry->ha_cb_arg);
  if (rc != 0) {
    att_err = rc;
    rc = BLE_HS_EAPP;
...
int ble_att_svr_tx_error_rsp(uint16_t conn_handle, uint16_t cid,
 ...
 *                                  field.
 */
static int ble_att_svr_tx_rsp(uint16_t conn_handle, uint16_t cid, int hs_status,
  ...
  if (do_tx) {
    ...
    if (hs_status != 0) {
      ...
      if (om != NULL) {
        ble_att_svr_tx_error_rsp(conn_handle, cid, om, att_op, err_handle,
                                 err_status);
        om = NULL;
      }
    }
...
static int ble_att_svr_build_mtu_rsp(uint16_t conn_handle,
                                     struct os_mbuf **rxom,
...
int ble_att_svr_rx_mtu(uint16_t conn_handle, uint16_t cid,
                       struct os_mbuf **rxom) {
  struct ble_att_mtu_cmd *cmd;
  struct ble_l2cap_chan *chan;
  struct ble_hs_conn *conn;
  ...
  rc = ble_att_svr_tx_rsp(conn_handle, BLE_L2CAP_CID_ATT, rc, txom,
                          BLE_ATT_OP_MTU_REQ, att_err, 0);
  if (rc == 0) {
    ble_hs_lock();
...
static int ble_att_svr_fill_info(uint16_t start_handle, uint16_t end_handle,
                                 struct os_mbuf *om, uint16_t mtu,
                                 uint8_t *format) {
...
static int ble_att_svr_build_find_info_rsp(uint16_t conn_handle, uint16_t cid,
...
int ble_att_svr_rx_find_info(uint16_t conn_handle, uint16_t cid,
                             struct os_mbuf **rxom) {
  ...
  rc = ble_att_svr_build_find_info_rsp(conn_handle, cid, start_handle,
                                       end_handle, rxom, &txom, &att_err);
  if (rc != 0) {
    err_handle = start_handle;
    goto done;
  }
...
static int ble_att_svr_is_valid_group_end(const ble_uuid_t *uuid_group,
 ...
 */
static int
ble_att_svr_fill_type_value(uint16_t conn_handle, uint16_t start_handle,
                            uint16_t end_handle, ble_uuid16_t attr_type,
  ...
  STAILQ_FOREACH(ha, &ble_att_svr_list, ha_next) {
    ...
    if (first) {
      ...
      rc =
        ble_att_svr_fill_type_value_entry(txom, first, prev, mtu, out_att_err);
      if (rc != BLE_HS_EAGAIN) {
        goto done;
      }
    ...
    if (ble_uuid_cmp(ha->ha_uuid, &attr_type.u) == 0) {
      rc = ble_att_svr_read_flat(conn_handle, ha, 0, sizeof buf, buf, &attr_len,
                                 out_att_err);
      if (rc != 0) {
        goto done;
      }
      /* value is at the end of req */
      rc = os_mbuf_cmpf(rxom, sizeof(struct ble_att_find_type_value_req), buf,
                        attr_len);
      if (rc == 0) {
   ...
   */
  if (first) {
    rc = ble_att_svr_fill_type_value_entry(txom, first, prev, mtu, out_att_err);
    if (rc == BLE_HS_EAGAIN) {
      rc = 0;
    }
  } else {
  ...
  any_entries =
    OS_MBUF_PKTHDR(txom)->omp_len > BLE_ATT_FIND_TYPE_VALUE_RSP_BASE_SZ;
  if (rc == 0 && !any_entries) {
...
static int ble_att_svr_build_find_type_value_rsp(
  uint16_t conn_handle, uint16_t cid, uint16_t start_handle,
  uint16_t end_handle, ble_uuid16_t attr_type, struct os_mbuf **rxom,
  ...
  rc = ble_att_svr_fill_type_value(conn_handle, start_handle, end_handle,
                                   attr_type, *rxom, txom, mtu, out_att_err);
  if (rc != 0) {
    goto done;
  }
...
int ble_att_svr_rx_find_type_value(uint16_t conn_handle, uint16_t cid,
  ...
  rc = ble_att_svr_build_find_type_value_rsp(conn_handle, cid, start_handle,
                                             end_handle, attr_type, rxom, &txom,
                                             &att_err);
  if (rc != 0) {
    err_handle = start_handle;
    goto done;
  ...
  rc = ble_att_svr_tx_rsp(conn_handle, cid, rc, txom,
                          BLE_ATT_OP_FIND_TYPE_VALUE_REQ, att_err, err_handle);
  return rc;
}
...
static int ble_att_svr_build_read_type_rsp(
  uint16_t conn_handle, uint16_t cid, uint16_t start_handle,
  uint16_t end_handle, const ble_uuid_t *uuid, struct os_mbuf **rxom,
  struct os_mbuf **out_txom, uint8_t *att_err, uint16_t *err_handle) {
...
int ble_att_svr_rx_read_type(uint16_t conn_handle, uint16_t cid,
  ...
  ble_hs_unlock();
#endif
  rc = ble_att_svr_build_read_type_rsp(conn_handle, cid, start_handle,
                                       end_handle, &uuid.u, rxom, &txom,
                                       &att_err, &err_handle);
  if (rc != 0) {
    goto done;
  }
...
int ble_att_svr_rx_read_blob(uint16_t conn_handle, uint16_t cid,
  ...
  *rxom = NULL;
  ...
  rc = ble_att_svr_read_handle(conn_handle, err_handle, offset, txom, &att_err);
  if (rc != 0) {
    goto done;
  }
...
static int ble_att_svr_build_read_mult_rsp(uint16_t conn_handle, uint16_t cid,
                                           struct os_mbuf **rxom,
                                           struct os_mbuf **out_txom,
                                           uint8_t *att_err,
                                           uint16_t *err_handle) {
...
static int ble_att_svr_build_read_mult_rsp_var(
  uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom,
  struct os_mbuf **out_txom, uint8_t *att_err, uint16_t *err_handle) {
  struct os_mbuf *txom;
  uint16_t handle;
  uint16_t mtu;
...
static int ble_att_svr_service_uuid(struct ble_att_svr_entry *entry,
  ...
  rc = ble_att_svr_read_flat(BLE_HS_CONN_HANDLE_NONE, entry, 0, sizeof(val),
                             val, &attr_len, out_att_err);
  if (rc != 0) {
    return rc;
  }
...
static int ble_att_svr_read_group_type_entry_write(
  struct os_mbuf *om, uint16_t mtu, uint16_t start_group_handle,
  uint16_t end_group_handle, const ble_uuid_t *service_uuid) {
  uint8_t *buf;
  int len;
 ...
 */
static int ble_att_svr_build_read_group_type_rsp(
  uint16_t conn_handle, uint16_t cid, uint16_t start_handle,
  uint16_t end_handle, const ble_uuid_t *group_uuid, struct os_mbuf **rxom,
  struct os_mbuf **out_txom, uint8_t *att_err, uint16_t *err_handle) {
  ...
  *rxom = NULL;
  ...
  rsp = ble_att_cmd_prepare(BLE_ATT_OP_READ_GROUP_TYPE_RSP, sizeof(*rsp), txom);
  if (rsp == NULL) {
    *att_err = BLE_ATT_ERR_INSUFFICIENT_RES;
    rc = BLE_HS_ENOMEM;
    goto done;
    ...
    if (start_group_handle != 0) {
      ...
      if (!ble_att_svr_is_valid_read_group_type(entry->ha_uuid)) {
        ...
        rc = ble_att_svr_read_group_type_entry_write(
          txom, mtu, start_group_handle, end_group_handle, &service_uuid.u);
        start_group_handle = 0;
        end_group_handle = 0;
        if (rc != 0) {
          *err_handle = entry->ha_handle_id;
...
int ble_att_svr_rx_read_group_type(uint16_t conn_handle, uint16_t cid,
  ...
  rc = ble_att_svr_build_read_group_type_rsp(conn_handle, cid, start_handle,
                                             end_handle, &uuid.u, rxom, &txom,
                                             &att_err, &err_handle);
  if (rc != 0) {
    goto done;
  ...
  rc = ble_att_svr_tx_rsp(conn_handle, cid, rc, txom,
                          BLE_ATT_OP_READ_GROUP_TYPE_REQ, att_err, err_handle);
  return rc;
}

static int ble_att_svr_build_write_rsp(struct os_mbuf **rxom,
                                       struct os_mbuf **out_txom,
                                       uint8_t *att_err) {
  struct os_mbuf *txom;
  int rc;
...
int ble_att_svr_rx_write_no_rsp(uint16_t conn_handle, uint16_t cid,
                                struct os_mbuf **rxom) {
...
int ble_att_svr_rx_signed_write(uint16_t conn_handle, uint16_t cid,
                                struct os_mbuf **rxom) {
  ...
  os_mbuf_copydata(
    *rxom,
    ...
    BLE_ATT_SIGNED_WRITE_CMD_BASE_SZ - BLE_ATT_SIGNED_WRITE_DATA_OFFSET, sign);

  /* Strip the signature from the end of the mbuf. */
  os_mbuf_adj(*rxom, -(BLE_ATT_SIGNED_WRITE_CMD_BASE_SZ -
                       BLE_ATT_SIGNED_WRITE_DATA_OFFSET));
  ...
  memcpy(&message[1 + OS_MBUF_PKTLEN(*rxom)], &value_sec.sign_counter,
         sizeof(value_sec.sign_counter));
  ...
  if (memcmp(sign, &value_sec.sign_counter, sizeof(value_sec.sign_counter)) !=
      0) {
    rc = BLE_HS_EAUTHEN;
    goto err;
  }
  ...
  /* Comparing signature */
  if (memcmp(&sign[sizeof(value_sec.sign_counter)], &cmac[sizeof(cmac) / 2],
             sizeof(cmac) / 2) != 0) {
    rc = BLE_HS_EAUTHEN;
    goto err;
  }

  /* Signature matches, increment sign counter and pass the data to the upper
   * layer */
...
int ble_att_svr_write_local(uint16_t attr_handle, struct os_mbuf *om) {
  ...
  rc = ble_att_svr_write_handle(BLE_HS_CONN_HANDLE_NONE, attr_handle, 0, &om,
                                NULL);
...
static int ble_att_svr_insert_prep_entry(uint16_t conn_handle, uint16_t handle,
                                         uint16_t offset,
  ...
  rc = os_mbuf_appendfrom(
    prep_entry->bape_value, rxom, sizeof(struct ble_att_prep_write_cmd),
  ...
  prep_prev = ble_att_svr_prep_find_prev(&conn->bhc_att_svr, handle, offset);
  if (prep_prev == NULL) {
    SLIST_INSERT_HEAD(&conn->bhc_att_svr.basc_prep_list, prep_entry, bape_next);
  } else {
...
int ble_att_svr_rx_prep_write(uint16_t conn_handle, uint16_t cid,
  ...
  ble_hs_lock();
  rc =
    ble_att_svr_insert_prep_entry(conn_handle, le16toh(req->bapc_handle),
                                  le16toh(req->bapc_offset), *rxom, &att_err);
  ble_hs_unlock();
  ...
  *rxom = NULL;
  ...
  if (rc == 0) {
    ...
    if (flags) {
      ...
      att_err = ble_att_svr_prep_write(conn_handle, &prep_list, &err_handle);
      if (att_err != 0) {
        rc = BLE_HS_EAPP;
      }
    }
...
int ble_att_svr_rx_notify_multi(uint16_t conn_handle, uint16_t cid,
  ...
  while (pkt_len > 0) {
    rc = ble_att_svr_pullup_req_base(rxom, sizeof(struct ble_att_tuple_list),
                                     NULL);
    if (rc != 0) {
      return BLE_HS_ENOMEM;
    }
    ...
    if (attr_len > BLE_ATT_ATTR_MAX_LEN) {
      BLE_HS_LOG_ERROR("attr length (%d) > max (%d)", attr_len,
 ...
 * @return                      0 on success; nonzero on failure.
 */
static int ble_att_svr_build_indicate_rsp(struct os_mbuf **rxom,
                                          struct os_mbuf **out_txom,
                                          uint8_t *out_att_err) {
  struct os_mbuf *txom;
  int rc;
...
void ble_att_svr_stop(void) { ble_att_svr_free_start_mem(); }
...
int ble_att_svr_init(void) {
  ...
  if (MYNEWT_VAL(BLE_ATT_SVR_MAX_PREP_ENTRIES) > 0) {
    rc = os_mempool_init(
      &ble_att_svr_prep_entry_pool, MYNEWT_VAL(BLE_ATT_SVR_MAX_PREP_ENTRIES),
      sizeof(struct ble_att_prep_entry), ble_att_svr_prep_entry_mem,
...
int ble_att_get_database_size(int *out_size) {
  ...
  for (entry = STAILQ_FIRST(&ble_att_svr_list); entry != NULL;
    ...
    } else if (uuid->value == 0x2901 || uuid->value == 0x2902 ||
               uuid->value == 0x2903 || uuid->value == 0x2904 ||
               uuid->value == 0x2905) {
      /* handle + type */
      size += 4;
 ...
 */
uint8_t ble_att_svr_security_mode_1_level() {
  struct ble_att_svr_entry *entry;
