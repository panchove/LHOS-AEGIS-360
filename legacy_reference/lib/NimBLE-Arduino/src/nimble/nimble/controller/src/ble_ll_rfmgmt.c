...
static void ble_ll_rfmgmt_enable(void) {
  OS_ASSERT_CRITICAL();
...
static void ble_ll_rfmgmt_disable(void) {
  OS_ASSERT_CRITICAL();
...
static void ble_ll_rfmgmt_timer_reschedule(void) {
...
static void ble_ll_rfmgmt_timer_exp(void *arg) {
  g_ble_ll_rfmgmt_data.timer_scheduled = false;
  ble_ll_rfmgmt_enable();
}

static void ble_ll_rfmgmt_release_ev(struct ble_npl_event *ev) {
  struct ble_ll_rfmgmt_data *rfmgmt = &g_ble_ll_rfmgmt_data;
  uint32_t now;
  bool can_disable;
  uint8_t lls;
  ...
  if (rfmgmt->enable_scan && CPUTIME_GEQ(now, rfmgmt->enable_scan_at)) {
    ...
    can_disable = false;
             ...
             CPUTIME_GEQ(now, rfmgmt->enable_sched_at)) {
...
static uint32_t ble_ll_rfmgmt_ticks_to_enabled(void) {
  struct ble_ll_rfmgmt_data *rfmgmt = &g_ble_ll_rfmgmt_data;
  uint32_t rem_ticks;
  uint32_t now;
