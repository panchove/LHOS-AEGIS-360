...
static int
ble_ead_generate_randomizer(uint8_t randomizer[BLE_EAD_RANDOMIZER_SIZE]) {
  int err;
...
static int
ble_ead_generate_nonce(const uint8_t iv[BLE_EAD_IV_SIZE],
...
static int ead_encrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE],
  ...
  err = ble_aes_ccm_encrypt(
    session_key, nonce, payload, payload_size, ble_ead_aad, BLE_EAD_AAD_SIZE,
    &encrypted_payload[BLE_EAD_RANDOMIZER_SIZE], BLE_EAD_MIC_SIZE);

  if (err != 0) {
    BLE_HS_LOG(DEBUG, "Failed to encrypt the payload (ble_ccm_encrypt err %d)",
               err);
    return -1;
  }
...
int ble_ead_encrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE],
  ...
  if (payload_size == 0) {
    BLE_HS_LOG(
      DEBUG,
      "payload_size is set to 0. The encrypted result will only contain the "
      "Randomizer and the MIC.");
  }

  return ead_encrypt(session_key, iv, NULL, payload, payload_size,
                     encrypted_payload);
}

static int ead_decrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE],
                       const uint8_t iv[BLE_EAD_IV_SIZE],
  ...
  size_t encrypted_ad_data_size =
    encrypted_payload_size - BLE_EAD_RANDOMIZER_SIZE;
  ...
  err = ble_aes_ccm_decrypt(session_key, nonce, encrypted_ad_data, payload_size,
                            ble_ead_aad, BLE_EAD_AAD_SIZE, payload,
                            BLE_EAD_MIC_SIZE);

  if (err != 0) {
    BLE_HS_LOG(DEBUG, "Failed to decrypt the data (ble_ccm_decrypt err %d)",
               err);
    return -1;
  }
...
int ble_ead_decrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE],
  ...
  if (encrypted_payload_size < BLE_EAD_RANDOMIZER_SIZE + BLE_EAD_MIC_SIZE) {
    ...
    return BLE_HS_EINVAL;
  } else if (encrypted_payload_size ==
             BLE_EAD_RANDOMIZER_SIZE + BLE_EAD_MIC_SIZE) {
    BLE_HS_LOG(
      WARN,
      "encrypted_payload_size not large enough to contain encrypted data.");
  }

  return ead_decrypt(session_key, iv, encrypted_payload, encrypted_payload_size,
                     payload);
}
