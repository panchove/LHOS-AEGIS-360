...
static void (*msg_cb)(uint32_t opcode, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf);
...
uint16_t bt_mesh_primary_addr(void) { return dev_primary_addr; }
...
int bt_mesh_msg_send(struct bt_mesh_msg_ctx *ctx, struct os_mbuf *buf,
...
uint8_t bt_mesh_elem_count(void) { return dev_comp->elem_count; }
...
bool bt_mesh_model_has_key(struct bt_mesh_model *mod, uint16_t key) {
  ...
  for (i = 0; i < ARRAY_SIZE(mod->keys); i++) {
    if (mod->keys[i] == key ||
        (mod->keys[i] == BT_MESH_KEY_DEV_ANY && BT_MESH_IS_DEV_KEY(key))) {
      return true;
    }
  }
...
static const struct bt_mesh_model_op *find_op(struct bt_mesh_elem *elem,
  ...
  for (i = 0U; i < count; i++) {
    ...
    if (CONFIG_BT_MESH_MODEL_VND_MSG_CID_FORCE && cid != UINT32_MAX &&
...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  ...
  BT_DBG("app_idx 0x%04x src 0x%04x dst 0x%04x", rx->ctx.app_idx, rx->ctx.addr,
         rx->ctx.recv_dst);
  BT_DBG("len %u: %s", buf->om_len, bt_hex(buf->om_data, buf->om_len));
  ...
  for (i = 0; i < dev_comp->elem_count; i++) {
    ...
    if ((op->len >= 0) && (buf->om_len < (size_t)op->len)) {
      ...
      BT_ERR("Invalid message size for OpCode 0x%08" PRIx32, opcode);
      continue;
    }
...
int bt_mesh_model_send(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *msg, const struct bt_mesh_send_cb *cb,
                       void *cb_data) {
  ...
  return bt_mesh_msg_send(ctx, msg, bt_mesh_model_elem(model)->addr, cb,
                          cb_data);
}
...
const struct bt_mesh_comp *bt_mesh_comp_get(void) { return dev_comp; }

void bt_mesh_model_extensions_walk(
  struct bt_mesh_model *model,
  enum bt_mesh_walk (*cb)(struct bt_mesh_model *mod, void *user_data),
  void *user_data) {
...
static int mod_set_pub(struct bt_mesh_model *mod, char *val) {
  ...
  BT_DBG("Restored model publication, dst 0x%04x app_idx 0x%03x", pub.addr,
         pub.key);

  return 0;
}

static int mod_data_set(struct bt_mesh_model *mod, char *name, char *len_rd) {
  char *next;
...
static int mod_set(bool vnd, int argc, char **argv, char *val) {
  ...
  BT_DBG("Decoded mod_key 0x%04x as elem_idx %u mod_idx %u", mod_key, elem_idx,
         mod_idx);
...
static void store_pending_mod_bind(struct bt_mesh_model *mod, bool vnd) {
  ...
  if (count) {
    val =
      settings_str_from_bytes(keys, count * sizeof(keys[0]), buf, sizeof(buf));
    if (!val) {
...
static void store_pending_mod_sub(struct bt_mesh_model *mod, bool vnd) {
  ...
  if (count) {
    val = settings_str_from_bytes(groups, count * sizeof(groups[0]), buf,
                                  sizeof(buf));
    if (!val) {
...
void bt_mesh_access_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_access conf");
