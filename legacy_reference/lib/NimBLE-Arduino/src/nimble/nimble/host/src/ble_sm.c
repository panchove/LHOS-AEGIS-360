...
static void ble_sm_dbg_assert_not_inserted(struct ble_sm_proc *proc) {
  ...
  STAILQ_FOREACH(cur, &ble_sm_procs, next) { BLE_HS_DBG_ASSERT(cur != proc); }
#endif
}
...
static void ble_sm_proc_remove(struct ble_sm_proc *proc,
...
static void ble_sm_fill_store_value(const ble_addr_t *peer_addr,
                                    int authenticated, int sc,
...
static void ble_sm_persist_keys(struct ble_sm_proc *proc) {
  ...
  if (proc->peer_keys.addr_valid) {
    ...
    if (memcmp(BLE_ADDR_ANY->val, &conn->bhc_peer_rpa_addr.val, 6) != 0) {
      ...
      if (p_dev_rec != NULL) {
        ...
        memcpy(p_dev_rec->peer_sec.peer_addr.val, proc->peer_keys.addr, 6);
  ...
  memcpy(value_rpa_rec.peer_rpa_addr.val, conn->bhc_peer_rpa_addr.val,
         sizeof conn->bhc_peer_rpa_addr.val);
...
static void ble_sm_exec(struct ble_sm_proc *proc, struct ble_sm_result *res,
                        void *arg) {
  ble_sm_state_fn *cb;
 ...
 */
static int ble_sm_read_bond(uint16_t conn_handle,
...
static void ble_sm_key_dist(struct ble_sm_proc *proc,
                            uint8_t *out_init_key_dist,
...
static int ble_sm_chk_store_overflow_by_type(int obj_type,
...
static int ble_sm_chk_store_overflow(uint16_t conn_handle) {
  ...
  rc =
    ble_sm_chk_store_overflow_by_type(BLE_STORE_OBJ_TYPE_PEER_SEC, conn_handle);
  if (rc != 0) {
    return rc;
  }

  rc =
    ble_sm_chk_store_overflow_by_type(BLE_STORE_OBJ_TYPE_OUR_SEC, conn_handle);
  if (rc != 0) {
    return rc;
  }
...
static int ble_sm_start_encrypt_tx(struct hci_start_encrypt *params) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_START_ENCRYPT), &cmd, sizeof(cmd),
    NULL, 0);
}
...
static void ble_sm_enc_event_rx(uint16_t conn_handle, uint8_t evt_status,
                                int encrypted) {
  struct ble_sm_result res;
  struct ble_sm_proc *proc;
  int authenticated;
...
static int ble_sm_retrieve_ltk(uint16_t ediv, uint64_t rand,
...
int ble_sm_ltk_req_rx(const struct ble_hci_ev_le_subev_lt_key_req *ev) {
  ...
  if (res.app_status == 0) {
    if (restore) {
      store_rc =
        ble_sm_retrieve_ltk(le16toh(ev->div), le64toh(ev->rand),
...
static void ble_sm_random_exec(struct ble_sm_proc *proc,
                               struct ble_sm_result *res, void *arg) {
  if (proc->flags & BLE_SM_PROC_F_SC) {
...
static void ble_sm_confirm_exec(struct ble_sm_proc *proc,
                                struct ble_sm_result *res, void *arg) {
  if (!(proc->flags & BLE_SM_PROC_F_SC)) {
...
static void ble_sm_pair_exec(struct ble_sm_proc *proc,
                             struct ble_sm_result *res, void *arg) {
...
static void ble_sm_pair_req_rx(uint16_t conn_handle, struct os_mbuf **om,
  ...
  if (proc != NULL) {
    ...
    if (conn->bhc_flags & BLE_HS_CONN_F_MASTER) {
      ...
      /* Fail if Secure Connections Only mode is on and SC is not supported by
       * peer
       */
...
static void ble_sm_sec_req_exec(struct ble_sm_proc *proc,
                                struct ble_sm_result *res, void *arg) {
...
static void ble_sm_key_exch_success(struct ble_sm_proc *proc,
                                    struct ble_sm_result *res) {
...
static void ble_sm_key_exch_exec(struct ble_sm_proc *proc,
                                 struct ble_sm_result *res, void *arg) {
  ...
  if (our_key_dist & BLE_SM_PAIR_KEY_DIST_ID) {
    ...
    addr_info =
      ble_sm_cmd_get(BLE_SM_OP_IDENTITY_ADDR_INFO, sizeof(*addr_info), &txom);
    if (!addr_info) {
      rc = BLE_HS_ENOMEM;
      goto err;
    }
...
static void ble_sm_key_rxed(struct ble_sm_proc *proc,
                            struct ble_sm_result *res) {
