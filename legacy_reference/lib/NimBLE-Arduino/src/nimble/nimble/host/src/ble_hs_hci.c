...
static struct err_code core_err_code_list[] = {
  ...
  {BLE_HS_EREJECT,
   ": BLE_HS_EREJECT (Peer rejected a connection parameter update request)"},
  {BLE_HS_EUNKNOWN, ": BLE_HS_EUNKNOWN (Unexpected failure; catch all)"},
  {BLE_HS_EROLE, ": BLE_HS_EROLE (Operation requires different role (e.g., "
                 "central vs. peripheral))"},
  {BLE_HS_ETIMEOUT_HCI,
   ": BLE_HS_ETIMEOUT_HCI (HCI request timed out; controller unresponsive)"},
  {BLE_HS_ENOMEM_EVT, ": BLE_HS_ENOMEM_EVT (Controller failed to send event "
                      "due to memory exhaustion)"},
  {BLE_HS_ENOADDR, ": BLE_HS_ENOADDR (Operation requires an identity address "
                   "but none configured)"},
  {BLE_HS_ENOTSYNCED, ": BLE_HS_ENOTSYNCED (Attempt to use the host before it "
                      "is synced with controller)"},
...
static struct err_code err_code_list[] = {
  {BLE_HS_HCI_ERR(BLE_ERR_UNKNOWN_HCI_CMD),
   ": BLE_ERR_UNKNOWN_HCI_CMD (Unknown HCI Command)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNK_CONN_ID),
   ": BLE_ERR_UNK_CONN_ID (Unknown Connection Identifier)"},
  ...
  {BLE_HS_HCI_ERR(BLE_ERR_AUTH_FAIL),
   ": BLE_ERR_AUTH_FAIL (Authentication Failure)"},
  {BLE_HS_HCI_ERR(BLE_ERR_PINKEY_MISSING),
   ": BLE_ERR_PINKEY_MISSING (PIN or Key Missing)"},
  {BLE_HS_HCI_ERR(BLE_ERR_MEM_CAPACITY),
   ": BLE_ERR_MEM_CAPACITY (Memory Capacity Exceeded)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_SPVN_TMO),
   ": BLE_ERR_CONN_SPVN_TMO (Connection Timeout)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_LIMIT),
   ": BLE_ERR_CONN_LIMIT (Connection Limit Exceeded)"},
  {BLE_HS_HCI_ERR(BLE_ERR_SYNCH_CONN_LIMIT),
   ": BLE_ERR_SYNCH_CONN_LIMIT (Synchronous Connection Limit To A Device "
   "Exceeded)"},
  {BLE_HS_HCI_ERR(BLE_ERR_ACL_CONN_EXISTS),
   ": BLE_ERR_ACL_CONN_EXISTS (ACL Connection Already Exists)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CMD_DISALLOWED),
   ": BLE_ERR_CMD_DISALLOWED (Command Disallowed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_REJ_RESOURCES),
   ": BLE_ERR_CONN_REJ_RESOURCES (Connection Rejected due to Limited "
   "Resources)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_REJ_SECURITY),
   ": BLE_ERR_CONN_REJ_SECURITY (Connection Rejected Due To Security Reasons)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_REJ_BD_ADDR),
   ": BLE_ERR_CONN_REJ_BD_ADDR (Connection Rejected due to Unacceptable "
   "BD_ADDR)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_ACCEPT_TMO),
   ": BLE_ERR_CONN_ACCEPT_TMO (Connection Accept Timeout Exceeded)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNSUPPORTED),
   ": BLE_ERR_UNSUPPORTED (Unsupported Feature or Parameter Value)"},
  {BLE_HS_HCI_ERR(BLE_ERR_INV_HCI_CMD_PARMS),
   ": BLE_ERR_INV_HCI_CMD_PARMS (Invalid HCI Command Parameters)"},
  {BLE_HS_HCI_ERR(BLE_ERR_REM_USER_CONN_TERM),
   ": BLE_ERR_REM_USER_CONN_TERM (Remote User Terminated Connection)"},
  {BLE_HS_HCI_ERR(BLE_ERR_RD_CONN_TERM_RESRCS),
   ": BLE_ERR_RD_CONN_TERM_RESRCS (Remote Device Terminated Connection due to "
   "Low Resources)"},
  {BLE_HS_HCI_ERR(BLE_ERR_RD_CONN_TERM_PWROFF),
   ": BLE_ERR_RD_CONN_TERM_PWROFF (Remote Device Terminated Connection due to "
   "Power Off)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_TERM_LOCAL),
   ": BLE_ERR_CONN_TERM_LOCAL (Connection Terminated By Local Host)"},
  {BLE_HS_HCI_ERR(BLE_ERR_REPEATED_ATTEMPTS),
   ": BLE_ERR_REPEATED_ATTEMPTS (Repeated Attempts)"},
  {BLE_HS_HCI_ERR(BLE_ERR_NO_PAIRING),
   ": BLE_ERR_NO_PAIRING (Pairing Not Allowed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNK_LMP), ": BLE_ERR_UNK_LMP (Unknown LMP PDU)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNSUPP_REM_FEATURE),
   ": BLE_ERR_UNSUPP_REM_FEATURE (Unsupported Remote Feature / Unsupported LMP "
   "Feature)"},
  {BLE_HS_HCI_ERR(BLE_ERR_SCO_OFFSET),
   ": BLE_ERR_SCO_OFFSET (SCO Offset Rejected)"},
  {BLE_HS_HCI_ERR(BLE_ERR_SCO_ITVL),
   ": BLE_ERR_SCO_ITVL (SCO Interval Rejected)"},
  {BLE_HS_HCI_ERR(BLE_ERR_SCO_AIR_MODE),
   ": BLE_ERR_SCO_AIR_MODE (SCO Air Mode Rejected)"},
  {BLE_HS_HCI_ERR(BLE_ERR_INV_LMP_LL_PARM),
   ": BLE_ERR_INV_LMP_LL_PARM (Invalid LMP Parameters / Invalid LL "
   "Parameters)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNSPECIFIED),
   ": BLE_ERR_UNSPECIFIED (Unspecified Error)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNSUPP_LMP_LL_PARM),
   ": BLE_ERR_UNSUPP_LMP_LL_PARM (Unsupported LMP Parameter Value / "
   "Unsupported LL Parameter Value)"},
  {BLE_HS_HCI_ERR(BLE_ERR_NO_ROLE_CHANGE),
   ": BLE_ERR_NO_ROLE_CHANGE (Role Change Not Allowed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_LMP_LL_RSP_TMO),
   ": BLE_ERR_LMP_LL_RSP_TMO (LMP Response Timeout / LL Response Timeout)"},
  {BLE_HS_HCI_ERR(BLE_ERR_LMP_COLLISION),
   ": BLE_ERR_LMP_COLLISION (LMP Error Transaction Collision)"},
  {BLE_HS_HCI_ERR(BLE_ERR_LMP_PDU), ": BLE_ERR_LMP_PDU (LMP PDU Not Allowed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_ENCRYPTION_MODE),
   ": BLE_ERR_ENCRYPTION_MODE (Encryption Mode Not Acceptable)"},
  {BLE_HS_HCI_ERR(BLE_ERR_LINK_KEY_CHANGE),
   ": BLE_ERR_LINK_KEY_CHANGE (Link Key cannot be Changed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNSUPP_QOS),
   ": BLE_ERR_UNSUPP_QOS (Requested QoS Not Supported)"},
  {BLE_HS_HCI_ERR(BLE_ERR_INSTANT_PASSED),
   ": BLE_ERR_INSTANT_PASSED (Instant Passed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNIT_KEY_PAIRING),
   ": BLE_ERR_UNIT_KEY_PAIRING (Pairing With Unit Key Not Supported)"},
  {BLE_HS_HCI_ERR(BLE_ERR_DIFF_TRANS_COLL),
   ": BLE_ERR_DIFF_TRANS_COLL (Different Transaction Collision)"},
  {BLE_HS_HCI_ERR(BLE_ERR_QOS_PARM),
   ": BLE_ERR_QOS_PARM (QoS Unacceptable Parameter)"},
  {BLE_HS_HCI_ERR(BLE_ERR_QOS_REJECTED),
   ": BLE_ERR_QOS_REJECTED (QoS Rejected)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CHAN_CLASS),
   ": BLE_ERR_CHAN_CLASS (Channel Classification Not Supported)"},
  {BLE_HS_HCI_ERR(BLE_ERR_INSUFFICIENT_SEC),
   ": BLE_ERR_INSUFFICIENT_SEC (Insufficient Security)"},
  {BLE_HS_HCI_ERR(BLE_ERR_PARM_OUT_OF_RANGE),
   ": BLE_ERR_PARM_OUT_OF_RANGE (Parameter Out Of Mandatory Range)"},
  {BLE_HS_HCI_ERR(BLE_ERR_PENDING_ROLE_SW),
   ": BLE_ERR_PENDING_ROLE_SW (Role Switch Pending)"},
  {BLE_HS_HCI_ERR(BLE_ERR_RESERVED_SLOT),
   ": BLE_ERR_RESERVED_SLOT (Reserved Slot Violation)"},
  {BLE_HS_HCI_ERR(BLE_ERR_ROLE_SW_FAIL),
   ": BLE_ERR_ROLE_SW_FAIL (Role Switch Failed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_INQ_RSP_TOO_BIG),
   ": BLE_ERR_INQ_RSP_TOO_BIG (Extended Inquiry Response Too Large)"},
  {BLE_HS_HCI_ERR(BLE_ERR_SEC_SIMPLE_PAIR),
   ": BLE_ERR_SEC_SIMPLE_PAIR (Secure Simple Pairing Not Supported By Host)"},
  {BLE_HS_HCI_ERR(BLE_ERR_HOST_BUSY_PAIR),
   ": BLE_ERR_HOST_BUSY_PAIR (Host Busy - Pairing)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_REJ_CHANNEL),
   ": BLE_ERR_CONN_REJ_CHANNEL (Connection Rejected due to No Suitable Channel "
   "Found)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CTLR_BUSY), ": BLE_ERR_CTLR_BUSY (Controller Busy)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_PARMS),
   ": BLE_ERR_CONN_PARMS (Unacceptable Connection Parameters)"},
  {BLE_HS_HCI_ERR(BLE_ERR_DIR_ADV_TMO),
   ": BLE_ERR_DIR_ADV_TMO (Directed Advertising Timeout)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_TERM_MIC),
   ": BLE_ERR_CONN_TERM_MIC (Connection Terminated due to MIC Failure)"},
  {BLE_HS_HCI_ERR(BLE_ERR_CONN_ESTABLISHMENT),
   ": BLE_ERR_CONN_ESTABLISHMENT (Connection Failed to be Established)"},
  {BLE_HS_HCI_ERR(BLE_ERR_MAC_CONN_FAIL),
   ": BLE_ERR_MAC_CONN_FAIL (MAC Connection Failed)"},
  {BLE_HS_HCI_ERR(BLE_ERR_COARSE_CLK_ADJ),
   ": BLE_ERR_COARSE_CLK_ADJ (Coarse Clock Adjustment Rejected but Will Try to "
   "Adjust Using Clock Dragging)"},
  {BLE_HS_HCI_ERR(BLE_ERR_TYPE0_SUBMAP_NDEF),
   ": BLE_ERR_TYPE0_SUBMAP_NDEF (Type0 Submap Not Defined)"},
  {BLE_HS_HCI_ERR(BLE_ERR_UNK_ADV_INDENT),
   ": BLE_ERR_UNK_ADV_INDENT (Unknown Advertising Identifier)"},
  {BLE_HS_HCI_ERR(BLE_ERR_LIMIT_REACHED),
   ": BLE_ERR_LIMIT_REACHED (Limit Reached)"},
  {BLE_HS_HCI_ERR(BLE_ERR_OPERATION_CANCELLED),
   ": BLE_ERR_OPERATION_CANCELLED (Operation Cancelled by Host)"},
  {BLE_HS_HCI_ERR(BLE_ERR_PACKET_TOO_LONG),
   ": BLE_ERR_PACKET_TOO_LONG (Packet Too Long)"},
  {BLE_HS_HCI_ERR(BLE_ERR_MAX), ": BLE_ERR_MAX"}};
#pragma GCC diagnostic pop

static void esp_core_err_to_name(int error_code, uint16_t *opcode) {
  for (int i = 0;
       i < sizeof(core_err_code_list) / sizeof(core_err_code_list[0]); i++) {
    if (core_err_code_list[i].error_code == error_code) {
      if (opcode == NULL) {
        MODLOG_DFLT(INFO, "core_err=0x%02X %s\n", error_code,
                    core_err_code_list[i].msg);
      } else {
        MODLOG_DFLT(INFO, "ogf=0x%02x, ocf=0x%04x, core_err=0x%02X %s\n",
                    BLE_HCI_OGF(*opcode), BLE_HCI_OCF(*opcode), error_code,
                    core_err_code_list[i].msg);
      }
      break;
    }
...
#define BLE_HS_HCI_FRAG_DATABUF_SIZE                                           \
  (BLE_ACL_MAX_PKT_SIZE + BLE_HCI_DATA_HDR_SZ +                                \
   sizeof(struct os_mbuf_pkthdr) + sizeof(struct ble_mbuf_hdr) +               \
   sizeof(struct os_mbuf))
#else
#define BLE_HS_HCI_FRAG_DATABUF_SIZE                                           \
  (BLE_ACL_MAX_PKT_SIZE + BLE_HCI_DATA_HDR_SZ + BLE_HS_CTRL_DATA_HDR_SZ +      \
   sizeof(struct os_mbuf_pkthdr) + sizeof(struct ble_mbuf_hdr) +               \
   sizeof(struct os_mbuf))
#endif
...
static int ble_hs_hci_rx_cmd_complete(const void *data, int len,
...
static int ble_hs_hci_rx_cmd_status(const void *data, int len,
                                    struct ble_hs_hci_ack *out_ack) {
...
static int ble_hs_hci_process_ack(uint16_t expected_opcode, uint8_t *params_buf,
                                  uint8_t params_buf_len,
                                  struct ble_hs_hci_ack *out_ack) {
  int rc;
  ...
  case BLE_HCI_EVCODE_COMMAND_STATUS:
    rc = ble_hs_hci_rx_cmd_status(ble_hs_hci_ack->data, ble_hs_hci_ack->length,
                                  out_ack);
    break;
...
int ble_hs_hci_cmd_tx_no_rsp(uint16_t opcode, const void *cmd,
                             uint8_t cmd_len) {
  int rc;
...
int ble_hs_hci_send_vs_cmd(uint16_t ocf, const void *cmdbuf, uint8_t cmdlen,
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_VENDOR, ocf), cmdbuf, cmdlen,
                         rspbuf, rsplen);
...
//         if((data[2] ==  BLE_HCI_LE_SUBEV_ADV_RPT) || (data[2] ==
//         BLE_HCI_LE_SUBEV_DIRECT_ADV_RPT) || (data[2] ==
//         BLE_HCI_LE_SUBEV_EXT_ADV_RPT) || (data[2] ==
...
int ble_hs_hci_rx_evt(uint8_t *hci_ev, void *arg) {
  ...
  // #if ((BT_HCI_LOG_INCLUDED == TRUE) && SOC_ESP_NIMBLE_CONTROLLER &&
  // CONFIG_BT_CONTROLLER_ENABLED)
  //     uint16_t len = hci_ev[1] + 3;
  //     if (host_recv_adv_packet(hci_ev)) {
  //         bt_hci_log_record_hci_adv(HCI_LOG_DATA_TYPE_ADV, &hci_ev[1], len -
  //         2);
  //     } else {
...
static struct os_mbuf *ble_hs_hci_frag_alloc(uint16_t frag_size, void *arg) {
  ...
  om =
    os_mbuf_get_pkthdr(&ble_hs_hci_frag_mbuf_pool, sizeof(struct ble_mbuf_hdr));
#else
...
static struct os_mbuf *ble_hs_hci_acl_hdr_prepend(struct os_mbuf *om,
                                                  uint16_t handle,
                                                  uint8_t pb_flag) {
...
#if SOC_ESP_NIMBLE_CONTROLLER && CONFIG_BT_CONTROLLER_ENABLED
    frag =
      mem_split_frag(&txom, BLE_ACL_MAX_PKT_SIZE, ble_hs_hci_frag_alloc, NULL);
#else
...
uint32_t ble_hs_hci_get_le_supported_feat(void) { return ble_hs_hci_sup_feat; }

void ble_hs_hci_set_hci_version(uint8_t hci_version) {
  ble_hs_hci_version = hci_version;
}

uint8_t ble_hs_hci_get_hci_version(void) { return ble_hs_hci_version; }
...
void ble_hs_hci_init(void) {
  ...
  rc = mem_init_mbuf_pool(ble_hs_hci_frag_data, &ble_hs_hci_frag_mempool,
                          &ble_hs_hci_frag_mbuf_pool, 1,
                          BLE_HS_HCI_FRAG_MEMBLOCK_SIZE, "ble_hs_hci_frag");
  BLE_HS_DBG_ASSERT_EVAL(rc == 0);
}
