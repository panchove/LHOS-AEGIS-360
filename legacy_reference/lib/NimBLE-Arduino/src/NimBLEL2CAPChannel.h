 ...
 * from the "server" (which waits for the connection to be opened) and the
 * "client" (which opens the connection) point of view.
 */
class NimBLEL2CAPChannel {
public:
  /// @brief Open an L2CAP channel via the specified PSM and MTU.
  /// @param[in] psm The PSM to use.
  /// @param[in] mtu The MTU to use. Note that this is the local MTU. Upon
  /// opening the channel, the final MTU will be negotiated to be the minimum of
  /// local and remote.
  /// @param[in] callbacks The callbacks to use. NOTE that these callbacks are
  /// called from the context of the NimBLE bluetooth task (`nimble_host`) and
  /// MUST be handled as fast as possible.
  /// @return True if the channel was opened successfully, false otherwise.
  static NimBLEL2CAPChannel *connect(NimBLEClient *client, uint16_t psm,
                                     uint16_t mtu,
                                     NimBLEL2CAPChannelCallbacks *callbacks);

  /// @brief Write data to the channel.
  ///
  /// If the size of the data exceeds the MTU, the data will be split into
  /// multiple fragments.
  /// @return true on success, after the data has been sent.
  /// @return false, if the data can't be sent.
  ///
  /// NOTE: This function will block until the data has been sent or an error
  /// occurred.
...
protected:
  NimBLEL2CAPChannel(uint16_t psm, uint16_t mtu,
                     NimBLEL2CAPChannelCallbacks *callbacks);
  ~NimBLEL2CAPChannel();
...
private:
  ...
  const uint16_t mtu; // The requested (local) MTU of the channel, might be
                      // larger than negotiated MTU
  ...
  int writeFragment(std::vector<uint8_t>::const_iterator begin,
