...
static void store_app_key(uint16_t app_idx) {
  ...
  key.net_idx = app->net_idx, key.updated = app->updated,
...
app_key_update_find(uint16_t key_idx, struct app_key_update **free_slot) {
...
int bt_mesh_keys_resolve(struct bt_mesh_msg_ctx *ctx,
                         struct bt_mesh_subnet **sub, const uint8_t **app_key,
                         uint8_t *aid) {
  struct app_key *app = NULL;
...
void bt_mesh_app_key_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_app_key conf");
#endif
}
