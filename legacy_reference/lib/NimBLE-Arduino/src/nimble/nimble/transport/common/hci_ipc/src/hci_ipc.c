...
static void hci_ipc_alloc(struct hci_ipc_sm *sm) {
  assert(sm->hdr.type);
  assert(sm->buf == NULL);
...
static bool hci_ipc_has_hdr(struct hci_ipc_sm *sm) {
  return sm->hdr_len == sizeof(sm->hdr);
}

static void hci_ipc_frame(struct hci_ipc_sm *sm) {
  assert(sm->hdr.type);
  assert(sm->buf);
  assert(sm->rem_len == 0);
...
static uint16_t hci_ipc_copy_to_hdr(struct hci_ipc_sm *sm, const uint8_t *buf,
                                    uint16_t len) {
...
static uint16_t hci_ipc_copy_to_buf(struct hci_ipc_sm *sm, const uint8_t *buf,
                                    uint16_t len) {
  int rc;
