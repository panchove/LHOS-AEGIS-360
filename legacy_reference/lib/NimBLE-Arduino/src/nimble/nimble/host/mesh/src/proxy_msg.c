...
static void proxy_sar_timeout(struct ble_npl_event *work) {
  ...
  if (role->conn_handle) {
    rc = ble_gap_terminate(role->conn_handle, BLE_ERR_REM_USER_CONN_TERM);
    assert(rc == 0);
  }
}

int bt_mesh_proxy_msg_recv(struct bt_mesh_proxy_role *role, const void *buf,
                           uint16_t len) {
...
static void proxy_msg_init(struct bt_mesh_proxy_role *role) {
  ...
  net_buf_simple_init_with_data(
    role->buf, &bufs[role->conn_handle * CONFIG_BT_MESH_PROXY_MSG_LEN],
