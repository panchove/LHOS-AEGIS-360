...
static os_membuf_t ble_l2cap_sig_proc_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_L2CAP_SIG_MAX_PROCS), sizeof(struct ble_l2cap_sig_proc))];
...
static uint8_t ble_l2cap_sig_next_id(void) {
 ...
 */
static struct ble_l2cap_sig_proc *ble_l2cap_sig_proc_alloc(void) {
 ...
 */
static void ble_l2cap_sig_proc_free(struct ble_l2cap_sig_proc *proc) {
  int rc;
 ...
 */
static int ble_l2cap_sig_proc_matches(struct ble_l2cap_sig_proc *proc,
...
static void ble_l2cap_sig_proc_set_timer(struct ble_l2cap_sig_proc *proc) {
  proc->exp_os_ticks =
    ble_npl_time_get() +
...
static void ble_l2cap_sig_process_status(struct ble_l2cap_sig_proc *proc,
                                         int status) {
  if (status == 0) {
...
static void ble_l2cap_sig_update_call_cb(struct ble_l2cap_sig_proc *proc,
                                         int status) {
...
int ble_l2cap_sig_update_req_rx(uint16_t conn_handle,
                                struct ble_l2cap_sig_hdr *hdr,
                                struct os_mbuf **om) {
...
static int ble_l2cap_sig_update_rsp_rx(uint16_t conn_handle,
  ...
  proc = ble_l2cap_sig_proc_extract(conn_handle, BLE_L2CAP_SIG_PROC_OP_UPDATE,
                                    hdr->identifier);
  if (proc == NULL) {
    return 0;
  }
...
int ble_l2cap_sig_update(uint16_t conn_handle,
  ...
  ble_hs_lock();
  rc = ble_hs_misc_conn_chan_find_reqd(conn_handle, BLE_L2CAP_CID_SIG, &conn,
                                       &chan);
  if (rc != 0) {
    ble_hs_unlock();
    goto done;
...
static int ble_l2cap_sig_coc_err2ble_hs_err(uint16_t l2cap_coc_err) {
...
static int ble_l2cap_sig_ble_hs_err2coc_err(uint16_t ble_hs_err) {
  switch (ble_hs_err) {
  case BLE_HS_ENOTSUP:
...
static void ble_l2cap_event_coc_connected(struct ble_l2cap_chan *chan,
                                          uint16_t status) {
...
static int ble_l2cap_event_coc_accept(struct ble_l2cap_chan *chan,
...
#if MYNEWT_VAL(BLE_L2CAP_ENHANCED_COC)
static void ble_l2cap_event_coc_reconfigured(uint16_t conn_handle,
                                             uint16_t status,
                                             struct ble_l2cap_chan *chan,
                                             bool peer) {
  struct ble_l2cap_event event = {};
...
static int ble_l2cap_sig_credit_base_reconfig_req_rx(
  uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr, struct os_mbuf **om) {
  ...
  if ((req->mps < BLE_L2CAP_ECOC_MIN_MTU) ||
      (req->mtu < BLE_L2CAP_ECOC_MIN_MTU)) {
    rsp->result = htole16(BLE_L2CAP_ERR_RECONFIG_UNACCEPTED_PARAM);
    goto failed;
  }
...
static void ble_l2cap_sig_coc_reconfig_cb(struct ble_l2cap_sig_proc *proc,
                                          int status) {
  int i;
  struct ble_l2cap_chan *chan[BLE_L2CAP_MAX_COC_CONN_REQ] = {0};
  struct ble_hs_conn *conn;
...
static int ble_l2cap_sig_credit_base_reconfig_rsp_rx(
  uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr, struct os_mbuf **om) {
  struct ble_l2cap_sig_proc *proc;
  struct ble_l2cap_sig_credit_base_reconfig_rsp *rsp;
  int rc;

  proc = ble_l2cap_sig_proc_extract(conn_handle, BLE_L2CAP_SIG_PROC_OP_RECONFIG,
                                    hdr->identifier);
  if (!proc) {
    return 0;
  }
...
static int ble_l2cap_sig_credit_base_con_req_rx(uint16_t conn_handle,
                                                struct ble_l2cap_sig_hdr *hdr,
                                                struct os_mbuf **om) {
  int rc;
  ...
  if ((req->mtu < BLE_L2CAP_ECOC_MIN_MTU) ||
      (req->mps < BLE_L2CAP_ECOC_MIN_MTU)) {
    rsp->result = htole16(BLE_L2CAP_COC_ERR_INVALID_PARAMETERS);
    goto failed;
  }
  ...
  for (i = 0; i < num_of_scids; i++) {
    ...
    if (rc != 0) {
      if (i == 0) {
        /* In case it is very first channel we cannot create it means PSM is
         * incorrect or we are out of resources. Just send a response now.
         */
        rsp->result = htole16(ble_l2cap_sig_ble_hs_err2coc_err(rc));
        goto failed;
      } else {
        /* We cannot create number of channels req by peer due to limited
         * resources. */
...
static int ble_l2cap_sig_credit_base_con_rsp_rx(uint16_t conn_handle,
                                                struct ble_l2cap_sig_hdr *hdr,
                                                struct os_mbuf **om) {
  struct ble_l2cap_sig_proc *proc;
  struct ble_l2cap_sig_credit_base_connect_rsp *rsp;
  ...
  proc = ble_l2cap_sig_proc_extract(conn_handle, BLE_L2CAP_SIG_PROC_OP_CONNECT,
                                    hdr->identifier);
  if (!proc) {
    return 0;
  }
...
static int ble_l2cap_sig_coc_req_rx(uint16_t conn_handle,
                                    struct ble_l2cap_sig_hdr *hdr,
                                    struct os_mbuf **om) {
  int rc;
...
static int ble_l2cap_sig_coc_rsp_rx(uint16_t conn_handle,
                                    struct ble_l2cap_sig_hdr *hdr,
                                    struct os_mbuf **om) {
  struct ble_l2cap_sig_proc *proc;
  struct ble_l2cap_sig_le_con_rsp *rsp;
...
#if MYNEWT_VAL(BLE_L2CAP_ENHANCED_COC)
int ble_l2cap_sig_ecoc_connect(uint16_t conn_handle, uint16_t psm, uint16_t mtu,
...
int ble_l2cap_sig_coc_reconfig(uint16_t conn_handle,
...
static int ble_l2cap_sig_disc_req_rx(uint16_t conn_handle,
                                     struct ble_l2cap_sig_hdr *hdr,
                                     struct os_mbuf **om) {
  ...
  if (!chan) {
    os_mbuf_free_chain(txom);
    ble_hs_unlock();
    ble_l2cap_sig_reject_invalid_cid_tx(conn_handle, hdr->identifier, req->dcid,
                                        req->scid);
    return 0;
  }
...
static void ble_l2cap_sig_coc_disconnect_cb(struct ble_l2cap_sig_proc *proc,
                                            int status) {
  struct ble_l2cap_chan *chan;
  struct ble_l2cap_event event;
  struct ble_hs_conn *conn;
...
static int ble_l2cap_sig_disc_rsp_rx(uint16_t conn_handle,
                                     struct ble_l2cap_sig_hdr *hdr,
                                     struct os_mbuf **om) {
  struct ble_l2cap_sig_disc_rsp *rsp;
  struct ble_l2cap_sig_proc *proc;
  ...
  proc = ble_l2cap_sig_proc_extract(
    conn_handle, BLE_L2CAP_SIG_PROC_OP_DISCONNECT, hdr->identifier);
  if (!proc) {
    return 0;
  }
...
static int ble_l2cap_sig_le_credits_rx(uint16_t conn_handle,
                                       struct ble_l2cap_sig_hdr *hdr,
                                       struct os_mbuf **om) {
...
static int ble_l2cap_sig_rx_reject(uint16_t conn_handle,
                                   struct ble_l2cap_sig_hdr *hdr,
                                   struct os_mbuf **om) {
  struct ble_l2cap_sig_proc *proc;
  proc = ble_l2cap_sig_proc_extract(conn_handle, BLE_L2CAP_SIG_PROC_OP_CONNECT,
...
static int ble_l2cap_sig_rx(struct ble_l2cap_chan *chan) {
  struct ble_l2cap_sig_hdr hdr;
...
int ble_l2cap_sig_init(void) {
  ...
  rc = os_mempool_init(&ble_l2cap_sig_proc_pool,
                       ...
                       ble_l2cap_sig_proc_mem, "ble_l2cap_sig_proc_pool");
  if (rc != 0) {
    return rc;
  }
