...
bool NimBLERemoteValueAttribute::writeValue(const uint8_t *data, size_t length,
                                            bool response) const {
  ...
  // Check if the data length is longer than we can write in one connection
  // event. If so we must do a long write which requires a response.
  if (length <= mtu && !response) {
    rc = ble_gattc_write_no_rsp_flat(pClient->getConnHandle(), getHandle(),
                                     data, length);
    goto Done;
  }
  ...
  do {
    if (length > mtu) {
      ...
      rc =
        ble_gattc_write_long(pClient->getConnHandle(), getHandle(), 0, om,
                             NimBLERemoteValueAttribute::onWriteCB, &taskData);
    } else {
      rc = ble_gattc_write_flat(pClient->getConnHandle(), getHandle(), data,
                                length, NimBLERemoteValueAttribute::onWriteCB,
                                &taskData);
    }
    ...
    case BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_LONG):
      NIMBLE_LOGE(LOG_TAG,
                  "Long write not supported by peer; Truncating length to %d",
                  mtu);
      retryCount++;
      length = mtu;
      break;
...
Done:
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "<< writeValue failed, rc: %d %s", rc,
 ...
 */
int NimBLERemoteValueAttribute::onWriteCB(uint16_t conn_handle,
                                          const ble_gatt_error *error,
                                          ble_gatt_attr *attr, void *arg) {
  auto pTaskData = static_cast<NimBLETaskData *>(arg);
  const auto pAtt =
    static_cast<NimBLERemoteValueAttribute *>(pTaskData->m_pInstance);
...
NimBLEAttValue NimBLERemoteValueAttribute::readValue(time_t *timestamp) {
  ...
  int rc = 0;
  int retryCount = 1;
  NimBLETaskData taskData(const_cast<NimBLERemoteValueAttribute *>(this), 0,
                          &value);

  do {
    rc = ble_gattc_read_long(pClient->getConnHandle(), getHandle(), 0,
                             NimBLERemoteValueAttribute::onReadCB, &taskData);
    if (rc != 0) {
      goto Done;
    }
    ...
    case BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_LONG):
      ...
      rc = ble_gattc_read(pClient->getConnHandle(), getHandle(),
                          NimBLERemoteValueAttribute::onReadCB, &taskData);
      if (rc != 0) {
        goto Done;
      }
...
Done:
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "<< readValue failed rc=%d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  } else {
    NIMBLE_LOGD(LOG_TAG, "<< readValue");
  }
 ...
 * @return success == 0 or error code.
 */
int NimBLERemoteValueAttribute::onReadCB(uint16_t conn_handle,
                                         const ble_gatt_error *error,
                                         ble_gatt_attr *attr, void *arg) {
  auto pTaskData = static_cast<NimBLETaskData *>(arg);
  const auto pAtt =
