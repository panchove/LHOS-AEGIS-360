...
#define OS_MBUF_PKTHDR(__om)                                                   \
  ((struct os_mbuf_pkthdr *)(uintptr_t)(void *)((uint8_t *)&(__om)->om_data +  \
 ...
 * @return 0 on success, non-zero on failure.
 */
int r_os_mqueue_put(struct os_mqueue *, struct ble_npl_eventq *,
                    struct os_mbuf *);
 ...
 */
int r_os_mbuf_pool_init(struct os_mbuf_pool *, struct os_mempool *mp, uint16_t,
                        uint16_t);
 ...
 */
int r_os_mbuf_cmpf(const struct os_mbuf *om, int off, const void *data,
                   int len);
 ...
 * @return 0 on success, non-zero on failure.
 */
int os_mqueue_put(struct os_mqueue *, struct ble_npl_eventq *,
                  struct os_mbuf *);
