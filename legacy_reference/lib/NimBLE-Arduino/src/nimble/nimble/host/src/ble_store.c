...
static int ble_store_status(struct ble_store_status_event *event) {
  int rc;
...
static int ble_store_persist_sec(int obj_type,
  ...
  BLE_HS_DBG_ASSERT(value_sec->ltk_present || value_sec->irk_present ||
...
int ble_store_write_peer_sec(const struct ble_store_value_sec *value_sec) {
  ...
  if (ble_addr_cmp(&value_sec->peer_addr, BLE_ADDR_ANY) &&
    ...
    rc = ble_hs_pvcy_add_entry(value_sec->peer_addr.val,
                               value_sec->peer_addr.type, value_sec->irk);
    if (rc != 0) {
      return rc;
    }
  }
...
void ble_store_key_from_value_local_irk(
...
void ble_store_key_from_value_rpa_rec(
...
int ble_store_iterate(int obj_type, ble_store_iterator_fn *callback,
                      void *cookie) {
...
int ble_store_clear(void) {
  const uint8_t obj_types[] = {
    BLE_STORE_OBJ_TYPE_OUR_SEC,      BLE_STORE_OBJ_TYPE_PEER_SEC,
    BLE_STORE_OBJ_TYPE_CCCD,         BLE_STORE_OBJ_TYPE_CSFC,
    BLE_STORE_OBJ_TYPE_PEER_ADDR,    BLE_STORE_OBJ_TYPE_LOCAL_IRK,
#if MYNEWT_VAL(ENC_ADV_DATA)
