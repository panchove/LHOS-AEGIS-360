...
static uint8_t ble_uuid_base[16] = {0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00,
...
static int hex2val(char c, uint8_t *value) {
  if (c >= '0' && c <= '9') {
    *value = c - '0';
  } else if (c >= 'a' && c <= 'f') {
    *value = c - 'a' + 10;
...
char *ble_uuid_to_str(const ble_uuid_t *uuid, char *dst) {
  ...
  case BLE_UUID_TYPE_128:
    ...
    sprintf(dst,
            ...
            u8p[15], u8p[14], u8p[13], u8p[12], u8p[11], u8p[10], u8p[9],
            u8p[8], u8p[7], u8p[6], u8p[5], u8p[4], u8p[3], u8p[2], u8p[1],
            u8p[0]);
    break;
  default:
    dst[0] = '\0';
