...
static uint32_t get_auth_number(bt_mesh_output_action_t output,
  ...
  const uint32_t divider[PROV_IO_OOB_SIZE_MAX] = {
    10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000};
  uint32_t num = 0;

  bt_rand(&num, sizeof(num));

  if (output == BT_MESH_BLINK || output == BT_MESH_BEEP ||
      output == BT_MESH_VIBRATE || input == BT_MESH_PUSH ||
      input == BT_MESH_TWIST) {
...
int bt_mesh_prov_auth(bool is_provisioner, uint8_t method, uint8_t action,
                      uint8_t size) {
  ...
  case AUTH_METHOD_OUTPUT:
    ...
    atomic_set_bit(bt_mesh_prov_link.flags, NOTIFY_INPUT_COMPLETE);
    return bt_mesh_prov->output_number(
      output, get_auth_number(output, BT_MESH_NO_INPUT, size));
  case AUTH_METHOD_INPUT:
    ...
    return bt_mesh_prov->output_number(
      output, get_auth_number(BT_MESH_NO_OUTPUT, input, size));
...
const struct bt_mesh_prov *bt_mesh_prov_get(void) { return bt_mesh_prov; }
