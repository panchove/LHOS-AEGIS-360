...
#if MYNEWT_VAL(BLE_MESH_SETTINGS)
static struct net_key_update *
net_key_update_find(uint16_t key_idx, struct net_key_update **free_slot) {
...
void bt_mesh_kr_update(struct bt_mesh_subnet *sub, bool kr_flag, bool new_key) {
  ...
  if (sub->kr_phase == BT_MESH_KR_PHASE_1) {
     ...
     */
    key_refresh(sub, (kr_flag ? BT_MESH_KR_PHASE_2 : BT_MESH_KR_PHASE_3));
...
static int net_keys_create(struct bt_mesh_subnet_keys *keys,
  ...
  BT_DBG("NID 0x%02x EncKey %s", keys->msg.nid, bt_hex(keys->msg.enc, 16));
...
uint8_t bt_mesh_subnet_kr_phase_set(uint16_t net_idx, uint8_t *phase) {
  ...
  BT_WARN("Invalid KR transition: 0x%02x -> 0x%02x", sub->kr_phase, *phase);
...
bool bt_mesh_net_cred_find(struct bt_mesh_net_rx *rx, struct os_mbuf *in,
                           ...
                           bool (*cb)(struct bt_mesh_net_rx *rx,
                                      struct os_mbuf *in, struct os_mbuf *out,
...
static int net_key_set(int argc, char **argv, char *val) {
  ...
  return bt_mesh_subnet_set(net_idx, key.kr_phase, key.val[0],
                            (key.kr_phase != BT_MESH_KR_NORMAL) ? key.val[1]
                                                                : NULL);
}
#endif
...
void bt_mesh_net_key_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_net_key conf");
#endif
}
