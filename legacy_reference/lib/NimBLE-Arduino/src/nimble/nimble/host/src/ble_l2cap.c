...
static os_membuf_t ble_l2cap_chan_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_L2CAP_MAX_CHANS) + MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM),
...
int ble_l2cap_enhanced_connect(uint16_t conn_handle, uint16_t psm, uint16_t mtu,
                               uint8_t num, struct os_mbuf *sdu_rx[],
                               ble_l2cap_event_fn *cb, void *cb_arg) {
  return ble_l2cap_sig_ecoc_connect(conn_handle, psm, mtu, num, sdu_rx, cb,
                                    cb_arg);
}

int ble_l2cap_reconfig(struct ble_l2cap_chan *chans[], uint8_t num,
                       uint16_t new_mtu) {
  int i;
...
int ble_l2cap_reconfig_mtu_mps(struct ble_l2cap_chan *chans[], uint8_t num,
 ...
 */
int ble_l2cap_send(struct ble_l2cap_chan *chan, struct os_mbuf *sdu) {
...
void ble_l2cap_remove_rx(struct ble_hs_conn *conn,
                         struct ble_l2cap_chan *chan) {
...
static void ble_l2cap_append_rx(struct ble_l2cap_chan *chan,
                                struct os_mbuf *frag) {
...
static int ble_l2cap_rx_payload(struct ble_hs_conn *conn,
...
static uint16_t ble_l2cap_get_mtu(struct ble_l2cap_chan *chan) {
 ...
 */
int ble_l2cap_rx(struct ble_hs_conn *conn, struct hci_data_hdr *hci_hdr,
                 struct os_mbuf *om, ble_l2cap_rx_fn **out_rx_cb,
                 int *out_reject_cid) {
  struct ble_l2cap_chan *chan;
  struct ble_l2cap_hdr l2cap_hdr;
  uint8_t pb;
  ...
  *out_reject_cid = -1;
  ...
  case BLE_HCI_PB_FIRST_FLUSH:
    ...
    if (chan->dcid >= 0x0040 && chan->dcid <= 0x007F &&
        l2cap_hdr.len > (chan->my_coc_mps + BLE_L2CAP_SDU_SZ)) {
      /* Data exceeds MPS */
...
int ble_l2cap_init(void) {
  ...
  rc = os_mempool_init(
    &ble_l2cap_chan_pool,
    MYNEWT_VAL(BLE_L2CAP_MAX_CHANS) + MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM),
    sizeof(struct ble_l2cap_chan), ble_l2cap_chan_mem, "ble_l2cap_chan_pool");
  if (rc != 0) {
    return BLE_HS_EOS;
  }
  ...
  rc = stats_init_and_reg(STATS_HDR(ble_l2cap_stats),
                          STATS_SIZE_INIT_PARMS(ble_l2cap_stats, STATS_SIZE_32),
