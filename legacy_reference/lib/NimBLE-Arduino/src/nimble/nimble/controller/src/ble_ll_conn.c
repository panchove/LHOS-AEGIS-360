 ...
 *
 * 3) Make sure we are setting the schedule end time properly for both
 * peripheral and central. We should just set this to the end of the connection
 * event. We might want to guarantee a IFS time as well since the next event
 * needs to be scheduled prior to the start of the event to account for the time
 * it takes to get a frame ready (which is pretty much the IFS time).
 *
...
static bool ble_ll_conn_cth_flow_is_enabled(void) {
  return g_ble_ll_conn_cth_flow.enabled;
}

static bool ble_ll_conn_cth_flow_alloc_credit(struct ble_ll_conn_sm *connsm) {
...
static void ble_ll_conn_cth_flow_free_credit(struct ble_ll_conn_sm *connsm,
                                             uint16_t credits) {
  struct ble_ll_conn_cth_flow *cth = &g_ble_ll_conn_cth_flow;
  os_sr_t sr;
...
static void ble_ll_conn_cth_flow_error_fn(struct ble_npl_event *ev) {
  ...
  opcode =
    BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND, BLE_HCI_OCF_CB_HOST_NUM_COMP_PKTS);
...
#if MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED)
static void ble_ll_conn_css_update_list(struct ble_ll_conn_sm *connsm) {
...
static inline int ble_ll_conn_phy_should_update(uint8_t pref_mask,
                                                uint8_t curr_mask) {
...
void ble_ll_conn_itvl_to_ticks(uint32_t itvl, uint32_t *itvl_ticks,
                               uint8_t *itvl_usecs) {
 ...
 */
static int ble_ll_conn_is_empty_pdu(uint8_t *rxbuf) {
  int rc;
  uint8_t llid;
...
static uint8_t ble_ll_conn_calc_dci_csa1(struct ble_ll_conn_sm *conn) {
  uint8_t curchan;
  uint8_t remap_index;
  uint8_t bitpos;
 ...
 *
 */
static void ble_ll_conn_wait_txend(void *arg) {
  struct ble_ll_conn_sm *connsm;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LE_ENCRYPTION)
static void ble_ll_conn_start_rx_encrypt(void *arg) {
  struct ble_ll_conn_sm *connsm;
...
static void ble_ll_conn_start_rx_unencrypt(void *arg) {
  struct ble_ll_conn_sm *connsm;
...
static void ble_ll_conn_txend_encrypt(void *arg) {
  struct ble_ll_conn_sm *connsm;
...
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
static void ble_ll_conn_rxend_unencrypt(void *arg) {
  struct ble_ll_conn_sm *connsm;
...
static void ble_ll_conn_continue_rx_encrypt(void *arg) {
  struct ble_ll_conn_sm *connsm;
 ...
 * @return uint32_t
 */
static uint32_t ble_ll_conn_get_next_sched_time(struct ble_ll_conn_sm *connsm) {
  uint32_t ce_end;
 ...
 * @param connsm
 */
static void ble_ll_conn_chk_csm_flags(struct ble_ll_conn_sm *connsm) {
 ...
 */
static uint16_t ble_ll_conn_adjust_pyld_len(struct ble_ll_conn_sm *connsm,
                                            uint16_t pyld_len) {
 ...
 */
static int ble_ll_conn_event_start_cb(struct ble_ll_sched_item *sch) {
  int rc = 0;
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
  uint32_t usecs;
#endif
 ...
 * connection event. This will always return 'true' if we are a peripheral. If
 * we are a central, we must be able to send the next fragment and get a minimum
 ...
 */
static int ble_ll_conn_can_send_next_pdu(struct ble_ll_conn_sm *connsm,
                                         uint32_t begtime, uint32_t add_usecs) {
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
  int rc;
  uint16_t rem_bytes;
  uint32_t ticks;
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
static void ble_ll_conn_central_common_init(struct ble_ll_conn_sm *connsm) {
  /* Set central role */
...
int ble_ll_conn_set_data_len(struct ble_ll_conn_sm *connsm, uint16_t tx_octets,
...
static void ble_ll_conn_set_phy(struct ble_ll_conn_sm *connsm, int tx_phy,
                                int rx_phy) {
...
static void ble_ll_conn_init_phy(struct ble_ll_conn_sm *connsm, int phy) {
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
static void ble_ll_conn_create_set_params(struct ble_ll_conn_sm *connsm,
                                          uint8_t phy) {
  struct ble_ll_conn_create_params *cc_params;
...
static void ble_ll_conn_set_csa(struct ble_ll_conn_sm *connsm, bool chsel) {
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LE_CSA2)
  if (chsel) {
    connsm->flags.csa2 = 1;
 ...
 * @return int
 */
static int ble_ll_conn_next_event(struct ble_ll_conn_sm *connsm) {
  ...
  if (connsm->flags.terminate_started && CONN_IS_PERIPHERAL(connsm)) {
     ...
     * TERMINATE_IND sent as a Slave. Since we are here it means we are still
     * waiting for ACK. Make sure we catch it in next connection event.
     */
  ...
  if (connsm->flags.conn_update_sched &&
    ...
    if (ble_ll_sched_css_is_enabled() &&
        connsm->conn_role == BLE_LL_CONN_ROLE_CENTRAL) {
      BLE_LL_ASSERT(connsm->css_slot_idx_pending != BLE_LL_CONN_CSS_NO_SLOT);
    ...
    if (upd->winoffset != 0) {
      ...
      ble_ll_tmr_add(&connsm->anchor_point, &connsm->anchor_point_usecs, usecs);
    }
  ...
  if (connsm->flags.chanmap_update_sched &&
    ...
    connsm->chan_map_used = ble_ll_utils_chan_map_used_get(connsm->req_chanmap);
  ...
  if (connsm->conn_role == BLE_LL_CONN_ROLE_PERIPHERAL) {
    cur_ww = ble_ll_utils_calc_window_widening(
      connsm->anchor_point, connsm->last_anchor_point, connsm->central_sca);
 ...
 */
static int ble_ll_conn_created(struct ble_ll_conn_sm *connsm,
  ...
  /*
   * Set first connection event time. If peripheral the endtime is the receive
   * end time of the connect request. The actual connection starts 1.25 msecs
   * plus the transmit window offset from the end of the connection request.
   */
  rc = 1;
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
  if (connsm->conn_role == BLE_LL_CONN_ROLE_PERIPHERAL) {
...
static void ble_ll_conn_central_start(uint8_t phy, uint8_t csa,
                                      struct ble_ll_scan_addr_data *addrd,
                                      uint8_t *targeta) {
  struct ble_ll_conn_sm *connsm;
...
#if (BLE_LL_BT5_PHY_SUPPORTED == 1)
            if (BLE_LL_LLID_IS_CTRL(hdr_byte) && CONN_IS_PERIPHERAL(connsm) &&
  ...
  if (connsm->enc_data.enc_state > CONN_ENC_S_ENCRYPTED) {
    ...
    if (llid == BLE_LL_LLID_CTRL) {
      ...
      case BLE_LL_CTRL_ENC_RSP:
        /* If encryption has been paused, we don't want to send any packets from
         * the TX queue, as they would go unencrypted.
         */
 ...
 * @return int
 */
void ble_ll_conn_tx_pkt_in(struct os_mbuf *om, uint16_t handle,
                           uint16_t length) {
  uint8_t hdr_byte;
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
  if ((connsm->conn_role == BLE_LL_CONN_ROLE_CENTRAL) &&
      !ble_ll_conn_rem_feature_check(connsm, BLE_LL_FEAT_CONN_SUBRATING_HOST)) {
    return -ENOTSUP;
  }
#endif
...
void ble_ll_conn_module_init(void) {
  ...
  rc =
    stats_init_and_reg(STATS_HDR(ble_ll_conn_stats),
