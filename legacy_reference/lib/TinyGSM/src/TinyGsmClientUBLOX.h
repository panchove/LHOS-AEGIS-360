...
protected:
  bool gprsConnectImpl(const char *apn, const char *user = NULL,
    ...
    sendAT(
      GF("+UPSDA=0,3")); // Activate the PDP context associated with profile 0
  ...
  bool gprsDisconnectImpl() {
    sendAT(
      GF("+UPSDA=0,4")); // Deactivate the PDP context associated with profile 0
    if (waitResponse(360000L) != 1) {
      return false;
    }
...
protected:
  bool enableGPSImpl() {
    ...
    if (waitResponse(10000L, GF(GSM_NL "+UGPS:")) != 1) {
      return false;
    }
    return waitResponse(10000L) == 1;
  }
  ...
  String inline getUbloxLocationRaw(int8_t sensor) {
    ...
    if (waitResponse(120000L, GF(GSM_NL "+UULOC:")) != 1) {
      return "";
    }
    String res = stream.readStringUntil('\n');
    waitResponse();
  ...
  String getGsmLocationRawImpl() { return getUbloxLocationRaw(2); }
  String getGPSrawImpl() { return getUbloxLocationRaw(1); }
  ...
  inline bool getUbloxLocation(int8_t sensor, float *lat, float *lon,
    ...
    ialt =
      streamGetFloatBefore(','); // Estimated altitude, in meters - only forGNSS
