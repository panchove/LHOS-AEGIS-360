...
struct bt_mesh_friend {
  uint16_t lpn;
  uint8_t recv_delay;
  uint8_t fsn : 1, send_last : 1, pending_req : 1, pending_buf : 1,
    established : 1;
  int32_t poll_to;
  uint8_t num_elem;
...
struct bt_mesh_net_tx {
  ...
  uint16_t src;
  uint8_t xmit;
  uint8_t friend_cred : 1, aszmic : 1, aid : 6;
};

extern struct bt_mesh_net bt_mesh;

#define BT_MESH_NET_IVI_TX                                                     \
  (bt_mesh.iv_index - atomic_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS))
...
static inline void *net_buf_user_data(const struct os_mbuf *buf) {
   ...
   * for user_data, which is array of octets, just like os_mbuf's om_data. Let's
   * just use last octets (starting at start of om_data + total size of data
   * mbuf can hold - intended user_data size) of om_data as Zephyr's user_data.
   */
