...
#define SCAN_LATENCY MIN(MYNEWT_VAL(BLE_MESH_LPN_SCAN_LATENCY), LPN_RECV_DELAY)
...
#define REQ_ATTEMPTS(lpn)                                                      \
  MIN(REQ_ATTEMPTS_MAX, POLL_TIMEOUT / REQ_RETRY_DURATION(lpn))

#define POLL_TIMEOUT_MAX(lpn)                                                  \
  (POLL_TIMEOUT - (REQ_ATTEMPTS(lpn) * REQ_RETRY_DURATION(lpn)))

#define CLEAR_ATTEMPTS 3

#define LPN_CRITERIA                                                           \
  ((MYNEWT_VAL(BLE_MESH_LPN_MIN_QUEUE_SIZE)) |                                 \
...
static int32_t poll_timeout(struct bt_mesh_lpn *lpn) {
  ...
  if (lpn->poll_timeout < POLL_TIMEOUT_MAX(lpn)) {
    lpn->poll_timeout *= 2;
    lpn->poll_timeout = MIN(lpn->poll_timeout, POLL_TIMEOUT_MAX(lpn));
  }
...
static int send_friend_clear(void) {
  ...
  return bt_mesh_ctl_send(&tx, TRANS_CTL_OP_FRIEND_CLEAR, &req, sizeof(req),
                          &clear_sent_cb, NULL);
}
...
static void friend_req_sent(uint16_t duration, int err, void *user_data) {
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_LPN_ESTABLISHMENT)) {
    ...
    k_work_reschedule(&lpn->timer, duration + FRIEND_REQ_TIMEOUT);
...
static void req_sent(uint16_t duration, int err, void *user_data) {
...
#if MYNEWT_VAL(BLE_MESH_LOW_POWER_LOG_LVL) == LOG_LEVEL_DEBUG
  BT_DBG("req 0x%02x duration %u err %d state %s", lpn->sent_req, duration, err,
         state2str(lpn->state));
#endif
  ...
  if (lpn->established || IS_ENABLED(CONFIG_BT_MESH_LPN_ESTABLISHMENT)) {
     ...
     */
    k_work_reschedule(&lpn->timer, LPN_RECV_DELAY - SCAN_LATENCY);
  } else {
    lpn_set_state(BT_MESH_LPN_WAIT_UPDATE);
    k_work_reschedule(&lpn->timer, LPN_RECV_DELAY + duration + lpn->recv_win);
  }
}
...
static int send_friend_poll(void) {
  ...
  err = bt_mesh_ctl_send(&tx, TRANS_CTL_OP_FRIEND_POLL, &fsn, 1, &req_sent_cb,
                         NULL);
  if (err == 0) {
    lpn->pending_poll = 0;
...
static int friend_cred_create(struct bt_mesh_net_cred *cred,
  ...
  return bt_mesh_friend_cred_create(cred, bt_mesh_primary_addr(), lpn->frnd,
                                    lpn->lpn_counter, lpn->frnd_counter, key);
}

int bt_mesh_lpn_friend_offer(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
...
static void lpn_group_del(uint16_t group) {
  ...
  for (i = 0; i < ARRAY_SIZE(lpn->groups); i++) {
    if (lpn->groups[i] == group) {
      if (atomic_test_bit(lpn->added, i) || atomic_test_bit(lpn->pending, i)) {
...
static bool sub_update(uint8_t op) {
  ...
  if (bt_mesh_ctl_send(&tx, op, &req, 1 + g * 2, &req_sent_cb, NULL) < 0) {
    group_zero(lpn->pending);
    return false;
  }
...
static void lpn_timeout(struct ble_npl_event *work) {
  ...
  case BT_MESH_LPN_REQ_WAIT:
    bt_mesh_scan_enable();
    k_work_reschedule(&lpn->timer, lpn->adv_duration + FRIEND_REQ_SCAN);
    lpn_set_state(BT_MESH_LPN_WAIT_OFFER);
    break;
  case BT_MESH_LPN_WAIT_OFFER:
  ...
  case BT_MESH_LPN_ESTABLISHED:
    ...
    BT_ERR("No response from Friend after %u retries", lpn->req_attempts);
    lpn->req_attempts = 0;
    clear_friendship(false, false);
    break;
  case BT_MESH_LPN_RECV_DELAY:
...
int bt_mesh_lpn_friend_sub_cfm(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
...
int bt_mesh_lpn_friend_update(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  ...
  BT_DBG("flags 0x%02x iv_index 0x%08x md %u", msg->flags, (unsigned)iv_index,
         msg->md);
