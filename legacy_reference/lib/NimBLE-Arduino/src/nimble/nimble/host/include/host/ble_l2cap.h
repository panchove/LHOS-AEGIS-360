 ...
 *
 * This header file provides the public API for interacting with the L2CAP layer
 * of the BLE (Bluetooth Low Energy) stack. L2CAP is responsible for managing
 * logical channels between two connected BLE devices.
 *
 * @defgroup bt_host_l2cap Bluetooth Host Logical Link Control and Adaptation
 * Protocol
 * @ingroup bt_host
 * @{
 */
 ...
 *
 * This structure represents a request to update the L2CAP connection
 * parameters.
 */
 ...
 *
 * This structure represents a connection between the BLE host and a remote
 * device.
 */
struct ble_hs_conn;
...
/**
 * @defgroup ble_l2cap_coc_err_codes Connection-Oriented Channels (CoC) Error
 * Codes
 * @{
 */
...
/**
 * @defgroup ble_l2cap_coc_event_types Connection-Oriented Channel (CoC) Event
 * Types
 * @{
 */
 ...
 *
 * @param conn_handle   The connection handle associated with the signaling
 * update event.
 ...
 *
 * This structure holds the parameters required for initiating a signaling
 * update in the L2CAP layer. It defines the connection interval, slave latency,
 * and supervision timeout multiplier for the update.
 */
struct ble_l2cap_sig_update_params {
   ...
   * It specifies the maximum allowed latency between consecutive connection
   * events.
   */
   ...
   * The supervision timeout defines the time limit for detecting the loss of a
   * connection. This value is multiplied by the connection interval to
   * determine the supervision timeout duration.
   */
 ...
 *
 * This function initiates an L2CAP connection update procedure for the
 * specified connection handle. The update procedure is used to modify the
 * connection parameters, such as interval, latency, and timeout.
 *
 * @param conn_handle   The connection handle of the L2CAP connection.
 *
 * @param params        A pointer to a structure containing the desired update
 * parameters. This includes the new connection interval, slave latency, and
 *                      supervision timeout multiplier.
 *
 * @param cb            The callback function to be called when the update
 * request completes. The function signature for the callback is defined by
 ...
 *
 *  This structure is typically used to retrieve or provide information about an
 * existing L2CAP channel.
 */
 ...
 * function.
 *
 * @return          Integer value representing the status or result of the event
 * handling.
 */
 ...
 * success; A Bluetooth Host Error Code on failure: BLE_HS_CONN_HANDLE_NONE: if
 * the provided channel pointer is NULL.
 */
 ...
 * Multiplexer (PSM) and Maximum Transmission Unit (MTU) size. The server is
 * used to accept incoming L2CAP connections from remote clients. When a
 * connection request is received, the provided callback function will be
 * invoked with the corresponding event information.
 *
 ...
 *                  A non-zero value on failure.
 */
int ble_l2cap_create_server(uint16_t psm, uint16_t mtu, ble_l2cap_event_fn *cb,
                            void *cb_arg);
 ...
 * specified connection handle, Protocol/Service Multiplexer (PSM), Maximum
 * Transmission Unit (MTU) size, and receive SDU buffer. When the connection is
 * established or if there is an error during the connection process, the
 * provided callback function will be invoked with the corresponding event
 * information.
 *
 ...
 * @param cb            Pointer to the callback function to be invoked when the
 ...
 *
 * This function disconnects the specified L2CAP channel by sending a disconnect
 * signal.
 *
 * @param chan          Pointer to the L2CAP channel structure representing the
 ...
 * encapsulated in L2CAP frames and transmitted to the remote device.
 *
 * @param chan          Pointer to the L2CAP channel structure representing the
 * channel to send the SDU on.
 * @param sdu_tx        Pointer to the os_mbuf structure containing the SDU
 * (Service Data Unit) to send.
 *
 * @return              0 on success;
 *                      BLE_HS_ESTALLED: if there was not enough credits
 * available to send whole SDU. The application needs to wait for the event
 * 'BLE_L2CAP_EVENT_COC_TX_UNSTALLED' before being able to transmit more data;
 ...
 *
 * This function checks if the specified L2CAP channel is ready to receive an
 * SDU (Service Data Unit). It can be used to determine if the channel is in a
 * state where it can accept incoming data.
 *
 * @param chan          Pointer to the L2CAP channel structure to check.
 * @param sdu_rx        Pointer to the os_mbuf structure to receive the incoming
 * SDU.
 *
 ...
 *
 * This function retrieves information about the specified L2CAP channel and
 * populates the provided `ble_l2cap_chan_info` structure with the channel's
 * details.
 *
 * @param chan          Pointer to the L2CAP channel structure to retrieve
 * information from.
 * @param chan_info     Pointer to the `ble_l2cap_chan_info` structure to
 * populate with channel information.
 *
 * @return              0 on success;
 *                      A non-zero value on failure.
 */
