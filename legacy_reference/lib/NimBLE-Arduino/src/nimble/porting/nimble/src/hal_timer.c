...
static uint32_t nrf_read_timer_cntr(NRF_TIMER_Type *hwtimer) {
  uint32_t tcntr;
 ...
 */
static void nrf_timer_set_ocmp(struct nrf52_hal_timer *bsptimer,
                               uint32_t expiry) {
  int32_t delta_t;
  uint32_t temp;
  uint32_t cntr;
...
static void nrf_timer_disable_ocmp(NRF_TIMER_Type *hwtimer) {
  hwtimer->INTENCLR = NRF_TIMER_INT_MASK(NRF_TIMER_CC_INT);
}

static void nrf_rtc_disable_ocmp(NRF_RTC_Type *rtctimer) {
  rtctimer->INTENCLR = NRF_TIMER_INT_MASK(NRF_RTC_TIMER_CC_INT);
  rtctimer->INTENCLR = RTC_INTENCLR_TICK_Msk;
}

static uint32_t hal_timer_read_bsptimer(struct nrf52_hal_timer *bsptimer) {
  uint32_t low32;
  os_sr_t sr;
  uint32_t tcntr;
  NRF_RTC_Type *rtctimer;
 ...
 * @param bsptimer
 */
static void hal_timer_chk_queue(struct nrf52_hal_timer *bsptimer) {
  uint32_t tcntr;
  os_sr_t sr;
...
static void hal_timer_irq_handler(struct nrf52_hal_timer *bsptimer) {
  uint32_t compare;
  NRF_TIMER_Type *hwtimer;
...
#if MYNEWT_VAL(TIMER_5)
static void hal_rtc_timer_irq_handler(struct nrf52_hal_timer *bsptimer) {
  uint32_t overflow;
  uint32_t compare;
  uint32_t tick;
  NRF_RTC_Type *rtctimer;
...
#if MYNEWT_VAL(TIMER_0)
void nrf52_timer0_irq_handler(void) {
  hal_timer_irq_handler(&nrf52_hal_timer0);
}
#endif

#if MYNEWT_VAL(TIMER_1)
void nrf52_timer1_irq_handler(void) {
  hal_timer_irq_handler(&nrf52_hal_timer1);
}
#endif

#if MYNEWT_VAL(TIMER_2)
void nrf52_timer2_irq_handler(void) {
  hal_timer_irq_handler(&nrf52_hal_timer2);
}
#endif

#if MYNEWT_VAL(TIMER_3)
void nrf52_timer3_irq_handler(void) {
  hal_timer_irq_handler(&nrf52_hal_timer3);
}
#endif

#if MYNEWT_VAL(TIMER_4)
void nrf52_timer4_irq_handler(void) {
  hal_timer_irq_handler(&nrf52_hal_timer4);
}
#endif

#if MYNEWT_VAL(TIMER_5)
void nrf52_timer5_irq_handler(void) {
 ...
 * @return int
 */
int hal_timer_set_cb(int timer_num, struct hal_timer *timer,
