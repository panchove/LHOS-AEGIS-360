...
NimBLEExtAdvertising::NimBLEExtAdvertising()
    : m_deleteCallbacks{false}, m_pCallbacks{&defaultCallbacks},
 ...
 * @param [in] instId The extended advertisement instance ID to assign to this
 * data.
...
bool NimBLEExtAdvertising::setInstanceData(uint8_t instId,
  ...
  // If connectable or not scannable disable the callback for scan response
  // requests
  ...
  int rc = ble_gap_ext_adv_configure(instId, &adv.m_params, NULL,
                                     (pServer != nullptr)
  ...
  int rc = ble_gap_ext_adv_configure(
    instId, &adv.m_params, NULL, NimBLEExtAdvertising::handleGapEvent, NULL);
#endif

  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Advertising config error: rc = %d %s", rc,
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Unable to copy data: rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Invalid advertisement data: rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Error setting advertisement address: rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
 ...
 * @param [in] instId The extended advertisement instance ID to assign to this
 * data.
 * @param [in] data A reference to a NimBLEExtAdvertisement that contains the
 * data.
 */
bool NimBLEExtAdvertising::setScanResponseData(uint8_t instId,
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Unable to copy scan data: rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
...
bool NimBLEExtAdvertising::start(uint8_t instId, int duration, int maxEvents) {
  ...
  if (rc != 0 && rc != BLE_HS_EALREADY) {
    NIMBLE_LOGE(LOG_TAG, "Error enabling advertising; rc=%d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
...
bool NimBLEExtAdvertising::removeInstance(uint8_t instId) {
  if (stop(instId)) {
    ...
    NIMBLE_LOGE(LOG_TAG, "ble_gap_ext_adv_remove rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
...
bool NimBLEExtAdvertising::removeAll() {
  if (stop()) {
    ...
    NIMBLE_LOGE(LOG_TAG, "ble_gap_ext_adv_clear rc = %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
...
bool NimBLEExtAdvertising::stop(uint8_t instId) {
  ...
  NIMBLE_LOGE(LOG_TAG, "ble_gap_ext_adv_stop rc = %d %s", rc,
              NimBLEUtils::returnCodeToString(rc));
  return false;
} // stop
 ...
 * @param [in] pCallbacks A pointer to a callback to be invoked when an
 * advertisement stops.
 * @param [in] deleteCallbacks if true callback class will be deleted when
 * advertising is destructed.
 */
void NimBLEExtAdvertising::setCallbacks(
  NimBLEExtAdvertisingCallbacks *pCallbacks, bool deleteCallbacks) {
 ...
 * @param [in] instId The instance ID of the advertised data to get the status
 * of.
...
int NimBLEExtAdvertising::handleGapEvent(ble_gap_event *event, void *arg) {
  ...
  case BLE_GAP_EVENT_SCAN_REQ_RCVD: {
    pAdv->m_pCallbacks->onScanRequest(
      pAdv, event->scan_req_rcvd.instance,
...
/**
 * @brief Sets wether the advertisement should use legacy (BLE 4.0, 31 bytes
 * max) advertising.
...
/**
 * @brief Sets wether this advertisement should advertise as a connectable
 * device.
 ...
 * @details This will set a bitmask using the input parameters to allow
 * different \n combinations. If all inputs are false then all 3 channels will
 * be used.
 */
void NimBLEExtAdvertisement::setPrimaryChannels(bool ch37, bool ch38,
                                                bool ch39) {
 ...
 * @param [in] scanRequestWhitelistOnly If true, only allow scan requests from
 * those on the white list.
 * @param [in] connectWhitelistOnly If true, only allow connections from those
 * on the white list.
 */
...
/**
 * @brief Enable or disable direct advertisements to the peer set with
 ...
 * @param [in] mininterval Minimum value for advertising interval in 0.625ms
 * units, 0 = use default.
 */
 ...
 * @param [in] maxinterval Maximum value for advertising interval in 0.625ms
 * units, 0 = use default.
 */
 ...
 *
 * @details Anonymous advertising omits the device's address from the
 * advertisement.
 */
 ...
 * @param [in] enable If true the scan response request callback will be called
 ...
 * @details This will clear all data but preserves advertising parameter
 * settings.
 */
 ...
 * @return True if successful.
 * @details If the appearance value is 0 then it will be removed from the
 * advertisement if set previously.
 */
 ...
 * @return True if successful.
 * @details If the flag value is 0 then it will be removed from the
 * advertisement if set previously.
 */
...
bool NimBLEExtAdvertisement::setManufacturerData(const uint8_t *data,
  ...
  m_payload.erase(m_payload.end() - 2,
                  m_payload.end()); // Remove the header if failed.
  return false;
} // setManufacturerData
...
bool NimBLEExtAdvertisement::setManufacturerData(const std::string &data) {
  return setManufacturerData(reinterpret_cast<const uint8_t *>(data.data()),
                             data.length());
} // setManufacturerData
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setManufacturerData(
 ...
 * @param [in] isComplete If true the name is complete, if false it is
 * shortened.
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setName(const std::string &name, bool isComplete) {
  ...
  header[1] =
    isComplete ? BLE_HS_ADV_TYPE_COMP_NAME : BLE_HS_ADV_TYPE_INCOMP_NAME;

  if (addData(header, 2)) {
    return addData(reinterpret_cast<const uint8_t *>(name.data()),
                   name.length());
  }
...
bool NimBLEExtAdvertisement::addServiceUUID(const NimBLEUUID &serviceUUID) {
  ...
  m_payload.insert(m_payload.begin() + dataLoc + m_payload[dataLoc] + 1, uuid,
                   uuid + bytes);
 ...
 * @return True if successful or uuid not found, false if uuid error or data
 * could not be reset.
 */
bool NimBLEExtAdvertisement::removeServiceUUID(const NimBLEUUID &serviceUUID) {
  ...
  m_payload.erase(m_payload.begin() + uuidLoc,
                  m_payload.begin() + uuidLoc + bytes);
  m_payload[dataLoc] -= bytes;
  return true;
} // removeServiceUUID
 ...
 */
bool NimBLEExtAdvertisement::removeServices() { return true; } // removeServices
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setCompleteServices16(
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setCompleteServices32(
  const std::vector<NimBLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setPartialServices16(
  const std::vector<NimBLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setPartialServices32(
  const std::vector<NimBLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setServices(bool complete, uint8_t size,
  ...
  case 16:
    header[1] =
      complete ? BLE_HS_ADV_TYPE_COMP_UUIDS16 : BLE_HS_ADV_TYPE_INCOMP_UUIDS16;
    break;
  case 32:
    header[1] =
      complete ? BLE_HS_ADV_TYPE_COMP_UUIDS32 : BLE_HS_ADV_TYPE_INCOMP_UUIDS32;
    break;
  case 128:
    ...
    for (const auto &uuid : uuids) {
      if (uuid.bitSize() != size) {
        ...
        if (addData(uuid.getValue(), uuidBytes)) {
          ...
          m_payload.erase(m_payload.end() - 2 - (count * uuidBytes),
                          m_payload.end());
          return false;
        }
      }
 ...
 * @return True if successful, false if data length is too long or could not be
 * set.
 */
...
bool NimBLEExtAdvertisement::setServiceData(const NimBLEUUID &uuid,
  ...
  return setServiceData(uuid, reinterpret_cast<const uint8_t *>(data.data()),
                        data.length());
} // setServiceData
 ...
 * @details  Range = 0x0006(7.5ms) to 0x0C80(4000ms), values not within the
 * range will be limited to this range.
 * @return True if successful.
 */
bool NimBLEExtAdvertisement::setPreferredParams(uint16_t minInterval,
  ...
  maxInterval = std::max<uint16_t>(
    maxInterval, minInterval); // Max must be greater than or equal to min.
 ...
 * @return -1 if the data is not found, otherwise the index of the data in the
 * payload.
 */
...
std::string NimBLEExtAdvertisement::toString() const {
  std::string hexStr =
    NimBLEUtils::dataToHexString(&m_payload[0], m_payload.size());
  std::string str;
