...
#define SEG_RETRANSMIT_TIMEOUT(tx)                                             \
  (BT_MESH_ADDR_IS_UNICAST(tx->dst) ? SEG_RETRANSMIT_TIMEOUT_UNICAST(tx)       \
                                    : SEG_RETRANSMIT_TIMEOUT_GROUP)
/* How long to wait for available buffers before giving up */
#define BUF_TIMEOUT K_NO_WAIT

struct virtual_addr {
  uint16_t ref : 15, changed : 1;
  uint16_t addr;
  uint8_t uuid[16];
};
...
static struct seg_rx {
  ...
  uint16_t len;
  uint8_t hdr;
  uint8_t seg_n : 5, ctl : 1, in_use : 1, obo : 1;
  uint8_t ttl;
  uint32_t block;
  uint32_t last;
...
char _k_mem_slab_buffer_[OS_ALIGN(
  (BT_MESH_APP_SEG_SDU_MAX) * (CONFIG_BT_MESH_SEG_BUFS), OS_ALIGNMENT)];

struct k_mem_slab segs = {.num_blocks = CONFIG_BT_MESH_SEG_BUFS,
                          .block_size =
                            OS_ALIGN(BT_MESH_APP_SEG_SDU_MAX, OS_ALIGNMENT),
...
static int send_unseg(struct bt_mesh_net_tx *tx, struct os_mbuf *sdu,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
    if (!bt_mesh_friend_queue_has_space(tx->sub->net_idx, tx->src,
                                        tx->ctx->addr, NULL, 1)) {
    ...
    if (bt_mesh_friend_enqueue_tx(tx, BT_MESH_FRIEND_PDU_SINGLE, NULL, 1,
                                  buf) &&
...
static void seg_tx_unblock_check(struct seg_tx *tx) {
  ...
  for (i = 0; i < ARRAY_SIZE(seg_tx); ++i) {
    if (&seg_tx[i] != tx && seg_tx[i].blocked &&
...
static void schedule_retransmit(struct seg_tx *tx) {
  ...
  k_work_reschedule(&tx->retransmit,
                    tx->seg_o ? K_NO_WAIT : K_MSEC(SEG_RETRANSMIT_TIMEOUT(tx)));
}
...
static void seg_tx_send_unacked(struct seg_tx *tx) {
  ...
  for (; tx->seg_o <= tx->seg_n; tx->seg_o++) {
    ...
    seg = bt_mesh_adv_create(BT_MESH_ADV_DATA, tx->xmit, BUF_TIMEOUT);
    if (!seg) {
...
static int send_seg(struct bt_mesh_net_tx *net_tx, struct os_mbuf *sdu,
  ...
  BT_DBG("src 0x%04x dst 0x%04x app_idx 0x%04x aszmic %u sdu_len %u",
         net_tx->src, net_tx->ctx->addr, net_tx->ctx->app_idx, net_tx->aszmic,
         sdu->om_len);

  for (tx = NULL, i = 0; i < ARRAY_SIZE(seg_tx); i++) {
    if (seg_tx[i].nack_count) {
      blocked |= seg_tx_blocks(&seg_tx[i], net_tx->src, net_tx->ctx->addr);
    } else if (!tx) {
      tx = &seg_tx[i];
    }
  }
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) &&
      !bt_mesh_friend_queue_has_space(tx->sub->net_idx, net_tx->src, tx->dst,
                                      &tx->seq_auth, tx->seg_n + 1) &&
      BT_MESH_ADDR_IS_UNICAST(tx->dst)) {
    BT_ERR("Not enough space in Friend Queue for %u segments", tx->seg_n + 1);
    seg_tx_reset(tx);
    return -ENOBUFS;
  }
  ...
  for (seg_o = 0; sdu->om_len; seg_o++) {
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
      ...
      if (bt_mesh_friend_enqueue_tx(net_tx, type, ctl_op ? NULL : &tx->seq_auth,
                                    tx->seg_n + 1, seg) &&
...
static void seg_rx_assemble(struct seg_rx *rx, struct os_mbuf *buf,
  ...
  for (i = 0; i <= rx->seg_n; i++) {
    net_buf_simple_add_mem(
      buf, rx->seg[i], MIN(seg_len(rx->ctl), rx->len - (i * seg_len(rx->ctl))));
  }
...
static int sdu_recv(struct bt_mesh_net_rx *rx, uint8_t hdr, uint8_t aszmic,
  ...
  rx->ctx.app_idx = bt_mesh_app_key_find(ctx.crypto.dev_key, AID(&hdr), rx,
                                         sdu_try_decrypt, &ctx);
...
static int trans_ack(struct bt_mesh_net_rx *rx, uint8_t hdr,
  ...
  BT_DBG("OBO %u seq_zero 0x%04x ack 0x%08x", obo, seq_zero, (unsigned)ack);
...
static int ctl_recv(struct bt_mesh_net_rx *rx, uint8_t hdr, struct os_mbuf *buf,
                    uint64_t *seq_auth) {
...
static int trans_unseg(struct os_mbuf *buf, struct bt_mesh_net_rx *rx,
  ...
  if (bt_mesh_rpl_check(rx, NULL)) {
    BT_WARN("Replay: src 0x%04x dst 0x%04x seq 0x%06x", rx->ctx.addr,
            rx->ctx.recv_dst, (unsigned)rx->seq);
    return -EINVAL;
  }
...
static int send_ack(struct bt_mesh_subnet *sub, uint16_t src, uint16_t dst,
                    uint8_t ttl, uint64_t *seq_auth, uint32_t block,
                    uint8_t obo) {
  struct bt_mesh_msg_ctx ctx = {
    .net_idx = sub->net_idx,
  ...
  BT_DBG("SeqZero 0x%04x Block 0x%08x OBO %u", seq_zero, (unsigned)block, obo);
  ...
  return bt_mesh_ctl_send(&tx, TRANS_CTL_OP_ACK, buf, sizeof(buf), NULL, NULL);
}
...
static void seg_ack(struct ble_npl_event *work) {
  ...
  send_ack(rx->sub, rx->dst, rx->src, rx->ttl, &rx->seq_auth, rx->block,
           rx->obo);
...
static int trans_seg(struct os_mbuf *buf, struct bt_mesh_net_rx *net_rx,
  ...
  if (bt_mesh_rpl_check(net_rx, &rpl)) {
    BT_WARN("Replay: src 0x%04x dst 0x%04x seq 0x%06" PRIx32, net_rx->ctx.addr,
   ...
   */
  *seq_auth =
    SEQ_AUTH(BT_MESH_NET_IVI_RX(net_rx),
             (net_rx->seq -
  ...
  *seg_count = seg_n + 1;
  ...
  if (rx) {
    ...
    if (rx->in_use) {
      BT_DBG("Existing RX context. Block 0x%08x", (unsigned)rx->block);
      goto found_rx;
    }
   ...
   */
  if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && net_rx->friend_match &&
      !net_rx->local_match &&
      !bt_mesh_friend_queue_has_space(net_rx->sub->net_idx, net_rx->ctx.addr,
...
found_rx:
  ...
  if (seg_o == seg_n) {
    ...
    BT_DBG("Target len %u * %u + %u = %u", seg_n, seg_len(rx->ctl), buf->om_len,
           rx->len);
  ...
  *pdu_type = BT_MESH_FRIEND_PDU_COMPLETE;
  ...
  if (net_rx->ctl) {
    ...
    net_buf_simple_init_with_data(sdu, seg_buf->om_data,
                                  rx->len - APP_MIC_LEN(ASZMIC(hdr)));
...
int bt_mesh_trans_recv(struct os_mbuf *buf, struct bt_mesh_net_rx *rx) {
  ...
  BT_DBG("src 0x%04x dst 0x%04x seq 0x%08x friend_match %u", rx->ctx.addr,
         rx->ctx.recv_dst, (unsigned)rx->seq, rx->friend_match);
   ...
   */
  if ((MYNEWT_VAL(BLE_MESH_LOW_POWER)) && bt_mesh_lpn_established() &&
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && rx->friend_match && !err) {
    if (seq_auth == TRANS_SEQ_AUTH_NVAL) {
      bt_mesh_friend_enqueue_rx(rx, pdu_type, NULL, seg_count, buf);
    } else {
      bt_mesh_friend_enqueue_rx(rx, pdu_type, &seq_auth, seg_count, buf);
    }
  }
...
uint8_t bt_mesh_va_del(const uint8_t uuid[16], uint16_t *addr) {
  ...
  for (int i = 0; i < ARRAY_SIZE(virtual_addrs); i++) {
    if (virtual_addrs[i].ref && !memcmp(uuid, virtual_addrs[i].uuid,
                                        ARRAY_SIZE(virtual_addrs[i].uuid))) {
      va = &virtual_addrs[i];
      break;
    }
...
void bt_mesh_va_pending_store(void) {
  ...
  for (i = 0; (lab = bt_mesh_va_get(i)) != NULL; i++) {
    ...
    if (IS_VA_DEL(lab)) {
      ...
      val = settings_str_from_bytes(&va, sizeof(va), buf, sizeof(buf));
      if (!val) {
    ...
    if (err) {
      ...
      BT_DBG("%s %s value", IS_VA_DEL(lab) ? "Deleted" : "Stored", path);
    }
  }
}
...
void bt_mesh_va_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_hb_pub conf");
#endif
}

#else
void bt_mesh_va_pending_store(void) { /* Do nothing. */ }
