...
static void send_start(void) {
  ...
  bool oob_pub_key =
    bt_mesh_prov_link.conf_inputs.capabilities[3] == PUB_KEY_OOB;

  bt_mesh_prov_buf_init(start, PROV_START);
  net_buf_simple_add_u8(start, PROV_ALG_P256);

  memcpy(bt_mesh_prov_link.conf_inputs.start, &start->om_data[1],
         PDU_LEN_START);
  ...
  memcpy(bt_mesh_prov_link.conf_inputs.invite, &start->om_data[1],
         PDU_LEN_INVITE);
...
static void prov_capabilities(const uint8_t *data) {
  ...
  caps.output_actions = (bt_mesh_output_action_t)(sys_get_be16(&data[6]));
  caps.input_size = data[8];
  caps.input_actions = (bt_mesh_input_action_t)(sys_get_be16(&data[9]));
...
#if MYNEWT_VAL(BLE_MESH_CDB)
  prov_device.node = bt_mesh_cdb_node_alloc(prov_device.uuid, prov_device.addr,
                                            data[0], prov_device.net_idx);
  if (prov_device.node == NULL) {
...
#endif
  memcpy(bt_mesh_prov_link.conf_inputs.capabilities, data,
...
static void send_confirm(void) {
  ...
  if (bt_mesh_prov_conf(bt_mesh_prov_link.conf_key, bt_mesh_prov_link.rand,
                        bt_mesh_prov_link.auth, bt_mesh_prov_link.conf)) {
...
static void send_pub_key(void) {
  ...
  sys_memcpy_swap(net_buf_simple_add(buf, BT_PUB_KEY_COORD_LEN), key,
                  BT_PUB_KEY_COORD_LEN);
  sys_memcpy_swap(net_buf_simple_add(buf, BT_PUB_KEY_COORD_LEN),
                  &key[BT_PUB_KEY_COORD_LEN], BT_PUB_KEY_COORD_LEN);

  /* PublicKeyProvisioner */
  memcpy(bt_mesh_prov_link.conf_inputs.pub_key_provisioner, &buf->om_data[1],
         PDU_LEN_PUB_KEY);
...
static void prov_dh_key_gen(void) {
  ...
  sys_memcpy_swap(&remote_pk_le[BT_PUB_KEY_COORD_LEN],
...
static void send_prov_data(void) {
  ...
  err = bt_mesh_prov_nonce(bt_mesh_prov_link.dhkey, bt_mesh_prov_link.prov_salt,
                           nonce);
  if (err) {
  ...
  err = bt_mesh_dev_key(bt_mesh_prov_link.dhkey, bt_mesh_prov_link.prov_salt,
                        prov_device.node->dev_key);
  if (err) {
  ...
  if (sub == NULL) {
    BT_ERR("No subnet with net_idx %u", prov_device.node->net_idx);
    prov_fail(PROV_ERR_UNEXP_ERR);
    return;
  }
#endif
  ...
  BT_DBG("net_idx %u, iv_index 0x%08x, addr 0x%04x", prov_device.node->net_idx,
...
static void prov_complete(const uint8_t *data) {
  ...
  BT_DBG("key %s, net_idx %u, num_elem %u, addr 0x%04x",
         bt_hex(node->dev_key, 16), node->net_idx, node->num_elem, node->addr);
...
static void prov_random(const uint8_t *data) {
  ...
  if (bt_mesh_prov_salt(bt_mesh_prov_link.conf_salt, bt_mesh_prov_link.rand,
                        data, bt_mesh_prov_link.prov_salt)) {
...
static void prov_link_closed(void) { reset_state(); }

static void prov_link_opened(void) { send_invite(); }
...
int bt_mesh_prov_remote_pub_key_set(const uint8_t public_key[BT_PUB_KEY_LEN]) {
  ...
  /* Swap X and Y halves independently to big-endian */
  memcpy(bt_mesh_prov_link.conf_inputs.pub_key_device, public_key,
         PDU_LEN_PUB_KEY);
