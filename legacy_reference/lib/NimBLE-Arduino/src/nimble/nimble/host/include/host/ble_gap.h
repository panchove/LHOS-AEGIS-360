...
struct ble_gap_event {
  ...
  union {
    ...
    struct {
       ...
       * If the connection is established from periodic advertising with
       * responses and Role is 0x00  then the Advertising_Handle parameter shall
       * be set according to the periodic advertising train the connection was
       * established from
       */
      uint8_t adv_handle;
       ...
       * If the connection is established from periodic advertising with
       * responses and Role is 0x01, then the Sync_Handle parameter shall be set
       * according to the periodic advertising train the connection was
       * established from
       */
    ...
    /**
     * Represents a successful Link establishment attempt. Sometimes, in noisy
     * environment, even if BLE_GAP_EVENT_CONNECT is posted, the link
     * syncronization procedure may fail and link gets disconnected with reason
     * 0x3E. Application can wait for below event to ensure the link
     * syncronization is completed.   Valid for the following event types: o
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV_WITH_RESPONSES)
      /** Number of subevents. If zero, the periodic advertiser will be a
       * broadcaster, without responses.
       */
      uint8_t num_subevents;
      /** Interval between subevents (N * 1.25 ms) */
      uint8_t subevent_interval;
      /** Time between the advertising packet and the first response slot (N
       * * 1.25 ms). */
     ...
     * update procedure has completed and some parameters have changed  Valid
     * for the following event types: o BLE_GAP_EVENT_SUBRATE_CHANGE
     */
    struct {
      /** BLE_ERR_SUCCESS on success or error code on failure */
      uint8_t status;
    ...
    struct {
       ...
       *  0x01: CRC was incorrect, but Length and CTETime fields were used to
       * determine sampling points 0x02: CRC was incorrect, but the Controller
       * determined the position and length of the CTE in another way 0xFF:
...
struct ble_gap_multi_conn_params {
  ...
  /**
   * Additional arguments specifying the particulars of the connect procedure.
   * When extended adv is disabled or BLE_GAP_LE_PHY_1M_MASK is set in phy_mask
   * this parameter can't be specified to null.
   */
  ...
  /**
   * Additional arguments specifying the particulars of the connect procedure.
   * When BLE_GAP_LE_PHY_2M_MASK is set in phy_mask this parameter can't be
   * specified to null.
   */
  ...
  /**
   * Additional arguments specifying the particulars of the connect procedure.
   * When BLE_GAP_LE_PHY_CODED_MASK is set in phy_mask this parameter can't be
   * specified to null.
   */
  ...
  /**
   * The minimum length occupied by this connection in scheduler. 0 means
   * disable the optimization for this connection.
   */
...
#if MYNEWT_VAL(BLE_AOA_AOD)
  /**
   * Specifies the type of Constant Tone Extension (CTE) to which the receiver
   * should not synchronize. This parameter determines which types of packets
   * with specific CTE configurations are ignored during synchronization.
   *
   * Possible values:
   *   0: Do not sync to packets with an AoA Constant Tone Extension.
   *   1: Do not sync to packets with an AoD Constant Tone Extension with 1 μs
   * slots. 2: Do not sync to packets with an AoD Constant Tone Extension with 2
   * μs slots. 3: Do not sync to packets with a type 3 Constant Tone Extension
   * (currently reserved for future use). 4: Do not sync to packets without a
 ...
 * When sync transfer arrives, BLE_GAP_EVENT_PERIODIC_TRANSFER is sent to the
 * user. After that, sync transfer reception on that connection is terminated
 * and user needs to call this API again when expect to receive next sync
 * transfers.
 *
 * Note: If ACL connection gets disconnected before sync transfer arrived, user
 * will not receive BLE_GAP_EVENT_PERIODIC_TRANSFER. Instead, sync transfer
 * reception is terminated by the host automatically.
 *
 * @param conn_handle        Handle identifying connection.
 ...
 * @param cte_request_interval Interval between CTE requests in connection
 * interval.
 ...
 *
 * @param switch_sampling_rates On success, stores the supported switching and
 * sampling rates.
 * @param num_antennae         On success, stores the number of antennae.
 * @param max_switch_pattern_len On success, stores the maximum switching
 * pattern length.
 ...
 *                                      - BLE_OWN_ADDR_RPA_PUBLIC_DEFAULT
 *                                      - BLE_OWN_ADDR_RPA_RANDOM_DEFAULT
 * @param advertising_handle    The advertising_Handle identifying the periodic
 advertising train
 *                                  Range: 0x00 to 0xEF or 0xFF
 * @param subevent              The Subevent parameter is used to identify the
 subevent where a connection request shall be initiated from a periodic
 advertising train. The Advertising_Handle and Subevent parameters shall be set
 to 0xFF if these parameters are not used.
 * @param peer_addr             The address of the peer to connect to.
 *                                  If this parameter is NULL, the white list
 *                                  is used.
 * @param duration_ms           The duration of the discovery procedure.
 ...
 * @param common_factor         The greatest common factor of all intervals in
 * 0.625ms units.
 * @return                      0 on success;
 *
 */
...
/**
 * Read LE Suggested Default Data Length in controller (OGF = 0x08, OCF =
 * 0x0024).
 *
 * @param out_sugg_max_tx_octets    The Host's suggested value for the
 * Controller's maximum transmitted number of payload octets in LL Data PDUs to
 * be used for new connections. (Range 0x001B-0x00FB).
 * @param out_sugg_max_tx_time      The Host's suggested value for the
 * Controller's maximum packet transmission time for packets containing LL Data
 * PDUs to be used for new connections. (Range 0x0148-0x4290).
 *
 * @return                          0 on success,
 *                                  other error code on failure.
 */
...
/**
 * Configure LE Suggested Default Data Length in controller (OGF = 0x08, OCF =
 * 0x0024).
 *
 * @param sugg_max_tx_octets    The Host's suggested value for the Controller's
 * maximum transmitted number of payload octets in LL Data PDUs to be used for
 * new connections. (Range 0x001B-0x00FB).
 * @param sugg_max_tx_time      The Host's suggested value for the Controller's
 * maximum packet transmission time for packets containing LL Data PDUs to be
 * used for new connections. (Range 0x0148-0x4290).
 *
 * @return                      0 on success,
 *                              other error code on failure.
 */
 ...
 * list from the controller. If a peer is connected, the connection is
 * terminated.
 *
 ...
 * field is sent or received in request by a peripheral.
 *
 ...
 *                         tx_channel = (Frequency -2402)/2, tx_channel range =
 ...
 *                         rx_channel = (Frequency -2402)/2, tx_channel range =
 * 0x00-0x27, Frequency range = 2402 MHz to 2480 MHz.
 *
 * @return                 0 on success; nonzero on failure
 */
 ...
 * @param test_data_len    Length in bytes of payload data in each packet
 * @param payload	   Packet payload type. Valid range: 0x00 - 0x07
 * @param phy              Phy used by transmitter 1M phy: 0x01, 2M phy:0x02,
 * coded phy:0x03.
 *
 ...
 * 0x00-0x27, Frequency range: 2402 MHz to 2480 MHz
 *
 * @param index 	  modulation index, 0x00:standard modulation index,
 * 0x01:stable modulation index
 * @param phy             Phy type used by the receiver, 1M phy: 0x01, 2M
 * phy:0x02, coded phy:0x03
 *
 * @return                0 on success; nonzero on failure
 */
...
/**
 * Set Read Remote Version Information is used to retrieve the version,
 manufacturer,
