...
void phy_debug_init(void) {
  ...
  nrf_ppi_channel_endpoint_setup(
    NRF_PPI, 17, (uint32_t)&(NRF_RADIO->EVENTS_READY),
  ...
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 20, (uint32_t)&(NRF_GPIOTE->TASKS_SET[PHY_GPIOTE_DEBUG_1]));
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 21, (uint32_t)&(NRF_GPIOTE->TASKS_SET[PHY_GPIOTE_DEBUG_1]));
#endif
  ...
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 26, (uint32_t)&(NRF_GPIOTE->TASKS_SET[PHY_GPIOTE_DEBUG_2]));
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 27, (uint32_t)&(NRF_GPIOTE->TASKS_CLR[PHY_GPIOTE_DEBUG_2]));
#endif

#if PHY_USE_DEBUG_3
  phy_gpiote_configure(PHY_GPIOTE_DEBUG_3,
                       MYNEWT_VAL(BLE_PHY_DBG_TIME_WFR_PIN));

#if NRF52840_XXAA
  nrf_ppi_channel_endpoint_setup(
    NRF_PPI, 18, (uint32_t)&(NRF_RADIO->EVENTS_RXREADY),
    (uint32_t)&(NRF_GPIOTE->TASKS_SET[PHY_GPIOTE_DEBUG_3]));
#else
  nrf_ppi_channel_endpoint_setup(
    NRF_PPI, 18, (uint32_t)&(NRF_RADIO->EVENTS_READY),
    (uint32_t)&(NRF_GPIOTE->TASKS_SET[GIDX_DEBUG_3]));
#endif
  nrf_ppi_channel_endpoint_setup(
    NRF_PPI, 19, (uint32_t)&(NRF_RADIO->EVENTS_DISABLED),
  ...
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 4, (uint32_t)&(NRF_GPIOTE->TASKS_CLR[PHY_GPIOTE_DEBUG_3]));
  nrf_ppi_fork_endpoint_setup(
    NRF_PPI, 5, (uint32_t)&(NRF_GPIOTE->TASKS_CLR[PHY_GPIOTE_DEBUG_3]));
#endif
}
#endif /* PHY_USE_DEBUG */
...
void phy_txpower_set(int8_t dbm) { NRF_RADIO->TXPOWER = dbm; }
