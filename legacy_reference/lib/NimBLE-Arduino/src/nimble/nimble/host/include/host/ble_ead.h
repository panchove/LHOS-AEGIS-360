...
/* This value is used to set the directionBit of the CCM  nonce to the MSB of
 * the Randomizer field (see Supplement to the Bluetooth Core Specification v11,
 * Part A 1.23.3)
 */
 ...
 *                      encryption.
 * @iv                  Initialisation Vector used to generate the nonce. It
 * must be changed each time the Session Key changes.
 ...
 *                      the MIC. Size must be at least @ref
 * BLE_EAD_RANDOMIZER_SIZE + @p payload_size + @ref BLE_EAD_MIC_SIZE. Use @ref
 *                      BLE_EAD_ENCRYPTED_PAYLOAD_SIZE to get the right size.
 *
 * @return              0 on success;
 *                      BLE_HS_EINVAL if the specified value is not
 *                      within the allowed range.
 *                      BLE_HS_ECANCEL if error occurred during the random
 * number generation
 */
 ...
 * @iv                          Initialisation Vector used to generate the
 * `nonce`.
 * @encrypted_payload           Encrypted Advertising Data received. This
 *                              should only contain the advertising data from
 * the received advertising structure, not the length nor the type.
 * @encrypted_payload_size      Size of the received advertising data in
 *                              bytes. Should be equal to the length field of
 * the received advertising structure, minus the size of the type (1 byte).
 * @payload                     Decrypted advertising payload. Use @ref
 *                              BLE_EAD_DECRYPTED_PAYLOAD_SIZE to get the right
 * size.
 *
 * @return                      0 on success;
 *                              BLE_HS_EINVAL if the specified value is not
