 ...
 * @param [in] maxLen - The maximum length in bytes that the characteristic
 * value can hold. (Default: 512 bytes for esp32, 20 for all others).
 * @param [in] pService - pointer to the service instance this characteristic
 * belongs to.
 */
NimBLECharacteristic::NimBLECharacteristic(const char *uuid,
                                           uint16_t properties, uint16_t maxLen,
...
NimBLECharacteristic::NimBLECharacteristic(const NimBLEUUID &uuid,
                                           uint16_t properties, uint16_t maxLen,
                                           NimBLEService *pService)
    : NimBLELocalValueAttribute{uuid, 0, maxLen},
      m_pCallbacks{&defaultCallback}, m_pService{pService} {
...
NimBLEDescriptor *NimBLECharacteristic::createDescriptor(const NimBLEUUID &uuid,
  ...
  if (uuid == NimBLEUUID(static_cast<uint16_t>(0x2904))) {
    NIMBLE_LOGW(LOG_TAG,
                "0x2904 descriptor should be created with create2904()");
...
/**
 * @brief Create a Characteristic Presentation Format Descriptor for this
 * characteristic.
...
void NimBLECharacteristic::addDescriptor(NimBLEDescriptor *pDescriptor) {
  ...
  for (const auto &dsc : m_vDescriptors) {
    if (dsc == pDescriptor) {
      pDescriptor->setCharacteristic(
        this); // Update the characteristic pointer in the descriptor.
      return;
    }
  }
 ...
 * @param[in] pDescriptor A pointer to the descriptor instance to remove from
 * the characteristic.
 * @param[in] deleteDsc If true it will delete the descriptor instance and free
 * it's resources.
 */
void NimBLECharacteristic::removeDescriptor(NimBLEDescriptor *pDescriptor,
  ...
  pDescriptor->setRemoved(deleteDsc ? NIMBLE_ATT_REMOVE_DELETE
 ...
 */
NimBLEDescriptor *
NimBLECharacteristic::getDescriptorByUUID(const char *uuid) const {
 ...
 */
NimBLEDescriptor *
NimBLECharacteristic::getDescriptorByUUID(const NimBLEUUID &uuid) const {
  for (const auto &dsc : m_vDescriptors) {
    if (dsc->getUUID() == uuid) {
      return dsc;
    }
 ...
 */
NimBLEDescriptor *
NimBLECharacteristic::getDescriptorByHandle(uint16_t handle) const {
  for (const auto &dsc : m_vDescriptors) {
 ...
 * @param[in] connHandle Connection handle to send an individual indication, or
 * BLE_HS_CONN_HANDLE_NONE to send the indication to all subscribed clients.
 ...
 * @return True if the indication was sent successfully, false otherwise.
 */
bool NimBLECharacteristic::indicate(const uint8_t *value, size_t length,
 ...
 * @param[in] connHandle Connection handle to send an individual notification,
 * or BLE_HS_CONN_HANDLE_NONE to send the notification to all subscribed
 * clients.
 ...
 * @return True if the notification was sent successfully, false otherwise.
 */
bool NimBLECharacteristic::notify(const uint8_t *value, size_t length,
                                  uint16_t connHandle) const {
 ...
 * @param[in] value A pointer to the data to send.
 * @param[in] length The length of the data to send.
 * @param[in] isNotification if true sends a notification, false sends an
 * indication.
 ...
 */
bool NimBLECharacteristic::sendValue(const uint8_t *value, size_t length,
                                     bool isNotification,
                                     uint16_t connHandle) const {
  int rc = 0;
  ...
  if (value != nullptr && length > 0) { // custom notification value
    ...
    if (connHandle !=
        BLE_HS_CONN_HANDLE_NONE) { // only sending to specific peer
    ...
    for (const auto &ch : NimBLEDevice::getServer()->getPeerDevices()) {
      // Must re-create the data buffer on each iteration because it is freed by
      // the calls bellow.
      om = ble_hs_mbuf_from_flat(value, length);
      if (!om) {
        rc = BLE_HS_ENOMEM;
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "failed to send value, rc=%d %s", rc,
...
void NimBLECharacteristic::writeEvent(const uint8_t *val, uint16_t len,
 ...
 * @param [in] pCharacteristic The characteristic that is the source of the
 * event.
 * @param [in] connInfo A reference to a NimBLEConnInfo instance containing the
 * peer info.
 */
void NimBLECharacteristicCallbacks::onRead(
 ...
 * peer info.
 */
void NimBLECharacteristicCallbacks::onWrite(
  NimBLECharacteristic *pCharacteristic, NimBLEConnInfo &connInfo) {
 ...
 * @details The status code for success is 0 for notifications and BLE_HS_EDONE
 * for indications, any other value is an error.
 */
void NimBLECharacteristicCallbacks::onStatus(
  NimBLECharacteristic *pCharacteristic, int code) {
 ...
 */
void NimBLECharacteristicCallbacks::onStatus(
  NimBLECharacteristic *pCharacteristic, NimBLEConnInfo &connInfo, int code) {
  NIMBLE_LOGD("NimBLECharacteristicCallbacks", "onStatus: default");
} // onStatus
 ...
 */
void NimBLECharacteristicCallbacks::onSubscribe(
  NimBLECharacteristic *pCharacteristic, NimBLEConnInfo &connInfo,
  uint16_t subValue) {
