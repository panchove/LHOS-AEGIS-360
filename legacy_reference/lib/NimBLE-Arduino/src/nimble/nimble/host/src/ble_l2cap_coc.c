...
static os_membuf_t ble_l2cap_coc_srv_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM), sizeof(struct ble_l2cap_coc_srv))];
...
static int ble_l2cap_coc_rx_fn(struct ble_l2cap_chan *chan) {
  ...
  BLE_HS_LOG(
    DEBUG, "Received partial sdu_len=%d, credits left=%d, current_sdu_idx=%d\n",
    OS_MBUF_PKTLEN(rx_sdu), rx->credits, chan->coc_rx.current_sdu_idx);

  return 0;
}

void ble_l2cap_coc_set_new_mtu_mps(struct ble_l2cap_chan *chan, uint16_t mtu,
                                   uint16_t mps) {
  chan->my_coc_mps = mps;
  chan->coc_rx.mtu = mtu;
...
int ble_l2cap_coc_create_srv_chan(struct ble_hs_conn *conn, uint16_t psm,
  ...
  *chan =
    ble_l2cap_coc_chan_alloc(conn, psm, srv->mtu, NULL, srv->cb, srv->cb_arg);
  if (!*chan) {
    return BLE_HS_ENOMEM;
  }
...
static void ble_l2cap_event_coc_unstalled(struct ble_l2cap_chan *chan,
                                          int status) {
  struct ble_l2cap_event event = {};
...
static int ble_l2cap_coc_continue_tx(struct ble_l2cap_chan *chan) {
  ...
  while (tx->credits) {
    ...
    BLE_HS_LOG(DEBUG, "Sent %d bytes, credits=%d, to send %d bytes \n", len,
               tx->credits, OS_MBUF_PKTLEN(tx->sdus[0]) - tx->data_offset);
...
int ble_l2cap_coc_recv_ready(struct ble_l2cap_chan *chan,
   ...
   */
  if (chan->disable_auto_credit_update == false &&
...
int ble_l2cap_coc_init(void) {
  ...
  return os_mempool_init(&ble_l2cap_coc_srv_pool,
                         ...
                         ble_l2cap_coc_srv_mem, "ble_l2cap_coc_srv_pool");
}
