...
void net_buf_put(struct ble_npl_eventq *fifo, struct os_mbuf *om) {
...
int bt_encrypt_be(const uint8_t *key, const uint8_t *plaintext,
  ...
  if (mbedtls_aes_crypt_ecb(&s, MBEDTLS_AES_ENCRYPT, plaintext, enc_data) !=
      0) {
    mbedtls_aes_free(&s);
    return BLE_HS_EUNKNOWN;
  }
...
void k_work_submit(struct ble_npl_callout *w) { ble_npl_callout_reset(w, 0); }
...
uint32_t k_uptime_get_32(void) { return k_uptime_get(); }
...
void bt_conn_get_info(struct ble_hs_conn *conn,
...
static int set_ad(const struct bt_data *ad, size_t ad_len, uint8_t *buf,
                  uint8_t *buf_len) {
  int i;
  ...
  for (i = 0; i < ad_len; i++) {
    ...
    memcpy(&buf[*buf_len], ad[i].data, ad[i].data_len);
...
static int ble_adv_conf_adv_instance(const struct ble_gap_adv_params *param,
                                     int *instance) {
...
int bt_le_adv_start(const struct ble_gap_adv_params *param,
  ...
  err = ble_gap_adv_start(g_mesh_addr_type, NULL, BLE_HS_FOREVER, param, NULL,
                          NULL);
  if (err) {
    BT_ERR("Advertising failed: err %d", err);
    return err;
...
int bt_le_adv_stop() { return ble_gap_adv_stop(); }
...
void net_buf_slist_init(struct net_buf_slist_t *list) { STAILQ_INIT(list); }
...
int create_free_list(struct k_mem_slab *slab) {
  ...
  if (((slab->block_size | (uintptr_t)slab->buffer) & (sizeof(void *) - 1)) !=
      0) {
    return -EINVAL;
  }
