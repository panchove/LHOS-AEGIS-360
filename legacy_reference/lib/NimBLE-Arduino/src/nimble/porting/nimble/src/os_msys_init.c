...
static STAILQ_HEAD(, os_mbuf_pool)
  g_msys_pool_list = STAILQ_HEAD_INITIALIZER(g_msys_pool_list);
...
#define SYSINIT_MSYS_1_MEMBLOCK_SIZE OS_ALIGN(OS_MSYS_1_BLOCK_SIZE, 4)
...
#define SYSINIT_MSYS_2_MEMBLOCK_SIZE OS_ALIGN(OS_MSYS_2_BLOCK_SIZE, 4)
#define SYSINIT_MSYS_2_MEMPOOL_SIZE                                            \
  OS_MEMPOOL_SIZE(OS_MSYS_2_BLOCK_COUNT, SYSINIT_MSYS_2_MEMBLOCK_SIZE)
#ifdef ESP_PLATFORM
 ...
 */
static int os_msys_sanity_min_count(int idx) {
  switch (idx) {
  case 0:
...
static int os_msys_sanity(struct os_sanity_check *sc, void *arg) {
...
static void os_msys_init_once(void *data, struct os_mempool *mempool,
                              struct os_mbuf_pool *mbuf_pool, int block_count,
                              int block_size, const char *name) {
  int rc;

  rc =
    mem_init_mbuf_pool(data, mempool, mbuf_pool, block_count, block_size, name);
...
#if OS_MSYS_1_BLOCK_COUNT > 0
  os_msys_init_1_data = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) * SYSINIT_MSYS_1_MEMPOOL_SIZE));
...
#if OS_MSYS_2_BLOCK_COUNT > 0
  os_msys_init_2_data = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) * SYSINIT_MSYS_2_MEMPOOL_SIZE));
...
#if OS_MSYS_1_BLOCK_COUNT > 0
  os_msys_init_once(os_msys_init_1_data, &os_msys_init_1_mempool,
                    &os_msys_init_1_mbuf_pool, OS_MSYS_1_BLOCK_COUNT,
                    SYSINIT_MSYS_1_MEMBLOCK_SIZE, "msys_1");
#endif

#if OS_MSYS_2_BLOCK_COUNT > 0
  os_msys_init_once(os_msys_init_2_data, &os_msys_init_2_mempool,
                    &os_msys_init_2_mbuf_pool, OS_MSYS_2_BLOCK_COUNT,
                    SYSINIT_MSYS_2_MEMBLOCK_SIZE, "msys_2");
#endif
