 ...
 *
 * Encapsulates a L2CAP server that can hold multiple services. Every service is
 * represented by a channel object and an assorted set of callbacks.
 */
...
public:
  ...
  /// @return the newly created object, if the server registration was
  /// successful.
  NimBLEL2CAPChannel *createService(const uint16_t psm, const uint16_t mtu,
