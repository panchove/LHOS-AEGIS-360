...
#define BT_DBG(fmt, ...)                                                       \
  BLE_MESH_LOG(DEBUG, "%s: " fmt "\n", __func__, ##__VA_ARGS__);
#define BT_INFO(fmt, ...)                                                      \
  BLE_MESH_LOG(INFO, "%s: " fmt "\n", __func__, ##__VA_ARGS__);
#define BT_WARN(fmt, ...)                                                      \
  BLE_MESH_LOG(WARN, "%s: " fmt "\n", __func__, ##__VA_ARGS__);
#define BT_ERR(fmt, ...)                                                       \
  BLE_MESH_LOG(ERROR, "%s: " fmt "\n", __func__, ##__VA_ARGS__);
...
int bt_encrypt_be(const uint8_t *key, const uint8_t *plaintext,
                  uint8_t *enc_data);
...
#define CONFIG_BT_MESH_IV_UPDATE_SEQ_LIMIT                                     \
  MYNEWT_VAL(BLE_MESH_IV_UPDATE_SEQ_LIMIT)
...
#define CONFIG_BT_MESH_MODEL_VND_MSG_CID_FORCE                                 \
  MYNEWT_VAL(BLE_MESH_MODEL_VND_MSG_CID_FORCE)
...
#define CONFIG_BT_MESH_NETWORK_TRANSMIT_COUNT                                  \
  MYNEWT_VAL(BLE_MESH_NETWORK_TRANSMIT_COUNT)
#define CONFIG_BT_MESH_NETWORK_TRANSMIT_INTERVAL                               \
  MYNEWT_VAL(BLE_MESH_NETWORK_TRANSMIT_INTERVAL)
#define CONFIG_BT_MESH_RELAY_ENABLED MYNEWT_VAL(BLE_MESH_RELAY_ENABLED)
#define CONFIG_BT_MESH_RELAY_RETRANSMIT_INTERVAL                               \
...
#define CONFIG_BT_MESH_RELAY_RETRANSMIT_COUNT                                  \
  MYNEWT_VAL(BLE_MESH_RELAY_RETRANSMIT_COUNT)
#define CONFIG_BT_MESH_GATT_PROXY_ENABLED                                      \
  MYNEWT_VAL(BLE_MESH_GATT_PROXY_ENABLED)
...
static inline void k_sem_give(struct k_sem *sem) { ble_npl_sem_release(sem); }

static inline void k_sem_reset(struct k_sem *sem) { ble_npl_sem_init(sem, 0); }
...
#define NET_BUF_SLIST_FOR_EACH_NODE(head, var)                                 \
  STAILQ_FOREACH(var, head, omp_next)

#if MYNEWT_VAL(BLE_MESH_SETTINGS)

#define settings_load conf_load
int settings_bytes_from_str(char *val_str, void *vp, int *len);
char *settings_str_from_bytes(const void *vp, int vp_len, char *buf,
                              int buf_len);
