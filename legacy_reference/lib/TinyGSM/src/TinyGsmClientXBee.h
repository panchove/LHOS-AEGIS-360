...
public:
  ...
  public:
    ...
    int peek() override { return at->stream.peek(); }
...
public:
  explicit TinyGsmXBee(Stream &stream)
      : stream(stream), guardTime(TINY_GSM_XBEE_GUARD_TIME),
        beeType(XBEE_UNKNOWN), resetPin(-1), savedIP(IPAddress(0, 0, 0, 0)),
        savedHost(""), savedHostIP(IPAddress(0, 0, 0, 0)),
        savedOperatingIP(IPAddress(0, 0, 0, 0)), inCommandMode(false),
  ...
  TinyGsmXBee(Stream &stream, int8_t resetPin)
      : stream(stream), guardTime(TINY_GSM_XBEE_GUARD_TIME),
        beeType(XBEE_UNKNOWN), resetPin(resetPin),
        savedIP(IPAddress(0, 0, 0, 0)), savedHost(""),
  ...
  String getModemNameImpl() { return getBeeName(); }
  ...
  void setBaudImpl(uint32_t baud) {
    ...
    switch (baud) {
    case 2400:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x1);
      break;
    case 4800:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x2);
      break;
    case 9600:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x3);
      break;
    case 19200:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x4);
      break;
    case 38400:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x5);
      break;
    case 57600:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x6);
      break;
    case 115200:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x7);
      break;
    case 230400:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x8);
      break;
    case 460800:
      changesMade |= changeSettingIfNeeded(GF("BD"), 0x9);
      break;
    case 921600:
  ...
  String getModemInfoImpl() { return sendATGetString(GF("HS")); }
...
public:
  XBeeType getBeeType() { return beeType; }

  String getBeeName() {
    switch (beeType) {
    case XBEE_S6B_WIFI:
      return "Digi XBee Wi-Fi";
    case XBEE_LTE1_VZN:
      return "Digi XBee Cellular LTE Cat 1";
    case XBEE_3G:
      return "Digi XBee Cellular 3G";
    case XBEE3_LTE1_ATT:
      return "Digi XBee3 Cellular LTE CAT 1";
    case XBEE3_LTEM_ATT:
      return "Digi XBee3 Cellular LTE-M";
    default:
      return "Digi XBee";
    }
  }
...
public:
  RegStatus getRegistrationStatus() {
    ...
    if (!inCommandMode)
      return REG_UNKNOWN; // Return immediately
...
protected:
  ...
  bool isGprsConnectedImpl() { return isNetworkConnected(); }
...
protected:
  ...
  String getIMSIImpl() { return sendATGetString(GF("II")); }
...
protected:
  ...
  bool sendSMSImpl(const String &number, const String &text) {
    ...
    streamWrite(text);
    stream.write(static_cast<char>(0x0D)); // close off with the carriage return
