...
static void ble_ll_trace_module_send_desc(void) {
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "0 ll_sched lls=%u cputime=%u start_time=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod, "1 ll_rx_start lls=%u pdu_type=%x");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "2 ll_rx_end pdu_type=%x len=%u flags=%x");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "3 ll_wfr_timer_exp lls=%u xcvr=%u rx_start=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod, "4 ll_ctrl_rx opcode=%u len=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "5 ll_conn_ev_start conn_handle=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "6 ll_conn_ev_end conn_handle=%u event_cntr=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "7 ll_conn_end conn_handle=%u event_cntr=%u err=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod, "8 ll_conn_tx len=%u offset=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "9 ll_conn_rx conn_sn=%u pdu_nesn=%u");
  os_trace_module_desc(&g_ble_ll_trace_mod,
                       "10 ll_adv_txdone inst=%u chanset=%x");
...
void ble_ll_trace_init(void) {
  ble_ll_trace_off = os_trace_module_register(&g_ble_ll_trace_mod, "ble_ll", 12,
