...
#define CHECK_CACHE_CONN_STATE(cache_state, cb, cb_arg, opcode, s_handle,      \
                               e_handle, p_uuid)                               \
  op = &conn->pending_op;                                                      \
  switch (cache_state) {                                                       \
  case SVC_DISC_IN_PROGRESS:                                                   \
    ...
    ble_gattc_cache_conn_fill_op(op, s_handle, e_handle, p_uuid, cb, cb_arg,   \
                                 opcode);                                      \
    return 0;                                                                  \
...
static struct ble_gattc_cache_conn_svc *ble_gattc_cache_conn_svc_find_range(
  struct ble_gattc_cache_conn *ble_gattc_cache_conn, uint16_t attr_handle);

static struct ble_gattc_cache_conn_svc *
ble_gattc_cache_conn_svc_find(struct ble_gattc_cache_conn *ble_gattc_cache_conn,
                              uint16_t svc_start_handle,
                              struct ble_gattc_cache_conn_svc **out_prev);

int ble_gattc_cache_conn_svc_is_empty(
  const struct ble_gattc_cache_conn_svc *svc);

uint16_t
ble_gattc_cache_conn_chr_end_handle(const struct ble_gattc_cache_conn_svc *svc,
                                    const struct ble_gattc_cache_conn_chr *chr);

int ble_gattc_cache_conn_chr_is_empty(
  const struct ble_gattc_cache_conn_svc *svc,
  const struct ble_gattc_cache_conn_chr *chr);

static struct ble_gattc_cache_conn_chr *
ble_gattc_cache_conn_chr_find(const struct ble_gattc_cache_conn_svc *svc,
                              uint16_t chr_def_handle,
                              struct ble_gattc_cache_conn_chr **out_prev);

static void ble_gattc_cache_conn_disc_chrs(
  struct ble_gattc_cache_conn *ble_gattc_cache_conn);

static void ble_gattc_cache_conn_disc_incs(
  struct ble_gattc_cache_conn *ble_gattc_cache_conn);

static void ble_gattc_cache_conn_disc_dscs(struct ble_gattc_cache_conn *peer);

struct ble_gattc_cache_conn *ble_gattc_cache_conn_find(uint16_t conn_handle) {
  struct ble_gattc_cache_conn *ble_gattc_cache_conn;
...
ble_gattc_cache_conn_find_by_addr(ble_addr_t peer_addr) {
  struct ble_gattc_cache_conn *ble_gattc_cache_conn;
  SLIST_FOREACH(ble_gattc_cache_conn, &ble_gattc_cache_conns, next) {
    if (memcmp(&ble_gattc_cache_conn->ble_gattc_cache_conn_addr, &peer_addr,
               sizeof(peer_addr)) == 0) {
      return ble_gattc_cache_conn;
    }
  }
...
ble_gattc_cache_conn_dsc_find_prev(const struct ble_gattc_cache_conn_chr *chr,
                                   uint16_t dsc_handle) {
...
static struct ble_gattc_cache_conn_dsc *
ble_gattc_cache_conn_dsc_find(const struct ble_gattc_cache_conn_chr *chr,
                              uint16_t dsc_handle,
...
static struct ble_gattc_cache_conn_chr *
ble_gattc_cache_conn_chr_find_range(const struct ble_gattc_cache_conn_svc *svc,
                                    uint16_t dsc_handle) {
...
int ble_gattc_cache_conn_dsc_add(ble_addr_t peer_addr, uint16_t chr_val_handle,
  ...
  if (chr == NULL) {
     ...
     * happen.
     */
    BLE_HS_LOG(ERROR, "Couldn't find characteristc for dsc handle = %d",
               gatt_dsc->handle);
    assert(0);
    return BLE_HS_EUNKNOWN;
  }
...
uint16_t ble_gattc_cache_conn_chr_end_handle(
  const struct ble_gattc_cache_conn_svc *svc,
  const struct ble_gattc_cache_conn_chr *chr) {
...
static struct ble_gattc_cache_conn_chr *
ble_gattc_cache_conn_chr_find_prev(const struct ble_gattc_cache_conn_svc *svc,
...
static int ble_gattc_cache_conn_db_hash_read(uint16_t conn_handle,
  ...
  res = ble_hs_mbuf_to_flat(attr->om, peer->database_hash, sizeof(uint8_t) * 16,
                            NULL);
  return res;
}

const struct ble_gattc_cache_conn_svc *ble_gattc_cache_conn_svc_find_uuid(
  const struct ble_gattc_cache_conn *ble_gattc_cache_conn,
...
ble_gattc_cache_conn_chr_find_uuid(
  const struct ble_gattc_cache_conn *ble_gattc_cache_conn,
...
int ble_gattc_cache_conn_chr_add(ble_addr_t peer_addr,
                                 uint16_t svc_start_handle,
  ...
  BLE_HS_LOG(DEBUG,
             "Characteristc added with def_handle = %d and val_handle = %d",
...
static struct ble_gattc_cache_conn_svc *
ble_gattc_cache_conn_svc_find_prev(struct ble_gattc_cache_conn *peer,
                                   uint16_t svc_start_handle) {
...
ble_gattc_cache_conn_svc_find(struct ble_gattc_cache_conn *ble_gattc_cache_conn,
  ...
  prev =
    ble_gattc_cache_conn_svc_find_prev(ble_gattc_cache_conn, svc_start_handle);
  if (prev == NULL) {
...
const struct ble_gattc_cache_conn_dsc *ble_gattc_cache_conn_dsc_find_uuid(
  const struct ble_gattc_cache_conn *ble_gattc_cache_conn,
  const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid,
  ...
  chr = ble_gattc_cache_conn_chr_find_uuid(ble_gattc_cache_conn, svc_uuid,
                                           chr_uuid);
  if (chr == NULL) {
    return NULL;
  }
...
int ble_gattc_cache_conn_inc_add(ble_addr_t peer_addr,
...
size_t ble_gattc_cache_conn_get_db_size(struct ble_gattc_cache_conn *peer) {
  if (peer == NULL) {
    return 0;
  }
  ...
  SLIST_FOREACH(svc, &peer->svcs, next) {
    ...
    SLIST_FOREACH(chr, &svc->chrs, next) {
      db_size++;
      SLIST_FOREACH(dsc, &chr->dscs, next) { db_size++; }
    }
  }
...
static void ble_gattc_cache_conn_cache_peer(struct ble_gattc_cache_conn *peer) {
...
static void service_sanity_check(struct ble_gattc_cache_conn_svc_list *svcs) {
  ...
  SLIST_FOREACH(svc, svcs, next) {
    if (svc->svc.end_handle == 65535) {
      ...
      SLIST_FOREACH(dsc, &prev->dscs, next) { end_handle = dsc->dsc.handle; }
...
ble_gattc_cache_conn_disc_complete(struct ble_gattc_cache_conn *peer, int rc) {
  ...
  if (rc == 0) {
    ...
    chr = ble_gattc_cache_conn_chr_find_uuid(
      peer, BLE_UUID16_DECLARE(BLE_GATT_SVC_UUID16),
  ...
  if (op->cb) {
    ...
    case BLE_GATT_OP_DISC_ALL_SVCS:
      rc = ble_gattc_cache_conn_search_all_svcs(peer->conn_handle, op->cb,
                                                op->cb_arg);
      if (rc != 0) {
    ...
    case BLE_GATT_OP_DISC_SVC_UUID:
      rc = ble_gattc_cache_conn_search_svc_by_uuid(peer->conn_handle, &op->uuid,
                                                   op->cb, op->cb_arg);
      if (rc != 0) {
    ...
    case BLE_GATT_OP_FIND_INC_SVCS:
      rc = ble_gattc_cache_conn_search_inc_svcs(
        peer->conn_handle, op->start_handle, op->end_handle, op->cb,
        op->cb_arg);
      if (rc != 0) {
    ...
    case BLE_GATT_OP_DISC_ALL_CHRS:
      rc = ble_gattc_cache_conn_search_all_chrs(
        peer->conn_handle, op->start_handle, op->end_handle, op->cb,
        op->cb_arg);
      if (rc != 0) {
    ...
    case BLE_GATT_OP_DISC_CHR_UUID:
      rc = ble_gattc_cache_conn_search_chrs_by_uuid(
        peer->conn_handle, op->start_handle, op->end_handle, &op->uuid, op->cb,
        op->cb_arg);
      if (rc != 0) {
    ...
    case BLE_GATT_OP_DISC_ALL_DSCS:
      rc = ble_gattc_cache_conn_search_all_dscs(
        peer->conn_handle, op->start_handle, op->end_handle, op->cb,
        op->cb_arg);
      if (rc != 0) {
...
static int ble_gattc_cache_conn_inc_disced(uint16_t conn_handle,
                                           const struct ble_gatt_error *error,
...
static int ble_gattc_cache_conn_svc_disced(uint16_t conn_handle,
                                           const struct ble_gatt_error *error,
                                           const struct ble_gatt_svc *service,
                                           void *arg) {
  struct ble_gattc_cache_conn *peer;
...
static int ble_gattc_cache_conn_disc(struct ble_gattc_cache_conn *peer) {
  int rc;
  ...
  rc = ble_gattc_disc_all_svcs(peer->conn_handle,
                               ble_gattc_cache_conn_svc_disced, peer);
  return rc;
}

static int ble_gattc_cache_conn_on_read(uint16_t conn_handle,
                                        const struct ble_gatt_error *error,
  ...
  res =
    ble_gattc_cache_check_hash((struct ble_gattc_cache_conn *)arg, attr->om);
  if (res == 0) {
...
int ble_gattc_cache_conn_create(uint16_t conn_handle,
  ...
  memcpy(&cache_conn->ble_gattc_cache_conn_addr, &ble_gattc_cache_conn_addr,
         sizeof(ble_addr_t));
...
static int ble_gattc_cache_conn_dsc_disced(uint16_t conn_handle,
                                           const struct ble_gatt_error *error,
                                           uint16_t chr_val_handle,
  ...
  switch (error->status) {
  case 0:
    rc = ble_gattc_cache_conn_dsc_add(peer->ble_gattc_cache_conn_addr,
                                      chr_val_handle, dsc);
    break;
  ...
  SLIST_FOREACH(svc, &peer->svcs, next) {
    SLIST_FOREACH(chr, &svc->chrs, next) {
      if (!ble_gattc_cache_conn_chr_is_empty(svc, chr) &&
        ...
        rc =
          ble_gattc_disc_all_dscs(peer->conn_handle, chr->chr.val_handle,
...
static int ble_gattc_cache_conn_chr_disced(uint16_t conn_handle,
                                           const struct ble_gatt_error *error,
  ...
  switch (error->status) {
  case 0:
    rc = ble_gattc_cache_conn_chr_add(peer->ble_gattc_cache_conn_addr,
                                      peer->cur_svc->svc.start_handle, chr);
...
static void ble_gattc_cache_conn_disc_chrs(struct ble_gattc_cache_conn *peer) {
  struct ble_gattc_cache_conn_svc *svc;
  int rc;
...
static void ble_gattc_cache_conn_disc_incs(struct ble_gattc_cache_conn *peer) {
  struct ble_gattc_cache_conn_svc *svc;
  int rc;
  ...
  svc = peer->cur_svc;
  rc = ble_gattc_find_inc_svcs(peer->conn_handle, svc->svc.start_handle,
                               svc->svc.end_handle,
 ...
 */
void ble_gattc_cache_conn_update(uint16_t conn_handle, uint16_t start_handle,
                                 uint16_t end_handle) {
  ...
  peer = ble_gattc_cache_conn_find(conn_handle);
  if (peer == NULL) {
    BLE_HS_LOG(ERROR, "Cannot find connection with conn_handle %d",
               conn_handle);
  }
...
void ble_gattc_cache_conn_free_mem(void) {
...
int ble_gattc_cache_conn_init() {
  ...
  max_svcs =
    (MYNEWT_VAL(BLE_MAX_CONNECTIONS)) * (MYNEWT_VAL(BLE_GATT_CACHING_MAX_SVCS));
  max_chrs =
    (MYNEWT_VAL(BLE_MAX_CONNECTIONS)) * (MYNEWT_VAL(BLE_GATT_CACHING_MAX_CHRS));
  max_dscs =
    (MYNEWT_VAL(BLE_MAX_CONNECTIONS)) * (MYNEWT_VAL(BLE_GATT_CACHING_MAX_DSCS));
  /* Free memory first in case this function gets called more than once. */
  ble_gattc_cache_conn_free_mem();

  ble_gattc_cache_conn_mem = malloc(OS_MEMPOOL_BYTES(
    max_ble_gattc_cache_conns, sizeof(struct ble_gattc_cache_conn)));
  ...
  rc = os_mempool_init(&ble_gattc_cache_conn_pool, max_ble_gattc_cache_conns,
                       sizeof(struct ble_gattc_cache_conn),
                       ble_gattc_cache_conn_mem, "ble_gattc_cache_conn_pool");
  if (rc != 0) {
    rc = BLE_HS_EOS;
    goto err;
  }
  ...
  ble_gattc_cache_conn_svc_mem =
    malloc(OS_MEMPOOL_BYTES(max_svcs, sizeof(struct ble_gattc_cache_conn_svc)));
  ...
  rc = os_mempool_init(&ble_gattc_cache_conn_svc_pool, max_svcs,
                       sizeof(struct ble_gattc_cache_conn_svc),
                       ble_gattc_cache_conn_svc_mem,
  ...
  ble_gattc_cache_conn_chr_mem =
    malloc(OS_MEMPOOL_BYTES(max_chrs, sizeof(struct ble_gattc_cache_conn_chr)));
  ...
  rc = os_mempool_init(&ble_gattc_cache_conn_chr_pool, max_chrs,
                       sizeof(struct ble_gattc_cache_conn_chr),
                       ble_gattc_cache_conn_chr_mem,
  ...
  ble_gattc_cache_conn_dsc_mem =
    malloc(OS_MEMPOOL_BYTES(max_dscs, sizeof(struct ble_gattc_cache_conn_dsc)));
  ...
  rc = os_mempool_init(&ble_gattc_cache_conn_dsc_pool, max_dscs,
                       sizeof(struct ble_gattc_cache_conn_dsc),
                       ble_gattc_cache_conn_dsc_mem,
 ...
 */
static struct ble_gatt_error *ble_gattc_cache_error(int status,
                                                    uint16_t att_handle) {
...
static int ble_gattc_cache_conn_verify(struct ble_gattc_cache_conn *conn) {
  ...
  if (conn->cache_state == CACHE_LOADED) {
    ...
    rc = ble_gattc_read_by_uuid(
      conn->conn_handle, 0x0001, 0xFFFF,
      BLE_UUID16_DECLARE(BLE_SVC_GATT_CHR_DATABASE_HASH_UUID16),
...
static void ble_gattc_cache_search_all_svcs_cb(struct ble_npl_event *ev) {
  ...
  SLIST_FOREACH(svc, &conn->svcs, next) {
    if (svc->type == BLE_GATT_SVC_TYPE_PRIMARY) {
      dcb(conn->conn_handle, ble_gattc_cache_error(status, 0), &svc->svc,
          op->cb_arg);
    }
  }
  status = BLE_HS_EDONE;
...
static void ble_gattc_cache_conn_fill_op(struct ble_gattc_cache_conn_op *op,
                                         ...
                                         uint16_t end_handle, ble_uuid_t uuid,
                                         void *cb, void *cb_arg,
                                         uint8_t cb_type) {
  op->cb = cb;
  op->cb_arg = cb_arg;
...
int ble_gattc_cache_conn_search_all_svcs(uint16_t conn_handle,
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         BLE_GATT_OP_DISC_ALL_SVCS, 0, 0, uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev, ble_gattc_cache_search_all_svcs_cb,
                     &conn->conn_handle);
...
ble_gattc_cache_conn_search_svc_by_uuid_cb(struct ble_npl_event *ev) {
  ...
  SLIST_FOREACH(svc, &conn->svcs, next) {
    if (svc->type == BLE_GATT_SVC_TYPE_PRIMARY &&
        ble_uuid_cmp(&svc->svc.uuid.u, &op->uuid) == 0) {
...
int ble_gattc_cache_conn_search_svc_by_uuid(uint16_t conn_handle,
                                            const ble_uuid_t *uuid,
                                            ble_gatt_disc_svc_fn *cb,
                                            void *cb_arg) {
  struct ble_gattc_cache_conn *conn;
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         BLE_GATT_OP_DISC_SVC_UUID, 0, 0, *uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev, ble_gattc_cache_conn_search_svc_by_uuid_cb,
                     &conn->conn_handle);
  ble_npl_eventq_put((struct ble_npl_eventq *)ble_hs_evq_get(), &conn->disc_ev);
  return 0;
}

static void ble_gattc_cache_conn_search_inc_svcs_cb(struct ble_npl_event *ev) {
  ...
  SLIST_FOREACH(svc, &conn->svcs, next) {
    if (svc->type == BLE_GATT_SVC_TYPE_SECONDARY &&
        (svc->svc.start_handle >= op->start_handle &&
         svc->svc.end_handle <= op->end_handle)) {
      dcb(conn->conn_handle, ble_gattc_cache_error(status, 0), &svc->svc,
          op->cb_arg);
    }
  }
...
int ble_gattc_cache_conn_search_inc_svcs(uint16_t conn_handle,
                                         uint16_t start_handle,
                                         uint16_t end_handle,
                                         ble_gatt_disc_svc_fn *cb,
                                         void *cb_arg) {
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         BLE_GATT_OP_DISC_ALL_CHRS, start_handle, end_handle,
                         uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev, ble_gattc_cache_conn_search_inc_svcs_cb,
                     &conn->conn_handle);
  ble_npl_eventq_put((struct ble_npl_eventq *)ble_hs_evq_get(), &conn->disc_ev);
  return 0;
}

static void ble_gattc_cache_conn_search_all_chrs_cb(struct ble_npl_event *ev) {
  struct ble_gattc_cache_conn *conn;
  struct ble_gattc_cache_conn_svc *svc;
  struct ble_gattc_cache_conn_chr *chr;
  struct ble_gattc_cache_conn_op *op;
...
int ble_gattc_cache_conn_search_all_chrs(uint16_t conn_handle,
                                         uint16_t start_handle,
                                         uint16_t end_handle,
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         ...
                         uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev, ble_gattc_cache_conn_search_all_chrs_cb,
                     &conn->conn_handle);
  ble_npl_eventq_put((struct ble_npl_eventq *)ble_hs_evq_get(), &conn->disc_ev);
  return 0;
}
...
int ble_gattc_cache_conn_search_chrs_by_uuid(
  uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle,
  const ble_uuid_t *uuid, ble_gatt_chr_fn *cb, void *cb_arg) {
  struct ble_gattc_cache_conn *conn;
  struct ble_gattc_cache_conn_op *op;
  int rc;
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         BLE_GATT_OP_DISC_CHR_UUID, start_handle, end_handle,
                         *uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev,
                     ble_gattc_cache_conn_search_chrs_by_uuid_cb,
                     &conn->conn_handle);
  ble_npl_eventq_put((struct ble_npl_eventq *)ble_hs_evq_get(), &conn->disc_ev);
  return 0;
}

static void ble_gattc_cache_conn_search_all_dscs_cb(struct ble_npl_event *ev) {
  struct ble_gattc_cache_conn *conn;
  struct ble_gattc_cache_conn_svc *svc;
  struct ble_gattc_cache_conn_chr *chr;
  struct ble_gattc_cache_conn_dsc *dsc;
  ...
  SLIST_FOREACH(dsc, &chr->dscs, next) {
    dcb(conn_handle, ble_gattc_cache_error(status, 0), chr->chr.val_handle,
        &dsc->dsc, op->cb_arg);
  }
  status = BLE_HS_EDONE;
  dcb(conn_handle, ble_gattc_cache_error(status, 0), chr->chr.val_handle,
...
int ble_gattc_cache_conn_search_all_dscs(uint16_t conn_handle,
                                         uint16_t start_handle,
                                         uint16_t end_handle,
  ...
  CHECK_CACHE_CONN_STATE(conn->cache_state, cb, cb_arg,
                         BLE_GATT_OP_DISC_ALL_DSCS, start_handle, end_handle,
                         uuid);
  /* put the event in the queue to mimic the gattc behaviour */
  ble_npl_event_init(&conn->disc_ev, ble_gattc_cache_conn_search_all_dscs_cb,
                     &conn->conn_handle);
  ble_npl_eventq_put((struct ble_npl_eventq *)ble_hs_evq_get(), &conn->disc_ev);
  return 0;
}
