...
static inline void adv_send(struct os_mbuf *buf) {
  ...
  duration =
    ((BT_MESH_TRANSMIT_COUNT(BT_MESH_ADV(buf)->xmit) + 1) * (adv_int + 10));
#else
   ...
   */
  duration = (adv_int + ((BT_MESH_TRANSMIT_COUNT(BT_MESH_ADV(buf)->xmit) + 1) *
                         (adv_int + 10)));
  ...
  BT_DBG("type %u om_len %u: %s", BT_MESH_ADV(buf)->type, buf->om_len,
  ...
  while (1) {
    if (MYNEWT_VAL(BLE_MESH_GATT_SERVER)) {
      ...
      while (!ev) {
        ...
        ev = ble_npl_eventq_get(&bt_mesh_adv_queue,
                                ble_npl_time_ms_to_ticks32(adv_timeout));
        bt_le_adv_stop();
      }
    } else {
...
void bt_mesh_adv_buf_ready(void) { /* Will be handled automatically */ }
