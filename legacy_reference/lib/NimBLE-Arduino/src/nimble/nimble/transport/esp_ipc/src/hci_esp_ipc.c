...
static int ble_transport_dummy_host_recv_cb(hci_trans_pkt_ind_t type,
                                            uint8_t *data, uint16_t len) {
  /* Dummy function */
  return 0;
}

static int ble_transport_host_recv_cb(hci_trans_pkt_ind_t type, uint8_t *data,
                                      uint16_t len) {
  int rc;
...
void ble_transport_free(void *buf) { r_ble_hci_trans_buf_free(buf); }

#endif /* !CONFIG_IDF_TARGET_ESP32 && !CONFIG_IDF_TARGET_ESP32C3 &&            \
