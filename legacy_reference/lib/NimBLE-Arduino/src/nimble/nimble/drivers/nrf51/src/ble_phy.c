...
void ble_phy_rxpdu_copy(uint8_t *dptr, struct os_mbuf *rxpdu) {
  ...
  while (true) {
    ...
    __asm__ volatile(".syntax unified              \n"
                     ...
                     "   adds %[src], %[src], r4   \n"
                     "   adds %[dst], %[dst], r4   \n"
                     : [dst] "+l"(dst), [src] "+l"(src), [len] "+l"(copy_len)
                     :
                     : "r3", "r4", "memory");
 ...
 *
 */
int ble_phy_set_start_time(uint32_t cputime, uint8_t rem_usecs) {
  uint32_t next_cc;
  uint32_t cur_cc;
  uint32_t cntr;
  uint32_t delta;
...
void ble_phy_wfr_enable(int txrx, uint8_t tx_phy_mode, uint32_t wfr_usecs) {
  ...
  if (txrx == BLE_PHY_WFR_ENABLE_TXRX) {
    ...
    end_time = NRF_TIMER0->CC[0] + XCVR_RX_START_DELAY_USECS + wfr_usecs +
               ble_phy_mode_pdu_start_off(BLE_PHY_MODE_1M) +
               BLE_LL_JITTER_USECS;
  }
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LE_ENCRYPTION) ||                               \
  MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PRIVACY)
...
static void ble_phy_rx_start_isr(void) {
  int rc;
  uint32_t state;
  uint32_t usecs;
  uint32_t ticks;
  ...
  NRF_RADIO->INTENCLR =
    RADIO_INTENCLR_DISABLED_Msk | RADIO_INTENCLR_ADDRESS_Msk;
...
int ble_phy_tx(ble_phy_tx_pducb_t pducb, void *pducb_arg, uint8_t end_trans) {
   ...
   */
  NRF_PPI->CHENCLR =
    PPI_CHEN_CH4_Msk | PPI_CHEN_CH5_Msk | PPI_CHEN_CH23_Msk | PPI_CHEN_CH25_Msk;
