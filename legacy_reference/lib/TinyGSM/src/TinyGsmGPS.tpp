...
class TinyGsmGPS {
 public:
  ...
  bool getGPS(float* lat, float* lon, float* speed = 0, float* alt = 0,
              int* usat = 0, float* hdop = 0, int* year = 0,
              int* month = 0, int* day = 0, int* hour = 0, int* minute = 0,
              float* second = 0) {
    return thisModem().getGPSImpl(lat, lon, speed, alt, usat, hdop,
  ...
  bool getGPSTime(int* year, int* month, int* day, int* hour, int* minute,
    ...
    float lat = 0;
    float lon = 0;
    return thisModem().getGPSImpl(&lat, &lon, 0, 0, 0, 0, year, month, day,
                                  hour, minute, second);
  }

  String setGNSSMode(uint8_t mode) {
    return thisModem().setGNSSModeImpl(mode);
  }
