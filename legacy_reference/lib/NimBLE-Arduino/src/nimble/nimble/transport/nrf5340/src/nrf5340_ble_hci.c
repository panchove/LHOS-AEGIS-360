...
static int nrf5340_ble_hci_acl_tx(struct os_mbuf *om) {
...
#if !MYNEWT_VAL(BLE_CONTROLLER)
static int nrf5340_ble_hci_iso_tx(struct os_mbuf *om) {
  struct hci_ipc_hdr hdr;
  struct os_mbuf *x;
  int rc;
...
static void nrf5340_ble_hci_init(void) {
...
uint16_t hci_ipc_atomic_get(volatile uint16_t *num) {
  int ret;
...
void hci_ipc_atomic_put(volatile uint16_t *num) {
  __asm__ volatile(".syntax unified              \n"
                   "1: ldrexh r1, [%[addr]]      \n"
                   "   add r1, r1, #1            \n"
                   "   strexh r2, r1, [%[addr]]  \n"
