...
int ble_aes_ccm_encrypt_be(const uint8_t *key, const uint8_t *plaintext,
                           uint8_t *enc_data) {
...
static int ble_aes_ccm_calculate_X0(const uint8_t key[16], const uint8_t *aad,
                                    uint8_t aad_len, size_t mic_size,
                                    uint8_t msg_len, uint8_t b[16],
                                    uint8_t X0[16]) {
  int i, j, err;
...
static int ble_aes_ccm_crypt(const uint8_t key[16], const uint8_t nonce[13],
                             const uint8_t *in_msg, uint8_t *out_msg,
                             size_t msg_len) {
...
int ble_aes_ccm_decrypt(const uint8_t key[16], uint8_t nonce[13],
                        const uint8_t *enc_msg, size_t msg_len,
                        const uint8_t *aad, size_t aad_len, uint8_t *out_msg,
                        size_t mic_size) {
  uint8_t mic[16];
  ...
  ble_aes_ccm_auth(key_reversed, nonce, out_msg, msg_len, aad, aad_len, mic,
                   mic_size);
...
int ble_aes_ccm_encrypt(const uint8_t key[16], uint8_t nonce[13],
                        const uint8_t *msg, size_t msg_len, const uint8_t *aad,
                        size_t aad_len, uint8_t *out_msg, size_t mic_size) {
  /** MIC starts after encrypted message and is part of encrypted advertisement
   * data */
  ...
  /** Calculating MIC */
  ble_aes_ccm_auth(key_reversed, nonce, msg, msg_len, aad, aad_len, mic,
                   mic_size);
