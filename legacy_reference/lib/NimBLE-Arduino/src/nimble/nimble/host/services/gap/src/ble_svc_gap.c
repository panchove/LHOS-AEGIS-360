...
#define BLE_SVC_GAP_NAME_MAX_LEN MYNEWT_VAL(BLE_SVC_GAP_DEVICE_NAME_MAX_LENGTH)
    ...
    .characteristics =
      (struct ble_gatt_chr_def[]){
        ...
        },
        {
          ...
          .uuid =
            BLE_UUID16_DECLARE(BLE_SVC_GAP_CHR_UUID16_LE_GATT_SECURITY_LEVELS),
          .access_cb = ble_svc_gap_access,
          .flags = BLE_GATT_CHR_F_READ,
        },
        {
...
#if MYNEWT_VAL(BLE_SVC_GAP_GATT_SECURITY_LEVEL)
static int ble_svc_gap_security_level_read_access(uint16_t conn_handle,
                                                  struct os_mbuf *om) {
...
#endif
#if PPCP_ENABLED
  uint16_t ppcp[4] = {htole16(MYNEWT_VAL(BLE_SVC_GAP_PPCP_MIN_CONN_INTERVAL)),
...
const char *ble_svc_gap_device_name(void) { return ble_svc_gap_name; }
...
uint16_t ble_svc_gap_device_appearance(void) { return ble_svc_gap_appearance; }
...
void ble_svc_gap_deinit(void) { ble_gatts_free_svcs(); }
