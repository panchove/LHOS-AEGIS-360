...
protected:
  ...
  bool getGPSImpl(float *lat, float *lon, float *speed = 0, float *alt = 0,
                  int *usat = 0, float *hdop = 0, int *year = 0, int *month = 0,
    ...
    if (fixMode == 1 || fixMode == 2 || fixMode == 3) {
      ...
      iusat = streamGetIntBefore(',');  // GPS satellite valid numbers
      ...
      ihdop = streamGetFloatBefore(',');  // Horizontal Dilution Of Precision
      ...
      if (lat != NULL)
        *lat =
          (floor(ilat / 100) + fmod(ilat, 100.) / 60) * (north == 'N' ? 1 : -1);
      if (lon != NULL)
        *lon =
          (floor(ilon / 100) + fmod(ilon, 100.) / 60) * (east == 'E' ? 1 : -1);
      if (speed != NULL)
        *speed = ispeed * 1.852;
      if (alt != NULL)
        *alt = ialt;
...
protected:
  ...
  //   Serial.println("Sending CIPSEND to modem");
  ...
  // }
  uint32_t modemSend(const void *buff, uint32_t len, uint8_t mux) {
    uint32_t totalSentBytes = 0; // Track the total sent bytes
    while (len > 0) {
      size_t chunkSize =
        len > 1500 ? 1500 : len; // Maximum chunk size is 1500 bytes
      sendAT(GF("+CIPSEND="), mux, ',', (uint16_t)chunkSize);

      if (waitResponse(GF(">")) != 1) {
        // If ">" response not received, return the partial sum of sent bytes
        return totalSentBytes;
      }

      stream.write(reinterpret_cast<const uint8_t *>(buff), chunkSize);
      stream.flush();

      if (waitResponse(GF(GSM_NL "+CIPSEND:")) != 1) {
        // If "+CIPSEND:" response not received, return the partial sum of sent
        // bytes
        return totalSentBytes;
      }
      ...
      uint32_t sentBytes = streamGetIntBefore('\n');

      totalSentBytes += sentBytes;
      len -= chunkSize; // Update remaining length
      buff =
        static_cast<const uint8_t *>(buff) + chunkSize; // Update buffer pointer
    }
    return totalSentBytes;
  }
