...
#else
#define BLE_HS_ATT_SVR_QUEUED_WRITE_TMO MYNEWT_VAL(BLE_ATT_SVR_QUEUED_WRITE_TMO)
#endif
...
int ble_mqueue_init(struct ble_mqueue *mq, ble_npl_event_fn *ev_fn,
                    void *ev_arg);
struct os_mbuf *ble_mqueue_get(struct ble_mqueue *mq);
int ble_mqueue_put(struct ble_mqueue *mq, struct ble_npl_eventq *evq,
                   struct os_mbuf *om);
