...
NimBLEScan::NimBLEScan()
    : m_pScanCallbacks{&defaultScanCallbacks},
      // default interval + window, no whitelist scan filter,not limited scan,
      // no scan response, filter_duplicates
      m_scanParams{0, 0, BLE_HCI_SCAN_FILT_NO_WL, 0, 1, 1},
      m_pTaskData{nullptr}, m_maxResults{0xFF} {}
...
int NimBLEScan::handleGapEvent(ble_gap_event *event, void *arg) {
  ...
  case BLE_GAP_EVENT_DISC: {
    ...
    NimBLEClient *pClient =
      NimBLEDevice::getClientByPeerAddress(advertisedAddress);
    if (pClient != nullptr && pClient->isConnected()) {
      NIMBLE_LOGI(LOG_TAG, "Ignoring device: address: %s, already connected",
...
#if CONFIG_BT_NIMBLE_EXT_ADV
      // Same address but different set ID should create a new advertised
      // device.
    ...
    // If we haven't seen this device before; create a new instance and insert
    // it in the vector. Otherwise just update the relevant parameters of the
    // already known device.
    if (advertisedDevice == nullptr) {
      // Check if we have reach the scan results limit, ignore this one if so.
      // We still need to store each device when maxResults is 0 to be able to
      ...
      if (isLegacyAdv && event_type == BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP) {
        NIMBLE_LOGI(LOG_TAG, "Scan response without advertisement: %s",
                    advertisedAddress.toString().c_str());
      }

      advertisedDevice = new NimBLEAdvertisedDevice(event, event_type);
      pScan->m_scanResults.m_deviceVec.push_back(advertisedDevice);
      NIMBLE_LOGI(LOG_TAG, "New advertiser: %s",
                  advertisedAddress.toString().c_str());
    } else {
      advertisedDevice->update(event, event_type);
      if (isLegacyAdv) {
        if (event_type == BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP) {
          NIMBLE_LOGI(LOG_TAG, "Scan response from: %s",
                      advertisedAddress.toString().c_str());
        } else {
          NIMBLE_LOGI(LOG_TAG, "Duplicate; updated: %s",
                      advertisedAddress.toString().c_str());
        }
      }
    }
    ...
    // or extended advertisement scanning, report the result to the callback
    // now.
    if (pScan->m_scanParams.passive || !isLegacyAdv ||
        !advertisedDevice->isScannable()) {
      advertisedDevice->m_callbackSent++;
    ...
    // If not storing results and we have invoked the callback, delete the
    // device.
  ...
  case BLE_GAP_EVENT_DISC_COMPLETE: {
    ...
    if (pScan->m_pTaskData != nullptr) {
      NimBLEUtils::taskRelease(*pScan->m_pTaskData,
                               event->disc_complete.reason);
    }
 ...
 * The default is a passive scan. An active scan means that we will request a
 * scan response.
 ...
 * @param [in] enabled If set to 1 (true), scanned devices will only be reported
once.
 * If set to 0 duplicates will be reported each time they are seen.
 * If using extended scanning this can be set to 2 which will reset the
duplicate filter
 ...
 * @param [in] enabled If true, only limited discovery devices will be in scan
 * results.
 */
 ...
 * @details The interval is the time between the start of two consecutive scan
 * windows. When a new interval starts the controller changes the channel it's
 * scanning on.
 */
 ...
 * @param [in] windowMs How long during the interval to actively scan in
 * milliseconds.
 */
 ...
 */
bool NimBLEScan::isScanning() { return ble_gap_disc_active(); }
 ...
 */
void NimBLEScan::setPhy(Phy phyMask) { m_phy = phyMask; } // setScanPhy

/**
 * @brief Set the extended scanning period.
 * @param [in] periodMs The scan period in milliseconds
 * @details The period is the time between the start of two consecutive scan
 * periods. This works as a timer to restart scanning at the specified amount of
 * time in periodMs.
 ...
 * @param [in] duration The duration in milliseconds for which to scan. 0 ==
 * scan forever.
 * @param [in] isContinue Set to true to save previous scan results, false to
 * clear them.
 * @param [in] restart Set to true to restart the scan if already in progress.
 * this is useful to clear the duplicate filter so all devices are reported
 * again.
...
bool NimBLEScan::start(uint32_t duration, bool isContinue, bool restart) {
  ...
  // If scanning is already active, call the functions anyway as the parameters
  // can be changed.
  ...
  int rc = ble_gap_ext_disc(
    NimBLEDevice::m_ownAddrType,
    duration / 10, // 10ms units
    m_period, m_scanParams.filter_duplicates, m_scanParams.filter_policy,
    m_scanParams.limited, m_phy & SCAN_1M ? &scan_params : NULL,
    m_phy & SCAN_CODED ? &scan_params : NULL, NimBLEScan::handleGapEvent, NULL);
#else
  int rc = ble_gap_disc(NimBLEDevice::m_ownAddrType,
                        duration ? duration : BLE_HS_FOREVER, &m_scanParams,
                        NimBLEScan::handleGapEvent, NULL);
#endif
  switch (rc) {
  case 0:
  ...
  default:
    NIMBLE_LOGE(LOG_TAG, "Error starting scan; rc=%d, %s", rc,
...
void NimBLEScan::erase(const NimBLEAddress &address) {
  ...
  for (auto it = m_scanResults.m_deviceVec.begin();
       it != m_scanResults.m_deviceVec.end(); ++it) {
...
void NimBLEScan::erase(const NimBLEAdvertisedDevice *device) {
  NIMBLE_LOGD(LOG_TAG, "erase device: %s",
 ...
 * @param [in] is_continue Set to true to save previous scan results, false to
 * clear them.
...
/**
 * @brief Get iterator to the beginning of the vector of advertised device
 * pointers.
 * @return An iterator to the beginning of the vector of advertised device
 * pointers.
 */
std::vector<NimBLEAdvertisedDevice *>::const_iterator
NimBLEScanResults::begin() const {
...
NimBLEScanResults::getDevice(const NimBLEAddress &address) const {
  for (const auto &dev : m_deviceVec) {
...
void NimBLEScanCallbacks::onDiscovered(
  const NimBLEAdvertisedDevice *pAdvertisedDevice) {
  NIMBLE_LOGD(CB_TAG, "Discovered: %s", pAdvertisedDevice->toString().c_str());
}

void NimBLEScanCallbacks::onResult(
  const NimBLEAdvertisedDevice *pAdvertisedDevice) {
  NIMBLE_LOGD(CB_TAG, "Result: %s", pAdvertisedDevice->toString().c_str());
}

void NimBLEScanCallbacks::onScanEnd(const NimBLEScanResults &results,
                                    int reason) {
  NIMBLE_LOGD(CB_TAG, "Scan ended; reason %d, num results: %d", reason,
              results.getCount());
}
