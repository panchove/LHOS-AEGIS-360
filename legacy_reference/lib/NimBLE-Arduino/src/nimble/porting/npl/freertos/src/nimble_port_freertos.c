...
esp_err_t esp_nimble_enable(void *host_task) {
   ...
   */
  xTaskCreatePinnedToCore(host_task, "nimble_host", NIMBLE_HS_STACK_SIZE, NULL,
                          NIMBLE_HOST_TASK_PRIORITY, &host_task_h, NIMBLE_CORE);
  return ESP_OK;
}
 ...
 *
 */
void nimble_port_freertos_deinit(void) { esp_nimble_disable(); }
...
void nimble_port_freertos_init(TaskFunction_t host_task_fn) {
   ...
   */
  ll_task_h =
    xTaskCreateStatic(nimble_port_ll_task_func, "ll", NIMBLE_LL_STACK_SIZE,
   ...
   */
  host_task_h =
    xTaskCreateStatic(host_task_fn, "host", NIMBLE_HS_STACK_SIZE, NULL,
                      NIMBLE_HOST_TASK_PRIORITY, hs_stack, &hs_task_buffer);
}
