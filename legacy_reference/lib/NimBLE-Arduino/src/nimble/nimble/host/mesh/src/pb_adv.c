...
static struct os_mbuf *adv_buf_create(uint8_t retransmits) {
  ...
  buf = bt_mesh_adv_create(BT_MESH_ADV_PROV, BT_MESH_TRANSMIT(retransmits, 20),
                           BUF_TIMEOUT);
  if (!buf) {
...
static void gen_prov_cont(struct prov_rx *rx, struct os_mbuf *buf) {
  ...
  if (!link.rx.seg && next_transaction_id(link.rx.id) == rx->xact_id) {
    ...
    BT_WARN("Data for unknown transaction (0x%x != 0x%x)", rx->xact_id,
            link.rx.id);
    return;
  }
  ...
  if (seg == link.rx.last_seg && !(link.rx.seg & BIT(0))) {
    ...
    expect_len = (link.rx.buf->om_len - 20U - ((link.rx.last_seg - 1) * 23U));
    if (expect_len != buf->om_len) {
      BT_ERR("Incorrect last seg len: %u != %u", expect_len, buf->om_len);
      prov_failed(PROV_ERR_NVAL_FMT);
      return;
    }
  }
...
static void gen_prov_start(struct prov_rx *rx, struct os_mbuf *buf) {
  ...
  BT_DBG("%p len %u last_seg %u total_len %u fcs 0x%02x", link.rx.buf,
         buf->om_len, START_LAST_SEG(rx->gpc), link.rx.buf->om_len,
         link.rx.fcs);
...
static struct os_mbuf *ctl_buf_create(uint8_t op, const void *data,
...
static int prov_send_adv(struct os_mbuf *msg, prov_bearer_send_complete_t cb,
                         void *cb_data) {
  ...
  for (seg_id = 1U; msg->om_len > 0; seg_id++) {
    ...
    BT_DBG("seg %u len %u: %s", seg_id, seg_len, bt_hex(msg->om_data, seg_len));
...
static void link_open(struct prov_rx *rx, struct os_mbuf *buf) {
  ...
  if (atomic_test_bit(link.flags, ADV_LINK_ACTIVE)) {
    ...
    /* Ignore errors, message will be attempted again if we keep receiving link
     * open: */
  ...
  err =
    bearer_ctl_send_unacked(ctl_buf_create(LINK_ACK, NULL, 0, RETRANSMITS_ACK));
  if (err) {
    reset_adv_link();
    return;
  }
...
static int prov_link_open(const uint8_t uuid[16], int32_t timeout,
  ...
  return bearer_ctl_send(
    ctl_buf_create(LINK_OPEN, uuid, 16, RETRANSMITS_RELIABLE));
}
...
void pb_adv_reset(void) { reset_adv_link(); }
