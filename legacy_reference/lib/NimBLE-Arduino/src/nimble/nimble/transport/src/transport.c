...
#define POOL_ISO_COUNT (0)
  ...
  !MYNEWT_VAL_CHOICE(BLE_TRANSPORT_HS, native)
#define POOL_ACL_COUNT                                                         \
  ((MYNEWT_VAL(BLE_TRANSPORT_ACL_FROM_HS_COUNT)) +                             \
   (MYNEWT_VAL(BLE_TRANSPORT_ACL_FROM_LL_COUNT)))
#define POOL_ISO_COUNT                                                         \
  ((MYNEWT_VAL(BLE_TRANSPORT_ISO_FROM_HS_COUNT)) +                             \
...
#endif
#define POOL_ACL_SIZE                                                          \
  (OS_ALIGN(MYNEWT_VAL(BLE_TRANSPORT_ACL_SIZE) + BLE_MBUF_MEMBLOCK_OVERHEAD +  \
              BLE_HCI_DATA_HDR_SZ,                                             \
            OS_ALIGNMENT))
#define POOL_ISO_SIZE                                                          \
  (OS_ALIGN(MYNEWT_VAL(BLE_TRANSPORT_ISO_SIZE) + BLE_MBUF_MEMBLOCK_OVERHEAD +  \
              BLE_HCI_DATA_HDR_SZ,                                             \
            OS_ALIGNMENT))
...
static uint8_t
  pool_evt_lo_buf[OS_MEMPOOL_BYTES(POOL_EVT_LO_COUNT, POOL_EVT_SIZE)];
static struct os_mempool pool_evt_lo;
...
void *ble_transport_alloc_cmd(void) { return os_memblock_get(&pool_cmd); }

static void *try_alloc_evt(struct os_mempool *mp) {
...
#if POOL_ACL_COUNT > 0
static os_error_t ble_transport_acl_put(struct os_mempool_ext *mpe, void *data,
                                        void *arg) {
  os_error_t err;
  err = 0;
...
esp_err_t ble_buf_alloc(void) {
  ...
  pool_evt_buf = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) *
        OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_TRANSPORT_EVT_COUNT),
                        MYNEWT_VAL(BLE_TRANSPORT_EVT_SIZE))));

  pool_evt_lo_buf = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) *
        OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_TRANSPORT_EVT_DISCARDABLE_COUNT),
                        MYNEWT_VAL(BLE_TRANSPORT_EVT_SIZE))));

  pool_cmd_buf = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) * OS_MEMPOOL_SIZE(POOL_CMD_COUNT, POOL_CMD_SIZE)));

#if POOL_ACL_COUNT > 0
  pool_acl_buf = (os_membuf_t *)nimble_platform_mem_calloc(
    1, (sizeof(os_membuf_t) * OS_MEMPOOL_SIZE(POOL_ACL_COUNT, POOL_ACL_SIZE)));
  if (!pool_acl_buf) {
    ble_buf_free();
    return ESP_ERR_NO_MEM;
  }
#endif
#if POOL_ISO_COUNT > 0
  pool_iso_buf = (os_membuf_t *)nimble_platform_mem_calloc(
    1, sizeof(os_membuf_t) * OS_MEMPOOL_SIZE(POOL_ISO_COUNT, POOL_ISO_SIZE));
  if (!pool_iso_buf) {
    ble_buf_free();
    return ESP_ERR_NO_MEM;
  }
...
void ble_transport_init(void) {
  ...
  rc = os_mempool_init(&pool_cmd, POOL_CMD_COUNT, POOL_CMD_SIZE, pool_cmd_buf,
                       "transport_pool_cmd");
  SYSINIT_PANIC_ASSERT(rc == 0);

  rc = os_mempool_init(&pool_evt, POOL_EVT_COUNT, POOL_EVT_SIZE, pool_evt_buf,
                       "transport_pool_evt");
  SYSINIT_PANIC_ASSERT(rc == 0);
  ...
  rc = os_mbuf_pool_init(&mpool_acl, &pool_acl.mpe_mp, POOL_ACL_SIZE,
                         POOL_ACL_COUNT);
  SYSINIT_PANIC_ASSERT(rc == 0);
  ...
  rc = os_mbuf_pool_init(&mpool_iso, &pool_iso.mpe_mp, POOL_ISO_SIZE,
                         POOL_ISO_COUNT);
  SYSINIT_PANIC_ASSERT(rc == 0);
#endif
}
