 ...
 *
 * A BLE Characteristic is an identified value container that manages a value.
 * It is exposed by a BLE service and can be read and written to by a BLE
 * client.
 */
class NimBLECharacteristic : public NimBLELocalValueAttribute {
public:
  NimBLECharacteristic(const char *uuid,
                       uint16_t properties = NIMBLE_PROPERTY::READ |
  ...
  bool indicate(const uint8_t *value, size_t length,
  ...
  NimBLEDescriptor *createDescriptor(
    const char *uuid,
   ...
   * @param [in] connHandle Optional, a connection handle to send the
   * notification to.
  ...
  typename std::enable_if<
    !std::is_pointer<T>::value && !std::is_array<T>::value &&
      !Has_c_str_length<T>::value && !Has_data_size<T>::value,
    bool>::type
#endif
  ...
  /**
   * @brief Template to send a notification with a value from a class that has a
   * c_str() and length() method.
...
#else
  typename std::enable_if<
    Has_c_str_length<T>::value && !Has_data_size<T>::value, bool>::type
#endif
  notify(const T &s, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return notify(reinterpret_cast<const uint8_t *>(s.c_str()), s.length(),
                  connHandle);
  }

  /**
   * @brief Template to send a notification with a value from a class that has a
   * data() and size() method with value_type.
   * @param [in] v The value to send.
   * @param [in] connHandle Optional, a connection handle to send the
   * notification to.
   * @details Correctly calculates byte size for containers with multi-byte
   * element types.
   */
  template <typename T>
#ifdef _DOXYGEN_
  bool
#else
  typename std::enable_if<Has_data_size<T>::value && Has_value_type<T>::value,
                          bool>::type
#endif
  notify(const T &v, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return notify(reinterpret_cast<const uint8_t *>(v.data()),
                  v.size() * sizeof(typename T::value_type), connHandle);
  }

  /**
   * @brief Template to send a notification with a value from a class that has a
   * data() and size() method without value_type.
   * @param [in] v The value to send.
   * @param [in] connHandle Optional, a connection handle to send the
   * notification to.
   */
...
#endif
  notify(const T &v, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return notify(reinterpret_cast<const uint8_t *>(v.data()), v.size(),
                  connHandle);
  }
  ...
  indicate(const T &v, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return indicate(reinterpret_cast<const uint8_t *>(&v), sizeof(T),
                    connHandle);
  }

  /**
   * @brief Template to send a indication with a value from a class that has a
   * c_str() and length() method.
   * @param [in] s The value to send.
   * @param [in] connHandle Optional, a connection handle to send the
   * notification to.
  ...
  indicate(const T &s, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return indicate(reinterpret_cast<const uint8_t *>(s.c_str()), s.length(),
                    connHandle);
  }
...
#endif
  indicate(const T &v, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return indicate(reinterpret_cast<const uint8_t *>(v.data()),
                    v.size() * sizeof(typename T::value_type), connHandle);
  }
...
#endif
  indicate(const T &v, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    return indicate(reinterpret_cast<const uint8_t *>(v.data()), v.size(),
                    connHandle);
  }
  ...
  template <typename T>
  typename std::enable_if<
    !std::is_pointer<T>::value && !std::is_array<T>::value, bool>::type
  notify(const T &value, uint16_t connHandle = BLE_HS_CONN_HANDLE_NONE) const {
    if constexpr (Has_data_size<T>::value) {
      ...
      return notify(reinterpret_cast<const uint8_t *>(value.c_str()),
                    value.length(), connHandle);
    } else {
      return notify(reinterpret_cast<const uint8_t *>(&value), sizeof(value),
                    connHandle);
    }
  }
  ...
  indicate(const T &value,
    ...
    if constexpr (Has_data_size<T>::value) {
      ...
      return indicate(reinterpret_cast<const uint8_t *>(value.c_str()),
                      value.length(), connHandle);
    } else {
      return indicate(reinterpret_cast<const uint8_t *>(&value), sizeof(value),
                      connHandle);
    }
  }
#endif
...
private:
  ...
  void writeEvent(const uint8_t *val, uint16_t len,
                  NimBLEConnInfo &connInfo) override;
  bool sendValue(const uint8_t *value, size_t length,
...
/**
 * @brief Callbacks that can be associated with a %BLE characteristic to inform
 * of events.
 *
 * When a server application creates a %BLE characteristic, we may wish to be
 * informed when there is either a read or write request to the characteristic's
 * value. An application can register a sub-classed instance of this class and
 * will be notified when such an event happens.
 */
class NimBLECharacteristicCallbacks {
public:
