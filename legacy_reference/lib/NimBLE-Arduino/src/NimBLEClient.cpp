 ...
 * Design
 * ------
 * When we perform a getService() request, we are asking the BLE server to
 * return each of the services that it exposes.  For each service, we receive a
 * callback which contains details of the exposed service including its UUID.
 *
 ...
 *
 * Since there is a hierarchical relationship here, we will have the idea that
 * from a NimBLERemoteService will own zero or more remote characteristics and a
 * NimBLERemoteCharacteristic will own zero or more NimBLERemoteDescriptors.
 *
 * We will assume that a NimBLERemoteService contains a vector of owned
 * characteristics and that a NimBLERemoteCharacteristic contains a vector of
 * owned descriptors.
 */
...
NimBLEClient::NimBLEClient(const NimBLEAddress &peerAddress)
    : m_peerAddress(peerAddress), m_lastErr{0}, m_connectTimeout{30000},
      m_pTaskData{nullptr}, m_svcVec{}, m_pClientCallbacks{&defaultCallbacks},
      m_connHandle{BLE_HS_CONN_HANDLE_NONE}, m_terminateFailCount{0},
      m_asyncSecureAttempt{0}, m_config{},
...
/**
 * @brief Delete all service objects created by this client and clear the
 * vector.
 */
 ...
 * @param [in] pDevice A pointer to the advertised device instance to connect
 * to.
 * @param [in] deleteAttributes If true this will delete any attribute objects
 * this client may already\n have created when last connected.
 * @param [in] asyncConnect If true, the connection will be made asynchronously
 * and this function will return immediately.\n If false, this function will
 * block until the connection is established or the connection attempt times
 * out.
 * @param [in] exchangeMTU If true, the client will attempt to exchange MTU with
 * the server after connection.\n If false, the client will use the default MTU
 * size and the application will need to call exchangeMTU() later.
 * @return true on success.
 */
bool NimBLEClient::connect(const NimBLEAdvertisedDevice *pDevice,
                           bool deleteAttributes, bool asyncConnect,
                           bool exchangeMTU) {
...
/**
 * @brief Connect to the BLE Server using the address of the last connected
 * device, or the address\n passed to the constructor.
 * @param [in] deleteAttributes If true this will delete any attribute objects
 * this client may already\n have created when last connected.
 * @param [in] asyncConnect If true, the connection will be made asynchronously
 * and this function will return immediately.\n If false, this function will
 ...
 * @return true on success.
 */
bool NimBLEClient::connect(bool deleteAttributes, bool asyncConnect,
                           bool exchangeMTU) {
...
bool NimBLEClient::connect(const NimBLEAddress &address, bool deleteAttributes,
                           bool asyncConnect, bool exchangeMTU) {
  ...
  do {
#if CONFIG_BT_NIMBLE_EXT_ADV
    rc = ble_gap_ext_connect(NimBLEDevice::m_ownAddrType, peerAddr,
                             m_connectTimeout, m_phyMask, &m_connParams,
                             &m_connParams, &m_connParams,
                             NimBLEClient::handleGapEvent, this);

#else
    rc =
      ble_gap_connect(NimBLEDevice::m_ownAddrType, peerAddr, m_connectTimeout,
                      &m_connParams, NimBLEClient::handleGapEvent, this);
#endif
    switch (rc) {
    case 0:
      break;
    ...
    case BLE_HS_EBUSY:
      ...
      // Scan was active, stop it through the NimBLEScan API to release any
      // tasks and call the callback.
    ...
    default:
      NIMBLE_LOGE(LOG_TAG, "Failed to connect to %s, rc=%d; %s",
                  std::string(m_peerAddress).c_str(), rc,
  ...
  if (!NimBLEUtils::taskWait(taskData, m_connectTimeout + 1000)) {
    ...
    if (isConnected()) {
      ...
      // workaround; if the controller doesn't cancel the connection at the
      // timeout, cancel it here.
  ...
  rc = taskData.m_flags;
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Connection failed; status=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
 ...
 * Called automatically when a characteristic or descriptor requires encryption
 * or authentication to access it.
 * @param [in] async If true, the connection will be secured asynchronously and
 * this function will return immediately.\n If false, this function will block
 * until the connection is secured or the client disconnects.
 * @return True on success.
 * @details If async=false, this function will block and should not be used in a
 * callback.
 */
bool NimBLEClient::secureConnection(bool async) const {
  ...
  } while (taskData.m_flags == (BLE_HS_ERR_HCI_BASE + BLE_ERR_PINKEY_MISSING) &&
           retryCount--);
...
bool NimBLEClient::cancelConnect() const {
  ...
  if (rc != 0 && rc != BLE_HS_EALREADY) {
    NIMBLE_LOGE(LOG_TAG, "ble_gap_conn_cancel failed: rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
    return false;
  }
...
/**
 * @brief Set or unset a flag to delete this client when disconnected or
 * connection failed.
 * @param [in] deleteOnDisconnect Set the client to self delete when
 * disconnected.
 * @param [in] deleteOnConnectFail Set the client to self delete when a
 ...
 * @param [in] config The config options instance to set the client
 * configuration to.
 */
 ...
 */
bool NimBLEClient::updatePhy(uint8_t txPhyMask, uint8_t rxPhyMask,
                             uint16_t phyOptions) {
  int rc =
    ble_gap_set_prefered_le_phy(m_connHandle, txPhyMask, rxPhyMask, phyOptions);
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Failed to update phy; rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
...
bool NimBLEClient::getPhy(uint8_t *txPhy, uint8_t *rxPhy) {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Failed to read phy; rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
 ...
 * @param [in] latency The number of packets allowed to skip (extends max
 * interval).
 * @param [in] timeout The timeout time in 10ms units before disconnecting.
 * @param [in] scanInterval The scan interval to use when attempting to connect
 * in 0.625ms units.
 * @param [in] scanWindow The scan window to use when attempting to connect in
 * 0.625ms units.
 */
void NimBLEClient::setConnectionParams(uint16_t minInterval,
                                       uint16_t maxInterval, uint16_t latency,
                                       uint16_t timeout, uint16_t scanInterval,
                                       uint16_t scanWindow)
/*, uint16_t minConnEvtTime, uint16_t maxConnEvtTime)*/
{
  ...
  // m_connParams.min_ce_len = minConnEvtTime;     // Minimum length of
  // connection event in 0.625ms units m_connParams.max_ce_len = maxConnEvtTime;
 ...
 * @param [in] timeout The timeout time in 10ms units before disconnecting.
 */
bool NimBLEClient::updateConnParams(uint16_t minInterval, uint16_t maxInterval,
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Update params error: %d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
  }
 ...
 * * Can only be used after a connection has been established.
 * @details Sends a data length update request to the server the client is
 * connected to. The Data Length Extension (DLE) allows to increase the Data
 * Channel Payload from 27 bytes to up to 251 bytes. The server needs to support
 * the Bluetooth 4.2 specifications, to be capable of DLE.
 * @param [in] txOctets The preferred number of payload octets to use (Range
 * 0x001B-0x00FB).
 */
bool NimBLEClient::setDataLen(uint16_t txOctets) {
#if defined(CONFIG_NIMBLE_CPP_IDF) && !defined(ESP_IDF_VERSION) ||             \
  (ESP_IDF_VERSION_MAJOR * 100 + ESP_IDF_VERSION_MINOR * 10 +                  \
   ESP_IDF_VERSION_PATCH) < 432
  return false;
#else
  uint16_t txTime = (txOctets + 14) * 8;
  int rc = ble_gap_set_data_len(m_connHandle, txOctets, txTime);
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Set data length error: %d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
 ...
 * @return A NimBLEConnInfo instance with the data, or a NULL instance if not
 * found.
 */
 ...
 * @param [in] time The number of milliseconds before timeout, default is 30
 * seconds.
 */
 ...
 * @param [in] address The address of the peer that this client is connected or
 * should connect to.
 * @return True if successful.
 */
...
int NimBLEClient::getRssi() const {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Failed to read RSSI error code: %d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
    return 0;
  }
...
/**
 * @brief Get iterator to the beginning of the vector of remote service
 * pointers.
 * @return An iterator to the beginning of the vector of remote service
 * pointers.
 */
...
NimBLERemoteService *NimBLEClient::getService(const NimBLEUUID &uuid) {
  ...
  for (auto &it : m_svcVec) {
    if (it->getUUID() == uuid) {
      NIMBLE_LOGD(LOG_TAG, "<< getService: found the service with uuid: %s",
                  uuid.toString().c_str());
      return it;
    }
  }
  ...
  if (retrieveServices(&uuid)) {
    ...
    if (uuid.bitSize() == BLE_UUID_TYPE_16 ||
      ...
      uuid16.to16();
      // if the uuid was 128 bit but not of the BLE base type this check will
      // fail
...
NimBLEClient::getServices(bool refresh) {
  if (refresh) {
    deleteServices();
    if (!retrieveServices()) {
...
/**
 * @brief Retrieves the full database of attributes that the peripheral has
 * available.
 * @return True if successful.
 */
...
bool NimBLEClient::retrieveServices(const NimBLEUUID *uuidFilter) {
  ...
  if (uuidFilter == nullptr) {
    rc = ble_gattc_disc_all_svcs(m_connHandle,
                                 NimBLEClient::serviceDiscoveredCB, &taskData);
  } else {
    rc =
      ble_gattc_disc_svc_by_uuid(m_connHandle, uuidFilter->getBase(),
                                 NimBLEClient::serviceDiscoveredCB, &taskData);
  }

  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gattc_disc_all_svcs: rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
    return false;
  }
  ...
  m_lastErr = rc;
  NIMBLE_LOGE(LOG_TAG, "Could not retrieve services, rc=%d %s", rc,
              NimBLEUtils::returnCodeToString(rc));
  return false;
} // getServices
...
/**
 * @brief Get the value of a specific characteristic associated with a specific
 * service.
 * @param [in] serviceUUID The service that owns the characteristic.
 * @param [in] characteristicUUID The characteristic whose value we wish to
 * read.
...
/**
 * @brief Set the value of a specific characteristic associated with a specific
 * service.
 * @param [in] serviceUUID The service that owns the characteristic.
 * @param [in] characteristicUUID The characteristic whose value we wish to
 * write.
...
bool NimBLEClient::setValue(const NimBLEUUID &serviceUUID,
  ...
  if (pService != nullptr) {
    NimBLERemoteCharacteristic *pChar =
      pService->getCharacteristic(characteristicUUID);
    if (pChar != nullptr) {
 ...
 * @details When the MTU exchange is complete the API will call this and report
 * the new MTU.
 */
int NimBLEClient::exchangeMTUCb(uint16_t conn_handle,
  ...
  if (error->status != 0) {
    NIMBLE_LOGE(LOG_TAG, "exchangeMTUCb() rc=%d %s", error->status,
...
bool NimBLEClient::exchangeMTU() {
  int rc =
    ble_gattc_exchange_mtu(m_connHandle, NimBLEClient::exchangeMTUCb, this);
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "MTU exchange error; rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    m_lastErr = rc;
    return false;
  }
 ...
 * @param [in] arg A pointer to the client instance that registered for this
 * callback.
 */
int NimBLEClient::handleGapEvent(struct ble_gap_event *event, void *arg) {
  NimBLEClient *pClient = (NimBLEClient *)arg;
  int rc = 0;
  NimBLETaskData *pTaskData =
    pClient->m_pTaskData; // save a copy in case client is deleted
  ...
  case BLE_GAP_EVENT_DISCONNECT: {
    // workaround for bug in NimBLE stack where disconnect event argument is not
    // passed correctly
    pClient = NimBLEDevice::getClientByPeerAddress(
      event->disconnect.conn.peer_ota_addr);
    if (pClient == nullptr) {
      pClient = NimBLEDevice::getClientByPeerAddress(
    ...
    if (pClient == nullptr) {
      pClient =
        NimBLEDevice::getClientByHandle(event->disconnect.conn.conn_handle);
    }
    ...
    NIMBLE_LOGD(LOG_TAG, "disconnect; reason=%d, %s", rc,
                NimBLEUtils::returnCodeToString(rc));

    pClient->m_terminateFailCount = 0;
    pClient->m_asyncSecureAttempt = 0;

    // Don't call the disconnect callback if we are waiting for a connection to
    // complete and it fails
    if (rc == (BLE_HS_ERR_HCI_BASE + BLE_ERR_CONN_ESTABLISHMENT) &&
        pClient->m_config.asyncConnect) {
    ...
    if (pClient->m_config.deleteOnDisconnect) {
      // If we are set to self delete on disconnect but we have a task waiting
      // on the connection completion we will set the flag to delete on connect
      // fail instead of deleting here to prevent segmentation faults or double
      // deleting
      if (pTaskData != nullptr &&
          rc == (BLE_HS_ERR_HCI_BASE + BLE_ERR_CONN_ESTABLISHMENT)) {
  ...
  case BLE_GAP_EVENT_CONNECT: {
    // If we aren't waiting for this connection response we should drop the
    ...
    if (rc == BLE_ERR_UNSUPP_REM_FEATURE) {
      rc = 0; // Workaround: Ignore unsupported remote feature error as it is
              // not a real error.
    }
    ...
    if (rc == 0) {
      ...
      if (pClient->m_config.exchangeMTU) {
        ...
        return 0; // return as we may have a task waiting for the MTU before
                  // releasing it.
      }
    } else {
      pClient->m_connHandle = BLE_HS_CONN_HANDLE_NONE;
  ...
  case BLE_GAP_EVENT_TERM_FAILURE: {
    ...
    NIMBLE_LOGE(LOG_TAG, "Connection termination failure; rc=%d - retrying",
                event->term_failure.status);
    if (++pClient->m_terminateFailCount > 2) {
      ble_hs_sched_reset(BLE_HS_ECONTROLLER);
    } else {
      ble_gap_terminate(event->term_failure.conn_handle,
      ...
      for (const auto &chr : svc->m_vChars) {
        if (chr->getHandle() == event->notify_rx.attr_handle) {
          ...
          if (chr->m_notifyCallback != nullptr) {
            chr->m_notifyCallback(chr, event->notify_rx.om->om_data, data_len,
  ...
  case BLE_GAP_EVENT_IDENTITY_RESOLVED: {
    NimBLEConnInfo peerInfo;
    rc =
      ble_gap_conn_find(event->identity_resolved.conn_handle, &peerInfo.m_desc);
    if (rc != 0) {
      rc = 0;
      break;
    }
  ...
  case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE: {
    ...
    pClient->m_pClientCallbacks->onPhyUpdate(pClient, event->phy_updated.tx_phy,
  ...
  case BLE_GAP_EVENT_PASSKEY_ACTION: {
    ...
    if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
      NIMBLE_LOGD(LOG_TAG, "Passkey on device's display: %" PRIu32,
                  event->passkey.params.numcmp);
      pClient->m_pClientCallbacks->onConfirmPasskey(
        peerInfo, event->passkey.params.numcmp);
 ...
 * @param [in] pClientCallbacks A pointer to a class to receive the event
 * callbacks.
 * @param [in] deleteCallbacks If true this will delete the callback class sent
 ...
 */
int NimBLEClient::getLastError() const { return m_lastErr; } // getLastError
...
bool NimBLEClientCallbacks::onConnParamsUpdateRequest(
...
void NimBLEClientCallbacks::onPhyUpdate(NimBLEClient *pClient, uint8_t txPhy,
                                        uint8_t rxPhy) {
  NIMBLE_LOGD(CB_TAG, "onPhyUpdate: default, txPhy: %d, rxPhy: %d", txPhy,
              rxPhy);
} // onPhyUpdate
#
