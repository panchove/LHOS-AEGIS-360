...
static void print_hash_key(uint8_t *hash_key) {
...
static void print_addr(ble_addr_t addr) {
...
static void getFilename(char *buffer, uint8_t *hash) {
  sprintf(buffer, "%s%02x%02x%02x%02x", GATT_CACHE_PREFIX, hash[0], hash[1],
          hash[2], hash[3]);
}

static int cacheErase(cache_handle_t handle) {
...
static int cacheWrite(cache_handle_t handle, const char *key, const void *value,
                      size_t length) {
  if (cache_fn.write) {
...
static int cacheRead(cache_handle_t handle, const char *key, void *out_value,
                     size_t *length) {
  if (cache_fn.read) {
...
static void cacheClose(ble_addr_t addr) {
  uint8_t index = 0;
...
static bool cacheOpen(ble_addr_t addr, bool to_save, uint8_t *index) {
  char fname[255] = {0};
  int status = -1;
  ...
  if ((*index = ble_gattc_cache_find_addr(addr)) != INVALID_ADDR_NUM) {
    if (cache_env->cache_addr[*index].is_open) {
      ...
      memcpy(hash_key, cache_env->cache_addr[*index].hash_key,
             sizeof(uint8_t) * 16);
      getFilename(fname, hash_key);
      if (cache_fn.open) {
        if ((status = cache_fn.open(fname, READWRITE,
                                    &cache_env->cache_addr[*index].cache_fp)) ==
            0) {
...
void ble_gattc_cacheReset(ble_addr_t *addr) {
  ...
  if ((index = ble_gattc_cache_find_addr(*addr)) != INVALID_ADDR_NUM) {
    ...
    if (cache_env->num_addr > 0) {
      ...
      uint16_t length =
        cache_env->num_addr * (sizeof(ble_addr_t) + sizeof(uint8_t) * 16);
...
static void ble_gattc_fill_nv_attr_entry(ble_uuid_any_t uuid, uint16_t s_handle,
                                         uint16_t e_handle,
                                         ble_gatt_attr_type attr_type,
                                         bool is_primary,
                                         struct ble_gatt_nv_attr *nv_attr,
                                         int index, uint8_t properties) {
  ...
  BLE_HS_LOG(
    DEBUG,
    "uuid = %x, s = %d, e = %d, is_primary = %d, attr_type = %d, prop =%d",
    uuid.u16.value, s_handle, e_handle, is_primary, attr_type, properties);
}

static void ble_gattc_fill_nv_attr(struct ble_gattc_cache_conn *peer,
                                   size_t num_attr,
                                   struct ble_gatt_nv_attr *nv_attr) {
  ...
  SLIST_FOREACH(svc, &peer->svcs, next) {
    ble_gattc_fill_nv_attr_entry(svc->svc.uuid, svc->svc.start_handle,
                                 svc->svc.end_handle, BLE_GATT_ATTR_TYPE_SRVC,
    ...
    SLIST_FOREACH(chr, &svc->chrs, next) {
      ble_gattc_fill_nv_attr_entry(chr->chr.uuid, chr->chr.val_handle,
                                   chr->chr.def_handle, BLE_GATT_ATTR_TYPE_CHAR,
                                   false, nv_attr, index, chr->chr.properties);
      index++;
      SLIST_FOREACH(dsc, &chr->dscs, next) {
        ble_gattc_fill_nv_attr_entry(dsc->dsc.uuid, dsc->dsc.handle, 0,
                                     BLE_GATT_ATTR_TYPE_CHAR_DESCR, false,
                                     nv_attr, index, 0);
        index++;
      }
    }
...
static int ble_gattc_cache_addr_save(uint8_t *out_index, ble_addr_t addr,
                                     uint8_t *hash_key) {
  int rc;
  uint8_t num = ++(cache_env->num_addr);
  uint8_t index = 0;
  ...
  if (ble_gattc_cache_find_hash(hash_key) != INVALID_ADDR_NUM) {
    ...
    if ((index = ble_gattc_cache_find_addr(addr)) != INVALID_ADDR_NUM) {
      ...
      /*
          If the bd_addr isn't in the address list, added the bd_addr to the
         last of the address list.
       */
  ...
  memcpy(cache_env->cache_addr[insert_ind].hash_key, hash_key,
         sizeof(uint8_t) * 16);
...
static int handle_compare(const void *s1, const void *s2) {
  return ((struct ble_gatt_nv_attr *)s1)->s_handle -
         ((struct ble_gatt_nv_attr *)s2)->s_handle;
}

static void ble_gatts_sort_nv_attr(struct ble_gatt_nv_attr *nv_attr,
                                   size_t num_attr) {
...
void ble_gattc_cache_save(struct ble_gattc_cache_conn *peer, size_t num_attr) {
  ...
  nv_attr = (struct ble_gatt_nv_attr *)nimble_platform_mem_malloc(
    num_attr * sizeof(ble_gatt_nv_attr));
  if (nv_attr == NULL) {
  ...
  rc = ble_gattc_cache_addr_save(&index, peer->ble_gattc_cache_conn_addr,
                                 hash_key);
  if (rc != 0) {
  ...
  BLE_HS_LOG(INFO,
             "%s() wrote hash_key on index = %d, num_attr = %d, status = %d.",
             __func__, index, num_attr, rc);

  nimble_platform_mem_free(nv_attr);
  cacheClose(peer->ble_gattc_cache_conn_addr);
}

static struct ble_gatt_nv_attr *ble_gattc_cache_load_nv_attr(uint8_t index,
                                                             int *num_attr) {
  int rc;
  size_t length = 0;
  struct ble_gatt_nv_attr *nv_attr;
  ...
  *num_attr = length / (sizeof(ble_gatt_nv_attr));
  ...
  rc = cacheRead(cache_env->cache_addr[index].cache_fp, cache_key, nv_attr,
                 &length);

  BLE_HS_LOG(INFO, "%s, rc = %d, length = %d index = %d", __func__, rc, length,
             index);
  return nv_attr;
}

static int ble_gattc_add_svc_from_cache(ble_addr_t peer_addr,
                                        struct ble_gatt_nv_attr nv_attr) {
  struct ble_gatt_svc *gatt_svc;
  int rc;

  gatt_svc = (struct ble_gatt_svc *)nimble_platform_mem_malloc(
    sizeof(struct ble_gatt_svc));
  if (gatt_svc == NULL) {
    return BLE_HS_ENOMEM;
  }
...
static int ble_gattc_add_inc_from_cache(ble_addr_t peer_addr,
                                        struct ble_gatt_nv_attr nv_attr) {
  struct ble_gatt_svc *gatt_svc;
  int rc;
...
static int ble_gattc_add_chr_from_cache(ble_addr_t peer_addr,
                                        struct ble_gatt_nv_attr nv_attr) {
  struct ble_gatt_chr *gatt_chr;
  int rc;
  gatt_chr = (struct ble_gatt_chr *)nimble_platform_mem_malloc(
    sizeof(struct ble_gatt_chr));
  if (gatt_chr == NULL) {
    return BLE_HS_ENOMEM;
  }
...
static int ble_gattc_add_dsc_from_cache(ble_addr_t peer_addr,
                                        struct ble_gatt_nv_attr nv_attr) {
  struct ble_gatt_dsc *gatt_dsc;
  int rc;
  gatt_dsc = (struct ble_gatt_dsc *)nimble_platform_mem_malloc(
    sizeof(struct ble_gatt_dsc));
  if (gatt_dsc == NULL) {
    return BLE_HS_ENOMEM;
  }
...
int ble_gattc_cache_check_hash(struct ble_gattc_cache_conn *peer,
                               struct os_mbuf *om) {
...
int ble_gattc_cache_init(void *storage_cb) {
  ...
  if (cache_fn.open) {
    if ((rc = cache_fn.open(cache_addr, READWRITE, &fp)) == 0) {
      ...
      if ((rc = cacheRead(fp, cache_key, p_buf, &length)) != 0) {
        if (rc != 0) {
          BLE_HS_LOG(
            DEBUG,
            "%s, Line = %d, storage flash get blob data fail, err_code = 0x%x",
      ...
      for (uint8_t i = 0; i < num_addr; i++) {
        memcpy(&cache_env->cache_addr[i].addr,
               p_buf + i * (sizeof(ble_addr_t) + sizeof(uint8_t) * 16),
               sizeof(ble_addr_t));
    ...
    BLE_HS_LOG(ERROR, "%s, Line = %d, storage flash open fail, err_code = %x",
               __func__, __LINE__, rc);
    nimble_platform_mem_free(p_buf);
    return rc;
  }
