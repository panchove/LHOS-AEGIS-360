...
public:
  ...
  NimBLEAddress getAddress() const {
    return NimBLEAddress(m_desc.peer_ota_addr);
  }

  /** @brief Gets the ID address of the connected peer */
  NimBLEAddress getIdAddress() const {
    return NimBLEAddress(m_desc.peer_id_addr);
  }

  /** @brief Gets the connection handle (also known as the connection id) of the
   * connected peer */
  ...
  /** @brief Gets the allowable latency for this connection (unit = number of
   * intervals) */
  uint16_t getConnLatency() const { return m_desc.conn_latency; }

  /** @brief Gets the maximum transmission unit size for this connection (in
   * bytes) */
