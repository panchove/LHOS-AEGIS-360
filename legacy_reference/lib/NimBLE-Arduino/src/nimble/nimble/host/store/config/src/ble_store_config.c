...
int ble_store_config_compare_bond_count(const void *a, const void *b) {
  const struct ble_store_value_sec *sec_a =
    (const struct ble_store_value_sec *)a;
  const struct ble_store_value_sec *sec_b =
    (const struct ble_store_value_sec *)b;

  return sec_a->bond_count - sec_b->bond_count;
}

/* This function gets the stored device records of OUR_SEC object type, arranges
 * them in order of their bond count, and then updates them with new counts so
 * they're in sequence.
 */
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
int ble_restore_our_sec_nvs(void) {
  ...
  memcpy(temp_our_secs, ble_store_config_our_secs,
         ble_store_config_num_our_secs * sizeof(struct ble_store_value_sec));
  temp_count = ble_store_config_num_our_secs;

  qsort(temp_our_secs, temp_count, sizeof(struct ble_store_value_sec),
...
/* This function gets the stored device records of PEER_SEC object type,
 * arranges them in order of their bond count, and then updates them with new
 * counts so they're in sequence.
 */
int ble_restore_peer_sec_nvs(void) {
  ...
  memcpy(temp_peer_secs, ble_store_config_peer_secs,
         ble_store_config_num_peer_secs * sizeof(struct ble_store_value_sec));
  temp_count = ble_store_config_num_peer_secs;

  qsort(temp_peer_secs, temp_count, sizeof(struct ble_store_value_sec),
        ble_store_config_compare_bond_count);
...
ble_store_config_print_value_sec(const struct ble_store_value_sec *sec) {
  if (sec->ltk_present) {
    BLE_HS_LOG(DEBUG, "ediv=%u rand=%llu authenticated=%d ltk=", sec->ediv,
               sec->rand_num, sec->authenticated);
...
ble_store_config_write_our_sec(const struct ble_store_value_sec *value_sec) {
  ...
  if (idx == -1) {
    if (ble_store_config_num_our_secs >= MYNEWT_VAL(BLE_STORE_MAX_BONDS)) {
      BLE_HS_LOG(DEBUG,
                 "error persisting our sec; too many entries "
                 "(%d)\n",
                 ble_store_config_num_our_secs);
...
static int ble_store_config_delete_obj(void *values, int value_size, int idx,
                                       int *num_values) {
  uint8_t *dst;
  uint8_t *src;
  uint8_t move_count;
...
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
static int ble_store_config_delete_sec(const struct ble_store_key_sec *key_sec,
...
ble_store_config_write_peer_sec(const struct ble_store_value_sec *value_sec) {
  ...
  if (idx == -1) {
    if (ble_store_config_num_peer_secs >= MYNEWT_VAL(BLE_STORE_MAX_BONDS)) {
      BLE_HS_LOG(DEBUG,
                 "error persisting peer sec; too many entries "
                 "(%d)\n",
                 ble_store_config_num_peer_secs);
      return BLE_HS_ESTORE_CAP;
    }
  ...
  ble_store_config_peer_secs[idx].bond_count =
...
ble_store_config_delete_cccd(const struct ble_store_key_cccd *key_cccd) {
  ...
  rc = ble_store_config_delete_obj(ble_store_config_cccds,
                                   sizeof *ble_store_config_cccds, idx,
...
static int ble_store_config_read_cccd(const struct ble_store_key_cccd *key_cccd,
...
ble_store_config_delete_ead(const struct ble_store_key_ead *key_ead) {
  ...
  rc = ble_store_config_delete_obj(ble_store_config_eads,
                                   sizeof *ble_store_config_eads, idx,
...
static int ble_store_config_read_ead(const struct ble_store_key_ead *key_ead,
...
static int ble_store_config_delete_local_irk(
  const struct ble_store_key_local_irk *key_irk) {
  int idx;
  int rc;
  ...
  rc = ble_store_config_delete_obj(ble_store_config_local_irks,
                                   sizeof *ble_store_config_local_irks, idx,
...
static int ble_store_config_write_local_irk(
  const struct ble_store_value_local_irk *value_irk) {
...
static int
ble_store_config_read_rpa_rec(const struct ble_store_key_rpa_rec *key_rpa_rec,
...
static int ble_store_config_write_rpa_rec(
  const struct ble_store_value_rpa_rec *value_rpa_rec) {
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
...
static int ble_store_config_delete_rpa_rec(
  const struct ble_store_key_rpa_rec *key_rpa_rec) {
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
  int idx;
  int rc;
  ...
  rc = ble_store_config_delete_obj(ble_store_config_rpa_recs,
                                   sizeof *ble_store_config_rpa_recs, idx,
                                   &ble_store_config_num_rpa_recs);
  if (rc != 0) {
    return rc;
  }
...
ble_store_config_delete_csfc(const struct ble_store_key_csfc *key_csfc) {
  ...
  rc = ble_store_config_delete_obj(ble_store_config_csfcs,
                                   sizeof *ble_store_config_csfcs, idx,
                                   &ble_store_config_num_csfcs);
...
static int ble_store_config_read_csfc(const struct ble_store_key_csfc *key_csfc,
