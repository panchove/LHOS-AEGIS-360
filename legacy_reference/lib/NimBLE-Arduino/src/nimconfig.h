...
/** @brief Un-comment to change the number of simultaneous connections (esp
 * controller max is 9) */
// #define CONFIG_BT_NIMBLE_MAX_CONNECTIONS 3

/** @brief Un-comment to enable storing the timestamp when an attribute value is
 * updated\n This allows for checking the last update time using getTimeStamp()
 * or getValue(time_t*)\n If disabled, the timestamp returned from these
 * functions will be 0.\n Disabling timestamps will reduce the memory used for
 * each value.\n 1 = Enabled, 0 = Disabled; Default = Disabled
 */
// #define CONFIG_NIMBLE_CPP_ATT_VALUE_TIMESTAMP_ENABLED 0

/** @brief Uncomment to set the default allocation size (bytes) for each
 * attribute if\n not specified when the constructor is called. This is also the
 * size used when a remote\n characteristic or descriptor is constructed before
 * a value is read/notified.\n Increasing this will reduce reallocations but
 * increase memory footprint.\n Default value is 20. Range: 1 : 512
...
/** @brief Un-comment to set the max number of extended advertising instances
 * (Range: 0 - 4) */
// #define CONFIG_BT_NIMBLE_MAX_EXT_ADV_INSTANCES 1

/** @brief Un-comment to set the max extended advertising data size (Range: 31 -
 * 1650) */
...
/** @brief Un-comment to change the maximum number of periodically synced
 * devices */
...
/** @brief Un-comment to set the debug log messages level from the NimBLE host
 * stack.\n Values: 0 = DEBUG, 1 = INFO, 2 = WARNING, 3 = ERROR, 4 = CRITICAL,
 * 5+ = NONE\n Uses approx. 32kB of flash memory.
 */
// #define CONFIG_BT_NIMBLE_LOG_LEVEL 5

/** @brief Un-comment to set the debug log messages level from the NimBLE CPP
...
/** @brief Un-comment to see NimBLE host return codes as text debug log
 * messages. Uses approx. 7kB of flash memory.
 */
...
/** @brief Un-comment to see advertisement types as text while scanning in debug
 * log messages. Uses approx. 250 bytes of flash memory.
 */
...
/** @brief Un-comment to change the maximum number of CCCD subscriptions to
 * store */
 ...
 * responses MBUFs are allocated out of msys_1 pool. This may need to be
 * increased if\n you are sending large blocks of data with a low MTU. E.g: 512
 * bytes with 23 MTU will fail.
 */
 ...
 * @details This could save approximately 8k of flash if already using mbedtls
...
/* Must have max instances and data length set if extended advertising is
 * enabled */
#if CONFIG_BT_NIMBLE_EXT_ADV
...
void nimble_cpp_assert(const char *file, unsigned line)
  __attribute((weak, noreturn));
#define NIMBLE_ATT_VAL_FILE                                                    \
