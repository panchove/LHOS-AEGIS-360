...
#if defined(ARDUINO_ARCH_SAMD) && defined(PIN_SPI_MISO) &&                     \
  defined(PIN_SPI_MOSI) && defined(PIN_SPI_SCK) && (PIN_SPI_MISO == 10) &&     \
  (PIN_SPI_MOSI == 8) && (PIN_SPI_SCK == 9)
...
public:
  ...
  void setPins(int cs = MCP2515_DEFAULT_CS_PIN,
               int irq = MCP2515_DEFAULT_INT_PIN);
