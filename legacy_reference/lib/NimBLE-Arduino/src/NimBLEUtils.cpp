...
#endif
constexpr uint32_t TASK_BLOCK_BIT =
  (1 << CONFIG_NIMBLE_CPP_FREERTOS_TASK_BLOCK_BIT);
#endif
...
#else
  return ble_npl_sem_pend(static_cast<ble_npl_sem *>(taskData.m_pHandle),
                          ticks) == BLE_NPL_OK;
#endif
} // taskWait
...
#if defined INC_FREERTOS_H
    xTaskNotify(static_cast<TaskHandle_t>(taskData.m_pHandle), TASK_BLOCK_BIT,
                eSetBits);
#else
