...
#define USECS_PER_TICK                                                         \
  ((1000000 + MYNEWT_VAL(OS_CPUTIME_FREQ) - 1) / MYNEWT_VAL(OS_CPUTIME_FREQ))
...
static inline uint32_t ble_ll_tmr_get(void) { return os_cputime_get32(); }

static inline uint32_t ble_ll_tmr_t2u(uint32_t ticks) {
...
static inline uint32_t ble_ll_tmr_u2t(uint32_t usecs) {
#if MYNEWT_VAL(OS_CPUTIME_FREQ) == 31250
  return usecs / 32;
#endif
...
static inline uint32_t ble_ll_tmr_u2t_up(uint32_t usecs) {
  return ble_ll_tmr_u2t(usecs + (USECS_PER_TICK - 1));
}

static inline uint32_t ble_ll_tmr_u2t_r(uint32_t usecs, uint8_t *rem_us) {
  uint32_t ticks;
...
static inline void ble_ll_tmr_add(uint32_t *ticks, uint8_t *rem_us,
                                  uint32_t usecs) {
  uint32_t t_ticks;
  uint8_t t_rem_us;
...
static inline void ble_ll_tmr_add_u(uint32_t *ticks, uint8_t *rem_us,
                                    uint8_t usecs) {
...
static inline void ble_ll_tmr_sub(uint32_t *ticks, uint8_t *rem_us,
                                  uint32_t usecs) {
  uint32_t t_ticks;
  uint8_t t_rem_us;
...
static inline void ble_ll_tmr_init(struct ble_ll_tmr *tmr, ble_ll_tmr_cb *cb,
                                   void *arg) {
  os_cputime_timer_init(&tmr->t, cb, arg);
}

static inline void ble_ll_tmr_start(struct ble_ll_tmr *tmr, uint32_t tgt) {
  os_cputime_timer_start(&tmr->t, tgt);
}

static inline void ble_ll_tmr_stop(struct ble_ll_tmr *tmr) {
