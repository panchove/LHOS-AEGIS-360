 ...
 */
uint16_t NimBLEBeacon::getMajor() { return m_beaconData.major; } // getMajor
 ...
 */
uint16_t NimBLEBeacon::getMinor() { return m_beaconData.minor; } // getMinor
...
void NimBLEBeacon::setData(const uint8_t *data, uint8_t length) {
  if (length != sizeof(BeaconData)) {
    NIMBLE_LOGE(LOG_TAG, "Data length must be %d bytes, sent: %d",
                sizeof(BeaconData), length);
    return;
  }
