...
static void get_nvs_key_string(int obj_type, int index, char *key_string) {
 ...
 *                       -1 if mismatch
 */
static int get_nvs_matching_index(void *nvs_val, void *db_list, int db_num,
                                  size_t item_size) {
...
static int get_nvs_peer_record(char *key_string,
  ...
  err = nvs_get_blob(nimble_handle, key_string, p_dev_rec, &required_size);
...
static int get_nvs_db_value(int obj_type, char *key_string,
  ...
  if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
    err = nvs_get_blob(nimble_handle, key_string, &val->cccd, &required_size);
  } else if (obj_type == BLE_STORE_OBJ_TYPE_CSFC) {
    err = nvs_get_blob(nimble_handle, key_string, &val->csfc, &required_size);
#if MYNEWT_VAL(ENC_ADV_DATA)
  } else if (obj_type == BLE_STORE_OBJ_TYPE_ENC_ADV_DATA) {
    err = nvs_get_blob(nimble_handle, key_string, &val->ead, &required_size);
#endif
  } else if (obj_type == BLE_STORE_OBJ_TYPE_LOCAL_IRK) {
    err =
      nvs_get_blob(nimble_handle, key_string, &val->local_irk, &required_size);

  } else if (obj_type == BLE_STORE_OBJ_TYPE_PEER_ADDR) {
    err =
      nvs_get_blob(nimble_handle, key_string, &val->rpa_rec, &required_size);

  } else {
    err = nvs_get_blob(nimble_handle, key_string, &val->sec, &required_size);
  }
 ...
 */
static int get_nvs_db_attribute(int obj_type, bool empty, void *value,
                                int num_value) {
  ...
  for (i = 1; i <= max_limit; i++) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
      if (empty) {
        ESP_LOGD(LOG_TAG, "Empty NVS index found = %d for obj_type = %d", i,
                 obj_type);
        return i;
      }
    } else if (err == ESP_OK) {
      ...
      if (value) {
#if MYNEWT_VAL(BLE_HOST_BASED_PRIVACY)
        if (obj_type == BLE_STORE_OBJ_TYPE_PEER_DEV_REC) {
          err = get_nvs_matching_index(
            &p_dev_rec.peer_sec,
            &((struct ble_hs_dev_records *)value)->peer_sec, num_value,
...
#endif
        {
          if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
            ...
            err =
              get_nvs_matching_index(&cur.local_irk, value, num_value,
                                     sizeof(struct ble_store_value_local_irk));

          } else if (obj_type == BLE_STORE_OBJ_TYPE_PEER_ADDR) {
            err =
              get_nvs_matching_index(&cur.rpa_rec, value, num_value,
 ...
 */
static int ble_nvs_delete_value(int obj_type, int8_t index) {
  esp_err_t err;
  nvs_handle_t nimble_handle;
  char key_string[NIMBLE_NVS_STR_NAME_MAX_LEN];
...
static int ble_nvs_write_key_value(char *key, const void *value,
 ...
 */
static int ble_store_nvs_write(int obj_type, const union ble_store_value *val) {
  char key_string[NIMBLE_NVS_STR_NAME_MAX_LEN];
  ...
  if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
    return ble_nvs_write_key_value(key_string, &val->cccd,
                                   sizeof(struct ble_store_value_cccd));
  } else if (obj_type == BLE_STORE_OBJ_TYPE_CSFC) {
    return ble_nvs_write_key_value(key_string, &val->csfc,
                                   sizeof(struct ble_store_value_csfc));
#if MYNEWT_VAL(ENC_ADV_DATA)
  } else if (obj_type == BLE_STORE_OBJ_TYPE_ENC_ADV_DATA) {
    return ble_nvs_write_key_value(key_string, &val->ead,
                                   sizeof(struct ble_store_value_ead));
#endif
  } else if (obj_type == BLE_STORE_OBJ_TYPE_LOCAL_IRK) {
    return ble_nvs_write_key_value(key_string, &val->local_irk,
                                   sizeof(struct ble_store_value_local_irk));

  } else if (obj_type == BLE_STORE_OBJ_TYPE_PEER_ADDR) {
    return ble_nvs_write_key_value(key_string, &val->rpa_rec,
                                   sizeof(struct ble_store_value_rpa_rec));

  } else {
    return ble_nvs_write_key_value(key_string, &val->sec,
                                   sizeof(struct ble_store_value_sec));
  }
}
...
/* If Host based privacy is enabled */
static int
ble_store_nvs_peer_records(int obj_type,
  ...
  return ble_nvs_write_key_value(key_string, p_dev_rec,
                                 sizeof(struct ble_hs_dev_records));
}
#endif

static int populate_db_from_nvs(int obj_type, void *dst, int *db_num) {
  ...
  for (i = 1; i <= get_nvs_max_obj_value(obj_type); i++) {
...
#endif
    {
      if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
        ...
        ESP_LOGD(LOG_TAG, "Local IRK in RAM is filled up from NVS index = %d",
                 i);
        memcpy(db_item, &cur.local_irk,
               sizeof(struct ble_store_value_local_irk));
 ...
 */
static int ble_nvs_restore_sec_keys(void) {
  esp_err_t err;
  int flag = 0;
  ...
  err =
    populate_db_from_nvs(BLE_STORE_OBJ_TYPE_OUR_SEC, ble_store_config_our_secs,
                         &ble_store_config_num_our_secs);
  if (err != ESP_OK) {
    ESP_LOGE(LOG_TAG, "NVS operation failed for 'our sec'");
    return err;
  }
  ESP_LOGD(LOG_TAG, "ble_store_config_our_secs restored %d bonds",
           ble_store_config_num_our_secs);

  err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_PEER_SEC,
  ...
  for (int i = 0; i < MYNEWT_VAL(BLE_STORE_MAX_BONDS) - 1; i++) {
    if ((ble_store_config_our_secs[i].bond_count >
         ble_store_config_our_secs[i + 1].bond_count) ||
        (ble_store_config_peer_secs[i].bond_count >
         ble_store_config_peer_secs[i + 1].bond_count)) {
      flag = 1;
      break;
    }
  }
  ...
  ble_store_config_our_bond_count =
    ble_store_config_our_secs[ble_store_config_num_our_secs - 1].bond_count;
  ble_store_config_peer_bond_count =
    ble_store_config_peer_secs[ble_store_config_num_peer_secs - 1].bond_count;
...
#if MYNEWT_VAL(ENC_ADV_DATA)
  err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_ENC_ADV_DATA,
...
#endif
  err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_LOCAL_IRK,
  ...
  err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_PEER_ADDR,
...
#if MYNEWT_VAL(BLE_HOST_BASED_PRIVACY)
static int ble_nvs_restore_peer_records(void) {
  esp_err_t err;
  ...
  ESP_LOGD(LOG_TAG, "peer_dev_rec restored %d records",
           ble_store_num_peer_dev_rec);
...
int ble_store_config_persist_cccds(void) {
  ...
  if (nvs_count < ble_store_config_num_cccds) {
    ...
    nvs_idx =
      get_nvs_db_attribute(BLE_STORE_OBJ_TYPE_CCCD, 0, ble_store_config_cccds,
                           ble_store_config_num_cccds);
    if (nvs_idx == -1) {
...
int ble_store_config_persist_csfcs(void) {
  ...
  if (nvs_count < ble_store_config_num_csfcs) {
    ...
    /* NVS db count more than RAM count, delete operation */
    nvs_idx =
      get_nvs_db_attribute(BLE_STORE_OBJ_TYPE_CSFC, 0, ble_store_config_csfcs,
                           ble_store_config_num_csfcs);
    if (nvs_idx == -1) {
...
int ble_store_config_persist_eads(void) {
  ...
  if (nvs_count < ble_store_config_num_eads) {
    ...
    /* NVS db count more than RAM count, delete operation */
    nvs_idx =
      get_nvs_db_attribute(BLE_STORE_OBJ_TYPE_ENC_ADV_DATA, 0,
                           ble_store_config_eads, ble_store_config_num_eads);
    if (nvs_idx == -1) {
...
int ble_store_config_persist_local_irk(void) {
  ...
  if (nvs_count < ble_store_config_num_local_irks) {
    ...
    val.local_irk =
      ble_store_config_local_irks[ble_store_config_num_local_irks - 1];
...
int ble_store_persist_peer_records(void) {
  ...
  if (nvs_count < ble_store_num_peer_dev_rec) {
    ...
    return ble_store_nvs_peer_records(BLE_STORE_OBJ_TYPE_PEER_DEV_REC,
                                      &peer_rec);
