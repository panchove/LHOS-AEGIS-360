...
static void ble_store_config_serialize_arr(const void *arr, int obj_sz,
...
static int ble_store_config_deserialize_arr(const char *enc, void *out_arr,
...
static int ble_store_config_persist_sec_set(
  uint32_t setting_name, const struct ble_store_value_sec *secs, int num_secs) {
  char buf[BLE_STORE_CONFIG_SEC_SET_ENCODE_SZ];
  int rc;

  ble_store_config_serialize_arr(secs, sizeof *secs, num_secs, buf, sizeof buf);
...
int ble_store_config_persist_our_secs(void) {
  ...
  rc = ble_store_config_persist_sec_set(
    OUR_SEC_LABEL, ble_store_config_our_secs, ble_store_config_num_our_secs);
  if (rc != 0) {
    return rc;
  }
...
int ble_store_config_persist_peer_secs(void) {
  ...
  rc = ble_store_config_persist_sec_set(
    PEER_SEC_LABEL, ble_store_config_peer_secs, ble_store_config_num_peer_secs);
  if (rc != 0) {
    return rc;
  }
...
int ble_store_config_persist_cccds(void) {
  ...
  ble_store_config_serialize_arr(ble_store_config_cccds,
                                 ...
                                 ble_store_config_num_cccds, buf, sizeof buf);
...
int ble_store_config_persist_csfcs(void) {
  ...
  ble_store_config_serialize_arr(ble_store_config_csfcs,
                                 ...
                                 ble_store_config_num_csfcs, buf, sizeof buf);
...
int ble_store_config_persist_eads(void) {
  ...
  ble_store_config_serialize_arr(ble_store_config_eads,
                                 ...
                                 ble_store_config_num_eads, buf, sizeof buf);
...
int ble_store_config_persist_rpa_recs(void) {
  ...
  ble_store_config_serialize_arr(
    ble_store_config_rpa_recs, sizeof *ble_store_config_rpa_recs,
    ble_store_config_num_rpa_recs, buf, sizeof buf);
...
int ble_store_config_persist_local_irk(void) {
  ...
  ble_store_config_serialize_arr(
    ble_store_config_local_irks, sizeof *ble_store_config_local_irks,
    ble_store_config_num_local_irks, buf, sizeof buf);
...
void ble_store_config_conf_init(void) {
  ...
  if (rc == 0) {
    rc = ble_store_config_deserialize_arr(
      (char *)val_addr, ble_store_config_our_secs,
      sizeof *ble_store_config_our_secs, &ble_store_config_num_our_secs);
    if (rc != 0) {
    ...
    if (rc == 0) {
      rc = ble_store_config_deserialize_arr(
        (char *)val_addr, ble_store_config_peer_secs,
        sizeof *ble_store_config_peer_secs, &ble_store_config_num_peer_secs);
      if (rc != 0) {
    ...
    if (rc == 0) {
      rc = ble_store_config_deserialize_arr(
        (char *)val_addr, ble_store_config_cccds,
        sizeof *ble_store_config_cccds, &ble_store_config_num_cccds);
      if (rc != 0) {
    ...
    if (rc == 0) {
      rc = ble_store_config_deserialize_arr(
        (char *)val_addr, ble_store_config_csfcs,
        sizeof *ble_store_config_csfcs, &ble_store_config_num_csfcs);
      if (rc != 0) {
    ...
    if (rc == 0) {
      rc = ble_store_config_deserialize_arr(
        (char *)val_addr, ble_store_config_eads, sizeof *ble_store_config_eads,
    ...
    if (rc == 0) {
      rc = ble_store_config_deserialize_arr(
        (char *)val_addr, ble_store_config_rpa_recs,
        sizeof *ble_store_config_rpa_recs, &ble_store_config_num_rpa_recs);
      if (rc != 0) {
  ...
  if (rc == 0) {
    rc = ble_store_config_deserialize_arr(
      (char *)val_addr, ble_store_config_local_irks,
      sizeof *ble_store_config_local_irks, &ble_store_config_num_local_irks);
    if (rc != 0) {
