...
static ble_gattc_resume_fn *const ble_gattc_resume_dispatch[BLE_GATT_OP_CNT] = {
...
static os_membuf_t ble_gattc_proc_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_GATT_MAX_PROCS), sizeof(struct ble_gattc_proc))];
...
static void ble_gattc_log_proc_init(const char *name) {
  BLE_HS_LOG(INFO, "GATT procedure initiated: %s", name);
}

static void ble_gattc_log_uuid(const ble_uuid_t *uuid) {
...
static void ble_gattc_log_disc_svc_uuid(struct ble_gattc_proc *proc) {
  ble_gattc_log_proc_init("discover service by uuid; uuid=");
  ble_gattc_log_uuid(&proc->disc_svc_uuid.service_uuid.u);
  BLE_HS_LOG(INFO, "\n");
}

static void ble_gattc_log_find_inc_svcs(struct ble_gattc_proc *proc) {
...
static void ble_gattc_log_disc_all_chrs(struct ble_gattc_proc *proc) {
...
static void ble_gattc_log_disc_chr_uuid(struct ble_gattc_proc *proc) {
...
static void ble_gattc_log_disc_all_dscs(struct ble_gattc_proc *proc) {
...
static void ble_gattc_log_read(uint16_t att_handle) {
  ble_gattc_log_proc_init("read; ");
  BLE_HS_LOG(INFO, "att_handle=%d\n", att_handle);
}

static void ble_gattc_log_read_uuid(uint16_t start_handle, uint16_t end_handle,
                                    const ble_uuid_t *uuid) {
  ble_gattc_log_proc_init("read by uuid; ");
  BLE_HS_LOG(INFO, "start_handle=%d end_handle=%d uuid=", start_handle,
             end_handle);
  ble_gattc_log_uuid(uuid);
  BLE_HS_LOG(INFO, "\n");
}

static void ble_gattc_log_read_long(struct ble_gattc_proc *proc) {
  ble_gattc_log_proc_init("read long; ");
  BLE_HS_LOG(INFO, "att_handle=%d\n", proc->read_long.handle);
}

static void ble_gattc_log_read_mult(const uint16_t *handles,
...
static void ble_gattc_log_write(uint16_t att_handle, uint16_t len,
                                int expecting_rsp) {
  const char *name;
...
static void ble_gattc_log_signed_write(uint16_t att_handle, uint16_t len) {
  ble_gattc_log_proc_init("signed write; ");
  BLE_HS_LOG(INFO, "att_handle=%d len=%d\n", att_handle, len);
}

static void ble_gattc_log_write_long(struct ble_gattc_proc *proc) {
  ble_gattc_log_proc_init("write long; ");
  BLE_HS_LOG(INFO, "att_handle=%d len=%d\n", proc->write_long.attr.handle,
             OS_MBUF_PKTLEN(proc->write_long.attr.om));
}

static void ble_gattc_log_write_reliable(struct ble_gattc_proc *proc) {
  int i;
...
static void ble_gattc_log_notify(uint16_t att_handle) {
  ble_gattc_log_proc_init("notify; ");
  BLE_HS_LOG(INFO, "att_handle=%d\n", att_handle);
}

static void ble_gattc_log_multi_notify(struct ble_gatt_notif *tuples,
                                       uint16_t num) {
...
static void ble_gattc_log_indicate(uint16_t att_handle) {
...
static const void *ble_gattc_rx_entry_find(uint8_t op, const void *rx_entries,
                                           int num_entries) {
  struct gen_entry {
    uint8_t op;
    void (*cb)(void);
 ...
 */
static struct ble_gattc_proc *ble_gattc_proc_alloc(void) {
...
static void ble_gattc_proc_prepare(struct ble_gattc_proc *proc,
 ...
 */
static void ble_gattc_proc_free(struct ble_gattc_proc *proc) {
  int rc;
  int i;
...
static void ble_gattc_proc_insert(struct ble_gattc_proc *proc) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static void ble_gattc_proc_set_exp_timer(struct ble_gattc_proc *proc) {
  proc->exp_os_ticks =
    ble_npl_time_get() +
    ble_npl_time_ms_to_ticks32(BLE_GATTC_UNRESPONSIVE_TIMEOUT_MS);
}

static void ble_gattc_proc_set_resume_timer(struct ble_gattc_proc *proc) {
...
static void ble_gattc_process_status(struct ble_gattc_proc *proc, int status) {
  switch (status) {
  case 0:
 ...
 */
static int ble_gattc_process_resume_status(struct ble_gattc_proc *proc,
                                           int status) {
  switch (status) {
  case 0:
    return 0;
...
static ble_gattc_tmo_fn *ble_gattc_tmo_dispatch_get(uint8_t op) {
  BLE_HS_DBG_ASSERT(op < BLE_GATT_OP_CNT);
 ...
 */
static int ble_gattc_proc_matches_conn_op(struct ble_gattc_proc *proc,
                                          void *arg) {
...
static int ble_gattc_proc_matches_conn_cid_op(struct ble_gattc_proc *proc,
                                              void *arg) {
  const struct ble_gattc_criteria_conn_op *criteria;
...
static int ble_gattc_proc_matches_expired(struct ble_gattc_proc *proc,
                                          void *arg) {
...
static int ble_gattc_proc_matches_conn_rx_entry(struct ble_gattc_proc *proc,
                                                void *arg) {
...
static void ble_gattc_extract(ble_gattc_match_fn *cb, void *arg, int max_procs,
...
static struct ble_gattc_proc *ble_gattc_extract_one(ble_gattc_match_fn *cb,
                                                    void *arg) {
  struct ble_gattc_proc_list dst_list;
...
static void ble_gattc_extract_by_conn_op(uint16_t conn_handle, uint8_t op,
                                         int max_procs,
                                         struct ble_gattc_proc_list *dst_list) {
  ...
  ble_gattc_extract(ble_gattc_proc_matches_conn_op, &criteria, max_procs,
                    dst_list);
}
...
ble_gattc_extract_by_conn_cid_op(uint16_t conn_handle, uint16_t psm, uint8_t op,
  ...
  ble_gattc_extract(ble_gattc_proc_matches_conn_cid_op, &criteria, max_procs,
                    dst_list);
}

static struct ble_gattc_proc *
ble_gattc_extract_first_by_conn_cid_op(uint16_t conn_handle, uint16_t cid,
                                       uint8_t op) {
  struct ble_gattc_proc_list dst_list;
...
static int ble_gattc_proc_matches_stalled(struct ble_gattc_proc *proc,
                                          void *unused) {
  return proc->flags & BLE_GATTC_PROC_F_STALLED;
}

static void ble_gattc_extract_stalled(struct ble_gattc_proc_list *dst_list) {
 ...
 *                                  occurs.
 */
static int32_t ble_gattc_extract_expired(struct ble_gattc_proc_list *dst_list) {
  struct ble_gattc_criteria_exp criteria;
...
ble_gattc_extract_with_rx_entry(uint16_t conn_handle, uint16_t cid,
  ...
  proc = ble_gattc_extract_one(ble_gattc_proc_matches_conn_rx_entry, &criteria);
 ...
 *                                  null on failure.
 */
#define BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, cid, rx_entries,            \
                                      out_rx_entry)                            \
  ble_gattc_extract_with_rx_entry((conn_handle), (cid), (rx_entries),          \
...
static void ble_gattc_resume_procs(void) {
...
static int32_t ble_gattc_ticks_until_resume(void) {
  ble_npl_time_t now;
  int32_t diff;
...
static void ble_gattc_proc_timeout(struct ble_gattc_proc *proc) {
  ble_gattc_tmo_fn *cb;
 ...
 *                                  is no callback).
 */
static int ble_gattc_mtu_cb(struct ble_gattc_proc *proc, int status,
  ...
  if (proc->mtu.cb == NULL) {
    ...
    rc = proc->mtu.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
                      mtu, proc->mtu.cb_arg);
  }
...
static void ble_gattc_mtu_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_mtu_tx(struct ble_gattc_proc *proc) {
...
int ble_gattc_exchange_mtu(uint16_t conn_handle, ble_gatt_mtu_fn *cb,
                           void *cb_arg) {
  struct ble_gattc_proc *proc;
  int rc;
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_svcs_cb(struct ble_gattc_proc *proc,
  ...
  if (proc->disc_all_svcs.cb == NULL) {
    ...
    rc = proc->disc_all_svcs.cb(proc->conn_handle,
                                ble_gattc_error(status, att_handle), service,
                                proc->disc_all_svcs.cb_arg);
  }

  return rc;
}

static void ble_gattc_disc_all_svcs_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_all_svcs_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static void ble_gattc_disc_all_svcs_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
ble_gattc_disc_all_svcs_rx_adata(struct ble_gattc_proc *proc,
  ...
  case 16:
    rc =
      ble_uuid_init_from_att_buf(&service.uuid, adata->value, adata->value_len);
    if (rc != 0) {
      rc = BLE_HS_EBADDATA;
      goto done;
    }
 ...
 * fully processed.
 */
static int ble_gattc_disc_all_svcs_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
...
int ble_gattc_disc_all_svcs(uint16_t conn_handle, ble_gatt_disc_svc_fn *cb,
                            void *cb_arg) {
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_svc_uuid_cb(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
                                      struct ble_gatt_svc *service) {
  int rc;
  ...
  if (proc->disc_svc_uuid.cb == NULL) {
    ...
    rc = proc->disc_svc_uuid.cb(proc->conn_handle,
                                ble_gattc_error(status, att_handle), service,
                                proc->disc_svc_uuid.cb_arg);
  }

  return rc;
}

static void ble_gattc_disc_svc_uuid_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_svc_uuid_tx(struct ble_gattc_proc *proc) {
  ...
  rc = ble_att_clt_tx_find_type_value(
    proc->conn_handle, proc->cid, proc->disc_svc_uuid.prev_handle + 1, 0xffff,
    BLE_ATT_UUID_PRIMARY_SERVICE, val,
...
static int ble_gattc_disc_svc_uuid_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static void ble_gattc_disc_svc_uuid_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_svc_uuid_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
 ...
 *                                  is no callback).
 */
static int ble_gattc_find_inc_svcs_cb(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
                                      struct ble_gatt_svc *service) {
  int rc;
  ...
  if (proc->find_inc_svcs.cb == NULL) {
    ...
    rc = proc->find_inc_svcs.cb(proc->conn_handle,
                                ble_gattc_error(status, att_handle), service,
                                proc->find_inc_svcs.cb_arg);
  }

  return rc;
}

static void ble_gattc_find_inc_svcs_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_find_inc_svcs_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static void ble_gattc_find_inc_svcs_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 * proc.
 */
static int ble_gattc_find_inc_svcs_rx_read_rsp(struct ble_gattc_proc *proc,
                                               int status,
                                               struct os_mbuf **om) {
  struct ble_gatt_svc service;
  int rc;
 ...
 */
static int ble_gattc_find_inc_svcs_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
...
int ble_gattc_find_inc_svcs(uint16_t conn_handle, uint16_t start_handle,
                            uint16_t end_handle, ble_gatt_disc_svc_fn *cb,
                            void *cb_arg) {
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_chrs_cb(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
...
static void ble_gattc_disc_all_chrs_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_all_chrs_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static void ble_gattc_disc_all_chrs_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_all_chrs_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
...
#if MYNEWT_VAL(BLE_GATT_CACHING)
  rc = ble_gattc_cache_conn_search_all_chrs(conn_handle, start_handle,
                                            end_handle, cb, cb_arg);
  if (rc == 0) {
    return rc;
  }
#endif
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_chr_uuid_cb(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
                                      struct ble_gatt_chr *chr) {
  int rc;
...
static void ble_gattc_disc_chr_uuid_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_chr_uuid_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 * discover-characteristic-by-uuid proc.
 */
static void ble_gattc_disc_chr_uuid_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_chr_uuid_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
...
#if MYNEWT_VAL(BLE_GATT_CACHING)
  rc = ble_gattc_cache_conn_search_chrs_by_uuid(conn_handle, start_handle,
                                                end_handle, uuid, cb, cb_arg);
  if (rc == 0) {
    return rc;
  }
#endif
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_dscs_cb(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
  ...
  if (proc->disc_all_dscs.cb == NULL) {
    ...
    rc = proc->disc_all_dscs.cb(
      proc->conn_handle, ble_gattc_error(status, att_handle),
      proc->disc_all_dscs.chr_val_handle, dsc, proc->disc_all_dscs.cb_arg);
  }

  return rc;
}

static void ble_gattc_disc_all_dscs_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_all_dscs_tx(struct ble_gattc_proc *proc) {
  int rc;
...
static int ble_gattc_disc_all_dscs_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static void ble_gattc_disc_all_dscs_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_all_dscs_rx_complete(struct ble_gattc_proc *proc,
                                               int status) {
  int rc;
...
int ble_gattc_disc_all_dscs(uint16_t conn_handle, uint16_t start_handle,
                            uint16_t end_handle, ble_gatt_dsc_fn *cb,
                            void *cb_arg) {
...
#if MYNEWT_VAL(BLE_GATT_CACHING)
  rc = ble_gattc_cache_conn_search_all_dscs(conn_handle, start_handle,
                                            end_handle, cb, cb_arg);
  if (rc == 0) {
    return rc;
  }
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_cb(struct ble_gattc_proc *proc, int status,
  ...
  if (proc->read.cb == NULL) {
    ...
    rc = proc->read.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
                       attr, proc->read.cb_arg);
  }

  return rc;
}

static void ble_gattc_read_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 * read-characteristic-value proc.
 */
static int ble_gattc_read_rx_read_rsp(struct ble_gattc_proc *proc, int status,
                                      struct os_mbuf **om) {
  struct ble_gatt_attr attr;
...
static int ble_gattc_read_tx(struct ble_gattc_proc *proc) {
  int rc;
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_uuid_cb(struct ble_gattc_proc *proc, int status,
                                  uint16_t att_handle,
                                  struct ble_gatt_attr *attr) {
  int rc;
  ...
  if (proc->read_uuid.cb == NULL) {
    ...
    rc =
      proc->read_uuid.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
                         attr, proc->read_uuid.cb_arg);
  }

  return rc;
}

static void ble_gattc_read_uuid_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_read_uuid_rx_adata(struct ble_gattc_proc *proc,
                                        struct ble_att_read_type_adata *adata) {
  struct ble_gatt_attr attr;
  int rc;
 ...
 */
static int ble_gattc_read_uuid_rx_complete(struct ble_gattc_proc *proc,
                                           int status) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_read_uuid_tx(struct ble_gattc_proc *proc) {
  return ble_att_clt_tx_read_type(
    proc->conn_handle, proc->cid, proc->read_uuid.start_handle,
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_long_cb(struct ble_gattc_proc *proc, int status,
                                  uint16_t att_handle,
                                  struct ble_gatt_attr *attr) {
  int rc;
  ...
  if (proc->read_long.cb == NULL) {
    ...
    rc =
      proc->read_long.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
                         attr, proc->read_long.cb_arg);
  }

  return rc;
}

static void ble_gattc_read_long_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_read_long_tx(struct ble_gattc_proc *proc) {
  ...
  if (proc->read_long.offset == 0) {
    rc =
      ble_att_clt_tx_read(proc->conn_handle, proc->cid, proc->read_long.handle);
    if (rc != 0) {
      return rc;
    }
  } else {
    rc =
      ble_att_clt_tx_read_blob(proc->conn_handle, proc->cid,
                               proc->read_long.handle, proc->read_long.offset);
    if (rc != 0) {
      return rc;
    }
  }
...
static int ble_gattc_read_long_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static int ble_gattc_read_long_rx_read_rsp(struct ble_gattc_proc *proc,
                                           int status, struct os_mbuf **om) {
  struct ble_gatt_attr attr;
  uint16_t data_len;
  uint16_t mtu;
  int rc;
...
static int ble_gattc_read_mult_cb_var(struct ble_gattc_proc *proc, int status,
                                      uint16_t att_handle,
                                      struct os_mbuf **om) {
  ...
  proc->read_mult.cb_mult(proc->conn_handle,
                          ble_gattc_error(status, att_handle), &attr[0], i,
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_mult_cb(struct ble_gattc_proc *proc, int status,
  ...
  if (proc->read_mult.cb == NULL) {
    ...
    rc =
      proc->read_mult.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
                         &attr, proc->read_mult.cb_arg);
  }
...
static void ble_gattc_read_mult_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static void ble_gattc_read_mult_var_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static void ble_gattc_read_mult_var_err(struct ble_gattc_proc *proc, int status,
                                        uint16_t att_handle) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
  ble_gattc_read_mult_cb_var(proc, status, att_handle, NULL);
}

static int ble_gattc_read_mult_tx(struct ble_gattc_proc *proc) {
  int rc;

  rc = ble_att_clt_tx_read_mult(
    proc->conn_handle, proc->cid, proc->read_mult.handles,
...
static int ble_gattc_read_mult_internal(uint16_t conn_handle,
  ...
  ble_gattc_proc_prepare(proc, conn_handle,
                         variable ? BLE_GATT_OP_READ_MULT_VAR
...
int ble_gattc_read_mult(uint16_t conn_handle, const uint16_t *handles,
                        ...
                        void *cb_arg) {
  return ble_gattc_read_mult_internal(conn_handle, handles, num_handles, false,
                                      cb, NULL, cb_arg);
}
...
int ble_gattc_read_mult_var(uint16_t conn_handle, const uint16_t *handles,
  ...
  return ble_gattc_read_mult_internal(conn_handle, handles, num_handles, true,
                                      NULL, cb, cb_arg);
#else
  return BLE_HS_ENOTSUP;
#endif
...
int ble_gattc_signed_write(uint16_t conn_handle, uint16_t attr_handle,
  ...
  rc =
    ble_att_clt_tx_signed_write_cmd(conn_handle, BLE_L2CAP_CID_ATT, attr_handle,
 ...
 *                                  is no callback).
 */
static int ble_gattc_write_cb(struct ble_gattc_proc *proc, int status,
                              uint16_t att_handle) {
  struct ble_gatt_attr attr;
  int rc;
  ...
  if (proc->write.cb == NULL) {
    ...
    rc = proc->write.cb(proc->conn_handle, ble_gattc_error(status, att_handle),
...
static void ble_gattc_write_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 *                                  is no callback).
 */
static int ble_gattc_write_long_cb(struct ble_gattc_proc *proc, int status,
                                   uint16_t att_handle) {
  int rc;
  ...
  if (proc->write_long.cb == NULL) {
    ...
    rc = proc->write_long.cb(proc->conn_handle,
                             ...
                             &proc->write_long.attr, proc->write_long.cb_arg);
  }

  return rc;
}

static void ble_gattc_write_long_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_write_long_tx(struct ble_gattc_proc *proc) {
  struct os_mbuf *om;
  int write_len;
  int max_sz;
  int rc;
  ...
  max_sz = ble_att_mtu_by_cid(proc->conn_handle, proc->cid) -
  ...
  write_len = min(max_sz, OS_MBUF_PKTLEN(proc->write_long.attr.om) -
  ...
  rc =
    os_mbuf_appendfrom(om, proc->write_long.attr.om,
                       proc->write_long.attr.offset, proc->write_long.length);
  if (rc != 0) {
    rc = BLE_HS_ENOMEM;
    goto done;
  }
...
static int ble_gattc_write_long_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
...
static void ble_gattc_write_long_err(struct ble_gattc_proc *proc, int status,
  ...
  if (proc->write_long.attr.offset > 0 &&
      proc->write_long.attr.offset < OS_MBUF_PKTLEN(proc->write_long.attr.om)) {
 ...
 */
static int ble_gattc_write_long_rx_prep(struct ble_gattc_proc *proc, int status,
                                        uint16_t handle, uint16_t offset,
                                        struct os_mbuf **rxom) {
  struct os_mbuf *om;
  int rc;
  ...
  if (offset + OS_MBUF_PKTLEN(om) > OS_MBUF_PKTLEN(proc->write_long.attr.om)) {
    rc = BLE_HS_EBADDATA;
    goto err;
  }
  ...
  if (os_mbuf_cmpm(om, 0, proc->write_long.attr.om, offset,
 ...
 */
static int ble_gattc_write_long_rx_exec(struct ble_gattc_proc *proc,
                                        int status) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 *                                  is no callback).
 */
static int ble_gattc_write_reliable_cb(struct ble_gattc_proc *proc, int status,
                                       uint16_t att_handle) {
  int rc;
  ...
  if (proc->write_reliable.cb == NULL) {
    ...
    rc = proc->write_reliable.cb(
      proc->conn_handle, ble_gattc_error(status, att_handle),
      proc->write_reliable.attrs, proc->write_reliable.num_attrs,
...
static void ble_gattc_write_reliable_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_write_reliable_tx(struct ble_gattc_proc *proc) {
  struct ble_gatt_attr *attr;
  struct os_mbuf *om;
  uint16_t max_sz;
  int attr_idx;
  ...
  rc =
    os_mbuf_appendfrom(om, attr->om, attr->offset, proc->write_reliable.length);
  if (rc != 0) {
    rc = BLE_HS_ENOMEM;
    goto done;
  }

  rc = ble_att_clt_tx_prep_write(proc->conn_handle, proc->cid, attr->handle,
                                 attr->offset, om);
  om = NULL;
  if (rc != 0) {
    goto done;
...
static int ble_gattc_write_reliable_resume(struct ble_gattc_proc *proc) {
  int status;
  int rc;
 ...
 */
static int ble_gattc_write_reliable_rx_prep(struct ble_gattc_proc *proc,
  ...
  if (os_mbuf_cmpm(attr->om, offset, om, 0, proc->write_reliable.length) != 0) {
    rc = BLE_HS_EBADDATA;
    goto err;
  }
 ...
 */
static int ble_gattc_write_reliable_rx_exec(struct ble_gattc_proc *proc,
                                            int status) {
  ble_gattc_dbg_assert_proc_not_inserted(proc);
...
int ble_gatts_notify_multiple_custom(uint16_t conn_handle, size_t chr_count,
  ...
  if ((conn->bhc_gatt_svr.peer_cl_sup_feat[0] & 0x04) == 0) {
    for (i = 0; i < chr_count; i++) {
      rc =
        ble_att_clt_tx_notify(conn_handle, tuples[i].handle, tuples[i].value);
      if (rc != 0) {
        goto done;
      }
    }
  ...
  for (i = 0; i < chr_count; i++) {
    if (OS_MBUF_PKTLEN(txom) + OS_MBUF_PKTLEN(tuples[i].value) > mtu &&
        cur_chr_cnt < 2) {
      rc =
        ble_att_clt_tx_notify(conn_handle, tuples[i].handle, tuples[i].value);
      if (rc != 0) {
    ...
    /* Length */
    os_mbuf_append(txom, &OS_MBUF_PKTLEN(tuples[i].value), sizeof(uint16_t));
 ...
 */
static void ble_gatts_indicate_err(struct ble_gattc_proc *proc, int status,
                                   uint16_t att_handle) {
  int rc;
...
static void ble_gatts_indicate_tmo(struct ble_gattc_proc *proc) {
  BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
  ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 * proc.
 */
static void ble_gatts_indicate_rx_rsp(struct ble_gattc_proc *proc) {
  int rc;

  ble_gattc_dbg_assert_proc_not_inserted(proc);

  rc =
    ble_gatts_rx_indicate_ack(proc->conn_handle, proc->indicate.chr_val_handle);
  if (rc != 0) {
    return;
  }
...
void ble_gattc_rx_err(uint16_t conn_handle, uint16_t cid, uint16_t handle,
  ...
  proc =
    ble_gattc_extract_first_by_conn_cid_op(conn_handle, cid, BLE_GATT_OP_NONE);
  if (proc != NULL) {
    err_cb = ble_gattc_err_dispatch_get(proc->op);
    if (err_cb != NULL) {
 ...
 * GATT procedure.
 */
void ble_gattc_rx_mtu(uint16_t conn_handle, uint16_t cid, int status,
                      uint16_t chan_mtu) {
  struct ble_gattc_proc *proc;

  assert(cid == BLE_L2CAP_CID_ATT);

  proc = ble_gattc_extract_first_by_conn_cid_op(conn_handle, BLE_L2CAP_CID_ATT,
                                                BLE_GATT_OP_MTU);
  if (proc != NULL) {
 ...
 * find-information-response to the appropriate active GATT procedure.
 */
void ble_gattc_rx_find_info_complete(uint16_t conn_handle, uint16_t cid,
                                     int status) {
#if !NIMBLE_BLE_ATT_CLT_FIND_INFO
  return;
#endif
 ...
 * find-by-type-value-response to the appropriate active GATT procedure.
 */
void ble_gattc_rx_find_type_value_complete(uint16_t conn_handle, uint16_t cid,
                                           int status) {
#if !NIMBLE_BLE_ATT_CLT_FIND_TYPE
  return;
#endif
...
void ble_gattc_rx_read_type_adata(uint16_t conn_handle, uint16_t cid,
  ...
  proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(
    conn_handle, cid, ble_gattc_rx_read_type_elem_entries, &rx_entry);
  if (proc != NULL) {
 ...
 */
void ble_gattc_rx_read_type_complete(uint16_t conn_handle, uint16_t cid,
                                     int status) {
#if !NIMBLE_BLE_ATT_CLT_READ_TYPE
  return;
#endif
  ...
  proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(
    conn_handle, cid, ble_gattc_rx_read_type_complete_entries, &rx_entry);
  if (proc != NULL) {
...
void ble_gattc_rx_read_rsp(uint16_t conn_handle, uint16_t cid, int status,
  ...
  proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(
    conn_handle, cid, ble_gattc_rx_read_rsp_entries, &rx_entry);
  if (proc != NULL) {
...
void ble_gattc_rx_write_rsp(uint16_t conn_handle, uint16_t cid) {
  ...
  proc =
    ble_gattc_extract_first_by_conn_cid_op(conn_handle, cid, BLE_GATT_OP_WRITE);
  if (proc != NULL) {
...
void ble_gattc_rx_prep_write_rsp(uint16_t conn_handle, uint16_t cid, int status,
  ...
  proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, cid,
                                       ble_gattc_rx_prep_entries, &rx_entry);
  if (proc != NULL) {
 ...
 * GATT procedure.
 */
void ble_gattc_rx_exec_write_rsp(uint16_t conn_handle, uint16_t cid,
                                 int status) {
 ...
 */
int ble_gattc_any_jobs(void) { return !STAILQ_EMPTY(&ble_gattc_procs); }
...
int ble_gattc_init(void) {
  ...
  if (MYNEWT_VAL(BLE_GATT_MAX_PROCS) > 0) {
    rc = os_mempool_init(&ble_gattc_proc_pool, MYNEWT_VAL(BLE_GATT_MAX_PROCS),
                         sizeof(struct ble_gattc_proc), ble_gattc_proc_mem,
                         "ble_gattc_proc_pool");
    if (rc != 0) {
      return rc;
    }
  ...
  rc = stats_init_and_reg(STATS_HDR(ble_gattc_stats),
                          STATS_SIZE_INIT_PARMS(ble_gattc_stats, STATS_SIZE_32),
                          STATS_NAME_INIT_PARMS(ble_gattc_stats), "ble_gattc");
  if (rc != 0) {
    return BLE_HS_EOS;
  }
