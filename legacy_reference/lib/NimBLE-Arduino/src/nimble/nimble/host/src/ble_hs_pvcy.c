...
static int ble_hs_pvcy_set_addr_timeout(uint16_t timeout) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_RPA_TMO), &cmd, sizeof(cmd),
    NULL, 0);
}
...
uint16_t ble_hs_get_rpa_timeout(void) { return rpa_timeout; }

void ble_hs_reset_rpa_timeout(void) { rpa_timeout = 0; }
...
int ble_hs_pvcy_set_resolve_enabled(int enable) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_ADDR_RES_EN), &cmd,
    sizeof(cmd), NULL, 0);
}
#endif
...
int ble_hs_pvcy_remove_entry(uint8_t addr_type, const uint8_t *addr) {
  ...
  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RMV_RESOLV_LIST), &cmd,
    sizeof(cmd), NULL, 0);
#endif
  return rc;
}
...
static int ble_hs_pvcy_clear_entries(void) {
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_CLR_RESOLV_LIST), NULL, 0, NULL,
    0);
}
#endif

static int ble_hs_pvcy_add_entry_hci(const uint8_t *addr, uint8_t addr_type,
                                     const uint8_t *irk) {
  ...
  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_ADD_RESOLV_LIST), &cmd,
    sizeof(cmd), NULL, 0);
  if (rc != 0) {
    return rc;
  }
...
int ble_hs_pvcy_add_entry(const uint8_t *addr, uint8_t addr_type,
                          const uint8_t *irk) {
  int rc;
...
int ble_hs_pvcy_set_mode(const ble_addr_t *addr, uint8_t priv_mode) {
  ...
  return ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_PRIVACY_MODE), &cmd,
    sizeof(cmd), NULL, 0);
}

bool ble_hs_pvcy_enabled(void) { return ble_hs_pvcy_started; }
