...
static const uint8_t ble_base_uuid[] = {0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00,
...
NimBLEUUID::NimBLEUUID(const std::string &value) {
  if (value.length() == 4) {
    ...
    uint64_t first_half = (strtoull(&value[0], &position, 16) << 32) +
                          (strtoull(position + 1, &position, 16) << 16) +
                          strtoull(position + 1, &position, 16);
    uint64_t second_half = (strtoull(position + 1, &position, 16) << 48) +
                           strtoull(position + 1, nullptr, 16);
 ...
 * @param [in] fourth The last 64bit of the UUID, combining the last 2 parts of
 * the string equivalent
 */
NimBLEUUID::NimBLEUUID(uint32_t first, uint16_t second, uint16_t third,
                       uint64_t fourth) {
  m_uuid.u.type = BLE_UUID_TYPE_128;
...
} // NimBLEUUID(uint32_t first, uint16_t second, uint16_t third, uint64_t
  // fourth)
 ...
 */
uint8_t NimBLEUUID::bitSize() const { return this->m_uuid.u.type; } // bitSize
 ...
 * @note The type value should be checked, if no 16, 32 or 128 then the UUID is
 * not initialized.
 */
...
NimBLEUUID NimBLEUUID::fromString(const std::string &uuid) {
  uint8_t start = 0;
  if (strstr(uuid.c_str(), "0x") !=
      nullptr) { // If the string starts with 0x, skip those characters.
    start = 2;
  }

  uint8_t len =
    uuid.length() -
    start; // Calculate the length of the string we are going to use.
  if (len == 4) {
 ...
 * @details A UUID can be internally represented as 16bit, 32bit or the full
 * 128bit. This method will convert 16 or 32bit representations to the full
 * 128bit.
 * @return The NimBLEUUID converted to 128bit.
 */
const NimBLEUUID &NimBLEUUID::to128() {
  // If we either don't have a value or are already a 128 bit UUID, nothing
  // further to do.
  if (bitSize() != BLE_UUID_TYPE_128) {
    uint32_t val = bitSize() == BLE_UUID_TYPE_16
                     ? *reinterpret_cast<const uint16_t *>(getValue())
 ...
 * @details A UUID can be internally represented as 16bit, 32bit or the full
 * 128bit. This method will convert a 128bit uuid to 16bit if it contains the
 * ble base uuid.
 * @return The NimBLEUUID converted to 16bit if successful, otherwise the
 * original uuid.
 */
 ...
 * @details This is useful when comparing UUIDs or when the advertisement data
 * is reversed.
 */
...
bool NimBLEUUID::operator==(const NimBLEUUID &rhs) const {
  ...
  if (this->bitSize() != rhs.bitSize()) {
    ...
    if (this->bitSize() == BLE_UUID_TYPE_128) {
      memcpy(uuid128 + 12, rhs.getValue(),
             rhs.bitSize() == BLE_UUID_TYPE_16 ? 2 : 4);
      return memcmp(this->getValue(), uuid128, sizeof(ble_base_uuid)) == 0;

    } else if (rhs.bitSize() == BLE_UUID_TYPE_128) {
      memcpy(uuid128 + 12, getValue(),
             this->bitSize() == BLE_UUID_TYPE_16 ? 2 : 4);
  ...
  if (this->bitSize() == BLE_UUID_TYPE_16) {
    return *reinterpret_cast<const uint16_t *>(this->getValue()) ==
           *reinterpret_cast<const uint16_t *>(rhs.getValue());
  }

  if (this->bitSize() == BLE_UUID_TYPE_32) {
    return *reinterpret_cast<const uint32_t *>(this->getValue()) ==
           *reinterpret_cast<const uint32_t *>(rhs.getValue());
  }
