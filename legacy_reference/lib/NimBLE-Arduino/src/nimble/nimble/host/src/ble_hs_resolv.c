...
void ble_rpa_set_num_peer_dev_records(int num_rec) {
  ble_store_num_peer_dev_rec = num_rec;
}

int ble_rpa_remove_peer_dev_rec(struct ble_hs_dev_records *p_dev_rec) {
  int i;

  for (i = 0; i < ble_store_num_peer_dev_rec; i++) {
    if (!(memcmp(p_dev_rec, &peer_dev_rec[i],
  ...
  BLE_HS_LOG(DEBUG,
             " RPA: removed device at index = %d, no. of peer records"
             " = %d\n",
...
static void ble_rpa_peer_dev_rec_clear_all(void) {
  uint8_t i;
 ...
 *         NULL            otherwise
 */
struct ble_hs_dev_records *ble_rpa_find_peer_dev_rec(uint8_t *addr) {
...
static struct ble_hs_dev_records *ble_rpa_find_peer_dev_by_irk(uint8_t *irk) {
  struct ble_hs_dev_records *p_dev_rec = &peer_dev_rec[0];
  uint8_t i;

  for (i = 0; i < ble_store_num_peer_dev_rec; i++, p_dev_rec++) {
    if ((p_dev_rec->rec_used) && (!memcmp(irk, p_dev_rec->peer_sec.irk, 16))) {
      return p_dev_rec;
    }
  }
  return NULL;
...
static bool is_rpa_resolvable_by_peer_rec(struct ble_hs_dev_records *p_dev_rec,
                                          uint8_t *peer_add) {
...
static void ble_rpa_replace_id_with_rand_addr(uint8_t *addr_type,
                                              uint8_t *peer_addr) {
  ...
  if (p_dev_rec != NULL) {
    if (memcmp(p_dev_rec->rand_addr, p_dev_rec->identity_addr,
               BLE_DEV_ADDR_LEN)) {
       ...
       * with rand_addr to maintain status quo for new pairing/encryption
       * request. */
      ...
      if (conn != NULL) {
        ...
        memcpy(&conn->bhc_peer_rpa_addr.val[0], p_dev_rec->rand_addr,
               BLE_DEV_ADDR_LEN);
        conn->bhc_peer_addr.type = p_dev_rec->rand_addr_type;
        memcpy(&conn->bhc_peer_addr.val[0], p_dev_rec->rand_addr,
               BLE_DEV_ADDR_LEN);
        BLE_HS_LOG(DEBUG,
...
int ble_rpa_resolv_add_peer_rec(uint8_t *peer_addr) {
  struct ble_hs_dev_records *p_dev_rec =
    ...
    if (is_rpa_resolvable_by_peer_rec(p_dev_rec, peer_addr)) {
      ...
      if (rl) {
        ...
        *peer_addr_type = p_dev_rec->peer_sec.peer_addr.type;
        ...
        BLE_HS_LOG(DEBUG,
                   "RPA resolvable but RL doesn't exist; remove"
                   " peer rec at index = %d \n",
                   i);
...
void ble_rpa_replace_peer_params_with_rl(uint8_t *peer_addr, uint8_t *addr_type,
...
static void ble_hs_rand_prand_get(uint8_t *prand) {
  uint16_t sum;
  uint8_t rc;
 ...
 * @param local
 */
static void ble_hs_resolv_gen_priv_addr(struct ble_hs_resolv_entry *rl,
                                        int local) {
  uint8_t *irk = NULL;
  uint8_t *prand = NULL;
...
/* Called to generate private (RPA/NRPA) address and this address is set in
 * controller as Random address. This is necessary in Host based privacy because
 * controller is unaware of private address is being used */
 ...
 */
static void ble_hs_resolv_rpa_timer_cb(struct ble_npl_event *ev) {
  if (ble_host_rpa_enabled() || (nrpa_pvcy)) {
    BLE_HS_LOG(
      DEBUG,
      "RPA/NRPA Timeout; start active adv & scan with new Private address \n");
    ble_gap_preempt();
...
static bool is_irk_nonzero(uint8_t *irk) {
  int i;
  bool rc = false;
 ...
 * element).
 */
static int ble_hs_is_on_resolv_list(uint8_t *addr, uint8_t addr_type) {
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
  int i;
  struct ble_hs_resolv_entry *rl = &g_ble_hs_resolv_list[1];

  for (i = 1; i < g_ble_hs_resolv_data.rl_cnt; ++i) {
    if ((!memcmp(rl->rl_identity_addr, addr, BLE_DEV_ADDR_LEN)) ||
 ...
 */
struct ble_hs_resolv_entry *ble_hs_resolv_list_find(uint8_t *addr) {
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
  int i;
  struct ble_hs_resolv_entry *rl = &g_ble_hs_resolv_list[1];
 ...
 */
int ble_hs_resolv_list_rmv(uint8_t addr_type, uint8_t *ident_addr) {
  ...
  if (position) {
    memmove(&g_ble_hs_resolv_list[position],
            ...
            (g_ble_hs_resolv_data.rl_cnt - position) *
              sizeof(struct ble_hs_resolv_entry));
    --g_ble_hs_resolv_data.rl_cnt;
   ...
   * peer_address to its latest received OTA address, this helps when existing
   * bond at peer side is removed */
...
void ble_hs_resolv_list_clear_all(void) {
  ...
  memset(g_ble_hs_resolv_list, 0,
         BLE_RESOLV_LIST_SIZE * sizeof(struct ble_hs_resolv_entry));
 ...
 */
void ble_hs_resolv_nrpa_enable(void) { nrpa_pvcy = true; }

/**
 * Called by host stack to disable NRPA privacy flag
 */
void ble_hs_resolv_nrpa_disable(void) { nrpa_pvcy = false; }
/**
 ...
 * @return int
 */
void ble_hs_resolv_enable(bool enable) {
  int32_t tmo;
 ...
 * @return int
 */
int ble_hs_resolv_set_rpa_tmo(uint16_t tmo_secs) {
...
struct ble_hs_resolv_entry *ble_hs_resolv_rpa_addr(uint8_t *addr,
                                                   uint8_t addr_type) {
#if MYNEWT_VAL(BLE_STORE_MAX_BONDS)
  int i;
  struct ble_hs_resolv_entry *rl = &g_ble_hs_resolv_list[1];
...
void ble_hs_resolv_init(void) {
  g_ble_hs_resolv_data.rpa_tmo =
    ble_npl_time_ms_to_ticks32(MYNEWT_VAL(BLE_RPA_TIMEOUT) * 1000);

  ble_npl_callout_init(&g_ble_hs_resolv_data.rpa_timer, ble_hs_evq_get(),
                       ble_hs_resolv_rpa_timer_cb, NULL);
}
