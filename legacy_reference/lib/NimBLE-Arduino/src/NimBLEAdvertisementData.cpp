 ...
 * @return True if successful.
 */
bool NimBLEAdvertisementData::setManufacturerData(
 ...
 * @param [in] isComplete If true the name is complete, which will set the data
 * type accordingly.
...
bool NimBLEAdvertisementData::setName(const std::string &name,
  ...
  uint8_t data[BLE_HS_ADV_MAX_SZ];
  uint8_t length =
    2 + std::min<uint8_t>(name.length(), BLE_HS_ADV_MAX_FIELD_SZ);
  data[0] = length - 1;
  data[1] =
    isComplete ? BLE_HS_ADV_TYPE_COMP_NAME : BLE_HS_ADV_TYPE_INCOMP_NAME;
  memcpy(&data[2], name.c_str(),
         std::min<uint8_t>(name.length(), BLE_HS_ADV_MAX_FIELD_SZ));
  return addData(data, length);
} // setName
 ...
 * @return True if successful.
 */
bool NimBLEAdvertisementData::setCompleteServices16(
 ...
 * @return True if successful.
 */
bool NimBLEAdvertisementData::setCompleteServices32(
  const std::vector<NimBLEUUID> &uuids) {
 ...
 * @return True if successful.
 */
bool NimBLEAdvertisementData::setPartialServices16(
  const std::vector<NimBLEUUID> &uuids) {
 ...
 * @return True if successful.
 */
bool NimBLEAdvertisementData::setPartialServices32(
  const std::vector<NimBLEUUID> &uuids) {
 ...
 * @return True if successful.
 * @details The number of services will be truncated if the total length exceeds
 * 31 bytes.
 */
bool NimBLEAdvertisementData::setServices(
  bool complete, uint8_t size, const std::vector<NimBLEUUID> &uuids) {
  ...
  switch (size) {
  case 16:
    data[1] = (complete ? BLE_HS_ADV_TYPE_COMP_UUIDS16
                        : BLE_HS_ADV_TYPE_INCOMP_UUIDS16);
    break;
  case 32:
    data[1] = (complete ? BLE_HS_ADV_TYPE_COMP_UUIDS32
                        : BLE_HS_ADV_TYPE_INCOMP_UUIDS32);
    break;
  case 128:
    data[1] = (complete ? BLE_HS_ADV_TYPE_COMP_UUIDS128
