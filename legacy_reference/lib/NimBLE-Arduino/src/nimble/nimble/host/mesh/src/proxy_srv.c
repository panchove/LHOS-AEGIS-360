...
#define BT_UUID_16_ENCODE(w16) (((w16) >> 0) & 0xFF), (((w16) >> 8) & 0xFF)
...
static void proxy_filter_recv(uint16_t conn_handle, struct bt_mesh_net_rx *rx,
                              struct os_mbuf *buf) {
...
static void proxy_cfg(struct bt_mesh_proxy_role *role) {
  ...
  if (bt_mesh_rpl_check(&rx, NULL)) {
    BT_WARN("Replay: src 0x%04x dst 0x%04x seq 0x%06" PRIx32, rx.ctx.addr,
...
static int node_id_adv(struct bt_mesh_subnet *sub, int32_t duration) {
  struct ble_gap_adv_params fast_adv_param = {ADV_OPT_PROXY ADV_FAST_INT};
#if ADV_OPT_USE_NAME
  const char *name = CONFIG_BT_DEVICE_NAME;
  size_t name_len = strlen(name);
  struct bt_data sd = {
    .type = BT_DATA_NAME_COMPLETE, .data_len = name_len, .data = (void *)name};
#else
...
static int net_id_adv(struct bt_mesh_subnet *sub, int32_t duration) {
  struct ble_gap_adv_params slow_adv_param = {ADV_OPT_PROXY ADV_SLOW_INT};
#if ADV_OPT_USE_NAME
  const char *name = CONFIG_BT_DEVICE_NAME;
  size_t name_len = strlen(name);
  struct bt_data sd = {
...
static int gatt_proxy_advertise(struct bt_mesh_subnet *sub) {
  ...
  BT_DBG("Advertising %d ms for net_idx 0x%04x", (int)remaining, sub->net_idx);
...
void bt_mesh_proxy_gatt_disconnect(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(clients); i++) {
    ...
    if ((client->cli) &&
        (client->filter_type == ACCEPT || client->filter_type == REJECT)) {
      client->filter_type = NONE;
      rc =
        ble_gap_terminate(client->cli->conn_handle, BLE_ERR_REM_USER_CONN_TERM);
      assert(rc == 0);
    }
  }
}
...
bool bt_mesh_proxy_relay(struct os_mbuf *buf, uint16_t dst) {
  ...
  for (i = 0; i < ARRAY_SIZE(clients); i++) {
    ...
    err = bt_mesh_proxy_msg_send(client->cli, BT_MESH_PROXY_NET_PDU, msg);
...
static void gatt_connected(uint16_t conn_handle) {
  ...
  if (info.role != BLE_GAP_ROLE_SLAVE || !service_registered) {
    return;
  }
  ...
  client->cli =
    bt_mesh_proxy_role_setup(conn_handle, proxy_send, proxy_msg_recv);
...
int ble_mesh_proxy_gap_event(struct ble_gap_event *event, void *arg) {
  if ((event->type == BLE_GAP_EVENT_CONNECT) ||
    ...
    gatt_disconnected(event->disconnect.conn, event->disconnect.reason);
#if MYNEWT_VAL(BLE_MESH_PB_GATT)
    gatt_disconnected_pb_gatt(event->disconnect.conn, event->disconnect.reason);
#endif
...
#endif
    } else if (event->subscribe.attr_handle == svc_handles.prov_data_out_h) {
#if (MYNEWT_VAL(BLE_MESH_PB_GATT))
