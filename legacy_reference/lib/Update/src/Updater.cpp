...
bool UpdateClass::_enablePartition(const esp_partition_t *partition) {
  ...
  return ESP.partitionWrite(partition, 0, (uint32_t *)_skipBuffer,
                            ENCRYPTED_BLOCK_SIZE);
}

UpdateClass::UpdateClass()
    : _error(0), _buffer(0), _bufferLen(0), _size(0), _progress_callback(NULL),
      _progress(0), _paroffset(0), _command(U_FLASH), _partition(NULL) {}
...
bool UpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn,
  ...
  if (command == U_FLASH) {
    ...
    if (!_partition) {
      ...
      _paroffset =
        0x1000; // Offset for ffat, assuming size is already corrected
      if (!_partition) {
        _error = UPDATE_ERROR_NO_PARTITION;
        return false;
      }
  ...
  // allocate buffer in SPIRAM if possible
  _buffer = (uint8_t *)heap_caps_malloc(SPI_FLASH_SEC_SIZE,
                                        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!_buffer) {
    log_e("malloc failed");
    return false;
  }
  log_w("buffer allocated at SPIRAM");
  _size = size;
  _command = command;
  _md5.begin();
  return true;
...
void UpdateClass::abort() { _abort(UPDATE_ERROR_ABORT); }
...
bool UpdateClass::_writeBuffer() {
  ...
  bool block_erase =
    (_size - _progress >= SPI_FLASH_BLOCK_SIZE) &&
    (offset % SPI_FLASH_BLOCK_SIZE ==
     0); // if it's the block boundary, than erase the whole block from here
  bool part_head_sectors =
    _partition->address % SPI_FLASH_BLOCK_SIZE &&
    offset < (_partition->address / SPI_FLASH_BLOCK_SIZE + 1) *
               SPI_FLASH_BLOCK_SIZE; // sector belong to unaligned partition
                                     // heading block
  bool part_tail_sectors =
    offset >= (_partition->address + _size) / SPI_FLASH_BLOCK_SIZE *
                SPI_FLASH_BLOCK_SIZE; // sector belong to unaligned partition
                                      // tailing block
  if (block_erase || part_head_sectors || part_tail_sectors) {
    if (!ESP.partitionEraseRange(_partition, _progress,
                                 block_erase ? SPI_FLASH_BLOCK_SIZE
                                             : SPI_FLASH_SEC_SIZE)) {
  ...
  if ((_partition->encrypted ||
       _chkDataInBlock(_buffer + skip / sizeof(uint32_t), _bufferLen - skip)) &&
      !ESP.partitionWrite(_partition, _progress + skip,
                          (uint32_t *)_buffer + skip / sizeof(uint32_t),
                          _bufferLen - skip)) {
...
size_t UpdateClass::writeStream(Stream &data) {
  ...
  while (remaining()) {
    ...
    /*
    Init read&timeout counters and try to read, if read failed, increase
    counter, wait 100ms and try to read again. If counter > 300 (30 sec), give
    up/abort
    */
    toRead = 0;
    timeout_failures = 0;
...
void UpdateClass::printError(Print &out) { out.println(_err2str(_error)); }

const char *UpdateClass::errorString() { return _err2str(_error); }
...
bool UpdateClass::_chkDataInBlock(const uint8_t *data, size_t len) const {
  ...
  do {
    if (*(uint32_t *)data ^ 0xffffffff) // for SPI NOR flash empty blocks are
                                        // all one's, i.e. filled with 0xff byte
      return true;
