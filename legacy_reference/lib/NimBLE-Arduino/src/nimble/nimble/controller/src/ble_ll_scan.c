...
#define BLE_LL_SCAN_ENTRY_TYPE_LEGACY(addr_type) ((addr_type) & 1)
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
#define BLE_LL_SCAN_ENTRY_TYPE_EXT(addr_type, has_aux, is_anon, adi)           \
  (((adi >> 8) & 0xF0) | (1 << 3) | (is_anon << 2) | (has_aux << 1) |          \
   ((addr_type) & 1))
#endif
...
static os_membuf_t g_scan_dup_mem[OS_MEMPOOL_SIZE(
  MYNEWT_VAL(BLE_LL_NUM_SCAN_DUP_ADVS), sizeof(struct ble_ll_scan_dup_entry))];
...
static inline uint32_t ble_ll_scan_time_hci_to_ticks(uint16_t value) {
  return ble_ll_tmr_u2t(value * BLE_HCI_SCAN_ITVL);
}

/* See Vol 6 Part B Section 4.4.3.2. Active scanning backoff */
static void ble_ll_scan_req_backoff(struct ble_ll_scan_sm *scansm,
                                    int success) {
...
static void ble_ll_scan_refresh_nrpa(struct ble_ll_scan_sm *scansm) {
  ble_npl_time_t now;
...
static void ble_ll_scan_req_pdu_prepare(struct ble_ll_scan_sm *scansm,
...
static uint8_t ble_ll_scan_req_tx_pdu_cb(uint8_t *dptr, void *pducb_arg,
                                         uint8_t *hdr_byte) {
...
static int ble_ll_hci_send_legacy_ext_adv_report(
  uint8_t evtype, const uint8_t *addr, uint8_t addr_type, int8_t rssi,
  uint8_t adv_data_len, struct os_mbuf *adv_data, const uint8_t *inita,
  uint8_t inita_type) {
  struct ble_hci_ev_le_subev_ext_adv_rpt *ev;
  struct ext_adv_report *report;
  struct ble_hci_ev *hci_ev;
  ...
  if ((sizeof(*ev) + sizeof(ev->reports[0]) + adv_data_len) >
      BLE_HCI_MAX_DATA_LEN) {
...
static int ble_ll_hci_send_adv_report(uint8_t evtype, const uint8_t *addr,
  ...
  if ((sizeof(*ev) + sizeof(ev->reports[0]) + adv_data_len + 1) >
      BLE_HCI_MAX_DATA_LEN) {
    STATS_INC(ble_ll_stats, adv_evt_dropped);
    return -1;
  }
  ...
  ev_rssi = (int8_t *)(hci_ev->data + sizeof(*ev) + sizeof(ev->reports[0]) +
                       adv_data_len);
  *ev_rssi = rssi;
...
static int ble_ll_hci_send_dir_adv_report(const uint8_t *addr,
                                          uint8_t addr_type,
                                          const uint8_t *inita,
                                          uint8_t inita_type, int8_t rssi) {
...
static int ble_ll_scan_dup_update_legacy(uint8_t addr_type, const uint8_t *addr,
 ...
 * @param scansm
 */
static void ble_ll_scan_send_adv_report(uint8_t pdu_type, const uint8_t *adva,
                                        uint8_t adva_type, const uint8_t *inita,
                                        uint8_t inita_type, struct os_mbuf *om,
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
  if (scansm->ext_scanning) {
    rc = ble_ll_hci_send_legacy_ext_adv_report(
      evtype, adva, adva_type, hdr->rxinfo.rssi - ble_ll_rx_gain(),
      adv_data_len, om, inita, inita_type);
    goto done;
  }
#endif
...
static int ble_ll_scan_start(struct ble_ll_scan_sm *scansm) {
  ...
  rc = ble_phy_rx_set_start_time(ble_ll_tmr_get() + g_ble_ll_sched_offset_ticks,
                                 0);
...
static uint8_t ble_ll_scan_get_next_adv_prim_chan(uint8_t chan) {
  ++chan;
...
static uint32_t ble_ll_scan_move_window_to(struct ble_ll_scan_phy *scanp,
                                           uint32_t time) {
  uint32_t end_time;
...
static bool ble_ll_scan_is_inside_window(struct ble_ll_scan_phy *scanp,
                                         uint32_t time) {
  uint32_t start_time;
...
static int ble_ll_scan_sm_start(struct ble_ll_scan_sm *scansm) {
  ...
  if (scanp_next) {
    ...
    scanp_next->timing.start_time =
      scanp->timing.start_time + scanp->timing.window;
  }
...
static void ble_ll_scan_interrupted_event_cb(struct ble_npl_event *ev) {
  struct ble_ll_scan_sm *scansm = &g_ble_ll_scan_sm;
 ...
 * @param arg
 */
static void ble_ll_scan_event_proc(struct ble_npl_event *ev) {
  struct ble_ll_scan_sm *scansm;
  os_sr_t sr;
  bool start_scan;
  bool inside_window;
...
static int ble_ll_scan_rx_isr_end_on_adv(uint8_t pdu_type, uint8_t *rxbuf,
                                         struct ble_mbuf_hdr *hdr,
                                         struct ble_ll_scan_addr_data *addrd) {
  struct ble_ll_scan_sm *scansm = &g_ble_ll_scan_sm;
  struct ble_ll_scan_phy *scanp = scansm->scanp;
...
static bool ble_ll_scan_send_scan_req(uint8_t pdu_type, uint8_t *rxbuf,
                                      struct ble_mbuf_hdr *hdr,
                                      struct ble_ll_scan_addr_data *addrd) {
  struct ble_ll_scan_sm *scansm = &g_ble_ll_scan_sm;
  struct ble_mbuf_hdr_rxinfo *rxinfo = &hdr->rxinfo;
  ...
  ble_ll_scan_req_pdu_prepare(scansm, addrd->adva, addrd->adva_type, rpa_index);
 ...
 */
void ble_ll_scan_timer_cb(void *arg) {
  struct ble_ll_scan_sm *scansm;
...
static inline struct ble_ll_scan_dup_entry *ble_ll_scan_dup_new(void) {
  struct ble_ll_scan_dup_entry *e;
...
static int ble_ll_scan_dup_check_legacy(uint8_t addr_type, uint8_t *addr,
                                        uint8_t pdu_type) {
  struct ble_ll_scan_dup_entry *e;
  uint8_t type;
  int rc;
...
ble_ll_scan_rx_pkt_in_on_legacy(uint8_t pdu_type, struct os_mbuf *om,
  ...
  if (!BLE_MBUF_HDR_DEVMATCH(hdr) || !BLE_MBUF_HDR_CRC_OK(hdr) ||
      BLE_MBUF_HDR_IGNORED(hdr) || !scansm->scan_enabled) {
    return;
  }
  ...
  send_hci_report = !scansm->scan_filt_dups ||
                    !ble_ll_scan_dup_check_legacy(addrd->adv_addr_type,
                                                  addrd->adv_addr, pdu_type);
  if (send_hci_report) {
    /* Sending advertising report will also update scan_dup list */
    ble_ll_scan_send_adv_report(pdu_type, addrd->adv_addr, addrd->adv_addr_type,
                                addrd->targeta, addrd->targeta_type, om, hdr,
                                scansm);
  }
 ...
 * @param rxbuf
 */
void ble_ll_scan_rx_pkt_in(uint8_t ptype, struct os_mbuf *om,
                           struct ble_mbuf_hdr *hdr) {
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
...
int ble_ll_scan_hci_set_params(const uint8_t *cmdbuf, uint8_t len) {
  ...
  if ((scan_itvl < BLE_HCI_SCAN_ITVL_MIN) ||
      ...
      (scan_window > BLE_HCI_SCAN_WINDOW_MAX) || (scan_itvl < scan_window)) {
    return BLE_ERR_INV_HCI_CMD_PARMS;
  }
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static int ble_ll_scan_check_phy_params(uint8_t type, uint16_t itvl,
                                        uint16_t window) {
  /* Check scan type */
...
ble_ll_scan_duration_period_timers_restart(struct ble_ll_scan_sm *scansm) {
  ...
  if (scansm->duration_ticks) {
    ble_npl_callout_reset(&scansm->duration_timer, scansm->duration_ticks);
...
static void ble_ll_scan_duration_timer_cb(struct ble_npl_event *ev) {
...
static void ble_ll_scan_period_timer_cb(struct ble_npl_event *ev) {
  struct ble_ll_scan_sm *scansm = ble_npl_event_get_arg(ev);
 ...
 */
static int ble_ll_scan_set_enable(uint8_t enable, uint8_t filter_dups,
                                  uint16_t period, uint16_t dur, bool ext) {
  int rc;
  struct ble_ll_scan_sm *scansm;
  struct ble_ll_scan_phy *scanp;
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
int ble_ll_scan_initiator_start(struct ble_ll_conn_sm *connsm, uint8_t ext,
  ...
  if (init_phy_mask & BLE_PHY_MASK_1M) {
    ...
    scanp_uncoded->timing.interval =
      ble_ll_scan_time_hci_to_ticks(cc_scan->scan_params[PHY_UNCODED].itvl);
    scanp_uncoded->timing.window =
      ble_ll_scan_time_hci_to_ticks(cc_scan->scan_params[PHY_UNCODED].window);
  ...
  if (init_phy_mask & BLE_PHY_MASK_CODED) {
    ...
    scanp_coded->timing.interval =
      ble_ll_scan_time_hci_to_ticks(cc_scan->scan_params[PHY_CODED].itvl);
    scanp_coded->timing.window =
      ble_ll_scan_time_hci_to_ticks(cc_scan->scan_params[PHY_CODED].window);
 ...
 */
int ble_ll_scan_enabled(void) { return (int)g_ble_ll_scan_sm.scan_enabled; }
...
static void ble_ll_scan_common_init(void) {
  struct ble_ll_scan_sm *scansm;
  struct ble_ll_scan_phy *scanp;
  int i;
  ...
  ble_npl_event_init(&scansm->scan_interrupted_ev,
                     ble_ll_scan_interrupted_event_cb, NULL);
}
...
void ble_ll_scan_init(void) {
  ...
  err = os_mempool_init(&g_scan_dup_pool, MYNEWT_VAL(BLE_LL_NUM_SCAN_DUP_ADVS),
                        sizeof(struct ble_ll_scan_dup_entry), g_scan_dup_mem,
