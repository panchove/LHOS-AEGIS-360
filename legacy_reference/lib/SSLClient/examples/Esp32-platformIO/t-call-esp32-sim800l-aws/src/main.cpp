...
#define serialMonitor                                                          \
  Serial // Set serial for debug console (to the Serial Monitor)
...
#define MQTT_BROKER                                                            \
  "AWS_BROKER_ENDPOINT" // e.g. a2l1dde17xdr1y-ats.iot.eu-west-1.amazonaws.com
...
// To use this example create your thing in AWS and get the certs downloaded. Be
// sure to update the ca_cert.h with the root CA, client certificate, and
// private key. The root CA is the Amazon Root CA 1. The client certificate and
// private key are the ones you downloaded from AWS IoT Core. The client
// certificate and private key are in PEM format. The root CA is in PEM format
// as well.
 ...
 * Sets the CA certificate, client certificate, and private key for the MQTT
 * client. Sets the timeout for the SSL connection.
 */
...
bool getIpAddress(const char *domain, char *ipAddress, size_t size) {
      ...
      if (startChar != NULL) {
        ...
        if (startChar != NULL) {
          ...
          if (endChar != NULL && endChar > startChar &&
              (size_t)(endChar - startChar - 1) < size) {
            strncpy(ipAddress, startChar + 1, endChar - startChar - 1);
            ipAddress[endChar - startChar - 1] =
  ...
  ipAddress[0] =
    '\0'; // Ensure the buffer is set to empty string if no IP found
  return false;
}
