...
typedef int hci_transport_host_recv_fn(hci_trans_pkt_ind_t type, uint8_t *data,
                                       uint16_t len);

/**
 * @brief Initialize the HCI transport layer.
 *        It should be called before using any other functions in the transport
 * layer.
 *
...
/**
 * @brief Deinitialize the HCI transport layer for releasing any allocated
 * resources.
 */
...
/**
 * @brief Set the host's HCI callback which will be invoked when receiving
 * ACL/Events from controller.
