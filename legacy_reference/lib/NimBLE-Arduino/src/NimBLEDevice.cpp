...
NimBLEDeviceCallbacks *NimBLEDevice::m_pDeviceCallbacks =
  &defaultDeviceCallbacks;
 ...
 * @return A pointer to the server instance or nullptr if none have been
 * created.
 */
NimBLEServer *NimBLEDevice::getServer() { return m_pServer; } // getServer
 ...
 * @return A pointer to the L2CAP server instance or nullptr if none have been
 * created.
 */
 ...
 */
bool NimBLEDevice::startAdvertising(uint8_t instId, int duration,
                                    int maxEvents) {
 ...
 * @return The scanning object reference.  This is a singleton object.  The
 * caller should not try and release/delete it.
 */
#if CONFIG_BT_NIMBLE_ROLE_OBSERVER
 ...
 * @param [in] size The number of advertisements filtered before the cache is
 * reset.\n Range is 10-1000, a larger value will reduce how often the same
 ...
 * * CONFIG_BTDM_SCAN_DUPL_TYPE_DEVICE (0) (default)\n
     Filter by device address only, advertisements from the same address will be
 reported only once.
 ...
 * * CONFIG_BTDM_SCAN_DUPL_TYPE_DATA_DEVICE (2)\n
 ...
 reported only once,\n except if the data in the advertisement has changed, then
 ...
 * even if already seen in the current scan. If set to 0 the cache will never be
 * reset.
 */
void NimBLEDevice::setScanDuplicateCacheResetTime(uint16_t time) {
  if (m_initialized) {
    NIMBLE_LOGE(LOG_TAG,
                "Cannot change scan cache reset time while initialized");
    return;
  } else if (time > 1000) {
...
#if CONFIG_BT_NIMBLE_ROLE_CENTRAL
/**
 * @brief Creates a new client object, each client can connect to 1 peripheral
 * device.
 ...
 * @brief Creates a new client object, each client can connect to 1 peripheral
 * device.
 * @param [in] peerAddress A peer address reference that is copied to the new
 * client object, allows for calling NimBLEClient::connect(bool) without a
 * device or address parameter.
 * @return A pointer to the new client object, or nullptr on error.
 */
NimBLEClient *NimBLEDevice::createClient(const NimBLEAddress &peerAddress) {
  ...
  NIMBLE_LOGE(LOG_TAG, "Unable to create client; already at max: %d",
 ...
 * Checks if it is connected or trying to connect and disconnects/stops it
 * first.
 ...
 * @return A pointer to the client object with the specified connection handle
 * or nullptr.
 */
 ...
 * @return The power level currently used in esp_power_level_t or a negative
 * value on error.
 */
...
bool NimBLEDevice::setPower(int8_t dbm, NimBLETxPowerType type) {
  ...
  return false; // CONFIG_IDF_TARGET_ESP32P4 does not support
  ...
  bool success = false;
  esp_power_level_t espPwr =
    static_cast<esp_power_level_t>(dbm / 3 + ESP_PWR_LVL_N0);
...
#ifdef ESP_PLATFORM
#ifdef CONFIG_IDF_TARGET_ESP32P4
  return 0xFF; // CONFIG_IDF_TARGET_ESP32P4 does not support
               // esp_ble_tx_power_get
#else
  esp_ble_power_type_t espPwr =
    type == NimBLETxPowerType::Advertise ? ESP_BLE_PWR_TYPE_ADV
...
/**
 * @brief Setup local mtu that will be used to negotiate mtu during request from
 * client peer.
...
bool NimBLEDevice::setMTU(uint16_t mtu) {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Could not set local mtu value to: %d, rc: %d", mtu,
                rc);
  }
 ...
 */
uint16_t NimBLEDevice::getMTU() { return ble_att_preferred_mtu(); }
...
bool NimBLEDevice::whiteListAdd(const NimBLEAddress &address) {
  if (!NimBLEDevice::onWhiteList(address)) {
    ...
    int rc = ble_gap_wl_set(reinterpret_cast<ble_addr_t *>(&m_whiteList[0]),
                            m_whiteList.size());
    if (rc != 0) {
 ...
 */
size_t NimBLEDevice::getWhiteListCount() { return m_whiteList.size(); }

/**
 * @brief Gets the address at the vector index.
 * @param [in] index The vector index to retrieve the address from.
 * @returns The NimBLEAddress at the whitelist index or null address if not
 * found.
 */
...
bool NimBLEDevice::setDefaultPhy(uint8_t txPhyMask, uint8_t rxPhyMask) {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Failed to set default phy; rc=%d %s", rc,
...
/**
 * @brief Host reset, we pass the message so we don't make calls until
 * re-synced.
...
void NimBLEDevice::onReset(int reason) {
  ...
  NIMBLE_LOGE(LOG_TAG, "Host reset; reason=%d, %s", reason,
...
void NimBLEDevice::onSync(void) {
  ...
  // This check is needed due to potentially being called multiple times in
  // succession If this happens, the call to scan start may get stuck or cause
  // an advertising fault.
  if (m_synced) {
    return;
  }
...
bool NimBLEDevice::init(const std::string &deviceName) {
  if (!m_initialized) {
    ...
    // make sure the linker includes esp32-hal-bt.c so Arduino init doesn't
    // release BLE memory.
    btStarted();
#endif
  ...
  !defined(CONFIG_NIMBLE_CPP_IDF)
    ...
    bt_cfg.ble_max_act = MYNEWT_VAL(BLE_MAX_CONNECTIONS) +
                         MYNEWT_VAL(BLE_ROLE_BROADCASTER) +
    ...
    case 2:
      mode |=
        ((1UL << 2) |
         (1UL << 3)); // FILTER_DUPLICATE_ADDRESS | FILTER_DUPLICATE_ADVDATA
      break;
    default:
      mode |= (1UL << 0) |
              (1UL << 2); // FILTER_DUPLICATE_PDUTYPE | FILTER_DUPLICATE_ADDRESS
    }
    ...
    ble_hs_cfg.store_status_cb = [](struct ble_store_status_event *event,
                                    void *arg) {
  ...
  // Wait for host and controller to sync before returning and accepting new
  // tasks
  while (!m_synced) {
    ble_npl_time_delay(1);
  }
  ...
  m_initialized =
    true; // Set the initialization flag to ensure we are only initialized once.
  return true;
} // init

/**
 * @brief Shutdown the NimBLE stack/controller.
 * @param [in] clearAll If true, deletes all server/advertising/scan/client
 * objects after de-initializing.
 * @note If clearAll is true when called all objects created will be deleted and
 * any references to the created objects become invalid. If clearAll is false,
 * the objects will remain and can be used again after re-initializing the
 * stack. If the stack was not already initialized, the objects created can be
 * deleted when clearAll is true with no effect on the stack.
 */
  ...
  if (m_initialized) {
    ...
    if (rc == 0) {
      ...
      if (rc != ESP_OK) {
        NIMBLE_LOGE(
          LOG_TAG,
          "esp_nimble_hci_and_controller_deinit() failed with error: %d", rc);
      }
#endif
#endif
      m_initialized = false;
 ...
 */
bool NimBLEDevice::isInitialized() { return m_initialized; } // getInitialized
 ...
 * @return A NimBLEAddress object with the currently used address, or a NULL
 * address if not set.
 */
NimBLEAddress NimBLEDevice::getAddress() {
  ble_addr_t addr{};
  uint8_t type =
    m_ownAddrType & 1; // input must be random or public, odd values are random
 ...
 * * 0x00: BLE_OWN_ADDR_PUBLIC - Public address; Uses the hardware static
 * address.
 * * 0x01: BLE_OWN_ADDR_RANDOM - Random static address; Uses the hardware or
 * generated random static address.
 * * 0x02: BLE_OWN_ADDR_RPA_PUBLIC_DEFAULT - Resolvable private address,
 * defaults to public if no RPA available.
 * * 0x03: BLE_OWN_ADDR_RPA_RANDOM_DEFAULT - Resolvable private address,
 * defaults to random static if no RPA available.
 */
bool NimBLEDevice::setOwnAddrType(uint8_t type) {
  ...
  if (type == BLE_OWN_ADDR_RPA_PUBLIC_DEFAULT ||
      type == BLE_OWN_ADDR_RPA_RANDOM_DEFAULT) {
    // esp32 controller does not support RPA so we must use the random static
    // for calls to the stack the host will take care of the random private
    // address generation/setting.
    m_ownAddrType = BLE_OWN_ADDR_RANDOM;
 ...
 * @details To use the address generated the address type must be set to random
 * with `setOwnAddrType`.
 */
 ...
 * @param mitm If true we are capable of man in the middle protection, false if
 * not.
 * @param sc If true we will perform secure connection pairing, false we will
 * use legacy pairing.
 */
void NimBLEDevice::setSecurityAuth(bool bonding, bool mitm, bool sc) {
  NIMBLE_LOGD(LOG_TAG, "Setting bonding: %d, mitm: %d, sc: %d", bonding, mitm,
              sc);
...
/**
 * @brief If we are the initiator of the security procedure this sets the keys
 * we will distribute.
...
bool NimBLEDevice::startSecurity(uint16_t connHandle, int *rcPtr) {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gap_security_initiate: rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
  if (rcPtr) {
    *rcPtr = rc;
...
/**
 * @brief Inject the provided numeric comparison response into the Security
 * Manager.
 * @param [in] peerInfo Connection information for the peer.
...
bool NimBLEDevice::setCustomGapHandler(gap_event_handler handler, void *arg) {
  ...
  if (rc == BLE_HS_EALREADY) {
    ...
    NIMBLE_LOGE(LOG_TAG, "ble_gap_event_listener_register: rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
  }
