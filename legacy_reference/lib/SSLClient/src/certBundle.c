...
static int esp_crt_verify_callback(void *buf, mbedtls_x509_crt *crt, int data,
                                   uint32_t *flags);
static int esp_crt_check_signature(mbedtls_x509_crt *child,
...
static int esp_crt_check_signature(mbedtls_x509_crt *child,
  ...
  if ((ret = mbedtls_pk_verify_ext(
         child->private_sig_pk, child->private_sig_opts, &parent.pk,
         child->private_sig_md, hash, mbedtls_md_get_size(md_info),
         child->private_sig.p, child->private_sig.len)) != 0) {
#else
  if ((ret = mbedtls_pk_verify_ext(
         child->sig_pk, child->sig_opts, &parent.pk, child->sig_md, hash,
         mbedtls_md_get_size(md_info), child->sig.p, child->sig.len)) != 0) {
#endif
 ...
 */
int esp_crt_verify_callback(void *buf, mbedtls_x509_crt *crt, int depth,
                            uint32_t *flags) {
  ...
  if (crt_found) {
    ...
    ret = esp_crt_check_signature(
      child, s_crt_bundle.crts[middle] + CRT_HEADER_OFFSET + name_len, key_len);
  }
...
/* Initialize the bundle into an array so we can do binary search for certs,
   the bundle generated by the python utility is already presorted by subject
   name
 */
...
esp_err_t ssl_lib_crt_bundle_attach(void *conf) {
  ...
  // If no bundle has been set by the user then use the bundle embedded in the
  // binary
  if (s_crt_bundle.crts == NULL) {
