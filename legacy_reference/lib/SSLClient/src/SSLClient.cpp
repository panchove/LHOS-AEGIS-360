...
/**
 * @brief Terminates the current SSL connection and releases associated
 * resources.
 *
 * This function stops the SSL socket and logs relevant information based on the
 * state of the client connection. After the SSL connection is terminated, it
 * resets the connected flag and peek value of the SSL client.
 *
 ...
 *
 * Based on the class members `_pskIdent` and `_psKey`, this function will
 * decide whether to establish a connection using a pre-shared key or using a
 * certificate authority. If `_pskIdent` and `_psKey` are set, it uses them to
 * establish a connection. Otherwise, it uses `_CA_cert`, `_cert`, and
 * `_private_key` for the connection.
 *
 * @param ip The IP address of the server to connect to.
 * @param port The port number on the server to connect to.
 *
 * @return Returns 1 if the connection is established successfully, and 0
 * otherwise.
 */
 ...
 * process to the `connect` method that takes `IPAddress` and `port` as its
 * parameters.
 *
 * @param ip The IP address of the server to connect to.
 * @param port The port number on the server to connect to.
 ...
 *
 * This function determines the type of SSL connection (either with a Pre-Shared
 * Key (PSK) or with Certificates (CA)) based on the availability of the PSK
 * identifier and key. Depending on which credentials are set, it delegates to
 * the appropriate `connect` variant.
 *
 * @param host Pointer to a null-terminated string representing the hostname or
...
/**
 * @brief Establishes an SSL connection to the specified host and port with a
 * given timeout.
 *
 * This function acts as an overloaded variant of the `connect` function,
 * allowing the caller to specify a connection timeout value. It then delegates
 * the actual connection process to the other variant of the `connect` function.
 *
 * @param host    Pointer to a null-terminated string representing the hostname
 * or IP address of the server to connect to.
 * @param port    The port number on the server to connect to.
 * @param timeout The timeout value (in milliseconds) for the SSL connection. If
 * the connection is not established within this time, the attempt will fail.
 *
 * @return Returns 1 if the connection is established successfully, and 0
 * otherwise.
 */
...
/**
 * @brief Establishes an SSL connection to the specified IP address and port
 * using a certificate and private key for authentication.
 *
 * This function acts as an overloaded variant of the `connect` function,
 * accepting an `IPAddress` object instead of a host string. It then delegates
 * the actual connection process to the other variant of the `connect` function.
 *
 * @param ip            The IP address of the server to connect to, given as an
 * `IPAddress` object.
 * @param port          The port number on the server to connect to.
 * @param _CA_cert      Pointer to a null-terminated string containing the root
 * Certificate Authority (CA) certificate.
 * @param _cert         Pointer to a null-terminated string containing the
 * client's certificate.
 * @param _private_key  Pointer to a null-terminated string containing the
 * client's private key.
 *
 * @return Returns 1 if the connection is established successfully, and 0
 * otherwise.
 */
int SSLClient::connect(IPAddress ip, uint16_t port, const char *_CA_cert,
                       const char *_cert, const char *_private_key) {
 ...
 *
 * This function initializes an SSL connection to a server specified by its host
 * and port. It also accepts optional parameters for CA certificate, client
 * certificate, and client private key for SSL authentication. The function will
 * log important debugging information like timeout values and whether
 * certificates are provided.
 *
 * @param host          The hostname or IP address of the server.
 * @param port          The port number on the server to connect to.
 * @param _CA_cert      Pointer to the CA certificate for server verification.
 * NULL if not provided.
 * @param _cert         Pointer to the client certificate for client
 * authentication. NULL if not provided.
 * @param _private_key  Pointer to the private key corresponding to the client
 * certificate. NULL if not provided.
 *
 * @return 1 if the SSL connection is successfully established, 0 otherwise.
 */
int SSLClient::connect(const char *host, uint16_t port, const char *_CA_cert,
                       const char *_cert, const char *_private_key) {
  ...
  int ret = start_ssl_client(sslclient, host, port, _timeout, _CA_cert,
                             _use_ca_bundle, _cert, _private_key, NULL, NULL,
...
/**
 * @brief Establishes an SSL connection to the specified IP address and port
 * using a pre-shared key (PSK) for authentication.
 *
 * This function acts as an overloaded variant of the `connect` function,
 * accepting an `IPAddress` object instead of a host string. It then delegates
 * the actual connection process to the other variant of the `connect` function.
 ...
 * `IPAddress` object.
 * @param port     The port number on the server to connect to.
 * @param pskIdent Pointer to a null-terminated string containing the pre-shared
 * key identity.
 * @param psKey    Pointer to a null-terminated string containing the pre-shared
 * key.
 *
 * @return Returns 1 if the connection is established successfully, and 0
 * otherwise.
 */
int SSLClient::connect(IPAddress ip, uint16_t port, const char *pskIdent,
                       const char *psKey) {
  return connect(ip.toString().c_str(), port, _pskIdent, _psKey);
}

/**
 * @brief Establishes an SSL connection to the specified host and port using a
 * pre-shared key (PSK) for authentication.
 *
 * This function attempts to start an SSL client connection to a server using
 * the provided host, port, and pre-shared key details for authentication. If a
 * timeout has been set using
 * `_timeout`, it will be used for the handshake timeout.
 *
 * @param host     Pointer to a null-terminated string containing the host
 ...
 * otherwise.
 */
int SSLClient::connect(const char *host, uint16_t port, const char *pskIdent,
                       const char *psKey) {
  ...
  int ret =
    start_ssl_client(sslclient, host, port, _timeout, NULL, false, NULL, NULL,
                     _pskIdent, _psKey, _use_insecure, _alpn_protos);
  _lastError = ret;
 ...
 */
size_t SSLClient::write(uint8_t data) { return write(&data, 1); }
 ...
 * @return int  < 1 if client is connected and there is an error from
 * get_ssl_receive().
...
int SSLClient::read(uint8_t *buf, size_t size) {
  ...
  if (_peek >= 0) {
    ...
    if (!size || !avail) { // If there's no space left in the buffer (size) or
                           // no data left to read (avail)
      return 1;            // Return 1 to indicate one byte has been read.
    }
    buf++;      // Increment the buffer pointer.
    peeked = 1; // set peeked to 1 to indicate one byte has been read from the
                // peeked value.
  }
  ...
  if (res < 0) {
    stop();
    return peeked ? peeked : res; // If peeked is true return peeked, otherwise
                                  // return res, i.e. data_to_read error.
  }

  return res + peeked; // Return the number of bytes read + the number of bytes
                       // peeked.
}

/**
 * @brief Returns how many bytes of data are available to be read from the
 * sslclient.
 *
 * It takes into account both directly readable bytes and a potentially "peeked"
 * byte. If there's an error or the client is not connected, it handles these
...
int SSLClient::available() {
  int peeked = (_peek >= 0); // 1 if a byte has been peeked (available to read
 ...
 *
 * @return A byte indicating the connection status: `1` if connected, `0`
 * otherwise.
 */
 ...
 *
 * This function sets the client to use an insecure connection, which means that
 * the client will not verify the server's certificate during the SSL/TLS
 * handshake. This is useful for testing or development purposes, but it is not
 ...
 *
 * This function is used to specify the root CA certificate for the SSL/TLS
 * client. The root CA certificate is crucial for verifying the server's
 * identity during the SSL/TLS handshake. If the server's certificate is not
 * signed by this root CA or is not traceable back to this root CA, the
 ...
 *
 * This function is used to provide the client certificate that the SSL/TLS
 * client will use during the SSL/TLS handshake. When the server requests a
 * client certificate for authentication, the client provides this certificate.
 *
 ...
 * use for mutual authentication during the SSL/TLS handshake. In setups
 * requiring client authentication, the server will challenge the client to
 * prove its identity by using this private key.
 *
 ...
 *
 * This function configures the Pre-Shared Key and its associated identifier to
 * be used in the SSL/TLS handshake. PSK is an authentication method where both
 * sides of the connection (client and server) have a shared secret key,
 * eliminating the need for digital certificates.
 *
 * @param pskIdent The PSK identifier (usually a string that identifies which
 * key to use).
...
/**
 * @brief Verifies the SSL/TLS certificate against a specified fingerprint and
 * domain name.
 *
 * This function checks the certificate presented by the remote server against a
 * given fingerprint and domain name. It ensures that the communication is
 * genuine and not susceptible to a man-in-the-middle attack.
 *
 * @param fp          The expected fingerprint of the remote server's
 * certificate.
 * @param domain_name The expected domain name or Common Name (CN) of the remote
 * server.
 *
 * @return Returns `true` if the fingerprint and domain name of the remote
 * server's certificate match the provided values, otherwise `false`.
 */
 ...
 *
 * This function attempts to read the specified number of bytes from the given
 * stream into a buffer. If a previous buffer was allocated by this function, it
 * is freed before new memory is allocated. If the entire amount of requested
 * data cannot be read from the stream, the allocated buffer is freed.
 *
 * @note The function returns a pointer to a statically held character pointer,
 * which means subsequent calls to this function will affect the same memory
 * location. It's up to the caller to ensure they don't overwrite or double-free
 * this memory.
 *
 * @param stream The Stream object from which data is to be read.
 * @param size   The number of bytes to read from the stream.
 *
 * @return Returns a pointer to the allocated buffer containing the read data,
 * or nullptr if data could not be fully read or memory allocation failed.
 */
...
/**
 * @brief Loads the Certificate Authority (CA) certificate for the SSL client
 * from a provided stream.
 *
 * This function reads the CA certificate data from the given stream and sets it
 * for the SSL client. The CA certificate is used by the SSL client to verify
 * the server's certificate.
 *
 * @param stream The Stream object from which the CA certificate is read.
 * @param size   The number of bytes to read from the stream.
 *
 * @return Returns true if the CA certificate is successfully loaded and set;
 ...
 *
 * This function reads the certificate data from the given stream and sets it
 * for the SSL client. This certificate is used for client-side SSL
 * authentication.
 *
 * @param stream The Stream object from which the certificate is read.
 * @param size   The number of bytes to read from the stream.
 *
 * @return Returns true if the certificate is successfully loaded and set;
 * otherwise returns false.
 */
 ...
 *
 * This function reads the private key data from the given stream and sets it
 * for the SSL client. This key is used for client-side SSL authentication.
 *
 * @param stream The Stream object from which the private key is read.
 * @param size   The number of bytes to read from the stream.
 *
 * @return Returns true if the private key is successfully loaded and set;
 * otherwise returns false.
 */
 ...
 *
 * This function obtains a textual representation of the last SSL error
 * encountered and places it into the provided buffer. If there hasn't been an
 * error, the function will return 0.
 *
 * @param buf   Pointer to the buffer where the error description should be
 * stored.
 ...
 */
void SSLClient::setClient(Client *client) { sslclient->client = client; }
 ...
 */
void SSLClient::setTimeout(uint32_t milliseconds) { _timeout = milliseconds; }
