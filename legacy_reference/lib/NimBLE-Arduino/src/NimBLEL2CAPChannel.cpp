...
NimBLEL2CAPChannel::NimBLEL2CAPChannel(uint16_t psm, uint16_t mtu,
  ...
  NIMBLE_LOGI(LOG_TAG, "L2CAP COC 0x%04X initialized w/ L2CAP MTU %i",
              this->psm, this->mtu);
};
...
bool NimBLEL2CAPChannel::setupMemPool() {
  const size_t buf_blocks =
    CEIL_DIVIDE(mtu, L2CAP_BUF_BLOCK_SIZE) * L2CAP_BUF_SIZE_MTUS_PER_CHANNEL;
  NIMBLE_LOGD(LOG_TAG, "Computed number of buf_blocks = %d", buf_blocks);

  _coc_memory = malloc(OS_MEMPOOL_SIZE(buf_blocks, L2CAP_BUF_BLOCK_SIZE) *
                       sizeof(os_membuf_t));
  if (_coc_memory == 0) {
  ...
  auto rc = os_mempool_init(&_coc_mempool, buf_blocks, L2CAP_BUF_BLOCK_SIZE,
                            _coc_memory, "appbuf");
  if (rc != 0) {
  ...
  auto rc2 = os_mbuf_pool_init(&_coc_mbuf_pool, &_coc_mempool,
                               L2CAP_BUF_BLOCK_SIZE, buf_blocks);
  if (rc2 != 0) {
  ...
  if (!this->receiveBuffer) {
    NIMBLE_LOGE(LOG_TAG, "Can't malloc receive buffer: %d, %s", errno,
                strerror(errno));
    return false;
  }
...
int NimBLEL2CAPChannel::writeFragment(
  ...
  auto mtu =
    info.peer_coc_mtu < info.our_coc_mtu ? info.peer_coc_mtu : info.our_coc_mtu;
  ...
  while (retries--) {
    ...
    switch (res) {
    case 0:
      NIMBLE_LOGD(LOG_TAG, "L2CAP COC 0x%04X sent %d bytes.", this->psm,
                  toSend);
      return 0;
    ...
    case BLE_HS_ESTALLED:
      ...
      NIMBLE_LOGD(LOG_TAG, "L2CAP COC 0x%04X sent %d bytes.", this->psm,
                  toSend);
      NIMBLE_LOGW(LOG_TAG, "ble_l2cap_send returned BLE_HS_ESTALLED. Next send "
                           "will wait for unstalled event...");
      return 0;

    case BLE_HS_ENOMEM:
    case BLE_HS_EAGAIN:
    case BLE_HS_EBUSY:
      NIMBLE_LOGD(LOG_TAG, "ble_l2cap_send returned %d. Retrying shortly...",
                  res);
...
NimBLEL2CAPChannel *
NimBLEL2CAPChannel::connect(NimBLEClient *client, uint16_t psm, uint16_t mtu,
                            NimBLEL2CAPChannelCallbacks *callbacks) {
  if (!client->isConnected()) {
    NIMBLE_LOGE(LOG_TAG, "Client is not connected. Before connecting via "
                         "L2CAP, a GAP connection must have been established");
    return nullptr;
  };
  ...
  if (!sdu_rx) {
    NIMBLE_LOGE(LOG_TAG, "Can't allocate SDU buffer: %d, %s", errno,
                strerror(errno));
    return nullptr;
  }
  auto rc = ble_l2cap_connect(client->getConnHandle(), psm, mtu, sdu_rx,
                              NimBLEL2CAPChannel::handleL2capEvent, channel);
  if (rc != 0) {
...
int NimBLEL2CAPChannel::handleConnectionEvent(struct ble_l2cap_event *event) {
  ...
  NIMBLE_LOGI(
    ...
    psm, info.our_coc_mtu, info.our_l2cap_mtu, info.peer_coc_mtu,
    info.peer_l2cap_mtu);
  if (info.our_coc_mtu > info.peer_coc_mtu) {
    NIMBLE_LOGW(
      LOG_TAG,
      "L2CAP COC 0x%04X connected, but local MTU is bigger than remote MTU.",
      psm);
  }
  auto mtu =
    info.peer_coc_mtu < info.our_coc_mtu ? info.peer_coc_mtu : info.our_coc_mtu;
...
int NimBLEL2CAPChannel::handleDisconnectionEvent(
...
/* STATIC */
int NimBLEL2CAPChannel::handleL2capEvent(struct ble_l2cap_event *event,
                                         void *arg) {
  NIMBLE_LOGD(LOG_TAG, "handleL2capEvent: handling l2cap event %d",
              event->type);
