...
static int gatt_send(uint16_t conn_handle, const void *data, uint16_t len);
...
void gatt_disconnected_pb_gatt(struct ble_gap_conn_desc conn, uint8_t reason) {
  if (conn.role != BLE_GAP_ROLE_SLAVE || !service_registered) {
    return;
  }
...
int bt_mesh_pb_gatt_adv_start(void) {
  ...
  struct ble_gap_adv_params fast_adv_param = {ADV_OPT_PROV ADV_FAST_INT};
#if ADV_OPT_USE_NAME
  ...
  if (!prov_fast_adv) {
    struct ble_gap_adv_params slow_adv_param = {ADV_OPT_PROV ADV_SLOW_INT};
  ...
  err = bt_mesh_adv_start(&fast_adv_param, (60 * MSEC_PER_SEC), prov_ad,
                          ARRAY_SIZE(prov_ad), prov_sd, prov_sd_len);
  if (!err) {
    prov_fast_adv = false;
  }
