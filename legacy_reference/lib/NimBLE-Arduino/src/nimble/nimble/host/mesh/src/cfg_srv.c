...
static int send_app_key_status(struct bt_mesh_model *model,
  ...
  struct os_mbuf *msg =
    NET_BUF_SIMPLE(BT_MESH_MODEL_BUF_LEN(OP_APP_KEY_STATUS, 4));
...
static int app_key_add(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
...
static int app_key_del(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  uint16_t key_net_idx, key_app_idx;
  uint8_t status;
...
static int app_key_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  struct os_mbuf *msg = BT_MESH_MODEL_BUF(
    OP_APP_KEY_LIST, 3 + IDX_LEN(CONFIG_BT_MESH_APP_KEY_COUNT));
...
static int beacon_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf) {
...
static int beacon_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf) {
  int err = 0;
  struct os_mbuf *msg = BT_MESH_MODEL_BUF(OP_BEACON_STATUS, 1);
...
static int relay_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                     struct os_mbuf *buf) {
...
static int relay_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                     struct os_mbuf *buf) {
  struct os_mbuf *msg = BT_MESH_MODEL_BUF(OP_RELAY_STATUS, 2);
  int err = 0;
...
send_status:
  return send_mod_pub_status(model, ctx, elem_addr, pub_addr, vnd, mod, status,
                             mod_id);
}

static int mod_pub_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  ...
  BT_DBG("elem_addr 0x%04x pub_addr 0x%04x cred_flag %u", elem_addr, pub_addr,
         cred_flag);
  BT_DBG("pub_app_idx 0x%03x, pub_ttl %u pub_period 0x%02x", pub_app_idx,
         pub_ttl, pub_period);
...
static int send_mod_sub_status(struct bt_mesh_model *model,
  ...
  BT_DBG("status 0x%02x elem_addr 0x%04x sub_addr 0x%04x", status, elem_addr,
         sub_addr);
...
static int mod_sub_add(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
...
send_status:
  return send_mod_sub_status(model, ctx, status, elem_addr, sub_addr, mod_id,
                             vnd);
}

static int mod_sub_del(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  uint16_t elem_addr, sub_addr;
  struct bt_mesh_model *mod;
  struct bt_mesh_elem *elem;
...
static enum bt_mesh_walk mod_sub_list_visitor(struct bt_mesh_model *mod,
  ...
  for (i = 0; i < ARRAY_SIZE(mod->groups); i++) {
    ...
    if (net_buf_simple_tailroom(visit->msg) < 2 + BT_MESH_MIC_SHORT) {
  ...
  BT_DBG("sublist: model %u:%x: %u groups", mod->elem_idx, mod->id, count);

  return BT_MESH_WALK_CONTINUE;
}

static int mod_sub_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
...
static int net_key_del(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  uint16_t del_idx;
  ...
  if (ctx->net_idx == del_idx) {
    return send_net_key_status(model, ctx, del_idx, STATUS_CANNOT_REMOVE);
  }
...
static int net_key_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  struct os_mbuf *msg =
    BT_MESH_MODEL_BUF(OP_NET_KEY_LIST, IDX_LEN(CONFIG_BT_MESH_SUBNET_COUNT));
...
static int mod_app_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                       struct os_mbuf *buf) {
  struct os_mbuf *msg = NET_BUF_SIMPLE(
    MAX(BT_MESH_MODEL_BUF_LEN(OP_VND_MOD_APP_LIST, 9 + KEY_LIST_LEN),
        BT_MESH_MODEL_BUF_LEN(OP_SIG_MOD_APP_LIST, 9 + KEY_LIST_LEN)));
...
static void reset_send_end(int err, void *cb_data) { bt_mesh_reset(); }
...
static int friend_get(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf) {
  BT_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s", ctx->net_idx,
         ctx->app_idx, ctx->addr, buf->om_len,
         bt_hex(buf->om_data, buf->om_len));
...
static int friend_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf) {
  BT_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s", ctx->net_idx,
         ctx->app_idx, ctx->addr, buf->om_len,
         bt_hex(buf->om_data, buf->om_len));
...
static int heartbeat_sub_set(struct bt_mesh_model *model,
  ...
  BT_DBG("sub_src 0x%04x sub_dst 0x%04x period 0x%02x", sub_src, sub_dst,
         period_log);
...
void bt_mesh_model_reset(void) { bt_mesh_model_foreach(mod_reset, NULL); }
