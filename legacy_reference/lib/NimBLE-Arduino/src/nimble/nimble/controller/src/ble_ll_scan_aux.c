...
#define AUX_MEMPOOL_SIZE                                                       \
  (OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_LL_SCAN_AUX_SEGMENT_CNT),                    \
...
static inline uint8_t *ble_ll_scan_aux_get_own_addr(void) {
...
static int ble_ll_scan_aux_sched_cb(struct ble_ll_sched_item *sch) {
...
static struct ble_ll_scan_aux_data *ble_ll_scan_aux_alloc(void) {
...
static void ble_ll_scan_aux_free(struct ble_ll_scan_aux_data *aux) {
...
static int ble_ll_hci_ev_send_ext_adv_report(struct os_mbuf *rxpdu,
                                             struct ble_mbuf_hdr *rxhdr,
                                             struct ble_hci_ev **hci_ev) {
  struct ble_mbuf_hdr_rxinfo *rxinfo = &rxhdr->rxinfo;
  struct ble_hci_ev_le_subev_ext_adv_rpt *hci_subev;
  ...
  max_data_len = BLE_LL_MAX_EVT_LEN - sizeof(**hci_ev) - sizeof(*hci_subev) -
                 sizeof(*report);
  offset = 0;
  hci_ev_next = NULL;
  ...
  do {
     ...
     */
    if ((offset < data_len) || (rxinfo->flags & BLE_MBUF_HDR_F_AUX_PTR_WAIT)) {
...
static int ble_ll_hci_ev_send_ext_adv_report_for_aux(
  struct os_mbuf *rxpdu, struct ble_mbuf_hdr *rxhdr,
  struct ble_ll_scan_aux_data *aux, struct ble_ll_scan_addr_data *addrd) {
  struct ble_hci_ev *hci_ev;
  int rc;
...
static void ble_ll_scan_aux_break_ev(struct ble_npl_event *ev) {
...
static int ble_ll_scan_aux_phy_to_phy(uint8_t aux_phy, uint8_t *phy) {
  switch (aux_phy) {
  case 0:
    *phy = BLE_PHY_1M;
    break;
...
int ble_ll_scan_aux_sched(struct ble_ll_scan_aux_data *aux, uint32_t pdu_ticks,
  ...
  sch->end_time =
    sch->start_time + ble_ll_tmr_u2t_up(sch->remainder + 2 * aux_ww_us +
...
static int ble_ll_scan_aux_parse_to_aux_data(struct ble_ll_scan_aux_data *aux,
                                             uint8_t *rxbuf,
                                             struct ble_mbuf_hdr *rxhdr) {
  uint8_t pdu_hdr;
  uint8_t pdu_len;
...
static int ble_ll_scan_aux_ext_parse(uint8_t *rxbuf,
  ...
  if (!(eh_flags &
        ((1 << BLE_LL_EXT_ADV_ADVA_BIT) | (1 << BLE_LL_EXT_ADV_AUX_PTR_BIT)))) {
    return -1;
  }
...
int ble_ll_scan_aux_rx_isr_end_on_ext(struct ble_ll_scan_sm *scansm,
  ...
  if (do_match) {
    ...
    rc = ble_ll_scan_rx_filter(ble_ll_scan_get_own_addr_type(),
                               ble_ll_scan_get_filt_policy(), &addrd, NULL);
    if (rc < 0) {
      return -1;
    }
...
static uint8_t ble_ll_scan_aux_scan_req_tx_pdu_cb(uint8_t *dptr, void *arg,
                                                  uint8_t *hdr_byte) {
...
static bool ble_ll_scan_aux_send_scan_req(struct ble_ll_scan_aux_data *aux,
                                          struct ble_ll_scan_addr_data *addrd) {
  int rc;

  /* Check if we already scanned this device successfully */
  if (ble_ll_scan_have_rxd_scan_rsp(addrd->adv_addr, addrd->adv_addr_type, true,
                                    aux->adi)) {
    return false;
  }
...
int ble_ll_scan_aux_rx_isr_end(struct os_mbuf *rxpdu, uint8_t crcok) {
  ...
  rc = ble_ll_scan_rx_filter(ble_ll_scan_get_own_addr_type(), scan_filt_policy,
                             &addrd, &scan_ok);
  if (rc < 0) {
    rxinfo->flags |= BLE_MBUF_HDR_F_IGNORED;
    /*
...
static void ble_ll_scan_aux_sync_check(struct os_mbuf *rxpdu,
                                       struct ble_ll_scan_addr_data *addrd) {
  struct ble_mbuf_hdr *rxhdr = BLE_MBUF_HDR_PTR(rxpdu);
...
ble_ll_scan_aux_check_connect_rsp(uint8_t *rxbuf,
  ...
  if (!(eh_flags &
        ((1 << BLE_LL_EXT_ADV_ADVA_BIT) | (1 << BLE_LL_EXT_ADV_TARGETA_BIT)))) {
    return -1;
  }
...
ble_ll_scan_aux_rx_pkt_in_for_initiator(struct os_mbuf *rxpdu,
  ...
  if (ble_ll_scan_aux_check_connect_rsp(
        rxpdu->om_data, ble_ll_scan_get_pdu_data(), &addrd) < 0) {
    ble_ll_conn_send_connect_req_cancel();
    ble_ll_scan_aux_free(aux);
    ble_ll_scan_chk_resume();
    return;
...
void ble_ll_scan_aux_rx_pkt_in(struct os_mbuf *rxpdu,
                               struct ble_mbuf_hdr *rxhdr) {
  struct ble_ll_scan_addr_data addrd;
  struct ble_mbuf_hdr_rxinfo *rxinfo;
  struct ble_ll_scan_aux_data *aux;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PERIODIC_ADV)
  sync_check =
    ble_ll_sync_enabled() && !(aux->flags & BLE_LL_SCAN_AUX_F_AUX_CHAIN);
  ...
  if (!scan_duplicate) {
    rc = ble_ll_hci_ev_send_ext_adv_report_for_aux(rxpdu, rxhdr, aux, &addrd);
    ...
    if (rc == 0) {
      if (rxinfo->flags & BLE_MBUF_HDR_F_SCAN_REQ_TXD) {
        ...
        if (ble_ll_scan_get_filt_dups()) {
          ble_ll_scan_dup_update_ext(addrd.adv_addr_type, addrd.adv_addr, true,
                                     aux->adi);
        }
        if (aux->flags & BLE_LL_SCAN_AUX_F_SCANNED) {
          ble_ll_scan_add_scan_rsp_adv(addrd.adv_addr, addrd.adv_addr_type, 1,
                                       aux->adi);
        }
      }
    }
...
void ble_ll_scan_aux_init(void) {
  ...
  err = os_mempool_init(&aux_data_pool, MYNEWT_VAL(BLE_LL_SCAN_AUX_SEGMENT_CNT),
                        sizeof(struct ble_ll_scan_aux_data), aux_data_mem,
                        "ble_ll_scan_aux_data_pool");
  BLE_LL_ASSERT(err == 0);
}
