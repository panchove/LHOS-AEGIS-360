...
NimBLEAdvertising::NimBLEAdvertising()
    : m_advData{}, m_scanData{}, m_advParams{}, m_advCompCb{nullptr},
      m_slaveItvl{0}, m_duration{BLE_HS_FOREVER}, m_scanResp{false},
      m_advDataSet{false} {
...
/**
 * @brief Stops the current advertising and resets the advertising data to the
 * default values.
...
bool NimBLEAdvertising::setConnectableMode(uint8_t mode) {
  ...
  if (mode == BLE_GAP_CONN_MODE_NON) { // Non-connectable advertising doesn't
                                       // need flags.
 ...
 * @param [in] interval The advertising interval in 0.625ms units, 0 = use
 * default.
 */
 ...
 * @param [in] enable If true, scan response data will be available, false
 * disabled, default = disabled.
 * @details The scan response data is sent in response to a scan request from a
 * peer device.
 */
 ...
 * @param [in] duration The duration, in milliseconds, to advertise, 0 ==
 * advertise forever.
...
bool NimBLEAdvertising::start(uint32_t duration, const NimBLEAddress *dirAddr) {
  NIMBLE_LOGD(LOG_TAG, ">> Advertising start: duration=%" PRIu32 ", dirAddr=%s",
              duration, dirAddr ? dirAddr->toString().c_str() : "NULL");
  ...
  // Save the duration incase of host reset so we can restart with the same
  // params
  m_duration = duration;
  if (duration == 0) {
  ...
  int rc = ble_gap_adv_start(
    ...
    (dirAddr != nullptr) ? dirAddr->getBase() : NULL, duration, &m_advParams,
    (pServer != nullptr) ? NimBLEServer::handleGapEvent
                         : NimBLEAdvertising::handleGapEvent,
    this);
#else
  int rc =
    ble_gap_adv_start(NimBLEDevice::m_ownAddrType, NULL, duration, &m_advParams,
                      NimBLEAdvertising::handleGapEvent, this);
#endif
...
bool NimBLEAdvertising::stop() {
  ...
  if (rc != 0 && rc != BLE_HS_EALREADY) {
    NIMBLE_LOGE(LOG_TAG, "ble_gap_adv_stop rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
 ...
 */
void NimBLEAdvertising::setAdvertisingCompleteCallback(
  advCompleteCB_t callback) {
...
/**
 * @brief Set the advertisement data that is to be broadcast in a regular
 * advertisement.
 * @param [in] data The data to be broadcast.
 * @return True if the data was set successfully.
 */
bool NimBLEAdvertising::setAdvertisementData(
  const NimBLEAdvertisementData &data) {
  int rc =
    ble_gap_adv_set_data(&data.getPayload()[0], data.getPayload().size());
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gap_adv_set_data: %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
  ...
  m_advDataSet = true; // Set the flag that indicates the data was set already
                       // so we don't set it again.
  return true;
 ...
 * @return True if the data was set successfully.
 * @details The scan response data is sent in response to a scan request from a
 * peer device. If this is set without setting the advertisement data when
 * advertising starts this may be overwritten.
 */
bool NimBLEAdvertising::setScanResponseData(
  const NimBLEAdvertisementData &data) {
  int rc =
    ble_gap_adv_rsp_set_data(&data.getPayload()[0], data.getPayload().size());
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gap_adv_rsp_set_data: %d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
  ...
  m_scanData =
    data; // copy the data into the member object in case this is custom.
  return true;
} // setScanResponseData
...
/**
 * @brief Clear the advertisement and scan response data and set the flags to
 ...
 * If the appearance value is 0 then the appearance will not be in the
 * advertisement.
 ...
 * @details If scan response is enabled the name will be set in the scan
 * response data.
 */
