...
struct ble_gatts_aware_state
  ble_gatts_conn_aware_states[MYNEWT_VAL(BLE_STORE_MAX_BONDS)];
...
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
static struct ble_gatts_svc_entry *ble_gatts_svc_entry_alloc(void) {
...
static void ble_gatts_svc_entry_free(struct ble_gatts_svc_entry *entry) {
...
static struct ble_gatts_clt_cfg *ble_gatts_clt_cfg_alloc(void) {
...
static void ble_gatts_clt_cfg_free(struct ble_gatts_clt_cfg *cfg) {
...
void ble_gatts_set_clt_cfg_perm_flags(uint8_t flags) { perm_flags = flags; }
...
static uint8_t ble_gatts_chr_properties(const struct ble_gatt_chr_def *chr) {
  ...
  if (chr->flags & (BLE_GATT_CHR_F_RELIABLE_WRITE | BLE_GATT_CHR_F_AUX_WRITE)) {
...
static bool ble_gatts_mutable(void) {
  ...
  if (ble_gap_adv_active() || ble_gap_disc_active() || ble_gap_conn_active()) {
    return false;
  }
...
static int ble_gatts_val_access(uint16_t conn_handle, uint16_t attr_handle,
                                ...
                                struct os_mbuf **om,
                                ble_gatt_access_fn *access_cb, void *cb_arg) {
...
static int ble_gatts_cpfd_is_sane(const struct ble_gatt_cpfd *cpfd) {
  ...
  if ((cpfd->unit < 0x2700) ||
      ...
      (cpfd->unit == 0x27BB) || (cpfd->unit > 0x27C8)) {
    return 0;
  }

  if ((cpfd->name_space == BLE_GATT_CHR_NAMESPACE_BT_SIG) &&
      (cpfd->description > 0x0110)) {
    return 0;
  }
...
#else
static int ble_gatts_clt_cfg_find_idx(struct ble_gatts_clt_cfg *cfgs,
                                      uint16_t chr_val_handle) {
  struct ble_gatts_clt_cfg *cfg;
  int i;
...
static void ble_gatts_subscribe_event(uint16_t conn_handle,
                                      uint16_t attr_handle, uint8_t reason,
                                      uint8_t prev_flags, uint8_t cur_flags) {
  if ((prev_flags ^ cur_flags) & ~BLE_GATTS_CLT_CFG_F_RESERVED) {
    ble_gap_subscribe_event(conn_handle, attr_handle, reason,
 ...
 */
static int ble_gatts_clt_cfg_access_locked(
  struct ble_hs_conn *conn, uint16_t attr_handle, uint8_t att_op,
  uint16_t offset, struct os_mbuf *om, struct ble_store_value_cccd *out_cccd,
  uint8_t *out_prev_clt_cfg_flags, uint8_t *out_cur_clt_cfg_flags) {
...
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
  clt_cfg =
    ble_gatts_clt_cfg_find(&conn->bhc_gatt_svr.clt_cfgs, chr_val_handle);
#else
...
int ble_gatts_clt_cfg_access(uint16_t conn_handle, uint16_t attr_handle,
  ...
  if (conn == NULL) {
    ...
    rc = ble_gatts_clt_cfg_access_locked(conn, attr_handle, op, offset, *om,
                                         &cccd_value, &prev_flags, &cur_flags);
  }
  ...
  ble_gatts_subscribe_event(conn_handle, chr_val_handle,
                            BLE_GAP_SUBSCRIBE_REASON_WRITE, prev_flags,
                            cur_flags);
...
static int ble_gatts_register_clt_cfg_dsc(uint16_t *att_handle) {
  int rc;
  rc = ble_att_svr_register(uuid_ccc, perm_flags, 0, att_handle,
                            ble_gatts_clt_cfg_access, NULL);
  if (rc != 0) {
    return rc;
  }
...
static int ble_gatts_cafd_access(uint16_t conn_handle, uint16_t attr_handle,
                                 uint8_t op, uint16_t offset,
                                 struct os_mbuf **om, void *arg) {
...
static int ble_gatts_cpfd_access(uint16_t conn_handle, uint16_t attr_handle,
                                 uint8_t op, uint16_t offset,
                                 struct os_mbuf **om, void *arg) {
...
static int ble_gatts_register_cpfds(const struct ble_gatt_cpfd *cpfds) {
  int idx;
  int rc;
  ...
  if (idx > 1) {
     ...
     */
    rc =
      ble_att_svr_register(uuid_caf, BLE_ATT_F_READ, 0, NULL,
...
static int ble_gatts_register_chr(const struct ble_gatt_svc_def *svc,
  ...
  rc = ble_att_svr_register(chr->uuid, att_flags, chr->min_key_size,
                            &val_handle, ble_gatts_chr_val_access, (void *)chr);
  if (rc != 0) {
    return rc;
  }
  ...
  if (chr->descriptors != NULL) {
    for (dsc = chr->descriptors; dsc->uuid != NULL; dsc++) {
      rc =
        ble_gatts_register_dsc(svc, chr, dsc, def_handle, register_cb, cb_arg);
      if (rc != 0) {
        return rc;
      }
    }
...
static int ble_gatts_svc_type_to_uuid(uint8_t svc_type,
...
static int ble_gatts_register_round(int *out_num_registered,
...
void ble_gatts_connection_broken(uint16_t conn_handle) {
  ...
  if (conn != NULL) {
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
    memcpy(&clt_cfgs, &(conn->bhc_gatt_svr.clt_cfgs),
           sizeof(struct ble_gatts_clt_cfg_list));
      ...
      for (i = 0; i < MYNEWT_VAL(BLE_STORE_MAX_BONDS); i++) {
        if (memcmp(ble_gatts_conn_aware_states[i].peer_id_addr,
          ...
          if (conn->bhc_gatt_svr.half_aware) {
            ...
            ble_gatts_conn_aware_states[i].aware =
              conn->bhc_gatt_svr.aware_state;
  ...
  if (STAILQ_FIRST(&clt_cfgs) != NULL) {
    ...
    while ((clt_cfg = STAILQ_FIRST(&clt_cfgs)) != NULL) {
      ble_gatts_subscribe_event(conn_handle, clt_cfg->chr_val_handle,
                                BLE_GAP_SUBSCRIBE_REASON_TERM, clt_cfg->flags,
                                0);
...
int ble_gatts_start(void) {
...
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
    ble_gatts_svc_entry_mem = nimble_platform_mem_malloc(
      ble_hs_max_services * sizeof(struct ble_gatts_svc_entry));
    if (ble_gatts_svc_entry_mem == NULL) {
#else
    ble_gatts_svc_entries =
#ifdef ESP_PLATFORM
      nimble_platform_mem_malloc(ble_hs_max_services *
                                 sizeof *ble_gatts_svc_entries);
#else
  ...
  rc = os_mempool_init(&ble_gatts_clt_cfg_pool, num_elems,
                       sizeof(struct ble_gatts_clt_cfg), ble_gatts_clt_cfg_mem,
...
int ble_gatts_conn_init(struct ble_gatts_conn *gatts_conn) {
  ...
  if (ble_gatts_num_cfgable_chrs > 0) {
    ...
    memcpy(&(gatts_conn->clt_cfgs), &clt_cfgs,
           sizeof(struct ble_gatts_clt_cfg_list));
...
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
  clt_cfg = ble_gatts_clt_cfg_find(&ble_gatts_clt_cfgs, chr_val_handle);
  if (clt_cfg == NULL) {
#else
...
int ble_gatts_peer_cl_sup_feat_get(uint16_t conn_handle,
                                   uint8_t *out_supported_feat, uint8_t len) {
  struct ble_hs_conn *conn;
  int rc = 0;
...
void ble_gatts_bonding_established(uint16_t conn_handle) {
  ...
  memcpy(csfc.csfc, conn->bhc_gatt_svr.peer_cl_sup_feat,
         MYNEWT_VAL(BLE_GATT_CSFC_SIZE));
  ...
  /* store the bonded peer aware_state
     ...
     oldest bond */
  ble_hs_conn_addrs(conn, &addrs);
  new_idx = (last_conn_aware_state_index + 1) % MYNEWT_VAL(BLE_STORE_MAX_BONDS);
  ...
  ble_gatts_conn_aware_states[new_idx].half_aware =
    conn->bhc_gatt_svr.half_aware;
...
void ble_gatts_bonding_restored(uint16_t conn_handle) {
  ...
  memcpy(conn->bhc_gatt_svr.peer_cl_sup_feat, csfc_value.csfc,
         MYNEWT_VAL(BLE_GATT_CSFC_SIZE));
}
...
ble_gatts_find_svc_chr_attr(const ble_uuid_t *svc_uuid,
  ...
  while (1) {
    ...
    if (ble_uuid_u16(cur->ha_uuid) == BLE_ATT_UUID_CHARACTERISTIC &&
        next != NULL && ble_uuid_cmp(next->ha_uuid, chr_uuid) == 0) {
...
int ble_gatts_find_dsc(const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid,
  ...
  rc = ble_gatts_find_svc_chr_attr(svc_uuid, chr_uuid, &svc_entry, &att_chr);
  if (rc != 0) {
    return rc;
  }
...
#if MYNEWT_VAL(BLE_DYNAMIC_SERVICE)
static void ble_gatts_add_clt_cfg(struct ble_gatts_clt_cfg_list *clt_cfgs,
                                  uint16_t chr_val_handle,
                                  uint16_t allowed_flags, uint8_t flags) {
  struct ble_gatts_clt_cfg *cfg;
...
static void ble_gatts_remove_clt_cfg(struct ble_gatts_clt_cfg_list *clt_cfgs,
                                     uint16_t chr_val_handle) {
  struct ble_gatts_clt_cfg *cfg;
...
#if MYNEWT_VAL(BLE_GATT_CACHING)
static int ble_gatts_conn_unaware(struct ble_hs_conn *conn, void *arg) {
...
static int ble_gatts_update_conn_clt_cfg(struct ble_hs_conn *conn, void *arg) {
  ...
  case 2:
    /* removed */
    ble_gatts_remove_clt_cfg(&conn->bhc_gatt_svr.clt_cfgs, chr_val_handle);
    (conn->bhc_gatt_svr.num_clt_cfgs)--;
    return 0;
  default:
    BLE_HS_DBG_ASSERT(0);
...
static struct ble_gatts_clt_cfg *
ble_gatts_get_last_cfg(struct ble_gatts_clt_cfg_list *ble_gatts_clt_cfgs) {
  struct ble_gatts_clt_cfg *cfg, *prev;
  prev = NULL;
  STAILQ_FOREACH(cfg, ble_gatts_clt_cfgs, next) { prev = cfg; }
  return prev;
}
int ble_gatts_add_dynamic_svcs(const struct ble_gatt_svc_def *svcs) {
  ...
  rc =
    ble_gatts_register_svcs(ble_gatts_svc_defs[0], ble_hs_cfg.gatts_register_cb,
                            ble_hs_cfg.gatts_register_arg);
  if (rc != 0) {
    goto done;
#if BLE_HS_DEBUG
  ...
  while ((ha = ble_att_svr_find_by_uuid(ha, &uuid.u, 0xffff)) != NULL) {
    ...
    allowed_flags = ble_gatts_chr_clt_cfg_allowed(chr);
    if (allowed_flags != 0) {
      ble_gatts_add_clt_cfg(&ble_gatts_clt_cfgs, ha->ha_handle_id + 1,
                            allowed_flags, 0);
      /* update connections */
...
static int ble_gatts_deregister_svc(const ble_uuid_t *uuid) {
  int rc;
...
static int ble_gatts_remove_svc_entry(const ble_uuid_t *uuid) {
  struct ble_gatts_svc_entry *entry;
...
int ble_gatts_delete_svc(const ble_uuid_t *uuid) {
  ...
  while ((ha = ble_att_svr_find_by_uuid(ha, &uuid_chr.u,
                                        entry->end_group_handle)) != NULL) {
    chr = ha->ha_cb_arg;
    allowed_flags = ble_gatts_chr_clt_cfg_allowed(chr);
    if (allowed_flags != 0) {
    ...
    if (svc->characteristics != NULL) {
      for (c = 0; svc->characteristics[c].uuid != NULL; c++) {
        ...
        if (chr->cpfd != NULL) {
          ...
          /** If more than one CPFD is present for a characteristic, one CAFD is
           * required. */
          if (pf > 1) {
            res->cafds++;
            res->dscs++;
...
int ble_gatts_get_cfgable_chrs(void) { return ble_gatts_num_cfgable_chrs; }
...
void ble_gatts_lcl_svc_foreach(ble_gatt_svc_foreach_fn cb, void *arg) {
  ...
  STAILQ_FOREACH(entry, &ble_gatts_svc_entries, next) {
    cb(entry->svc, entry->handle, entry->end_group_handle, arg);
  }
#else
  int i;

  for (i = 0; i < ble_gatts_num_svc_entries; i++) {
    cb(ble_gatts_svc_entries[i].svc, ble_gatts_svc_entries[i].handle,
...
uint8_t ble_gatts_security_mode_1_level() {
...
int ble_gatts_init(void) {
  ...
  rc = stats_init_and_reg(STATS_HDR(ble_gatts_stats),
                          STATS_SIZE_INIT_PARMS(ble_gatts_stats, STATS_SIZE_32),
