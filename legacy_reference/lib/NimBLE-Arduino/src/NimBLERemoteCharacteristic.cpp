...
NimBLERemoteCharacteristic::NimBLERemoteCharacteristic(
  const NimBLERemoteService *svc, const ble_gatt_chr *chr)
    : NimBLERemoteValueAttribute{chr->uuid, chr->val_handle},
      m_pRemoteService{svc}, m_properties{chr->properties}, m_notifyCallback{},
...
/**
 * @brief Callback used by the API when a descriptor is discovered or search
 * complete.
 */
int NimBLERemoteCharacteristic::descriptorDiscCB(uint16_t connHandle,
  ...
  NIMBLE_LOGD(LOG_TAG, "Descriptor Discovery >> status: %d handle: %d", rc,
              (rc == 0) ? dsc->handle : -1);
 ...
 * @param [in] pFilter Pointer to a filter containing pointers to descriptor,
 * UUID, and task data.
 * @return True if successfully retrieved, success = BLE_HS_EDONE.
 */
bool NimBLERemoteCharacteristic::retrieveDescriptors(
  NimBLEDescriptorFilter *pFilter) const {
  NIMBLE_LOGD(LOG_TAG, ">> retrieveDescriptors() for characteristic: %s",
              getUUID().toString().c_str());

  const auto pSvc = getRemoteService();
  uint16_t endHandle = pSvc->getEndHandle();

  // Find the handle of the next characteristic to limit the descriptor search
  // range.
  ...
  for (auto it = chars.begin(); it != chars.end(); ++it) {
    if ((*it)->getHandle() == this->getHandle()) {
      ...
      if (next_it != chars.end()) {
        ...
        NIMBLE_LOGD(LOG_TAG, "Search range limited to handle 0x%04X",
                    endHandle);
      }
      break;
    }
  ...
  int rc = ble_gattc_disc_all_dscs(
    getClient()->getConnHandle(), getHandle(), endHandle,
    NimBLERemoteCharacteristic::descriptorDiscCB, pFilter);
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gattc_disc_all_dscs: rc=%d %s", rc,
  ...
  if (rc != BLE_HS_EDONE) {
    NIMBLE_LOGE(LOG_TAG, "<< retrieveDescriptors(): failed: rc=%d %s", rc,
                NimBLEUtils::returnCodeToString(rc));
    return false;
  }
  ...
  NIMBLE_LOGD(LOG_TAG, "<< retrieveDescriptors(): found %d descriptors.",
              m_vDescriptors.size() - prevDscCount);
  return true;
} // retrieveDescriptors

/**
 * @brief Get the descriptor instance with the given UUID that belongs to this
 * characteristic.
 * @param [in] uuid The UUID of the descriptor to find.
 * @return The Remote descriptor (if present) or nullptr if not present.
 */
NimBLERemoteDescriptor *
NimBLERemoteCharacteristic::getDescriptor(const NimBLEUUID &uuid) const {
 ...
 * @param [in] refresh If true the current descriptor vector will be cleared
 * and\n all descriptors for this characteristic retrieved from the
 * peripheral.\n If false the vector will be returned with the currently stored
...
NimBLERemoteCharacteristic::getDescriptors(bool refresh) const {
  if (refresh) {
    deleteDescriptors();
    retrieveDescriptors();
  }
...
/**
 * @brief Get iterator to the beginning of the vector of remote descriptor
 * pointers.
 * @return An iterator to the beginning of the vector of remote descriptor
 * pointers.
 */
std::vector<NimBLERemoteDescriptor *>::iterator
NimBLERemoteCharacteristic::begin() const {
 ...
 */
std::vector<NimBLERemoteDescriptor *>::iterator
NimBLERemoteCharacteristic::end() const {
 ...
 */
const NimBLERemoteService *
NimBLERemoteCharacteristic::getRemoteService() const {
  return m_pRemoteService;
} // getRemoteService

/**
 * @brief Subscribe or unsubscribe for notifications or indications.
 * @param [in] val 0x00 to unsubscribe, 0x01 for notifications, 0x02 for
 * indications.
 ...
 */
bool NimBLERemoteCharacteristic::setNotify(uint16_t val,
 ...
 * @param [in] notifications If true, subscribe for notifications, false
 * subscribe for indications.
 * @param [in] notifyCallback A callback to be invoked for a notification.
 * @param [in] response If true, require a write response from the descriptor
 * write operation. If NULL is provided then no callback is performed.
 * @return false if writing to the descriptor failed.
 */
bool NimBLERemoteCharacteristic::subscribe(bool notifications,
                                           notify_callback notifyCallback,
                                           bool response) const {
 ...
 * @param [in] response bool if true, require a write response from the
 ...
 * @details We maintain a vector called m_vDescriptors that contains pointers to
 * NimBLERemoteDescriptors object references. Since we allocated these in this
 * class, we are also responsible for deleting them. This method does just that.
 */
 ...
 */
size_t
NimBLERemoteCharacteristic::deleteDescriptor(const NimBLEUUID &uuid) const {
