 ...
 * message.
 */
#define BLE_HS_LOG(lvl, ...) BLE_HS_LOG_##lvl(__VA_ARGS__)
 ...
 *  "XX:XX:XX:XX:XX:XX" at different severity levels, such as DEBUG, INFO, WARN,
 * ERROR or CRITICAL.
 *
 *  @param lvl          The log level of the message.
...
#define BLE_HS_LOG_ADDR(lvl, addr)                                             \
  BLE_HS_LOG_##lvl("%02x:%02x:%02x:%02x:%02x:%02x", (addr)[5], (addr)[4],      \
