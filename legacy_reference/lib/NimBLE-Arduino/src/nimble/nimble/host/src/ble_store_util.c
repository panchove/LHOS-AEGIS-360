...
static int ble_store_util_iter_unique_peer(int obj_type,
  ...
  BLE_HS_DBG_ASSERT(obj_type == BLE_STORE_OBJ_TYPE_OUR_SEC ||
                    ...
                    obj_type == BLE_STORE_OBJ_TYPE_LOCAL_IRK);
...
int ble_store_util_bonded_peers(ble_addr_t *out_peer_id_addrs,
  ...
  rc = ble_store_iterate(BLE_STORE_OBJ_TYPE_OUR_SEC,
                         ble_store_util_iter_unique_peer, &set);
  if (rc != 0) {
    return rc;
  }
...
static int ble_store_util_iter_count(int obj_type, union ble_store_value *val,
                                     void *arg) {
  int *count;
...
int ble_store_util_count(int type, int *out_count) {
  ...
  *out_count = 0;
  rc = ble_store_iterate(type, ble_store_util_iter_count, out_count);
  if (rc != 0) {
    return rc;
  }
...
int ble_store_util_delete_oldest_peer(void) {
  ...
  rc = ble_store_util_bonded_peers(
    peer_id_addrs, &num_peers, sizeof peer_id_addrs / sizeof peer_id_addrs[0]);
  if (rc != 0) {
    return rc;
  }
 ...
 *                              Other nonzero on error.
 */
int ble_store_util_ead_peers(ble_addr_t *out_peer_id_addrs, int *out_num_peers,
                             int max_peers) {
  struct ble_store_util_peer_set set = {
    .peer_id_addrs = out_peer_id_addrs,
    .num_peers = 0,
...
int ble_store_util_delete_ead_oldest_peer(void) {
  ...
  rc = ble_store_util_ead_peers(peer_id_addrs, &num_peers,
