...
public:
  ...
  bool connect(bool deleteAttributes = true, bool asyncConnect = false,
               bool exchangeMTU = true);
  ...
  NimBLEAttValue getValue(const NimBLEUUID &serviceUUID,
  ...
  struct Config {
    uint8_t deleteCallbacks : 1; // Delete the callback object when the client
                                 // is deleted.
    uint8_t deleteOnDisconnect : 1;  // Delete the client when disconnected.
    uint8_t deleteOnConnectFail : 1; // Delete the client when a connection
                                     // attempt fails.
...
public:
   ...
   * @param [in] pClient A pointer to the calling client object.
   * @param [in] params A pointer to the struct containing the connection
   * parameters requested.
   * @return True to accept the parameters.
   */
  virtual bool onConnParamsUpdateRequest(NimBLEClient *pClient,
   ...
   * @param [in] connInfo A reference to a NimBLEConnInfo instance containing
   * the peer info.\n This can be used to check the status of the connection
   * encryption/pairing.
   */
   ...
   * @param [in] pClient A pointer to the client that the MTU change is
   * associated with.
