 ...
 */
  ...
  (defined(NRF52_SERIES) || defined(NRF53_SERIES))
...
#define NRF_PCNF0                                                              \
  (NRF_LFLEN_BITS << RADIO_PCNF0_LFLEN_Pos) | (RADIO_PCNF0_S1INCL_Msk) |       \
    (NRF_S0LEN << RADIO_PCNF0_S0LEN_Pos) |                                     \
    (NRF_S1LEN_BITS << RADIO_PCNF0_S1LEN_Pos)
#define NRF_PCNF0_1M                                                           \
  (NRF_PCNF0) | (RADIO_PCNF0_PLEN_8bit << RADIO_PCNF0_PLEN_Pos)
#define NRF_PCNF0_2M                                                           \
  (NRF_PCNF0) | (RADIO_PCNF0_PLEN_16bit << RADIO_PCNF0_PLEN_Pos)
#define NRF_PCNF0_CODED                                                        \
  (NRF_PCNF0) | (RADIO_PCNF0_PLEN_LongRange << RADIO_PCNF0_PLEN_Pos) |         \
...
static void phy_nrf52_errata_191(uint8_t new_phy_mode) {
  ...
  if (to_coded) {
    *(volatile uint32_t *)0x40001740 =
      ((*((volatile uint32_t *)0x40001740)) & 0x7fff00ff) | 0x80000000 |
      (((uint32_t)(196)) << 8);
  } else {
    *(volatile uint32_t *)0x40001740 =
...
#else
static uint32_t ble_phy_mode_pdu_start_off(int phy_mode) { return 40; }
#endif
...
void ble_phy_rxpdu_copy(uint8_t *dptr, struct os_mbuf *rxpdu) {
  ...
  while (true) {
    ...
    __asm__ volatile(".syntax unified              \n"
                     ...
                     "   adds %[src], %[src], r4   \n"
                     "   adds %[dst], %[dst], r4   \n"
                     : [dst] "+r"(dst), [src] "+r"(src), [len] "+r"(copy_len)
                     :
                     : "r3", "r4", "memory");
#endif
...
#if MYNEWT_VAL(BLE_PHY_VARIABLE_TIFS)
void ble_phy_tifs_set(uint16_t tifs) { g_ble_phy_data.tifs = tifs; }
#endif
...
static int ble_phy_set_start_now(void) {
  ...
  if (MYNEWT_VAL(BLE_FEM_LNA_TURN_ON_US) > BLE_PHY_T_RXENFAST) {
    radio_rem_us = 1 + MYNEWT_VAL(BLE_FEM_LNA_TURN_ON_US) - BLE_PHY_T_RXENFAST;
    fem_rem_us = 1;
  } else {
    radio_rem_us = 1;
    fem_rem_us = 1 + BLE_PHY_T_RXENFAST - MYNEWT_VAL(BLE_FEM_LNA_TURN_ON_US);
  }
#else
  radio_rem_us = 1;
#endif
...
static bool ble_phy_rx_start_isr(void) {
  ...
  if (g_ble_phy_data.phy_privacy && (dptr[3] & 0x40)) {
    ...
    nrf_radio_bcc_set(
      NRF_RADIO, (BLE_LL_PDU_HDR_LEN + adva_offset + BLE_DEV_ADDR_LEN) * 8 +
  ...
  rc =
    ble_ll_rx_start(dptr + 3, g_ble_phy_data.phy_chan, &g_ble_phy_data.rxhdr);
  if (rc >= 0) {
...
int ble_phy_init(void) {
  ...
  for (uint32_t index = 0;
       index < 32ul && NRF_FICR_NS->TRIMCNF[index].ADDR != 0xFFFFFFFFul;
       index++) {
  ...
  *(volatile uint32_t *)(NRF_RADIO_NS_BASE + 0x774) =
    (*(volatile uint32_t *)(NRF_RADIO_NS_BASE + 0x774) & 0xfffffffe) |
    0x01000000;
  ...
  NRF_RADIO->PCNF1 =
    NRF_MAXLEN | (RADIO_PCNF1_ENDIAN_Little << RADIO_PCNF1_ENDIAN_Pos) |
  ...
  if (!g_ble_phy_data.phy_stats_initialized) {
    rc = stats_init_and_reg(STATS_HDR(ble_phy_stats),
                            STATS_SIZE_INIT_PARMS(ble_phy_stats, STATS_SIZE_32),
                            STATS_NAME_INIT_PARMS(ble_phy_stats), "ble_phy");
    assert(rc == 0);
 ...
 */
int ble_phy_tx_power_round(int dbm) { return phy_txpower_round(dbm); }
...
static int ble_phy_set_access_addr(uint32_t access_addr) {
  ...
  NRF52_ERRATA_106_ENABLE_WORKAROUND || NRF52_ERRATA_107_ENABLE_WORKAROUND
#ifndef BABBLESIM
  if (nrf52_errata_102() || nrf52_errata_106() || nrf52_errata_107()) {
     ...
     * [107] RADIO: Immediate address match for access addresses containing MSBs
     * 0x00
     */
 ...
 */
int ble_phy_tx_power_get(void) { return g_ble_phy_data.phy_txpwr_dbm; }
...
uint8_t ble_phy_chan_get(void) { return g_ble_phy_data.phy_chan; }
 ...
 */
int ble_phy_state_get(void) { return g_ble_phy_data.phy_state; }
 ...
 * @return int
 */
int ble_phy_rx_started(void) { return g_ble_phy_data.phy_rx_started; }
 ...
 */
uint8_t ble_phy_max_data_pdu_pyld(void) { return BLE_LL_DATA_PDU_MAX_PYLD; }
...
void ble_phy_resolv_list_disable(void) { g_ble_phy_data.phy_privacy = 0; }
#endif
