...
int ble_att_clt_rx_error(uint16_t conn_handle, uint16_t cid,
                         struct os_mbuf **rxom) {
...
int ble_att_clt_rx_mtu(uint16_t conn_handle, uint16_t cid,
                       struct os_mbuf **rxom) {
...
int ble_att_clt_tx_find_info(uint16_t conn_handle, uint16_t cid,
...
int ble_att_clt_rx_find_info(uint16_t conn_handle, uint16_t cid,
                             struct os_mbuf **om) {
#if !NIMBLE_BLE_ATT_CLT_FIND_INFO
  return BLE_HS_ENOTSUP;
#endif
 ...
 * anyway
 */
int ble_att_clt_tx_find_type_value(uint16_t conn_handle, uint16_t cid,
                                   uint16_t start_handle, uint16_t end_handle,
  ...
  req = ble_att_cmd_get(BLE_ATT_OP_FIND_TYPE_VALUE_REQ,
                        sizeof(*req) + value_len, &txom);
  if (req == NULL) {
    return BLE_HS_ENOMEM;
  }
...
static int ble_att_clt_parse_find_type_value_hinfo(
...
int ble_att_clt_tx_read_type(uint16_t conn_handle, uint16_t cid,
                             uint16_t start_handle, uint16_t end_handle,
...
int ble_att_clt_rx_read_type(uint16_t conn_handle, uint16_t cid,
                             struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_READ_TYPE
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_rx_read(uint16_t conn_handle, uint16_t cid,
                        struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_READ
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_tx_read_blob(uint16_t conn_handle, uint16_t cid,
...
int ble_att_clt_rx_read_blob(uint16_t conn_handle, uint16_t cid,
                             struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_READ_BLOB
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_tx_read_mult(uint16_t conn_handle, uint16_t cid,
  ...
  req = ble_att_cmd_get(op, sizeof(req->handles[0]) * num_handles, &txom);
  if (req == NULL) {
    return BLE_HS_ENOMEM;
  }
...
int ble_att_clt_rx_read_mult(uint16_t conn_handle, uint16_t cid,
                             struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_READ_MULT
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_rx_read_mult_var(uint16_t conn_handle, uint16_t cid,
                                 struct os_mbuf **rxom) {
...
int ble_att_clt_rx_write(uint16_t conn_handle, uint16_t cid,
                         struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_WRITE
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_tx_signed_write_cmd(uint16_t conn_handle, uint16_t cid,
  ...
  cmd = ble_att_cmd_get(BLE_ATT_OP_SIGNED_WRITE_CMD, sizeof(*cmd), &txom2);
  if (cmd == NULL) {
    rc = BLE_HS_ENOMEM;
    goto err;
  }
   ...
   */
  len =
    BLE_ATT_SIGNED_WRITE_DATA_OFFSET + OS_MBUF_PKTLEN(txom) + sizeof(counter);
  ...
  /** Copying message */
  rc = os_mbuf_copydata(txom, 0, OS_MBUF_PKTLEN(txom),
                        &message[BLE_ATT_SIGNED_WRITE_DATA_OFFSET]);
  if (rc != 0) {
    goto err;
  }

  /** Copying sign counter */
  memcpy(&message[BLE_ATT_SIGNED_WRITE_DATA_OFFSET + OS_MBUF_PKTLEN(txom)],
  ...
  rc = os_mbuf_copyinto(txom, OS_MBUF_PKTLEN(txom), cmac + (sizeof(cmac) / 2),
                        sizeof(cmac) / 2);
  if (rc != 0) {
    goto err;
  }
...
int ble_att_clt_rx_exec_write(uint16_t conn_handle, uint16_t cid,
                              struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_EXEC_WRITE
  return BLE_HS_ENOTSUP;
#endif
...
int ble_att_clt_tx_indicate(uint16_t conn_handle, uint16_t cid, uint16_t handle,
                            struct os_mbuf *txom) {
...
int ble_att_clt_rx_indicate(uint16_t conn_handle, uint16_t cid,
                            struct os_mbuf **rxom) {
#if !NIMBLE_BLE_ATT_CLT_INDICATE
  return BLE_HS_ENOTSUP;
#endif
