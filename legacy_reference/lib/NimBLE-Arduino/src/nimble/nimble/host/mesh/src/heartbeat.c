...
static int64_t sub_remaining(void) {
  ...
  uint32_t rem_ms =
    k_ticks_to_ms_floor32(k_work_delayable_remaining_get(&sub_timer));
...
static void sub_end(struct ble_npl_event *work) { notify_sub_end(); }
...
static int heartbeat_send(const struct bt_mesh_send_cb *cb, void *cb_data) {
  ...
  return bt_mesh_ctl_send(&tx, TRANS_CTL_OP_HEARTBEAT, &hb, sizeof(hb), cb,
                          cb_data);
}
...
int bt_mesh_hb_recv(struct bt_mesh_net_rx *rx, struct os_mbuf *buf) {
  ...
  BT_DBG("src 0x%04x TTL %u InitTTL %u (%u hop%s) feat 0x%04x", rx->ctx.addr,
...
void bt_mesh_hb_pub_get(struct bt_mesh_hb_pub *get) { *get = pub; }
...
void bt_mesh_hb_sub_reset_count(void) { sub.count = 0; }
...
void bt_mesh_hb_suspend(void) { (void)k_work_cancel_delayable(&pub_timer); }
...
static int hb_pub_set(int argc, char **argv, char *val) {
  ...
  if (len != sizeof(hb_val)) {
    BT_ERR("Unexpected value length (%d != %zu)", len, sizeof(hb_val));
    return -EINVAL;
  }
  pub.dst = hb_val.dst;
