...
static uint16_t
  ble_hs_flow_mbuf_conn_handle[MYNEWT_VAL(BLE_TRANSPORT_ACL_FROM_LL_COUNT)];

static inline int ble_hs_flow_mbuf_index(const struct os_mbuf *om) {
...
static int ble_hs_flow_tx_num_comp_pkts(void) {
  uint8_t buf[sizeof(struct ble_hci_cb_host_num_comp_pkts_cp) +
  ...
  for (conn = ble_hs_conn_first(); conn != NULL;
    ...
    if (conn->bhc_completed_pkts > 0) {
       ...
       */
      rc =
        ble_hs_hci_cmd_tx_no_rsp(BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND,
...
static void ble_hs_flow_event_cb(struct ble_npl_event *ev) {
  int rc;
...
static void ble_hs_flow_inc_completed_pkts(struct ble_hs_conn *conn) {
  uint16_t num_free;
  ...
  if (ble_hs_flow_num_completed_pkts >
...
static os_error_t ble_hs_flow_acl_free(struct os_mempool_ext *mpe, void *data,
                                       void *arg) {
  struct ble_hs_conn *conn;
...
void ble_hs_flow_track_data_mbuf(struct os_mbuf *om) {
  ...
  hdr = (void *)om->om_data;
  ble_hs_flow_mbuf_conn_handle[idx] =
    BLE_HCI_DATA_HANDLE(hdr->hdh_handle_pb_bc);
#endif
}
...
int ble_hs_flow_startup(void) {
  ...
  rc = ble_hs_hci_cmd_tx(
    BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND, BLE_HCI_OCF_CB_SET_CTLR_TO_HOST_FC),
