...
static int preempt_any(struct ble_ll_sched_item *sch,
                       struct ble_ll_sched_item *item) {
  return 1;
}

static int preempt_none(struct ble_ll_sched_item *sch,
                        struct ble_ll_sched_item *item) {
  return 0;
}

static int preempt_any_except_conn(struct ble_ll_sched_item *sch,
                                   struct ble_ll_sched_item *item) {
...
static inline int ble_ll_sched_check_overlap(struct ble_ll_sched_item *sch1,
...
static void ble_ll_sched_preempt(struct ble_ll_sched_item *sch,
...
static inline void ble_ll_sched_q_head_changed(void) {
  ...
  TAILQ_FOREACH(entry, &g_ble_ll_sched_q, link) {
    ...
    if (ble_ll_sched_check_overlap(sch, entry)) {
      if (preempt_cb(sch, entry)) {
        ...
        if ((max_delay == 0) || LL_TMR_GEQ(sch->start_time, max_start_time)) {
          sch->enqueued = 0;
          goto done;
        }
...
int ble_ll_sched_conn_central_new(struct ble_ll_conn_sm *connsm,
   ...
   * central has to tell the peripheral how many transmit window offsets there
   * are from the earliest possible time to when the actual transmit start will
   ...
   * transmitting earlier than the peripheral thinks we could end up
   * transmitting before the window offset. Transmitting later is fine since we
   * have the transmit window to do so. Transmitting before is bad, since the
   * peripheral wont be listening. We could do better calculation if we wanted
   * to use a transmit window of 1 as opposed to 2, but for now we dont care.
   */
  ...
  if (calc_sch) {
    ...
    sch->end_time =
      earliest_start + ble_ll_tmr_u2t(MYNEWT_VAL(BLE_LL_CONN_INIT_SLOTS) *
                                      BLE_LL_SCHED_USECS_PER_SLOT);

    min_win_offset =
      ble_ll_tmr_u2t(MYNEWT_VAL(BLE_LL_CONN_INIT_MIN_WIN_OFFSET) *
                     BLE_LL_SCHED_USECS_PER_SLOT);
 ...
 */
static int ble_ll_sched_sync_overlaps_current(struct ble_ll_sched_item *sch) {
  uint32_t end_time;
  uint8_t state;
 ...
 */
static int ble_ll_sched_execute_item(struct ble_ll_sched_item *sch) {
  int rc;
  uint8_t lls;
 ...
 * @return int
 */
static void ble_ll_sched_run(void *arg) {
  struct ble_ll_sched_item *sch;
 ...
 */
void ble_ll_sched_stop(void) { ble_ll_tmr_stop(&g_ble_ll_sched_timer); }
...
int ble_ll_sched_init(void) {
  ...
  g_ble_ll_sched_css.period_slots =
    MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED_PERIOD_SLOTS);
#endif
...
void ble_ll_sched_css_set_conn_anchor(struct ble_ll_conn_sm *connsm) {
  ...
  if (diff < 0) {
    ble_ll_tmr_sub(&connsm->anchor_point, &connsm->anchor_point_usecs, -diff);
  } else if (diff > 0) {
    ble_ll_tmr_add(&connsm->anchor_point, &connsm->anchor_point_usecs, diff);
  }
}

#if !MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED_FIXED)
inline bool ble_ll_sched_css_is_enabled(void) {
  return g_ble_ll_sched_css.enabled;
}

inline uint32_t ble_ll_sched_css_get_slot_us(void) {
  return g_ble_ll_sched_css.slot_us;
}

inline uint32_t ble_ll_sched_css_get_period_slots(void) {
  return g_ble_ll_sched_css.period_slots;
}

inline uint32_t ble_ll_sched_css_get_conn_interval_us(void) {
  return ble_ll_sched_css_get_period_slots() * ble_ll_sched_css_get_slot_us() /
