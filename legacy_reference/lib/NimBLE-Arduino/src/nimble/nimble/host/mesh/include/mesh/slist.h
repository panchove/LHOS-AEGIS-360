...
#define SYS_SLIST_FOR_EACH_NODE(__sl, __sn)                                    \
  for (__sn = sys_slist_peek_head(__sl); __sn; __sn = sys_slist_peek_next(__sn))
...
#define SYS_SLIST_ITERATE_FROM_NODE(__sl, __sn)                                \
  for (__sn = __sn ? sys_slist_peek_next_no_check(__sn)                        \
       ...
       __sn; __sn = sys_slist_peek_next(__sn))
...
#define SYS_SLIST_FOR_EACH_NODE_SAFE(__sl, __sn, __sns)                        \
  for (__sn = sys_slist_peek_head(__sl), __sns = sys_slist_peek_next(__sn);    \
       __sn; __sn = __sns, __sns = sys_slist_peek_next(__sn))
...
#define SYS_SLIST_FOR_EACH_CONTAINER_SAFE(__sl, __cn, __cns, __n)              \
  for (__cn = SYS_SLIST_PEEK_HEAD_CONTAINER(__sl, __cn, __n),                  \
      __cns = SYS_SLIST_PEEK_NEXT_CONTAINER(__cn, __n);                        \
       __cn; __cn = __cns, __cns = SYS_SLIST_PEEK_NEXT_CONTAINER(__cn, __n))
 ...
 */
static inline void sys_slist_append_list(sys_slist_t *list, void *head,
                                         void *tail) {
  if (!list->tail) {
 ...
 */
static inline void sys_slist_remove(sys_slist_t *list, sys_snode_t *prev_node,
                                    sys_snode_t *node) {
  if (!prev_node) {
    list->head = node->next;
