...
static const char *const ble_gatt_chr_f_names[] = {
  "BROADCAST", "READ",         "WRITE_NO_RSP",    "WRITE",
  "NOTIFY",    "INDICATE",     "AUTH_SIGN_WRITE", "RELIABLE_WRITE",
  "AUX_WRITE", "READ_ENC",     "READ_AUTHEN",     "READ_AUTHOR",
  "WRITE_ENC", "WRITE_AUTHEN", "WRITE_AUTHOR",    NULL};

static const char *const ble_gatt_dsc_f_names[] = {
  "READ",      "WRITE",        "READ_ENC",     "READ_AUTHEN", "READ_AUTHOR",
  "WRITE_ENC", "WRITE_AUTHEN", "WRITE_AUTHOR", NULL};

#define BLE_CHR_FLAGS_STR_LEN 180

static char *ble_gatts_flags_to_str(uint16_t flags, char *buf,
...
static void ble_gatt_show_local_chr(const struct ble_gatt_svc_def *svc,
  ...
  for (chr = svc->characteristics; chr && chr->uuid; ++chr) {
    ...
    console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                   "%s\n",
                   " ", "uuid", ble_uuid_to_str(chr->uuid, uuid_buf));
    console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                   "%d\n",
                   " ", "def_handle", handle);
    console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
    ...
    console_printf(
      "%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
      "%s\n",
      " ", "flags",
      ble_gatts_flags_to_str(chr->flags, flags_buf, ble_gatt_chr_f_names));
    handle += 2;
    ...
    if ((chr->flags & BLE_GATT_CHR_F_NOTIFY) ||
      ...
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                     "%s\n",
                     " ", "uuid", ble_uuid_to_str(uuid_ccc, uuid_buf));
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                     "%d\n",
                     " ", "handle", handle);
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
    ...
    for (dsc = chr->descriptors; dsc && dsc->uuid; ++dsc) {
      ...
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                     "%s\n",
                     " ", "uuid", ble_uuid_to_str(dsc->uuid, uuid_buf));
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                     "%d\n",
                     " ", "handle", handle);
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
      ...
      console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                     "%s\n",
                     " ", "flags",
                     ble_gatts_flags_to_str(dsc->att_flags, flags_buf,
                                            ble_gatt_dsc_f_names));
      handle++;
    }
  }
}
...
static int ble_gatt_show_local_inc_svc(const struct ble_gatt_svc_def *svc,
  ...
  for (includes = &svc->includes[0]; *includes != NULL; ++includes) {
    ...
    console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                   "%s\n",
                   " ", "uuid", ble_uuid_to_str((*includes)->uuid, uuid_buf));
    console_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                   "%d\n",
                   " ", "attr handle", handle);
    ++num;
...
static void ble_gatt_show_local_svc(const struct ble_gatt_svc_def *svc,
  ...
  ble_gatt_show_local_chr(svc, handle, uuid_buf, flags_buf);
}
