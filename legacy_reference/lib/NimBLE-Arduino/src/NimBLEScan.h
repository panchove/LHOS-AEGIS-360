 ...
 * @details When a scan completes, we have a set of found devices.  Each device
 * is described by a NimBLEAdvertisedDevice object.  The number of items in the
 * set is given by getCount().  We can retrieve a device by calling getDevice()
 * passing in the index (starting at 0) of the desired device.
 */
 ...
 *
 * Scanning is associated with a %BLE client that is attempting to locate BLE
 * servers.
 */
class NimBLEScan {
public:
  ...
  bool isScanning();
  void setScanCallbacks(NimBLEScanCallbacks *pScanCallbacks,
...
public:
  ...
  /**
   * @brief Called when a new device is discovered, before the scan result is
  ...
  /**
   * @brief Called when a new scan result is complete, including scan response
   * data (if applicable).
   * @param [in] advertisedDevice The device for which the complete result is
   * available.
   */
