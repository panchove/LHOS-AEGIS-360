...
public:
  ...
  int pidDataRead(uint8_t mode, uint8_t pid, void *data, int length);
  ...
  int dtcRead(std::string *dtcs, int maxCodes);

  int clearAllStoredDTC();

  int supportedPidsRead();
  int parseDTCs(uint8_t *data, int length, std::string *dtcs, int maxCodes);
