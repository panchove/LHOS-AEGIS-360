...
int bt_mesh_net_encrypt(const uint8_t key[16], struct os_mbuf *buf,
  ...
  BT_DBG("IVIndex %u EncKey %s mic_len %u", (unsigned)iv_index, bt_hex(key, 16),
         mic_len);
  ...
  err = bt_ccm_encrypt(key, nonce, &buf->om_data[7], buf->om_len - 7, NULL, 0,
                       &buf->om_data[7], mic_len);
  if (!err) {
...
int bt_mesh_net_decrypt(const uint8_t key[16], struct os_mbuf *buf,
  ...
  BT_DBG("iv_index %u, key %s mic_len %u", (unsigned)iv_index, bt_hex(key, 16),
         mic_len);
  ...
  return bt_ccm_decrypt(key, nonce, &buf->om_data[7], buf->om_len - 7, NULL, 0,
                        &buf->om_data[7], mic_len);
}
...
int bt_mesh_app_encrypt(const uint8_t key[16],
  ...
  BT_DBG("dev_key %u src 0x%04x dst 0x%04x", ctx->dev_key, ctx->src, ctx->dst);
  ...
  err =
    bt_ccm_encrypt(key, nonce, buf->om_data, buf->om_len, ctx->ad,
                   ctx->ad ? 16 : 0, buf->om_data, APP_MIC_LEN(ctx->aszmic));
...
int bt_mesh_app_decrypt(const uint8_t key[16],
  ...
  err =
    bt_ccm_decrypt(key, nonce, buf->om_data, buf->om_len, ctx->ad,
                   ctx->ad ? 16 : 0, out->om_data, APP_MIC_LEN(ctx->aszmic));
  if (!err) {
