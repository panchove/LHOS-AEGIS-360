...
public:
  NimBLEDescriptor(const char *uuid, uint16_t properties, uint16_t maxLen,
                   NimBLECharacteristic *pCharacteristic = nullptr);

  NimBLEDescriptor(const NimBLEUUID &uuid, uint16_t properties, uint16_t maxLen,
                   NimBLECharacteristic *pCharacteristic = nullptr);
...
/**
 * @brief Callbacks that can be associated with a %BLE descriptors to inform of
 * events.
 *
 * When a server application creates a %BLE descriptor, we may wish to be
 * informed when there is either a read or write request to the descriptors
