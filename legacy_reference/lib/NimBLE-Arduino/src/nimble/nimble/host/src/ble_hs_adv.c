...
static int ble_hs_adv_set_flat_mbuf(uint8_t type, int data_len,
                                    const void *data, uint8_t *dst,
...
static int ble_hs_adv_set_array_uuid16(uint8_t type, uint8_t num_elems,
  ...
  rc = ble_hs_adv_set_hdr(type, num_elems * 2, max_len, dst, dst_len, om);
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_adv_set_array_uuid32(uint8_t type, uint8_t num_elems,
  ...
  rc = ble_hs_adv_set_hdr(type, num_elems * 4, max_len, dst, dst_len, om);
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_adv_set_array_uuid128(uint8_t type, uint8_t num_elems,
                                        const ble_uuid128_t *elems,
                                        uint8_t *dst, uint8_t *dst_len,
                                        uint8_t max_len, struct os_mbuf *om) {
  int rc;
  int i;

  rc = ble_hs_adv_set_hdr(type, num_elems * 16, max_len, dst, dst_len, om);
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_adv_set_array16(uint8_t type, uint8_t num_elems,
                                  const uint16_t *elems, uint8_t *dst,
                                  uint8_t *dst_len, uint8_t max_len,
                                  struct os_mbuf *om) {
  uint16_t tmp;
  int rc;
...
static int adv_set_fields(const struct ble_hs_adv_fields *adv_fields,
  ...
  if (adv_fields->flags != 0) {
    rc = ble_hs_adv_set_flat_mbuf(BLE_HS_ADV_TYPE_FLAGS, 1, &adv_fields->flags,
  ...
  if (adv_fields->name != NULL && adv_fields->name_len > 0) {
    ...
    rc = ble_hs_adv_set_flat_mbuf(type, adv_fields->name_len, adv_fields->name,
                                  dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  ...
  if (adv_fields->tx_pwr_lvl_is_present) {
    ...
    rc = ble_hs_adv_set_flat_mbuf(BLE_HS_ADV_TYPE_TX_PWR_LVL, 1, &tx_pwr_lvl,
                                  dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  ...
  if (adv_fields->slave_itvl_range != NULL) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_SLAVE_ITVL_RANGE, BLE_HS_ADV_SLAVE_ITVL_RANGE_LEN,
      adv_fields->slave_itvl_range, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->sol_uuids16 != NULL) {
    rc = ble_hs_adv_set_array_uuid16(
      BLE_HS_ADV_TYPE_SOL_UUIDS16, adv_fields->sol_num_uuids16,
      adv_fields->sol_uuids16, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->sol_uuids128 != NULL) {
    rc = ble_hs_adv_set_array_uuid128(
      BLE_HS_ADV_TYPE_SOL_UUIDS128, adv_fields->sol_num_uuids128,
      adv_fields->sol_uuids128, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->svc_data_uuid16 != NULL && adv_fields->svc_data_uuid16_len) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_SVC_DATA_UUID16, adv_fields->svc_data_uuid16_len,
      adv_fields->svc_data_uuid16, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->public_tgt_addr != NULL &&
    ...
    rc = ble_hs_adv_set_flat_mbuf(
      ...
      BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN * adv_fields->num_public_tgt_addrs,
      adv_fields->public_tgt_addr, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->random_tgt_addr != NULL &&
    ...
    rc = ble_hs_adv_set_flat_mbuf(
      ...
      BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN * adv_fields->num_random_tgt_addrs,
      adv_fields->random_tgt_addr, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->appearance_is_present) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_APPEARANCE, BLE_HS_ADV_APPEARANCE_LEN,
      &adv_fields->appearance, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->adv_itvl_is_present) {
    rc =
      ble_hs_adv_set_array16(BLE_HS_ADV_TYPE_ADV_ITVL, 1, &adv_fields->adv_itvl,
                             dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  ...
  if (adv_fields->device_addr != NULL) {
    ...
    dev_addr_ad[BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN] =
      adv_fields->device_addr_type;

    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_DEVICE_ADDR,
      BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN + BLE_HS_ADV_ADDR_TYPE_LEN,
      dev_addr_ad, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->le_role_is_present) {
    rc =
      ble_hs_adv_set_flat_mbuf(BLE_HS_ADV_TYPE_LE_ROLE, 1, &adv_fields->le_role,
                               dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  ...
  if (adv_fields->sol_uuids32 != NULL && adv_fields->sol_num_uuids32) {
    rc = ble_hs_adv_set_array_uuid32(
      BLE_HS_ADV_TYPE_SOL_UUIDS32, adv_fields->sol_num_uuids32,
      adv_fields->sol_uuids32, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->svc_data_uuid32 != NULL && adv_fields->svc_data_uuid32_len) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_SVC_DATA_UUID32, adv_fields->svc_data_uuid32_len,
      adv_fields->svc_data_uuid32, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->svc_data_uuid128 != NULL &&
    ...
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_SVC_DATA_UUID128, adv_fields->svc_data_uuid128_len,
      adv_fields->svc_data_uuid128, dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  if (adv_fields->le_supp_feat_is_present) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_LE_SUPP_FEAT, BLE_HS_ADV_LE_SUPP_FEAT_LEN,
      adv_fields->le_supp_feat, dst, &dst_len_local, max_len, om);
  }

  /*** 0x2f - Advertising interval - long. */
  if (adv_fields->adv_itvl_long_is_present &&
      adv_fields->adv_itvl_long > 0xFFFF) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_ADV_ITVL_LONG, BLE_HS_ADV_ADV_ITVL_LONG_LEN,
      &adv_fields->adv_itvl_long, dst, &dst_len_local, max_len, om);
  }
  ...
  if ((adv_fields->enc_adv_data != NULL) &&
    ...
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_ENC_ADV_DATA, adv_fields->enc_adv_data_len,
      adv_fields->enc_adv_data, dst, &dst_len_local, max_len, om);
  }
#endif
  ...
  if ((adv_fields->mfg_data != NULL) && (adv_fields->mfg_data_len >= 2)) {
    rc = ble_hs_adv_set_flat_mbuf(
      BLE_HS_ADV_TYPE_MFG_DATA, adv_fields->mfg_data_len, adv_fields->mfg_data,
      dst, &dst_len_local, max_len, om);
    if (rc != 0) {
      return rc;
    }
...
static int find_field_func(const struct ble_hs_adv_field *field,
                           void *user_data) {
