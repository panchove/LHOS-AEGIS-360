...
static const uint16_t g_ble_sca_ppm_tbl[8] = {500, 250, 150, 100,
                                              75,  50,  30,  20};
...
uint8_t ble_ll_utils_chan_map_remap(const uint8_t *chan_map,
                                    uint8_t remap_index) {
  uint8_t cntr;
  uint8_t mask;
...
#if __thumb2__
static inline uint32_t ble_ll_utils_csa2_perm(uint32_t val) {
...
#else
static uint32_t ble_ll_utils_csa2_perm(uint32_t in) {
  uint32_t out = 0;
  int i;
...
static inline uint32_t ble_ll_utils_csa2_mam(uint32_t a, uint32_t b) {
  return (17 * a + b) % 65536;
}

static uint16_t ble_ll_utils_csa2_prn_s(uint16_t counter, uint16_t ch_id) {
  uint32_t prn_s;
...
static uint16_t ble_ll_utils_csa2_prng(uint16_t counter, uint16_t ch_id) {
  uint16_t prn_s;
  uint16_t prn_e;
...
static uint16_t ble_ll_utils_csa2_chan2remap(uint16_t chan_idx,
...
static uint16_t ble_ll_utils_csa2_remap2chan(uint16_t remap_idx,
                                             const uint8_t *chan_map) {
  uint32_t u32 = 0;
  unsigned idx;
...
static uint16_t ble_ll_utils_csa2_calc_chan_idx(uint16_t prn_e,
                                                uint8_t num_used_chans,
                                                const uint8_t *chanm_map,
                                                uint16_t *remap_idx) {
  uint16_t chan_idx;
...
uint16_t ble_ll_utils_dci_iso_event(uint16_t counter, uint16_t chan_id,
  ...
  *prn_sub_lu = prn_s;
  ...
  chan_idx =
    ble_ll_utils_csa2_calc_chan_idx(prn_e, chan_map_used, chan_map, remap_idx);
...
uint16_t ble_ll_utils_dci_iso_subevent(uint16_t chan_id, uint16_t *prn_sub_lu,
  ...
  *prn_sub_lu = ble_ll_utils_csa2_mam(*prn_sub_lu, chan_id);
  ...
  d = MAX(1, MAX(MIN(3, chan_map_used - 5), MIN(11, (chan_map_used - 10) / 2)));
  *remap_idx =
    (*remap_idx + d + prn_sub_se * (chan_map_used - 2 * d + 1) / 65536) %
    chan_map_used;
