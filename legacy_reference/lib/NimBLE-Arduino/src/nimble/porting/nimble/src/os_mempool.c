...
STAILQ_HEAD(, os_mempool)
g_os_mempool_list = STAILQ_HEAD_INITIALIZER(g_os_mempool_list);

#if MYNEWT_VAL(OS_MEMPOOL_POISON)
static uint32_t os_mem_poison = 0xde7ec7ed;

static_assert(sizeof(struct os_memblock) % 4 == 0,
              "sizeof(struct os_memblock) shall be aligned to 4");
static_assert(sizeof(os_mem_poison) == 4, "sizeof(os_mem_poison) shall be 4");

static void os_mempool_poison(const struct os_mempool *mp, void *start) {
  uint32_t *p;
  uint32_t *end;
  int sz;
...
static void os_mempool_poison_check(const struct os_mempool *mp, void *start) {
  uint32_t *p;
  uint32_t *end;
  int sz;
...
static void os_mempool_guard(const struct os_mempool *mp, void *start) {
  uint32_t *tgt;
...
static void os_mempool_guard_check(const struct os_mempool *mp, void *start) {
  uint32_t *tgt;
...
static os_error_t os_mempool_init_internal(struct os_mempool *mp,
...
os_error_t os_mempool_ext_init(struct os_mempool_ext *mpe, uint16_t blocks,
  ...
  rc = os_mempool_init_internal(&mpe->mpe_mp, blocks, block_size, membuf, name,
                                OS_MEMPOOL_F_EXT);
  if (rc != 0) {
    return rc;
  }
...
void os_mempool_module_init(void) { STAILQ_INIT(&g_os_mempool_list); }
