...
#if MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED_FIXED)
static inline bool ble_ll_sched_css_is_enabled(void) { return true; }

static inline uint32_t ble_ll_sched_css_get_slot_us(void) {
  return MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED_SLOT_US);
}

static inline uint32_t ble_ll_sched_css_get_period_slots(void) {
  return MYNEWT_VAL(BLE_LL_CONN_STRICT_SCHED_PERIOD_SLOTS);
}

static inline uint32_t ble_ll_sched_css_get_conn_interval_us(void) {
