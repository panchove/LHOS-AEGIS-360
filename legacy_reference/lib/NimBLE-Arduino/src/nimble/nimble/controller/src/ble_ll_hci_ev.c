...
void ble_ll_hci_ev_subrate_change(struct ble_ll_conn_sm *connsm,
                                  uint8_t status) {
...
void ble_ll_hci_ev_send_vs_printf(uint8_t id, const char *fmt, ...) {
  ...
  va_start(ap, fmt);
  hci_ev->length +=
    vsnprintf((void *)ev->data, BLE_HCI_MAX_DATA_LEN - sizeof(*ev), fmt, ap);
  va_end(ap);
...
void ble_ll_hci_ev_send_vs_llcp_trace(uint8_t type, uint16_t handle,
                                      uint16_t count, void *pdu,
                                      size_t length) {
  struct ble_hci_ev_vs *ev;
  struct ble_hci_ev *hci_ev;
