...
static int ble_svc_gatt_srv_sup_feat_access(uint16_t conn_handle,
                                            uint16_t attr_handle,
                                            struct ble_gatt_access_ctxt *ctxt,
                                            void *arg);

static int ble_svc_gatt_cl_sup_feat_access(uint16_t conn_handle,
                                           uint16_t attr_handle,
                                           struct ble_gatt_access_ctxt *ctxt,
                                           void *arg);
...
static int ble_svc_gatt_srv_sup_feat_access(uint16_t conn_handle,
  ...
  os_mbuf_append(ctxt->om, &ble_svc_gatt_local_srv_sup_feat,
                 sizeof(ble_svc_gatt_local_srv_sup_feat));
...
uint16_t ble_svc_gatt_hash_handle() { return ble_svc_gatt_db_hash_handle; }
...
void ble_svc_gatt_init(void) {
  ...
  if (MYNEWT_VAL(BLE_EATT_CHAN_NUM) > 0 && ble_hs_cfg.eatt) {
    ble_svc_gatt_local_srv_sup_feat |=
      (1 << BLE_SVC_GATT_SRV_SUP_FEAT_EATT_BIT);
  }
  ...
  if (MYNEWT_VAL(BLE_ATT_SVR_NOTIFY_MULTI) > 0) {
    ble_svc_gatt_local_cl_sup_feat |=
      (1 << BLE_SVC_GATT_CLI_SUP_FEAT_MULT_NTF_BIT);
  }

  if (MYNEWT_VAL(BLE_GATT_CACHING) > 0) {
    ble_svc_gatt_local_cl_sup_feat |=
      (1 << BLE_SVC_GATT_CLI_SUP_FEAT_ROBUST_CATCHING_BIT);
  }
}

void ble_svc_gatt_deinit(void) { ble_gatts_free_svcs(); }
