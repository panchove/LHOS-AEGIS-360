...
#define SYNC_DATA_LEN(_advsm)                                                  \
  ((_advsm)->periodic_adv_data ? OS_MBUF_PKTLEN((_advsm)->periodic_adv_data)   \
                               : 0)
...
static struct ble_ll_adv_sm *ble_ll_adv_sm_find_configured(uint8_t instance) {
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static int ble_ll_adv_active_chanset_is_pri(struct ble_ll_adv_sm *advsm) {
  return (advsm->flags & BLE_LL_ADV_SM_FLAG_ACTIVE_CHANSET_MASK) == 0x10;
}

static int ble_ll_adv_active_chanset_is_sec(struct ble_ll_adv_sm *advsm) {
  return (advsm->flags & BLE_LL_ADV_SM_FLAG_ACTIVE_CHANSET_MASK) == 0x20;
}
#endif

static void ble_ll_adv_active_chanset_clear(struct ble_ll_adv_sm *advsm) {
  os_sr_t sr;
...
static void ble_ll_adv_active_chanset_set_pri(struct ble_ll_adv_sm *advsm) {
  os_sr_t sr;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static void ble_ll_adv_active_chanset_set_sec(struct ble_ll_adv_sm *advsm) {
  os_sr_t sr;
...
static void ble_ll_adv_flags_set(struct ble_ll_adv_sm *advsm, uint16_t flags) {
  os_sr_t sr;
...
static void ble_ll_adv_flags_clear(struct ble_ll_adv_sm *advsm,
                                   uint16_t flags) {
  os_sr_t sr;
...
static void ble_ll_adv_make_done(struct ble_ll_adv_sm *advsm,
...
static void ble_ll_adv_rpa_update(struct ble_ll_adv_sm *advsm) {
 ...
 * @param advsm
 */
void ble_ll_adv_chk_rpa_timeout(struct ble_ll_adv_sm *advsm) {
 ...
 */
static uint8_t ble_ll_adv_legacy_pdu_make(uint8_t *dptr, void *pducb_arg,
                                          uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static void ble_ll_adv_put_aux_ptr(uint8_t chan, uint8_t phy, uint32_t offset,
                                   uint8_t *dptr) {
  dptr[0] = chan;
 ...
 */
static uint8_t ble_ll_adv_pdu_make(uint8_t *dptr, void *pducb_arg,
                                   uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
  uint8_t pdu_type;
  uint8_t adv_mode;
  ...
  *hdr_byte = pdu_type;
  ...
  ext_hdr_flags =
    (1 << BLE_LL_EXT_ADV_DATA_INFO_BIT) | (1 << BLE_LL_EXT_ADV_AUX_PTR_BIT);
  ...
  if (AUX_CURRENT(advsm)->sch.enqueued) {
    offset = ble_ll_tmr_t2u(AUX_CURRENT(advsm)->start_time -
                            advsm->adv_pdu_start_time);
  } else {
    offset = 0;
  }
  /* Always use channel from 1st AUX */
  ble_ll_adv_put_aux_ptr(AUX_CURRENT(advsm)->chan, advsm->sec_phy, offset,
                         dptr);
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PERIODIC_ADV)
static void ble_ll_adv_put_syncinfo(struct ble_ll_adv_sm *advsm,
                                    struct ble_ll_conn_sm *connsm,
                                    uint8_t *conn_event_cnt, uint8_t *dptr) {
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PERIODIC_ADV_SYNC_TRANSFER)
 ...
 * Create the AUX PDU
 */
static uint8_t ble_ll_adv_aux_pdu_make(uint8_t *dptr, void *pducb_arg,
                                       uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
  ...
  if (aux->ext_hdr_flags & (1 << BLE_LL_EXT_ADV_AUX_PTR_BIT)) {
    if (!AUX_NEXT(advsm)->sch.enqueued) {
      ...
      offset = ble_ll_tmr_t2u(AUX_NEXT(advsm)->start_time -
                              advsm->rx_ble_hdr->beg_cputime);
      offset -= (advsm->rx_ble_hdr->rem_usecs +
                 ble_ll_pdu_us(12, advsm->sec_phy) + BLE_LL_IFS);
    } else {
    ...
    ble_ll_adv_put_aux_ptr(AUX_NEXT(advsm)->chan, advsm->sec_phy, offset, dptr);
  ...
  if (aux->data_len) {
    os_mbuf_copydata(*advsm->aux_data, aux->data_offset, aux->data_len, dptr);
  }
...
static uint8_t ble_ll_adv_aux_scannable_pdu_make(uint8_t *dptr, void *pducb_arg,
                                                 uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
  uint8_t pdu_type;
  uint8_t *ext_hdr_len;
...
static uint8_t ble_ll_adv_scan_rsp_legacy_pdu_make(uint8_t *dptr,
                                                   void *pducb_arg,
                                                   uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
 ...
 * @param advsm
 */
static uint8_t ble_ll_adv_scan_rsp_pdu_make(uint8_t *dptr, void *pducb_arg,
                                            uint8_t *hdr_byte) {
  struct ble_ll_adv_sm *advsm;
 ...
 */
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
static uint8_t ble_ll_adv_aux_conn_rsp_pdu_make(uint8_t *dptr, void *pducb_arg,
                                                uint8_t *hdr_byte) {
 ...
 *
 */
static void ble_ll_adv_tx_done(void *arg) {
  struct ble_ll_adv_sm *advsm;
 ...
 * @return int
 */
static int ble_ll_adv_tx_start_cb(struct ble_ll_sched_item *sch) {
  int rc;
  uint8_t end_trans;
  uint32_t txstart;
  struct ble_ll_adv_sm *advsm;
  ...
  /* Set channel */
  rc =
    ble_phy_setchan(advsm->adv_chan, BLE_ACCESS_ADDR_ADV, BLE_LL_CRCINIT_ADV);
  BLE_LL_ASSERT(rc == 0);
...
static void ble_ll_adv_set_sched(struct ble_ll_adv_sm *advsm) {
  uint32_t max_usecs;
  struct ble_ll_sched_item *sch;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static int ble_ll_adv_secondary_tx_start_cb(struct ble_ll_sched_item *sch) {
  int rc;
  uint8_t end_trans;
  uint32_t txstart;
  struct ble_ll_adv_sm *advsm;
...
static uint16_t ble_ll_adv_aux_calculate_payload(
  struct ble_ll_adv_sm *advsm, uint16_t props, struct os_mbuf *data,
  uint32_t data_offset, uint8_t *data_len_o, uint8_t *ext_hdr_flags_o) {
  ...
  if (first_pdu && !(props & BLE_HCI_LE_SET_EXT_ADV_PROP_ANON_ADV)) {
    ext_hdr_flags |= (1 << BLE_LL_EXT_ADV_ADVA_BIT);
    ext_hdr_len += BLE_LL_EXT_ADV_ADVA_SIZE;
  }
   ...
   */
  if (first_pdu && (props & BLE_HCI_LE_SET_EXT_ADV_PROP_DIRECTED) &&
  ...
  if (first_pdu && (props & BLE_HCI_LE_SET_EXT_ADV_PROP_INC_TX_PWR)) {
...
static void ble_ll_adv_aux_calculate(struct ble_ll_adv_sm *advsm,
                                     struct ble_ll_adv_aux *aux,
                                     uint16_t data_offset) {
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LE_CSA2)
  aux->chan =
    ble_ll_utils_dci_csa2(advsm->event_cntr++, advsm->channel_id,
                          g_ble_ll_data.chan_map_used, g_ble_ll_data.chan_map);
#else
  aux->chan = ble_ll_utils_remapped_channel(
    ble_ll_rand() % BLE_PHY_NUM_DATA_CHANS, g_ble_ll_data.chan_map);
#endif

  aux->data_offset = data_offset;
  aux->payload_len = ble_ll_adv_aux_calculate_payload(
    advsm, advsm->props, *advsm->aux_data, data_offset, &aux->data_len,
    &aux->ext_hdr_flags);
}

static bool ble_ll_adv_aux_check_data_itvl(struct ble_ll_adv_sm *advsm,
                                           uint16_t props, uint8_t pri_phy,
                                           uint8_t sec_phy,
                                           struct os_mbuf *data,
  ...
  max_usecs = 3 * (ble_ll_pdu_us(7, pri_phy) + 300) + BLE_LL_MAFS +
...
static void ble_ll_adv_aux_scheduled(struct ble_ll_adv_sm *advsm,
...
static void ble_ll_adv_aux_schedule_next(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_adv_aux *aux;
   ...
   */
  if (aux->auxptr_zero || (advsm->duration && LL_TMR_GT(aux_next->sch.end_time,
                                                        advsm->adv_end_time))) {
    ble_ll_sched_rmv_elem(&aux_next->sch);
  }
}

static void ble_ll_adv_aux_schedule_first(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_adv_aux *aux;
  struct ble_ll_sched_item *sch;
  uint32_t max_usecs;
  ...
  /* Set end time to maximum time this schedule item may take */
  if (advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_CONNECTABLE) {
    max_usecs = ble_ll_pdu_us(aux->payload_len, advsm->sec_phy) + BLE_LL_IFS +
                /* AUX_CONN_REQ */
                ble_ll_pdu_us(34 + 14, advsm->sec_phy) + BLE_LL_IFS +
                /* AUX_CONN_RSP */
     ...
     *
     * Note:
     *  1. aux->payload_len, which calculated by above
     * ble_ll_adv_aux_calulcate(), contains AUX_SCAN_RSP length.
...
static void ble_ll_adv_aux_set_start_time(struct ble_ll_adv_sm *advsm) {
   ...
   */
  adv_event_dur =
    (adv_pdu_dur * chans) + ((3 + g_ble_ll_sched_offset_ticks) * (chans - 1));

  advsm->aux[0].start_time =
    advsm->adv_event_start_time + adv_event_dur +
    ble_ll_tmr_u2t_up(BLE_LL_MAFS + MYNEWT_VAL(BLE_LL_SCHED_AUX_MAFS_DELAY));
}

static void ble_ll_adv_aux_schedule(struct ble_ll_adv_sm *advsm) {
  /*
...
int ble_ll_adv_set_adv_params(const uint8_t *cmdbuf, uint8_t len) {
  ...
  advsm->tx_power =
    ble_ll_tx_power_round(g_ble_ll_tx_power - g_ble_ll_tx_power_compensation);
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static void ble_ll_adv_update_did(struct ble_ll_adv_sm *advsm) {
...
static void ble_ll_adv_update_adv_scan_rsp_data(struct ble_ll_adv_sm *advsm) {
  if (!(advsm->flags & BLE_LL_ADV_SM_FLAG_NEW_ADV_DATA) &&
      !(advsm->flags & BLE_LL_ADV_SM_FLAG_NEW_SCAN_RSP_DATA)) {
    return;
  }
 ...
 * @param advsm
 */
static void ble_ll_adv_sm_stop(struct ble_ll_adv_sm *advsm) {
  os_sr_t sr;
...
static void ble_ll_adv_sm_stop_timeout(struct ble_ll_adv_sm *advsm) {
  ...
  if (ble_ll_hci_adv_mode_ext()) {
    ble_ll_hci_ev_send_adv_set_terminated(
      BLE_ERR_DIR_ADV_TMO, advsm->adv_instance, 0, advsm->events);
  }
#endif
  ...
  if (advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_HD_DIRECTED) {
    ble_ll_conn_comp_event_send(NULL, BLE_ERR_DIR_ADV_TMO, advsm->conn_comp_ev,
                                advsm);
    advsm->conn_comp_ev = NULL;
  }
#endif
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static void ble_ll_adv_sm_stop_limit_reached(struct ble_ll_adv_sm *advsm) {
  ble_ll_hci_ev_send_adv_set_terminated(BLE_ERR_LIMIT_REACHED,
                                        advsm->adv_instance, 0, advsm->events);
...
static void ble_ll_adv_scheduled(struct ble_ll_adv_sm *advsm,
                                 uint32_t sch_start, void *arg) {
  ...
  if (advsm->duration) {
    advsm->adv_end_time =
      advsm->adv_event_start_time + ble_ll_tmr_u2t(advsm->duration * 10000);
  }
#else
...
static uint8_t ble_ll_adv_sync_pdu_make(uint8_t *dptr, void *pducb_arg,
  ...
  if (sync->ext_hdr_flags & (1 << BLE_LL_EXT_ADV_AUX_PTR_BIT)) {
    if (!SYNC_NEXT(advsm)->sch.enqueued) {
      ...
      offset = ble_ll_tmr_t2u(SYNC_NEXT(advsm)->start_time - sync->start_time);
    }

    sync->auxptr_zero = offset == 0;

    ble_ll_adv_put_aux_ptr(SYNC_NEXT(advsm)->chan, advsm->sec_phy, offset,
                           dptr);
...
#if MYNEWT_VAL(BLE_LL_ISO_BROADCASTER)
  if (advsm->big) {
    biginfo_len = ble_ll_iso_big_biginfo_copy(
      advsm->big, dptr, sync->sch.start_time + g_ble_ll_sched_offset_ticks,
      sync->sch.remainder);
...
static void ble_ll_adv_sync_tx_done(struct ble_ll_adv_sm *advsm) {
 ...
 *
 */
static void ble_ll_adv_sync_tx_end(void *arg) {
...
static int ble_ll_adv_sync_tx_start_cb(struct ble_ll_sched_item *sch) {
  int rc;
  uint32_t txstart;
  struct ble_ll_adv_sm *advsm;
  struct ble_ll_adv_sync *sync;
...
static void ble_ll_adv_sync_calculate(struct ble_ll_adv_sm *advsm,
                                      struct ble_ll_adv_sync *sync,
                                      uint16_t data_offset, uint8_t chan) {
  uint16_t rem_data_len;
  uint8_t ext_hdr_len;
   ...
   */
  if (data_offset == 0 && (advsm->periodic_adv_props &
...
static void ble_ll_adv_periodic_schedule_first(struct ble_ll_adv_sm *advsm,
                                               bool first_pdu) {
  struct ble_ll_adv_sync *sync;
  struct ble_ll_sched_item *sch;
  uint32_t max_usecs;
   ...
   */
  chan = ble_ll_utils_dci_csa2(
    ++advsm->periodic_event_cntr, advsm->periodic_channel_id,
    advsm->periodic_num_used_chans, advsm->periodic_chanmap);
...
static void ble_ll_adv_sync_next_scheduled(struct ble_ll_adv_sm *advsm,
                                           uint32_t sch_start, void *arg) {
...
static void ble_ll_adv_periodic_schedule_next(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_adv_sync *sync;
  ...
  chan = ble_ll_utils_dci_csa2(
    advsm->periodic_chain_event_cntr++, advsm->periodic_channel_id,
    advsm->periodic_num_used_chans, advsm->periodic_chanmap);
...
static void ble_ll_adv_sync_schedule(struct ble_ll_adv_sm *advsm,
                                     bool first_pdu) {
  /*
   * For secondary channel we always start by scheduling two consecutive
   * auxiliary packets at once. Then, after sending one packet we try to
...
static void ble_ll_adv_reschedule_periodic_event(struct ble_ll_adv_sm *advsm) {
  advsm->periodic_adv_event_start_time += advsm->periodic_adv_itvl_ticks;
  ble_ll_adv_sync_schedule(advsm, false);
}

static void ble_ll_adv_update_periodic_data(struct ble_ll_adv_sm *advsm) {
 ...
 * @param ev
 */
static void ble_ll_adv_periodic_done(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_adv_sync *sync;
  struct ble_ll_adv_sync *sync_next;
...
static void ble_ll_adv_periodic_event_done(struct ble_npl_event *ev) {
  ble_ll_adv_periodic_done(ble_npl_event_get_arg(ev));
}

static void ble_ll_adv_sm_start_periodic(struct ble_ll_adv_sm *advsm) {
  uint32_t usecs;
  ...
  advsm->periodic_channel_id =
    ((advsm->periodic_access_addr & 0xffff0000) >> 16) ^
  ...
  advsm->periodic_adv_itvl_ticks =
    ble_ll_tmr_u2t_r(usecs, &advsm->periodic_adv_itvl_rem_usec);
  ...
  advsm->periodic_adv_event_start_time =
    advsm->adv_pdu_start_time + ble_ll_tmr_u2t(advsm->adv_itvl_usecs + 5000);

  ble_ll_adv_sync_schedule(advsm, true);
}

static void ble_ll_adv_sm_stop_periodic(struct ble_ll_adv_sm *advsm) {
  os_sr_t sr;
 ...
 * @return int
 */
static int ble_ll_adv_sm_start(struct ble_ll_adv_sm *advsm) {
  uint8_t adv_chan;
  uint8_t *addr;
  ...
  advsm->channel_id =
    ((access_addr & 0xffff0000) >> 16) ^ (access_addr & 0x0000ffff);
#endif
  ...
  advsm->adv_pdu_start_time = ble_ll_tmr_get() + ble_ll_tmr_u2t(start_delay_us);
 ...
 * @return int
 */
static int ble_ll_adv_set_enable(uint8_t instance, uint8_t enable, int duration,
                                 uint8_t events) {
  int rc;
  struct ble_ll_adv_sm *advsm;
...
static void ble_ll_adv_update_data_mbuf(struct os_mbuf **omp, bool new_data,
                                        uint16_t maxlen, const void *data,
                                        uint16_t datalen) {
  struct os_mbuf *om;
  int ret;
 ...
 * @return int
 */
static int ble_ll_adv_set_scan_rsp_data(const uint8_t *data, uint8_t datalen,
  ...
  if (advsm->adv_enabled) {
    ...
    if (!(advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_LEGACY) &&
        !ble_ll_adv_aux_check_data_itvl(
          advsm, advsm->props, advsm->pri_phy, advsm->sec_phy,
          advsm->new_scan_rsp_data, advsm->adv_itvl_usecs)) {
      os_mbuf_free_chain(advsm->new_scan_rsp_data);
      advsm->new_scan_rsp_data = NULL;
    ...
    if (!(advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_LEGACY) &&
        !ble_ll_adv_aux_check_data_itvl(advsm, advsm->props, advsm->pri_phy,
                                        advsm->sec_phy, advsm->scan_rsp_data,
...
int ble_ll_hci_set_scan_rsp_data(const uint8_t *cmdbuf, uint8_t len) {
 ...
 */
static int ble_ll_adv_set_adv_data(const uint8_t *data, uint8_t datalen,
                                   uint8_t instance, uint8_t operation) {
  struct ble_ll_adv_sm *advsm;
  bool new_data;
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static bool pri_phy_valid(uint8_t phy) {
  switch (phy) {
...
static bool sec_phy_valid(uint8_t phy) {
  switch (phy) {
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LE_CODED_PHY)
  case BLE_HCI_LE_PHY_CODED:
#endif
...
static struct ble_ll_adv_sm *ble_ll_adv_sm_get(uint8_t instance) {
  struct ble_ll_adv_sm *advsm;
  unsigned int i;
...
int ble_ll_adv_ext_set_param(const uint8_t *cmdbuf, uint8_t len,
  ...
  if (cmd->tx_power == 127) {
    ...
    advsm->tx_power = ble_ll_tx_power_round(
      MIN(cmd->tx_power, MYNEWT_VAL(BLE_LL_TX_PWR_MAX_DBM)) -
 ...
 * @return int BLE error code
 */
int ble_ll_adv_remove(const uint8_t *cmdbuf, uint8_t len) {
...
static bool ble_ll_adv_periodic_check_data_itvl(uint16_t payload_len,
                                                uint16_t props, uint16_t itvl,
                                                uint8_t phy) {
...
int ble_ll_adv_periodic_set_param(const uint8_t *cmdbuf, uint8_t len) {
  const struct ble_hci_le_set_periodic_adv_params_cp *cmd =
    (const void *)cmdbuf;
  struct ble_ll_adv_sm *advsm;
  uint16_t adv_itvl_min;
  uint16_t adv_itvl_max;
...
int ble_ll_adv_periodic_set_data(const uint8_t *cmdbuf, uint8_t len) {
   ...
   */
  if (!ble_ll_adv_periodic_check_data_itvl(
        payload_total_len, advsm->periodic_adv_props,
        advsm->periodic_adv_itvl_max, advsm->sec_phy)) {
    return BLE_ERR_PACKET_TOO_LONG;
  }
  ...
  case BLE_HCI_LE_SET_DATA_OPER_LAST:
  case BLE_HCI_LE_SET_DATA_OPER_COMPLETE:
    ble_ll_adv_flags_clear(advsm, BLE_LL_ADV_SM_FLAG_PERIODIC_DATA_INCOMPLETE);
    break;
  case BLE_HCI_LE_SET_DATA_OPER_INT:
  case BLE_HCI_LE_SET_DATA_OPER_FIRST:
  default:
    ble_ll_adv_flags_set(advsm, BLE_LL_ADV_SM_FLAG_PERIODIC_DATA_INCOMPLETE);
    break;
  }
...
int ble_ll_adv_periodic_enable(const uint8_t *cmdbuf, uint8_t len) {
  const struct ble_hci_le_set_periodic_adv_enable_cp *cmd =
    (const void *)cmdbuf;
  struct ble_ll_adv_sm *advsm;
  ...
  if (cmd->enable) {
     ...
     */
    if (!ble_ll_adv_periodic_check_data_itvl(
          SYNC_DATA_LEN(advsm), advsm->periodic_adv_props,
          advsm->periodic_adv_itvl_max, advsm->sec_phy)) {
      return BLE_ERR_PACKET_TOO_LONG;
    }
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PERIODIC_ADV_SYNC_TRANSFER)
static int ble_ll_adv_periodic_send_sync_ind(struct ble_ll_adv_sm *advsm,
                                             struct ble_ll_conn_sm *connsm,
...
int ble_ll_adv_periodic_set_info_transfer(const uint8_t *cmdbuf, uint8_t len,
                                          uint8_t *rspbuf, uint8_t *rsplen) {
  const struct ble_hci_le_periodic_adv_set_info_transfer_cp *cmd =
    (const void *)cmdbuf;
 ...
 */
static int ble_ll_adv_rx_req(uint8_t pdu_type, struct os_mbuf *rxpdu) {
  int rc;
  int resolved;
  uint8_t chk_wl;
  uint8_t txadd;
  ...
  if (pdu_type == BLE_ADV_PDU_TYPE_SCAN_REQ) {
    ...
    rc =
      ble_phy_tx(ble_ll_adv_scan_rsp_pdu_make, advsm, BLE_PHY_TRANSITION_NONE);
 ...
 */
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
static int ble_ll_adv_conn_req_rxd(uint8_t *rxbuf, struct ble_mbuf_hdr *hdr,
                                   struct ble_ll_adv_sm *advsm) {
  int valid;
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PRIVACY)
  uint8_t resolved;
  ...
  if (valid) {
    ...
    if (resolved) {
       ...
       * is used from now on.
       */
      memcpy(inita, g_ble_ll_resolv_list[advsm->adv_rpa_index].rl_identity_addr,
             BLE_DEV_ADDR_LEN);
    ...
    valid = ble_ll_conn_periph_start(
      rxbuf, addr_type, hdr,
      !(advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_LEGACY));
    if (valid) {
...
static void ble_ll_adv_reschedule_event(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_sched_item *sch;
  ...
  if (!sch->enqueued) {
    ...
    advsm->adv_event_start_time = sch->start_time + g_ble_ll_sched_offset_ticks;
    advsm->adv_pdu_start_time = advsm->adv_event_start_time;
  }
...

{
  ...
  if (advsm->adv_chan == final_adv_chan) {
     ...
     */
    advsm->adv_pdu_start_time =
      ble_ll_tmr_get() + g_ble_ll_sched_offset_ticks + 1;

#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
    /* If we're past aux (unlikely, but can happen), just drop an event */
    if (!(advsm->props & BLE_HCI_LE_SET_EXT_ADV_PROP_LEGACY) &&
        advsm->aux_active &&
        LL_TMR_GT(advsm->adv_pdu_start_time, AUX_CURRENT(advsm)->start_time)) {
      ble_ll_adv_drop_event(advsm);
      return;
    }
#endif
...
static void ble_ll_adv_event_done(struct ble_npl_event *ev) {
 ...
 * @param ev
 */
static void ble_ll_adv_sec_done(struct ble_ll_adv_sm *advsm) {
  struct ble_ll_adv_aux *aux;
  struct ble_ll_adv_aux *aux_next;
...
static void ble_ll_adv_sec_event_done(struct ble_npl_event *ev) {
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
  ble_npl_event_init(&advsm->adv_sec_txdone_ev, ble_ll_adv_sec_event_done,
                     advsm);
#endif
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_PERIODIC_ADV)
