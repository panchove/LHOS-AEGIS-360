...
#define NO_WAIT_PENDING_BITS                                                   \
  (BIT(BT_MESH_SETTINGS_NET_PENDING) | BIT(BT_MESH_SETTINGS_IV_PENDING) |      \
   BIT(BT_MESH_SETTINGS_SEQ_PENDING) | BIT(BT_MESH_SETTINGS_CDB_PENDING))

/* Pending flags that use CONFIG_BT_MESH_STORE_TIMEOUT */
#define GENERIC_PENDING_BITS                                                   \
  (BIT(BT_MESH_SETTINGS_NET_KEYS_PENDING) |                                    \
...
void bt_mesh_settings_store_schedule(enum bt_mesh_settings_flag flag) {
  ...
  remaining_ms =
    k_ticks_to_ms_floor32(k_work_delayable_remaining_get(&pending_store));
...
static void store_pending(struct ble_npl_event *work) {
  ...
  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_NET_PENDING)) {
    bt_mesh_net_pending_net_store();
  }

  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_IV_PENDING)) {
    bt_mesh_net_pending_iv_store();
  }

  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_SEQ_PENDING)) {
  ...
  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_CFG_PENDING)) {
    bt_mesh_cfg_pending_store();
  }

  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_MOD_PENDING)) {
    bt_mesh_model_pending_store();
  }

  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_VA_PENDING)) {
    bt_mesh_va_pending_store();
  }

#if IS_ENABLED(CONFIG_BT_MESH_CDB)
  if (atomic_test_and_clear_bit(pending_flags, BT_MESH_SETTINGS_CDB_PENDING)) {
...
void bt_mesh_settings_init(void) {
  ...
  SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failed to register bt_mesh_settings conf");
