 ...
 * @param instanceId The index of the service to return (used when multiple
 * services have the same UUID).
...
#if CONFIG_BT_NIMBLE_EXT_ADV
/**
 * @brief Retrieve the advertising object that can be used to advertise the
...
/**
 * @brief Called when the services are added/removed and sets a flag to indicate
 ...
 * @details Required to be called after setup of all services and
...
void NimBLEServer::start() {
  ...
  for (const auto &svc : m_svcVec) {
    ...
    // Set the descriptor handles now as the stack does not set these when the
    // service is started
    for (const auto &chr : svc->m_vChars) {
      for (auto &desc : chr->m_vDescriptors) {
        ble_gatts_find_dsc(svc->getUUID().getBase(), chr->getUUID().getBase(),
                           desc->getUUID().getBase(), &desc->m_handle);
      }
    }
  }
...
/**
 * @brief Set the server to automatically start advertising when a client
 * disconnects.
 ...
 * @return A NimBLEConnInfo instance with the peer connection information, or an
 * empty instance if not found.
 */
...
/**
 * @brief Get the connection information of a connected peer by connection
 * handle.
...
int NimBLEServer::handleGapEvent(ble_gap_event *event, void *arg) {
  ...
  case BLE_GAP_EVENT_CONNECT: {
    ...
    if (rc != 0) {
      NIMBLE_LOGE(LOG_TAG, "Connection failed rc = %d %s", rc,
                  NimBLEUtils::returnCodeToString(rc));
#if !CONFIG_BT_NIMBLE_EXT_ADV && CONFIG_BT_NIMBLE_ROLE_BROADCASTER
  ...
  case BLE_GAP_EVENT_DISCONNECT: {
    ...
    case BLE_HS_ENOTSYNCED:
      NIMBLE_LOGE(LOG_TAG, "Disconnect - host reset, rc=%d",
...
#if CONFIG_BT_NIMBLE_ROLE_CENTRAL
    if (pServer->m_pClient && pServer->m_pClient->m_connHandle ==
    ...
    pServer->m_pServerCallbacks->onDisconnect(pServer, peerInfo,
                                              event->disconnect.reason);
#if !CONFIG_BT_NIMBLE_EXT_ADV
    ...
    for (const auto &svc : pServer->m_svcVec) {
      for (const auto &chr : svc->m_vChars) {
        if (chr->getHandle() == event->subscribe.attr_handle) {
          rc =
            ble_gap_conn_find(event->subscribe.conn_handle, &peerInfo.m_desc);
          if (rc != 0) {
            break;
          }
          ...
          if (!peerInfo.isEncrypted() &&
              (chrProps & BLE_GATT_CHR_F_READ_AUTHEN ||
               chrProps & BLE_GATT_CHR_F_READ_AUTHOR ||
               chrProps & BLE_GATT_CHR_F_READ_ENC)) {
            NimBLEDevice::startSecurity(event->subscribe.conn_handle);
          }

          chr->m_pCallbacks->onSubscribe(
            chr, peerInfo,
  ...
  case BLE_GAP_EVENT_NOTIFY_RX: {
    if (pServer->m_pClient &&
        pServer->m_pClient->m_connHandle == event->notify_rx.conn_handle) {
...
#if CONFIG_BT_NIMBLE_ROLE_CENTRAL
    if (pServer->m_pClient &&
        pServer->m_pClient->m_connHandle == event->enc_change.conn_handle) {
      NimBLEClient::handleGapEvent(event, pServer->m_pClient);
    }
#endif
    break;
  ...
  case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE: {
    ...
    pServer->m_pServerCallbacks->onPhyUpdate(
      peerInfo, event->phy_updated.tx_phy, event->phy_updated.rx_phy);
    return 0;
  ...
  case BLE_GAP_EVENT_PASSKEY_ACTION: {
    ...
    if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
      ...
      pkey.passkey =
        NimBLEDevice::getSecurityPasskey(); // This is the passkey to be entered
                                            // on peer
      // if the (static)passkey is the default, check the callback for custom
      // value both values default to the same.
      if (pkey.passkey == 123456) {
        pkey.passkey = pServer->m_pServerCallbacks->onPassKeyDisplay();
      }
      rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
      NIMBLE_LOGD(LOG_TAG, "BLE_SM_IOACT_DISP; ble_sm_inject_io result: %d",
                  rc);
      ...
      pServer->m_pServerCallbacks->onConfirmPassKey(
      ...
      // }
      // rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
      // NIMBLE_LOGD(LOG_TAG, "BLE_SM_IOACT_OOB; ble_sm_inject_io result: %d",
      // rc);
 ...
 */
int NimBLEServer::handleGattEvent(uint16_t connHandle, uint16_t attrHandle,
                                  ble_gatt_access_ctxt *ctxt, void *arg) {
  NIMBLE_LOGD(LOG_TAG, "Gatt %s event",
  ...
  case BLE_GATT_ACCESS_OP_READ_CHR: {
    // Don't call readEvent if the buffer len is 0 (this is a follow up to a
    // previous read), or if this is an internal read (handle is NONE)
 ...
 *
 * As a BLE server operates, it will generate server level events such as a new
 * client connecting or a previous client disconnecting.  This function can be
 * called to register a callback handler that will be invoked when these events
 * are detected.
 *
 * @param [in] pCallbacks The callbacks to be invoked.
 * @param [in] deleteCallbacks if true callback class will be deleted when
 ...
 *
 * @details Immediately removes access to the service by clients, sends a
 * service changed indication, and removes the service (if applicable) from the
 * advertisements. The service is not deleted unless the deleteSvc parameter is
 * true, otherwise the service remains available and can be re-added in the
 * future. If desired a removed but not deleted service can be deleted later by
 * calling this method with deleteSvc set to true.
 *
 * @note The service will not be removed from the database until all open
 * connections are closed as it requires resetting the GATT server. In the
 * interim the service will have it's visibility disabled.
 *
 * @note Advertising will need to be restarted by the user after calling this as
 * we must stop advertising in order to remove the service.
 *
...
void NimBLEServer::removeService(NimBLEService *service, bool deleteSvc) {
  ...
  service->setRemoved(deleteSvc ? NIMBLE_ATT_REMOVE_DELETE
                                : NIMBLE_ATT_REMOVE_HIDE);
  serviceChanged();
#if !CONFIG_BT_NIMBLE_EXT_ADV && CONFIG_BT_NIMBLE_ROLE_BROADCASTER
...
/**
 * @brief Adds a service which was either already created but removed from
...
/**
 * @brief Resets the GATT server, used when services are added/removed after
 * initialization.
 */
 ...
 * @return True if successful.
 */
bool NimBLEServer::updatePhy(uint16_t connHandle, uint8_t txPhyMask,
                             uint8_t rxPhyMask, uint16_t phyOptions) {
  int rc =
    ble_gap_set_prefered_le_phy(connHandle, txPhyMask, rxPhyMask, phyOptions);
  if (rc != 0) {
 ...
 * @param [in] duration How long to advertise for in milliseconds, 0 = forever
 * (default).
 * @param [in] maxEvents Maximum number of advertisement events to send, 0 = no
 * limit (default).
 * @return True if advertising started successfully.
 * @details Start the server advertising its existence.  This is a convenience
 * function and is equivalent to retrieving the advertising object and invoking
 * start upon it.
 */
bool NimBLEServer::startAdvertising(uint8_t instId, int duration,
                                    int maxEvents) const {
 ...
 * @param [in] instId The extended advertisement instance ID to stop
 * advertising.
...
/**
 * @brief Start advertising.
 * @param [in] duration The duration in milliseconds to advertise for, default =
 * forever.
 * @return True if advertising started successfully.
 * @details Start the server advertising its existence. This is a convenience
 * function and is equivalent to retrieving the advertising object and invoking
 ...
 * @param [in] connHandle The connection handle of the client to get the MTU
 * value for.
 ...
 * @param [in] connHandle The connection handle of the peer to send the request
 * to.
 ...
 */
void NimBLEServer::updateConnParams(uint16_t connHandle, uint16_t minInterval,
 ...
 * The Data Length Extension (DLE) allows to increase the Data Channel Payload
 * from 27 bytes to up to 251 bytes. The peer needs to support the Bluetooth 4.2
 * specifications, to be capable of DLE.
 * @param [in] connHandle The connection handle of the peer to send the request
 * to.
 * @param [in] octets The preferred number of payload octets to use (Range
 * 0x001B-0x00FB).
 */
 ...
 * @param [in] connHandle The connection handle to create a client instance
 * from.
 * @return A pointer to the NimBLEClient instance or nullptr if there was an
 * error.
 * @note Only one instance is supported subsequent calls will overwrite the
 * previous client connection information and data.
 */
...
void NimBLEServerCallbacks::onPhyUpdate(NimBLEConnInfo &connInfo, uint8_t txPhy,
                                        uint8_t rxPhy) {
  NIMBLE_LOGD("NimBLEServerCallbacks",
              "onPhyUpdate: default, txPhy: %d, rxPhy: %d", txPhy, rxPhy);
} // onPhyUpdate
