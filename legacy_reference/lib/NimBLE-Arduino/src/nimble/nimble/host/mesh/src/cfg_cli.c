...
static int comp_data_status(struct bt_mesh_model *model,
  ...
  BT_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s", ctx->net_idx,
         ctx->app_idx, ctx->addr, buf->om_len,
         bt_hex(buf->om_data, buf->om_len));

  if (!bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, OP_DEV_COMP_DATA_STATUS,
                                 ctx->addr, (void **)&param)) {
    return -ENOENT;
  }
...
static int ttl_status(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
                      struct os_mbuf *buf) {
...
static int node_reset_status(struct bt_mesh_model *model,
                             struct bt_mesh_msg_ctx *ctx, struct os_mbuf *buf) {
  bool *param = NULL;
  BT_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x", ctx->net_idx, ctx->app_idx,
         ctx->addr);
...
static int mod_app_status(struct bt_mesh_model *model,
  ...
  if (param->elem_addr != elem_addr || param->mod_app_idx != mod_app_idx ||
...
static int get_state_u8(uint16_t net_idx, uint16_t addr, uint32_t op,
                        uint32_t rsp, uint8_t *val) {
...
static int set_state_u8(uint16_t net_idx, uint16_t addr, uint32_t op,
                        uint32_t rsp, uint8_t new_val, uint8_t *val) {
...
int bt_mesh_cfg_beacon_set(uint16_t net_idx, uint16_t addr, uint8_t val,
                           uint8_t *status) {
  return set_state_u8(net_idx, addr, OP_BEACON_SET, OP_BEACON_STATUS, val,
                      status);
}

int bt_mesh_cfg_ttl_get(uint16_t net_idx, uint16_t addr, uint8_t *ttl) {
  return get_state_u8(net_idx, addr, OP_DEFAULT_TTL_GET, OP_DEFAULT_TTL_STATUS,
                      ttl);
}

int bt_mesh_cfg_ttl_set(uint16_t net_idx, uint16_t addr, uint8_t val,
                        uint8_t *ttl) {
  return set_state_u8(net_idx, addr, OP_DEFAULT_TTL_SET, OP_DEFAULT_TTL_STATUS,
                      val, ttl);
}
...
int bt_mesh_cfg_friend_set(uint16_t net_idx, uint16_t addr, uint8_t val,
                           uint8_t *status) {
  return set_state_u8(net_idx, addr, OP_FRIEND_SET, OP_FRIEND_STATUS, val,
                      status);
}

int bt_mesh_cfg_gatt_proxy_get(uint16_t net_idx, uint16_t addr,
                               uint8_t *status) {
  return get_state_u8(net_idx, addr, OP_GATT_PROXY_GET, OP_GATT_PROXY_STATUS,
                      status);
}

int bt_mesh_cfg_gatt_proxy_set(uint16_t net_idx, uint16_t addr, uint8_t val,
                               uint8_t *status) {
  return set_state_u8(net_idx, addr, OP_GATT_PROXY_SET, OP_GATT_PROXY_STATUS,
                      val, status);
}
...
int bt_mesh_cfg_mod_app_bind(uint16_t net_idx, uint16_t addr,
  ...
  return mod_app_bind(net_idx, addr, elem_addr, mod_app_idx, mod_id, CID_NVAL,
                      status);
}
...
int bt_mesh_cfg_mod_app_unbind(uint16_t net_idx, uint16_t addr,
                               uint16_t elem_addr, uint16_t mod_app_idx,
                               uint16_t mod_id, uint8_t *status) {
  return mod_app_unbind(net_idx, addr, elem_addr, mod_app_idx, mod_id, CID_NVAL,
                        status);
}
...
static int mod_member_list_get(uint32_t op, uint32_t expect_op,
  ...
  BT_DBG("net_idx 0x%04x addr 0x%04x elem_addr 0x%04x", net_idx, addr,
         elem_addr);
...
int bt_mesh_cfg_mod_app_get(uint16_t net_idx, uint16_t addr, uint16_t elem_addr,
                            ...
                            size_t *app_cnt) {
  return mod_member_list_get(OP_SIG_MOD_APP_GET, OP_SIG_MOD_APP_LIST, net_idx,
                             addr, elem_addr, mod_id, CID_NVAL, status, apps,
                             app_cnt);
}
...
int bt_mesh_cfg_mod_app_get_vnd(uint16_t net_idx, uint16_t addr,
  ...
  return mod_member_list_get(OP_VND_MOD_APP_GET, OP_VND_MOD_APP_LIST, net_idx,
                             addr, elem_addr, mod_id, cid, status, apps,
                             app_cnt);
}

static int mod_sub(uint32_t op, uint16_t net_idx, uint16_t addr,
                   uint16_t elem_addr, uint16_t sub_addr, uint16_t mod_id,
...
int bt_mesh_cfg_mod_sub_add(uint16_t net_idx, uint16_t addr, uint16_t elem_addr,
  ...
  return mod_sub(OP_MOD_SUB_ADD, net_idx, addr, elem_addr, sub_addr, mod_id,
                 CID_NVAL, status);
}
...
int bt_mesh_cfg_mod_sub_del(uint16_t net_idx, uint16_t addr, uint16_t elem_addr,
  ...
  return mod_sub(OP_MOD_SUB_DEL, net_idx, addr, elem_addr, sub_addr, mod_id,
                 CID_NVAL, status);
}
...
int bt_mesh_cfg_mod_sub_overwrite(uint16_t net_idx, uint16_t addr,
  ...
  return mod_sub(OP_MOD_SUB_OVERWRITE, net_idx, addr, elem_addr, sub_addr,
...
static int mod_sub_va(uint32_t op, uint16_t net_idx, uint16_t addr,
                      uint16_t elem_addr, const uint8_t label[16],
                      uint16_t mod_id, uint16_t cid, uint16_t *virt_addr,
                      uint8_t *status) {
  ...
  BT_DBG("net_idx 0x%04x addr 0x%04x elem_addr 0x%04x label %s", net_idx, addr,
         elem_addr, bt_hex(label, 16));
...
int bt_mesh_cfg_mod_sub_va_add(uint16_t net_idx, uint16_t addr,
                               ...
                               uint8_t *status) {
  return mod_sub_va(OP_MOD_SUB_VA_ADD, net_idx, addr, elem_addr, label, mod_id,
...
int bt_mesh_cfg_mod_sub_va_del(uint16_t net_idx, uint16_t addr,
                               ...
                               uint16_t mod_id, uint16_t *virt_addr,
                               uint8_t *status) {
  return mod_sub_va(OP_MOD_SUB_VA_DEL, net_idx, addr, elem_addr, label, mod_id,
                    CID_NVAL, virt_addr, status);
}
...
int bt_mesh_cfg_mod_sub_va_overwrite(uint16_t net_idx, uint16_t addr,
                                     ...
                                     uint16_t *virt_addr, uint8_t *status) {
  return mod_sub_va(OP_MOD_SUB_VA_OVERWRITE, net_idx, addr, elem_addr, label,
...
int bt_mesh_cfg_mod_sub_get(uint16_t net_idx, uint16_t addr, uint16_t elem_addr,
                            ...
                            size_t *sub_cnt) {
  return mod_member_list_get(OP_MOD_SUB_GET, OP_MOD_SUB_LIST, net_idx, addr,
                             elem_addr, mod_id, CID_NVAL, status, subs,
                             sub_cnt);
}
...
int bt_mesh_cfg_mod_sub_get_vnd(uint16_t net_idx, uint16_t addr,
  ...
  return mod_member_list_get(OP_MOD_SUB_GET_VND, OP_MOD_SUB_LIST_VND, net_idx,
                             addr, elem_addr, mod_id, cid, status, subs,
                             sub_cnt);
}
...
int bt_mesh_cfg_mod_pub_set(uint16_t net_idx, uint16_t addr, uint16_t elem_addr,
                            ...
                            uint8_t *status) {
  if (pub->uuid) {
    return mod_pub_va_set(net_idx, addr, elem_addr, mod_id, CID_NVAL, pub,
                          status);
  } else {
...
int32_t bt_mesh_cfg_cli_timeout_get(void) { return msg_timeout; }

void bt_mesh_cfg_cli_timeout_set(int32_t timeout) { msg_timeout = timeout; }

int bt_mesh_comp_p0_get(struct bt_mesh_comp_p0 *page, struct os_mbuf *buf) {
  if (buf->om_len < 10) {
    return -EINVAL;
  }
...
bt_mesh_comp_p0_elem_pull(const struct bt_mesh_comp_p0 *page,
...
uint16_t bt_mesh_comp_p0_elem_mod(struct bt_mesh_comp_p0_elem *elem, int idx) {
  CHECKIF(idx >= elem->nsig) { return 0xffff; }
