...
static int hci_transport_controller_packet_rx(hci_driver_data_type_t data_type,
                                              uint8_t *data) {
...
static int hci_transport_host_packet_rx(hci_driver_data_type_t data_type,
                                        uint8_t *data, uint32_t length) {
  if (!s_hci_transport_env.host_recv_cb) {
    return -1;
  }
  return s_hci_transport_env.host_recv_cb((hci_trans_pkt_ind_t)data_type, data,
                                          length);
}

static int hci_transport_packet_rx(hci_driver_data_type_t data_type,
                                   uint8_t *data, uint32_t length,
...
static int hci_transport_controller_tx_dummy(void *data, void *arg) {
  return -1;
}

static int hci_transport_controller_evt_tx(uint8_t *hci_ev, void *arg) {
  uint32_t len;
  ...
  len = hci_ev[1] + 2;
  return s_hci_transport_env.driver_ops->hci_driver_tx(
    HCI_DRIVER_TYPE_EVT, hci_ev, len, HCI_DRIVER_DIR_C2H);
}

static int hci_transport_controller_acl_tx(struct os_mbuf *om, void *arg) {
  uint16_t len;
  if (esp_bt_controller_get_status() != ESP_BT_CONTROLLER_STATUS_ENABLED) {
  ...
  return s_hci_transport_env.driver_ops->hci_driver_tx(
    HCI_DRIVER_TYPE_ACL, (uint8_t *)om, len, HCI_DRIVER_DIR_C2H);
}

/* Functions for host Tx. */
int na_hci_transport_host_cmd_tx(uint8_t *data, uint32_t length) {
  return s_hci_transport_env.driver_ops->hci_driver_tx(
    HCI_DRIVER_TYPE_CMD, data, length, HCI_DRIVER_DIR_H2C);
}

int na_hci_transport_host_acl_tx(uint8_t *data, uint32_t length) {
  return s_hci_transport_env.driver_ops->hci_driver_tx(
    HCI_DRIVER_TYPE_ACL, data, length, HCI_DRIVER_DIR_H2C);
}

int na_hci_transport_host_callback_register(
  hci_transport_host_recv_fn *callback) {
...
int na_hci_transport_init(uint8_t hci_transport_mode) {
  ...
  r_ble_hci_trans_cfg_hs(hci_transport_controller_evt_tx, NULL,
                         hci_transport_controller_acl_tx, NULL);
...
void na_hci_transport_deinit(void) {
  ...
  r_ble_hci_trans_cfg_hs(
    (esp_hci_internal_rx_cmd_fn *)hci_transport_controller_tx_dummy, NULL,
