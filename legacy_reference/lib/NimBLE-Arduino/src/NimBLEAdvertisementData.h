...
class NimBLEUUID;
/**
 * @brief Advertisement data set by the programmer to be published by the BLE
 * server.
 */
class NimBLEAdvertisementData {
  // Only a subset of the possible BLE architected advertisement fields are
  // currently exposed.  Others will be exposed on demand/request or as time
  // permits.
  //
public:
  ...
  bool setServiceData(const NimBLEUUID &uuid, const uint8_t *data,
                      size_t length);
...
private:
  ...
  bool setServices(bool complete, uint8_t size,
