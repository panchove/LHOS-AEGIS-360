 ...
 * Apache 2.0 License. Additions Copyright (C) 2017 Evandro Luis Copercini,
 * Apache 2.0 License. Additions Copyright (C) 2019 Vadim Govorovski.
 */
#ifdef PLATFORMIO
#include <Arduino.h>
#endif
...
#error                                                                         \
  "Please configure IDF framework to include mbedTLS -> Enable pre-shared-key ciphersuites and activate at least one cipher"
#endif
...
static int _handle_error(int err, const char *function, int line) {
  ...
  if (err == MBEDTLS_ERR_NET_SEND_FAILED) {
    strncpy(error_buf, "Failed to send data - underlying network layer error",
            sizeof(error_buf) - 1);
...
static int client_net_recv(void *ctx, unsigned char *buf, size_t len) {
  ...
  // if (result > 0) {
  // esp_log_buffer_hexdump_internal("SSL.RD", buf, (uint16_t)result,
  // ESP_LOG_VERBOSE);
  // }
 ...
 * \return int      -2 if connect failed.
 */
int client_net_recv_timeout(void *ctx, unsigned char *buf, size_t len,
                            uint32_t timeout) {
  Client *client = (Client *)ctx;
  ...
  log_v("SSL client RX (received=%d expected=%zu in %lums)", result, len,
        millis() - start);
...
static int client_net_send(void *ctx, const unsigned char *buf, size_t len) {
  ...
  // esp_log_buffer_hexdump_internal("SSL.WR", buf, (uint16_t)len,
  // ESP_LOG_VERBOSE);BEDTLS_ERR_NET_SEND_FAILED;
...
/**
 * \brief Cleans up allocated resources and stops the SSL socket if an error
 * occurred.
 *
 * \param ssl_client Pointer to the SSL client context.
 * \param ca_cert_initialized Flag indicating if the CA certificate was
 * initialized.
 * \param client_cert_initialized Flag indicating if the client certificate was
 * initialized.
 * \param client_key_initialized Flag indicating if the client key was
 * initialized.
 ...
 */
void cleanup(sslclient__context *ssl_client, bool ca_cert_initialized,
             bool client_cert_initialized, bool client_key_initialized, int ret,
  ...
  if (ret != 0) {
    stop_ssl_socket(ssl_client, rootCABuff, cli_cert,
                    cli_key); // Stop SSL socket on error
  }
...
/**
 * \brief Starts the SSL client, handling initialization, authentication, and
 ...
 */
int start_ssl_client(sslclient__context *ssl_client, const char *host,
                     uint32_t port, int timeout, const char *rootCABuff,
                     bool useRootCABundle, const char *cli_cert,
                     const char *cli_key, const char *pskIdent,
  ...
  do {
    ret = init_tcp_connection(ssl_client, host,
                              port); // Step 1 - Initiate TCP connection
    if (ret != 0) {
      break;
    }
    ret = seed_random_number_generator(
      ssl_client); // Step 2 - Seed the random number generator
    if (ret == MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED || ret != 0) {
      break;
    }
    log_v("Random number generator seeded, ret: %d", ret);
    ret =
      set_up_tls_defaults(ssl_client); // Step 3 - Set up the SSL/TLS defaults
    if (ret != 0) { // MBEDTLS_ERR_XXX_ALLOC_FAILED undefined?
      break;
    }
    if (alpn_protos != NULL) {
      log_v("Setting ALPN protocols");
      if ((ret = mbedtls_ssl_conf_alpn_protocols(&ssl_client->ssl_conf,
                                                 alpn_protos)) != 0) {
        return handle_error(ret);
      }
    }
    log_v("SSL config defaults set, ret: %d", ret);
    ret = auth_root_ca_buff(
      ssl_client, rootCABuff, &ca_cert_initialized, pskIdent, psKey,
      insecure); // Step 4 route a - Set up required auth mode rootCaBuff
    if (ret != 0) {
      break;
    }
    log_v("SSL auth mode set, ret: %d", ret);
    ret = auth_client_cert_key(
      ssl_client, cli_cert, cli_key, &client_cert_initialized,
      &client_key_initialized); // Step 4 route b - Set up required
                                // auth mode cli_cert and cli_key
    if (ret != 0) {
      break;
    }
    log_v("SSL client cert and key set, ret: %d", ret);
    ret = set_hostname_for_tls(
      ssl_client, host); // Step 5 - Set the hostname for a TLS session
    if (ret != 0) {
      break;
    }
    log_v("SSL hostname set, ret: %d", ret);
    ret = set_io_callbacks_and_timeout(
      ssl_client,
      timeout); // Step 6 - Configure IO callbacks and set a
                // read timeout for the SSL client context
    if (ret != 0) {
      break;
    }
    log_v("SSL IO callbacks and timeout set, ret: %d", ret);
    ret = perform_ssl_handshake(ssl_client, cli_cert,
                                cli_key); // Step 7 - Perform SSL/TLS handshake
    if (ret != 0) {
      break;
    }
    int flags =
      verify_server_cert(ssl_client); // Step 8 - Verify the server certificate
    ret = flags;
    if (ret != 0) {
  ...
  cleanup(ssl_client, ca_cert_initialized, client_cert_initialized,
          client_key_initialized, ret, rootCABuff, cli_cert, cli_key);
...
/**
 * \brief             Initializes a TCP connection to a remote host on the
 * specified port.
 *
 * \param ssl_client  sslclient__context* - The SSL client context.
 ...
 *
 * This function initiates a TCP connection to a remote host on the specified
 * port using the provided SSL client context. It checks if the Client pointer
 * within the context is valid, attempts to establish the TCP connection, and
 * returns appropriate error codes if any issues are encountered.
 */
int init_tcp_connection(sslclient__context *ssl_client, const char *host,
                        uint32_t port) {
  ...
  if (!pClient->connect(host, port)) {
    log_e("Connection to server failed, is the signal good, server available "
          "at this address "
          "and timeout sufficient? %s:%d",
          host, port);
    return -2;
  }
 ...
 *
 * This function initializes the random number generator used in SSL/TLS
 * operations. It sets up the entropy source and uses it to seed the
 * deterministic random bit generator (DRBG). The DRBG is essential for
 * generating secure cryptographic keys and nonces during SSL/TLS communication.
...
int seed_random_number_generator(sslclient__context *ssl_client) {
  ...
  int ret = mbedtls_ctr_drbg_seed(&ssl_client->drbg_ctx, mbedtls_entropy_func,
                                  ...
                                  (const unsigned char *)persy, strlen(persy));
  return ret;
}
 ...
 * \param ssl_client  sslclient__context* - The SSL client context.
 *
 * \return int        0 if SSL/TLS configuration is successfully set up with
 * defaults.
 * \return int        An error code if the setup process fails.
 *
 * This function configures SSL/TLS settings with default values, including
 * specifying that it's used as a client, operating in a stream transport mode,
 * and applying the default preset. The SSL/TLS configuration is essential for
 * establishing secure communication over the network. If successful, the
 * function returns 0; otherwise, it returns an error code.
 */
int set_up_tls_defaults(sslclient__context *ssl_client) {
  log_v("Setting up the SSL/TLS defaults...");

  int ret = mbedtls_ssl_config_defaults(
    &ssl_client->ssl_conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
 ...
 * \param ca_cert_initialized bool* - Indicates whether CA certificate is
 * initialized.
 * \param pskIdent         const char* - The PSK identity.
 * \param psKey            const char* - The PSK key.
 * \param func_ret         int* - Pointer to an integer to hold the return
 * value.
 * \param insecure         bool - Flag indicating if the connection is insecure.
 *
 * \return int             0 if the SSL/TLS authentication options are
 * configured successfully.
 * \return int             An error code if the configuration process fails.
 *
 * This function configures SSL/TLS authentication options based on the provided
 * parameters. If `rootCABuff` is not NULL, it loads the root CA certificate and
 * configures SSL/TLS to require verification. If `pskIdent` and `psKey` are not
 * NULL, it sets up a pre-shared key (PSK) for authentication. If none of the
 * options are provided, it configures SSL/TLS with no verification. The
 * function may modify the value pointed to by `func_ret` to indicate errors. If
 * successful, the function returns 0; otherwise, it returns an error code, -1
 * for a null context.
 */
int auth_root_ca_buff(sslclient__context *ssl_client, const char *rootCABuff,
                      bool *ca_cert_initialized, const char *pskIdent,
                      const char *psKey, bool insecure) {
  ...
  if (insecure) {
    ...
    mbedtls_ssl_conf_authmode(&ssl_client->ssl_conf,
                              MBEDTLS_SSL_VERIFY_REQUIRED);
    ret = mbedtls_x509_crt_parse(&ssl_client->ca_cert,
                                 (const unsigned char *)rootCABuff,
                                 strlen(rootCABuff) + 1);
    ...
    mbedtls_ssl_conf_ca_chain(&ssl_client->ssl_conf, &ssl_client->ca_cert,
                              NULL);
    ...
    // set mbedtls config
    ret =
      mbedtls_ssl_conf_psk(&ssl_client->ssl_conf, psk, psk_len,
 ...
 *
 * This function initializes and loads the client's certificate and private key
 * into the provided SSL client context. It also provides a status of the
 * initialization of the certificate and key.
 *
 ...
 * \param[out] client_cert_initialized Pointer to a boolean indicating if the
 * client certificate was initialized.
 * \param[out] client_key_initialized Pointer to a boolean indicating if the
 * client key was initialized.
 *
 * \return 0 if successful, or a non-zero error code indicating a failure during
 * the initialization or parsing. Positive error codes indicate number of certs
 * that failed. Negative error codes indicate a PEM or x509 error.
 */
int auth_client_cert_key(sslclient__context *ssl_client, const char *cli_cert,
                         const char *cli_key, bool *client_cert_initialized,
  ...
  if (cli_cert != NULL && cli_key != NULL) {
    ...
    ret = mbedtls_x509_crt_parse(&ssl_client->client_cert,
                                 (const unsigned char *)cli_cert,
                                 strlen(cli_cert) + 1);
    if (ret != 0) {
      // if ret > 0 n certs failed, ret < 0 pem or x509 error code.
      return ret;
    ...
    ret = mbedtls_pk_parse_key(
      &ssl_client->client_key, (const unsigned char *)cli_key,
      strlen(cli_key) + 1, NULL, 0, mbedtls_ctr_drbg_random, &ctr_drbg);
    mbedtls_ctr_drbg_free(&ctr_drbg);
#else
    ret = mbedtls_pk_parse_key(&ssl_client->client_key,
                               (const unsigned char *)cli_key,
                               strlen(cli_key) + 1, NULL, 0);
#endif
    if (ret != 0) { // PK or PEM non-zero error codes
      mbedtls_x509_crt_free(
        &ssl_client->client_cert); // cert+key are free'd in pair
      return ret;
    } else {
    ...
    ret = mbedtls_ssl_conf_own_cert(
      &ssl_client->ssl_conf, &ssl_client->client_cert, &ssl_client->client_key);
  }
  return ret;
}
...
int set_hostname_for_tls(sslclient__context *ssl_client, const char *host) {
  ...
  if (ret == MBEDTLS_ERR_SSL_ALLOC_FAILED ||
      ret == MBEDTLS_ERR_SSL_BAD_INPUT_DATA || ret != 0) {
    log_e("Failed to set hostname for tls session");
    return ret;
  }

  mbedtls_ssl_conf_rng(&ssl_client->ssl_conf, mbedtls_ctr_drbg_random,
                       &ssl_client->drbg_ctx);
...
/**
 * \brief Configures IO callbacks and sets a read timeout for the SSL client
 * context.
 *
 * This function sets up the IO callbacks for sending, receiving, and receiving
 * with timeout for the provided SSL client context. It also configures the read
 * timeout for the SSL client context.
 *
 * \param ssl_client A pointer to the sslclient__context structure representing
 * the SSL client context.
 ...
 * \endcode
 *
 * \note The function assumes that the sslclient__context structure is properly
 * initialized and the client_net_send, client_net_recv, and
 * client_net_recv_timeout functions are correctly implemented.
 */
int set_io_callbacks_and_timeout(sslclient__context *ssl_client, int timeout) {
  ...
  mbedtls_ssl_set_bio(&ssl_client->ssl_ctx, ssl_client->client, client_net_send,
                      NULL, client_net_recv_timeout);
 ...
 *
 * This function initiates and manages the SSL/TLS handshake process. It also
 * checks for timeout conditions and handles client certificate and key if
 * provided.
 *
 * \param ssl_client A pointer to the sslclient__context structure representing
 * the SSL client context.
 * \param func_ret A pointer to an integer where a specific error code can be
 * stored for further analysis.
 * \param cli_cert A pointer to a character string representing the client's
 * certificate. If not needed, pass NULL.
 * \param cli_key A pointer to a character string representing the client's
 * private key. If not needed, pass NULL.
 *
 * \return int Returns 0 on successful handshake completion. Returns -1 if the
 * handshake process times out. Returns a mbedtls error code if any other error
 * occurs during the handshake process.
 *
 * Usage:
 * \code
 *      sslclient__context ssl_client;
 ...
 * CERTIFICATE-----"; const char *cli_key = "-----BEGIN PRIVATE
 * KEY-----\n...\n-----END PRIVATE KEY-----"; int ret =
 * perform_ssl_handshake(&ssl_client, &func_ret, cli_cert, cli_key); if(ret !=
 * 0) {
 *          // handle error
 *      }
 * \endcode
 *
 * \note This function assumes that the sslclient__context structure is properly
 * initialized and the mbedtls libraries are correctly configured.
 */
int perform_ssl_handshake(sslclient__context *ssl_client, const char *cli_cert,
                          const char *cli_key) {
  if (ssl_client == nullptr) {
    log_e("Uninitialised context!");
    return -1;
  ...
  log_v("Performing the SSL/TLS handshake, timeout %lu ms",
        ssl_client->handshake_timeout);
  unsigned long handshake_start_time = millis();
  log_d("calling mbedtls_ssl_handshake with ssl_ctx address %p",
        (void *)&ssl_client->ssl_ctx);
  ...
  if (cli_cert != NULL && cli_key != NULL) {
    log_d("Protocol is %s Ciphersuite is %s",
...
/**
 * \brief Verifies the server's certificate using the provided SSL client
 * context.
 *
 * This function performs a verification of the server's certificate to ensure
 * it's valid and trustworthy. The verification process checks the server
 * certificate against the provided root CA. If client certificate and key are
 * provided, they can be used for further verification or cleanup.
 *
 * \param ssl_client A pointer to the sslclient__context structure representing
 * the SSL client context.
 * \param ret The return value of the mbedtls_ssl_handshake function.
 * \param rootCABuff A pointer to a character string containing the root CA
 * certificate.
 * \param cli_cert A pointer to a character string representing the client's
 * certificate. If not needed, pass NULL.
 * \param cli_key A pointer to a character string representing the client's
 * private key. If not needed, pass NULL.
 *
 * \return int Returns 0 on successful verification. Returns a non-zero error
 * code on failure, which can be obtained from the mbedtls library.
 *
 * Usage:
 * \code
 *      sslclient__context ssl_client;
 *      const char *rootCABuff = "-----BEGIN CERTIFICATE-----\n...\n-----END
 * CERTIFICATE-----"; const char *cli_cert = "-----BEGIN
 * CERTIFICATE-----\n...\n-----END CERTIFICATE-----"; const char *cli_key =
 * "-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----"; int ret =
 * verify_server_cert(&ssl_client, rootCABuff, cli_cert, cli_key); if(ret != 0)
 * {
 *          // handle error
 *      }
 * \endcode
 *
 * \note This function assumes that the sslclient__context structure is properly
 * initialized and the mbedtls libraries are correctly configured. Also, ensure
 * that the root CA certificate is correct and corresponds to the CA that issued
 ...
 */
void stop_ssl_socket(sslclient__context *ssl_client, const char *rootCABuff,
                     const char *cli_cert, const char *cli_key) {
  log_d("Cleaning SSL connection.");

  // Stop the client connection
  if (ssl_client && ssl_client->client) {
    log_d("Stopping SSL client. Current client pointer address: %p",
          (void *)ssl_client->client);
...
#endif
    log_d("Freeing CA cert. Current ca_cert address: %p",
          (void *)&ssl_client->ca_cert);
...
#endif
    log_d("Freeing client cert and client key. Current client_cert address: "
          "%p, client_key "
          "address: %p",
  ...
  log_d("Freeing SSL context. Current ssl_ctx address: %p",
        (void *)&ssl_client->ssl_ctx);
  mbedtls_ssl_free(&ssl_client->ssl_ctx);

  log_d("Freeing SSL config. Current ssl_conf address: %p",
        (void *)&ssl_client->ssl_conf);
  mbedtls_ssl_config_free(&ssl_client->ssl_conf);

  log_d("Freeing DRBG context. Current drbg_ctx address: %p",
        (void *)&ssl_client->drbg_ctx);
  mbedtls_ctr_drbg_free(&ssl_client->drbg_ctx);

  log_d("Freeing entropy context. Current entropy_ctx address: %p",
        (void *)&ssl_client->entropy_ctx);
 ...
 */
int send_ssl_data(sslclient__context *ssl_client, const uint8_t *data,
                  size_t len) {
  if (ssl_client != nullptr) {
 ...
 */
int get_ssl_receive(sslclient__context *ssl_client, uint8_t *data,
                    size_t length) {
...
/**
 * \brief               Compare a name from certificate and domain name, return
 * true if they match.
 *
 * \param name          const string& - The name from certificate.
 * \param domainName    const string& - The domain name.
 * \return bool         True if the name from certificate and domain name match,
 * false otherwise.
 */
...
/**
 * \brief               Verifies certificate provided by the peer to match
 * specified SHA256 fingerprint.
 *
 * \param ssl_client    sslclient__context* - The ssl client context.
 * \param fp            const char* - The SHA256 fingerprint.
 * \param domain_name   const char* - The domain name.
 * \return bool         True if the certificate matches the fingerprint, false
 * otherwise.
 */
bool verify_ssl_fingerprint(sslclient__context *ssl_client, const char *fp,
  ...
  for (size_t i = 0; i < sizeof(fingerprint_local); ++i) {
    ...
    uint8_t high, low;
    if (!parse_hex_nibble(fp[pos], &high) ||
        !parse_hex_nibble(fp[pos + 1], &low)) {
      log_d("pos:%d len:%d invalid hex sequence: %c%c", pos, len, fp[pos],
            fp[pos + 1]);
      return false;
    }
...
/**
 * \brief               Checks if peer certificate has specified domain in CN or
 * SANs.
 *
 * \param ssl_client    sslclient__context* - The ssl client context.
 * \param domain_name   const char* - The domain name.
 * \return bool         True if the certificate has the domain name, false
 * otherwise.
 */
bool verify_ssl_dn(sslclient__context *ssl_client, const char *domain_name) {
  ...
  transform(domain_name_str.begin(), domain_name_str.end(),
            domain_name_str.begin(), ::tolower);
  ...
  while (common_name != nullptr) {
    ...
    if (!MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &common_name->oid)) {
      string common_name_str((const char *)common_name->val.p,
                             common_name->val.len);
