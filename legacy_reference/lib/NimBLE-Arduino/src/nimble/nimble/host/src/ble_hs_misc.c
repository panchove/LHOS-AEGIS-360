...
static int ble_hs_misc_restore_one_irk(int obj_type, union ble_store_value *val,
                                       void *cookie) {
  ...
  if (sec->irk_present) {
    rc =
      ble_hs_pvcy_add_entry(sec->peer_addr.val, sec->peer_addr.type, sec->irk);
    if (rc != 0) {
...
int ble_hs_misc_restore_irks(void) {
  ...
  rc = ble_store_iterate(BLE_STORE_OBJ_TYPE_PEER_SEC,
                         ble_hs_misc_restore_one_irk, NULL);
  return rc;
}
