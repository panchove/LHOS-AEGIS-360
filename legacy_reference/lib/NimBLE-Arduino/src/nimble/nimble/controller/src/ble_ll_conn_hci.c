...
int ble_ll_conn_hci_create_check_scan(struct ble_ll_conn_create_scan *p) {
...
static int ble_ll_conn_hci_create_check_params(
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
static void ble_ll_conn_hci_ext_create_set_fb_params(
...
static int ble_ll_conn_hci_ext_create_parse_params(
...
int ble_ll_conn_hci_ext_create(const uint8_t *cmdbuf, uint8_t len) {
  ...
  /* validate length */
  if (len <
      sizeof(*cmd) + __builtin_popcount(cmd->init_phy_mask) * sizeof(*params)) {
    return BLE_ERR_INV_HCI_CMD_PARMS;
  }
  ...
  ble_ll_conn_hci_ext_create_set_fb_params(cc_scan.init_phy_mask, cc_params_fb);
...
static int ble_ll_conn_process_conn_params(
...
static void ble_ll_conn_hci_update_complete_event(void *user_data) {
  ...
  if (connsm->flags.conn_update_host_w4reply) {
    ...
    if (rc == BLE_ERR_SUCCESS) {
      ...
      if (om) {
        dptr = om->om_data;
        rsp_opcode =
          ble_ll_ctrl_conn_param_reply(connsm, dptr, &connsm->conn_cp);
        dptr[0] = rsp_opcode;
...
int ble_ll_conn_hci_param_nrr(const uint8_t *cmdbuf, uint8_t len,
  ...
  if (connsm->flags.conn_update_host_w4reply) {
    /* XXX: check return code and deal */
    ble_ll_ctrl_reject_ind_send(connsm, connsm->host_reply_opcode, cmd->reason);
    connsm->flags.conn_update_host_w4reply = 0;
 ...
 */
static void ble_ll_conn_hci_cancel_conn_complete_event(void *user_data) {
  BLE_LL_ASSERT(g_ble_ll_conn_comp_ev);

  ble_ll_conn_comp_event_send(NULL, BLE_ERR_UNK_CONN_ID, g_ble_ll_conn_comp_ev,
                              NULL);
...
int ble_ll_set_default_sync_transfer_params(const uint8_t *cmdbuf,
                                            uint8_t len) {
