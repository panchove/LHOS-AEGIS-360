...
typedef int ble_att_rx_fn(uint16_t conn_handle, uint16_t cid,
                          struct os_mbuf **om);
...
int ble_att_conn_chan_find(uint16_t conn_handle, uint16_t cid,
  ...
  return ble_hs_misc_conn_chan_find(conn_handle, cid, out_conn, out_chan);
}

int ble_att_conn_chan_find_by_psm(uint16_t conn_handle, uint16_t psm,
                                  struct ble_hs_conn **out_conn,
                                  struct ble_l2cap_chan **out_chan) {
...
uint16_t ble_att_chan_mtu(const struct ble_l2cap_chan *chan) {
  ...
  if (ble_hs_cfg.eatt && chan->psm == BLE_EATT_PSM) {
    /* The ATT_MTU for the Enhanced ATT bearer shall be set to the minimum of
     * the MTU field values of the two devices. Reference: Core v5.4 Vol 3 Part
     * G 5.3.1 ATT_MTU
     */
...
static void ble_att_send_outstanding_after_response(uint16_t conn_handle) {
  struct ble_hs_conn *conn;
  struct ble_l2cap_chan *chan;
  int rc;
...
static int ble_att_rx_extended(uint16_t conn_handle, uint16_t cid,
                               struct os_mbuf **om) {
  const struct ble_att_rx_dispatch_entry *entry;
  uint8_t op;
  int rc;
...
uint16_t ble_att_preferred_mtu(void) { return ble_att_preferred_mtu_val; }
...
int ble_att_init(void) {
  ...
  rc = stats_init_and_reg(STATS_HDR(ble_att_stats),
                          STATS_SIZE_INIT_PARMS(ble_att_stats, STATS_SIZE_32),
