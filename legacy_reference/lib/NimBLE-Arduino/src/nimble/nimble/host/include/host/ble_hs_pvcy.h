...
/* Called to configure local(own) privacy (RPA/NRPA) when using Host based
 * privacy. In Host based privacy, as controller is not aware of RPA/NRPA
 * address is in use, we do it through 'BLE_ADDR_RANDOM (0x01)' addr_type route.
 * This is necessary so as to set the private address as random address in
 * controller. Remember to configure `BLE_SM_PAIR_KEY_DIST_ID` in our & their
 * key distributions for using RPA. For NRPA part of privacy it is not
 * necessary to configure key distributions in host, as anyway NRPA is
 * non-resolvable. Please call this API once host-controller are synced as we
 * set the private (RPA/NRPA) address using host-controller HCI commands.
 *
 ...
 *                       enable NRPA when param     = 2
 * (NIMBLE_HOST_ENABLE_NRPA) disable privacy when param = 0
 * (NIMBLE_HOST_DISABLE_PRIVACY)
 *
