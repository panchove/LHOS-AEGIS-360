// Implementation for ELA Innovation Blue PUCK Presence (PIR)
#include <modules/ble_devices/BleSensorData.h>
#include <modules/ble_devices/devices/ELA_PUCK_PIR.h>

static inline std::string toLowerPir(std::string s) {
  for (auto &c : s)
...
static inline bleSensorData parseKV(const std::string &e) {
  ...
  if (id.empty())
    return {"", ""};
  return {toLowerPir(id), v};
}

std::string ELA_PUCK_PIR_decoder::decode(const char *macAddress,
  ...
  if (pirBytes.size() == 2) {
    uint16_t raw = (pirBytes[1] << 8) | pirBytes[0];
    bool status = (raw & 0x0001) == 0x0001;
    uint16_t count = (raw >> 1) & 0x7FFF;
    out << "ble." << macAddress << ".presence.status:" << (status ? 1 : 0)
        << ",";
    out << "ble." << macAddress << ".presence.count:" << count << ",";
  }
